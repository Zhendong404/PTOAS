...
int main(int argc, char **argv) {
  ...
  bool cliTilelangPathSpecified = false;
  bool cliTilelangPkgPathSpecified = false;
  for (int i = 1; i < argc; ++i) {
    llvm::StringRef arg(argv[i]);
    if (arg == "--pto-arch" || arg.starts_with("--pto-arch="))
      cliArchSpecified = true;
    if (arg == "--tilelang-path" || arg.starts_with("--tilelang-path="))
      cliTilelangPathSpecified = true;
    if (arg == "--tilelang-pkg-path" || arg.starts_with("--tilelang-pkg-path="))
      cliTilelangPkgPathSpecified = true;
  }

  // Parse command line options
  llvm::cl::ParseCommandLineOptions(argc, argv, "PTO Assembler (ptoas)\n");

  if (!cliTilelangPathSpecified) {
    std::string detectedTilelangPath = detectInstalledTilelangPath(argv[0]);
    if (!detectedTilelangPath.empty())
      tilelangPath = detectedTilelangPath;
  }
  if (!cliTilelangPkgPathSpecified) {
    std::string detectedTilelangPkgPath = detectInstalledTilelangPkgPath(argv[0]);
    if (!detectedTilelangPkgPath.empty())
      tilelangPkgPath = detectedTilelangPkgPath;
  }
