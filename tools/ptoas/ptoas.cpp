...
using StringRefVector = llvm::SmallVector<llvm::StringRef, kStringRefInlineCapacity>;

} // namespace

static void printPTOASVersion(llvm::raw_ostream& os) { os << "ptoas " << PTOAS_RELEASE_VERSION << "\n"; }

static void collectEmitCFunctions(
    ModuleOp module, SmallVectorImpl<emitc::FuncOp>& declarations, SmallVectorImpl<emitc::FuncOp>& definitions,
...
static LogicalResult buildEmitCFunctionGraph(
    SmallVectorImpl<emitc::FuncOp>& definitions, llvm::DenseMap<StringAttr, emitc::FuncOp>& definitionsByName,
    llvm::DenseMap<Operation*, unsigned>& indegree, llvm::DenseMap<Operation*, SmallVector<Operation*>>& outgoing)
{
    ...
    for (auto caller : definitions) {
        ...
        if (hasCycle) {
            return caller.emitOpError() << "recursive function calls are not supported for EmitC C++ "
                                           "emission";
        }
    }
    return success();
...
static SmallVector<emitc::FuncOp> topologicallySortEmitCDefinitions(
    SmallVectorImpl<emitc::FuncOp>& definitions, llvm::DenseMap<Operation*, unsigned>& indegree,
...
static void reorderEmitCFunctionBlock(Block& body, ArrayRef<emitc::FuncOp> desiredOrder, Operation* anchor)
{
...
static LogicalResult reorderEmitCFunctions(ModuleOp module)
{
    ...
    llvm::DenseMap<Operation*, SmallVector<Operation*>> outgoing;
    if (failed(buildEmitCFunctionGraph(definitions, definitionsByName, indegree, outgoing)))
        return failure();

    SmallVector<emitc::FuncOp> sortedDefinitions = topologicallySortEmitCDefinitions(definitions, indegree, outgoing);
    if (sortedDefinitions.size() != definitions.size()) {
        return module.emitError() << "cyclic function call graph is not supported for EmitC C++ emission";
    }
...
static llvm::cl::opt<std::string> outputFilename(
    "o", llvm::cl::desc("Output filename"), llvm::cl::value_desc("filename"), llvm::cl::init("-"));

static llvm::cl::opt<bool> enableInsertSync(
    "enable-insert-sync", llvm::cl::desc("Enable automatic synchronization insertion pass"), llvm::cl::init(false));
...
static llvm::cl::opt<int> graphSyncSolverEventIdMax(
    ...
    llvm::cl::desc("Maximum EVENT_ID slots for the graph sync solver (default 8). "
                   "Lower values exercise the PIPE_ALL coloring fallback sooner."),
    llvm::cl::init(kDefaultGraphSyncSolverEventIdMax));

static llvm::cl::opt<bool> disableInferLayout(
    "disable-infer-layout", llvm::cl::desc("Disable PTO layout inference pass (static-only)"), llvm::cl::init(false));

static llvm::cl::opt<bool> emitAddPtrTrace(
    "emit-addptr-trace", llvm::cl::desc("Emit addptr trace comments in generated C++ output"), llvm::cl::init(false));

static llvm::cl::opt<bool> emitMlirIR(
    "emit-pto-ir", llvm::cl::desc("Emit PTO IR after lowering instead of C++"), llvm::cl::init(false));

static llvm::cl::opt<std::string> ptoTargetArch(
    "pto-arch", llvm::cl::desc("Target Ascend architecture for codegen: a3 or a5 (default: a3)"),
    llvm::cl::value_desc("a3|a5"), llvm::cl::init("a3"));

static llvm::cl::opt<std::string> ptoBuildLevel(
    "pto-level", llvm::cl::desc("Build level for pass pipeline: level1, level2, or level3 (default: level2)"),
    llvm::cl::value_desc("level1|level2|level3"), llvm::cl::init("level2"));
...
static bool parseAutoSyncTailHint(llvm::StringRef hintStr, std::string& normalized)
{
    ...
    if (s == "mte3-to-s-event0" || s == "mte3_to_s_event0" || s == "setwait-mte3-to-s-event0" ||
...
static bool parseMarkerArgs(llvm::StringRef argsRef, llvm::SmallVectorImpl<llvm::StringRef>& args)
{
    size_t partBegin = 0;
    int parenDepth = 0;
...
static std::optional<ParsedMarkerCall> findNextMarkerCall(
    const std::string& cpp, llvm::StringRef marker, size_t searchPos)
{
...
static bool rewriteMarkerCalls(std::string& cpp, llvm::StringRef marker, BuildReplacementFn buildReplacement)
{
    size_t searchPos = 0;
    bool changed = false;
    for (auto call = findNextMarkerCall(cpp, marker, searchPos); call;
        ...
        cpp.replace(call->markerPos, (call->rparenPos - call->markerPos) + 1, *replacement);
        changed = true;
...
static bool rewriteMarkerCallToMember(
    std::string& cpp, llvm::StringRef marker, llvm::StringRef memberName, unsigned expectedNumArgs)
{
    return rewriteMarkerCalls(cpp, marker, [&](const ParsedMarkerCall& call) -> std::optional<std::string> {
...
static void rewriteMarkerCallsToMembers(std::string& cpp, llvm::ArrayRef<MarkerRewriteSpec> rewrites)
{
    bool changed = true;
    while (changed) {
        changed = false;
        for (const MarkerRewriteSpec& rewrite : rewrites) {
            const bool markerChanged =
                rewriteMarkerCallToMember(cpp, rewrite.marker, rewrite.memberName, rewrite.expectedNumArgs);
            changed = changed || markerChanged;
        }
    }
}

static bool rewriteMarkerCallToField(
    std::string& cpp, llvm::StringRef marker, llvm::StringRef fieldName, size_t expectedNumArgs)
{
    return rewriteMarkerCalls(cpp, marker, [&](const ParsedMarkerCall& call) -> std::optional<std::string> {
        if (call.args.size() != expectedNumArgs)
            return std::nullopt;
...
static void rewriteAsyncEventMarkers(std::string& cpp)
{
    ...
    (void)rewriteMarkerCallToField(cpp, "PTOAS__PREFETCH_CTX_SESSION", "session", 1);
}
...
{
    llvm::SmallVector<emitc::ExpressionOp, kEmptyExpressionInlineCapacity> toErase;
...
static void materializeControlFlowOperands(Operation* rootOp)
{
    ...
    for (Operation* op : branches) {
        ...
        for (OpOperand& operand : op->getOpOperands()) {
            ...
            Attribute initAttr = getDefaultEmitCVariableInitAttr(builder, value.getType());
            if (!initAttr)
                continue;

            Value tmp = builder.create<emitc::VariableOp>(op->getLoc(), value.getType(), initAttr).getResult();
...
static bool rewriteMarkerCallToSubscript(
    std::string& cpp, llvm::StringRef marker, unsigned expectedNumArgs, bool isStore)
{
    return rewriteMarkerCalls(cpp, marker, [&](const ParsedMarkerCall& call) -> std::optional<std::string> {
        if (call.args.size() != expectedNumArgs)
            return std::nullopt;
        if (isStore) {
            return (call.args[0] + "[" + call.args[1] + "] = " + call.args[2]).str();
        }
        return (call.args[0] + "[" + call.args[1] + "]").str();
    });
}

static void rewriteMarkerCallsToSubscripts(std::string& cpp, llvm::ArrayRef<MarkerSubscriptRewriteSpec> rewrites)
{
    bool changed = true;
    while (changed) {
        changed = false;
        for (const MarkerSubscriptRewriteSpec& rewrite : rewrites) {
            const bool markerChanged =
                rewriteMarkerCallToSubscript(cpp, rewrite.marker, rewrite.expectedNumArgs, rewrite.isStore);
            changed = changed || markerChanged;
        }
    }
}
...
static bool rewriteAddPtrTraceMarkers(std::string& cpp, bool showTrace)
{
    size_t searchPos = 0;
    bool changed = false;
    for (auto call = findNextMarkerCall(cpp, "PTOAS__ADDPTR_TRACE", searchPos); call;
         call = findNextMarkerCall(cpp, "PTOAS__ADDPTR_TRACE", searchPos)) {
        if (call->rparenPos == std::string::npos) {
        ...
        cpp.replace(call->markerPos, (replaceEnd - call->markerPos) + 1, replacement);
        changed = true;
        searchPos = call->markerPos + replacement.size();
    }
    return changed;
...
static bool isGeneratedGlobalTensorDecl(llvm::StringRef trimmed, llvm::StringRef& decl, llvm::StringRef& varName)
{
    if (!trimmed.starts_with("GlobalTensor<") || !trimmed.ends_with(";") || trimmed.contains('=') ||
        trimmed.contains('(')) {
        return false;
    }
    ...
    if (!varName.starts_with("v") || varName.size() <= 1)
        return false;
    return llvm::all_of(varName.drop_front(1), [](char c) { return std::isdigit(c); });
}
...
static bool isLiteralInitializer(llvm::StringRef rhs)
{
    ...
    static const llvm::Regex kIntLiteral(R"(^[+-]?(0[xX][0-9A-Fa-f]+|[0-9]+)[uUlL]*$)");
    ...
    return kIntLiteral.match(rhs) || kFloatLiteral.match(rhs) || kHexFloatLiteral.match(rhs) ||
           kSpecialFloatLiteral.match(rhs);
}

static std::string normalizeConstInitializer(llvm::StringRef type, llvm::StringRef rhs)
{
    type = type.trim();
    rhs = rhs.trim();
    if (type == "bool") {
...
{
    return body.starts_with("return") || body.starts_with("goto ") || body.starts_with("if ") ||
           body.starts_with("if(") || body.starts_with("switch ") || body.starts_with("switch(") ||
           body.starts_with("for ") || body.starts_with("for(") || body.starts_with("while ") ||
           body.starts_with("while(") || body.starts_with("case ") || body == "default";
}

static bool splitDeclarationAndInitializer(llvm::StringRef body, llvm::StringRef& lhs, llvm::StringRef& rhs)
{
    lhs = body;
    rhs = {};
...
static bool parseDeclarationNameAndType(llvm::StringRef lhs, llvm::StringRef& type, llvm::StringRef& name)
{
...
static bool parseConstantDeclarationLine(llvm::StringRef line, ConstantDeclCandidate& candidate, std::string& valueName)
{
    llvm::StringRef trimmed = line.trim();
    if (trimmed.empty() || trimmed.starts_with("#") || trimmed.starts_with("//") || !trimmed.ends_with(";"))
        return false;
...
static bool parseGeneratedValueAssignment(llvm::StringRef line, llvm::StringRef& valueName, llvm::StringRef& rhs)
{
    llvm::StringRef trimmed = line.trim();
    if (trimmed.empty() || trimmed.starts_with("#") || trimmed.starts_with("//") || !trimmed.ends_with(";"))
        return false;
...
static void collectConstantDeclCandidates(
    ArrayRef<std::string> lines, size_t beginLine, size_t endLine, llvm::StringMap<ConstantDeclCandidate>& candidates)
{
...
static void rewriteConstantDeclCandidates(
    llvm::StringMap<ConstantDeclCandidate>& candidates, llvm::SmallVectorImpl<std::string>& lines,
    ...
    for (auto& entry : candidates) {
        ...
        if (info.hasInitializer) {
            ...
            if (!isLiteralInitializer(info.assignmentRhs))
                continue;
            initializer = normalizeConstInitializer(info.type, llvm::StringRef(info.assignmentRhs));
            eraseLine[info.assignmentLine] = true;
        }

        lines[info.declLine] = (info.indent + "const " + info.type + " " + valueName.str() + " = " + initializer + ";");
    }
}

static std::string joinRemainingLines(ArrayRef<std::string> lines, ArrayRef<bool> eraseLine, size_t reserveSize)
{
    std::string out;
...
static void rewriteScalarConstantDecls(std::string& cpp)
{
    ...
    auto rewriteSegment = [&lines, &eraseLine](size_t beginLine, size_t endLine) {
...
static std::optional<std::string> detectTextualModuleArch(llvm::StringRef text)
{
    ...
    llvm::Regex archRegex(R"ptoarch("?(pto\.target_arch)"?[[:space:]]*=[[:space:]]*"([[:alpha:][:digit:]_]+)")ptoarch");
    if (!archRegex.match(text, &matches) || matches.size() < kNumber3)
        return std::nullopt;
    return normalizeArchValue(matches[2]);
}

static bool isSupportedArch(llvm::StringRef arch) { return arch == "a3" || arch == "a5"; }

static int resolveEffectiveArch(llvm::StringRef buf, bool isPTOBC, bool cliArchSpecified, std::string& arch)
{
    arch = normalizeArchValue(ptoTargetArch);
    if (cliArchSpecified) {
        if (!isSupportedArch(arch)) {
            llvm::errs() << "Error: invalid --pto-arch='" << ptoTargetArch << "'. Expected 'a3' or 'a5'.\n";
            return 1;
        }
        return 0;
    }
...
static OwningOpRef<ModuleOp> decodePTOBCModule(llvm::StringRef buf, MLIRContext& context)
{
...
static OwningOpRef<ModuleOp> parseTextModule(
    std::unique_ptr<llvm::MemoryBuffer> buffer, MLIRContext& context, llvm::StringRef arch)
{
    llvm::SourceMgr sourceMgr;
    sourceMgr.AddNewSourceBuffer(std::move(buffer), llvm::SMLoc());
    pto::ScopedPTOParserTargetArch scopedParserArch(
        &context, arch == "a5" ? pto::PTOParserTargetArch::A5 : pto::PTOParserTargetArch::A3);
    return parseSourceFile<ModuleOp>(sourceMgr, &context);
}

static OwningOpRef<ModuleOp> loadInputModule(
    std::unique_ptr<llvm::MemoryBuffer> buffer, MLIRContext& context, llvm::StringRef arch, bool isPTOBC)
{
    if (isPTOBC)
...
static bool normalizeAutoSyncTailHints(ModuleOp module, MLIRContext& context)
{
    ...
    module->walk([&](mlir::func::FuncOp func) {
        auto hintAttr = func->getAttrOfType<mlir::StringAttr>("pto.auto_sync_tail_hint");
        if (!hintAttr)
            return;
        std::string normalizedHint;
        if (!parseAutoSyncTailHint(hintAttr.getValue(), normalizedHint)) {
            func.emitError("invalid pto.auto_sync_tail_hint '") << hintAttr.getValue()
                                                                << "'. Expected 'barrier-all' (or 'default') or "
                                                                   "'mte3-to-s-event0'.";
            invalidAutoSyncTailHint = true;
            return;
        }
        func->setAttr("pto.auto_sync_tail_hint", mlir::StringAttr::get(&context, normalizedHint));
    });
...
static int validateTAssignAndAutoSyncOptions(PTOBuildLevel effectiveLevel, bool hasTAssign)
{
    ...
    int enabledAutoSyncModes =
        (enableInsertSync ? 1 : 0) + (enableInjectBarrierAllSync ? 1 : 0) + (enableGraphSyncSolver ? 1 : 0);
...
static int validateAllocTileAddrMode(ModuleOp module, PTOBuildLevel effectiveLevel)
{
...
{
    pm.addNestedPass<mlir::func::FuncOp>(pto::createPTOAssignDefaultFrontendPipeIdPass());
    pm.addNestedPass<mlir::func::FuncOp>(pto::createPTOLowerFrontendPipeOpsPass());
    ...
    pm.addNestedPass<mlir::func::FuncOp>(pto::createPTOValidateIntToPtrUsesPass());
...
static void addAutoSyncPipeline(PassManager& pm)
{
    ...
    if (enableInjectBarrierAllSync) {
        pm.addNestedPass<mlir::func::FuncOp>(pto::createPTOInjectBarrierAllSyncPass());
        return;
    }
    ...
    pm.addNestedPass<mlir::func::FuncOp>(pto::createPTOGraphSyncSolverPass(graphSyncOpts));
}
...
static void addCppEmissionPipeline(PassManager& pm, llvm::StringRef arch)
{
    ...
    pm.addPass(pto::createEmitPTOManualPass(arch == "a3" ? pto::PTOArch::A3 : pto::PTOArch::A5));
    pm.addPass(emitc::createFormExpressionsPass());
    pm.addPass(mlir::createCSEPass());
}

static int openOutputStream(std::unique_ptr<llvm::ToolOutputFile>& outputFile, llvm::raw_ostream*& outputOS)
{
    outputOS = &llvm::outs();
    if (outputFilename == "-")
        return 0;
    std::error_code ec;
    outputFile = std::make_unique<llvm::ToolOutputFile>(outputFilename, ec, llvm::sys::fs::OF_None);
    if (ec) {
...
static int runMlirPipeline(
    PassManager& pm, ModuleOp module, llvm::raw_ostream& outputOS, llvm::ToolOutputFile* outputFile)
{
...
int main(int argc, char** argv)
{
    ...
    if (!fileOrErr) {
        llvm::errs() << "Error: Could not open input file: " << fileOrErr.getError().message() << "\n";
        return 1;
    }
    ...
    OwningOpRef<ModuleOp> module = loadInputModule(std::move(inputBuffer), context, arch, isPTOBC);
    if (!module) {
        llvm::errs() << "Error: Failed to " << (isPTOBC ? "decode PTOBC.\n" : "parse MLIR.\n");
        return 1;
    }
    if (cliArchSpecified || !module->getOperation()->hasAttr("pto.target_arch"))
        module->getOperation()->setAttr("pto.target_arch", mlir::StringAttr::get(&context, arch));
