...
static std::string renameSSAInText(const std::string& text, const std::unordered_map<std::string, std::string>& map)
{
    std::string out;
...
static void collectSSADefsFromSignature(const std::string& line, std::vector<std::string>& out)
{
...
static bool parseConstantLine(const std::string& line, std::string& imm, std::string& ty)
{
    ...
    static const std::regex re(
        R"(^[ \t]*%[-a-zA-Z$._0-9]+[ \t]*=[ \t]*arith\.constant[ \t]+(.+?)[ \t]*:[ \t]*([^ \t]+)(?:[ \t]+loc\(.+\))?[ \t]*$)");
    std::smatch m;
...
static bool findConstantDefinition(
    const std::vector<std::string>& lines, const std::string& name, std::string& imm, std::string& ty)
{
...
static std::string getCanonicalSSAName(
    const std::vector<std::string>& lines, const std::string& oldName,
    std::unordered_map<std::string, int>& constCounts, uint64_t& nextNonConst)
{
    std::string imm;
    std::string ty;
...
static void canonicalizeScalarFloatConstants(
    mlir::ModuleOp module, const mlir::AsmState::LocationMap& locMap, std::vector<std::string>& lines)
{
    // Match: "%name = arith.constant <anything> : fXX [loc(...)]"
    // We keep the prefix + the type suffix (+ optional loc suffix), replace the literal.
    const std::regex re(
        R"(^([ \t]*%[-a-zA-Z$._0-9]+[ \t]*=[ \t]*arith\.constant[ \t]+)(.+?)([ \t]*:[ \t]*f(16|32|64)(?:[ \t]+loc\(.+\))?[ \t]*$))");
...
std::string printModuleCanonical(mlir::ModuleOp module, const CanonicalPrintOptions& opt)
{
