...
struct Reader {
    ...
    uint32_t readU32LE()
    {
        ...
        uint32_t b2 = readU8();
        uint32_t b3 = readU8();
        return b0 | (b1 << kBitsPerByte) | (b2 << (kBitsPerByte * 2U)) | (b3 << (kBitsPerByte * 3U));
    }
    uint64_t readULEB()
    {
        uint64_t v;
...
static void parseTypesSection(
    const std::vector<uint8_t>& data, const std::vector<std::string>& strings, std::vector<TypeEntry>& types)
{
    Reader r{data.data(), data.data() + data.size()};
    uint64_t cnt = r.readULEB();
    types.clear();
...
static void parseAttrsSection(
    const std::vector<uint8_t>& data, const std::vector<std::string>& strings, std::vector<AttrEntry>& attrs)
{
    Reader r{data.data(), data.data() + data.size()};
    uint64_t cnt = r.readULEB();
    attrs.clear();
...
static void parseConstPoolSection(const std::vector<uint8_t>& data, std::vector<ConstEntryParsed>& consts)
{
    Reader r{data.data(), data.data() + data.size()};
    uint64_t cnt = r.readULEB();
    consts.clear();
...
static llvm::APInt rebuildAPIntFromBytes(llvm::ArrayRef<uint8_t> bytes, unsigned bitWidth)
{
...
static mlir::Attribute buildFloatConstAttr(BuildCtx& bc, const ConstEntryParsed& entry)
{
...
static mlir::Attribute buildIntegerConstAttr(BuildCtx& bc, const ConstEntryParsed& entry)
{
    auto ty = getType(bc, entry.typeId);
...
static void addAttrDictionary(mlir::OperationState& state, mlir::DictionaryAttr dict)
{
    for (auto attr : dict)
...
static void assignDecodedResults(BuildCtx& bc, size_t resStart, mlir::Operation* op, size_t numResults)
{
    for (size_t i = 0; i < numResults; ++i)
        bc.values[resStart + i] = op->getResult(i);
}

static llvm::SmallVector<uint64_t, kValueIdInlineCapacity> readValueIds(Reader& r, size_t count)
{
...
static KnownOpImmediates readKnownOpImmediates(Reader& r, const ptobc::v0::OpInfo& info)
{
...
static llvm::SmallVector<uint64_t, kValueIdInlineCapacity> readKnownOperandIds(
    const BuildCtx&, Reader& r, uint16_t opcode, uint8_t variant, const ptobc::v0::OpInfo& info,
    ...
    switch (info.operand_mode) {
        ...
        case 0x03:
            ...
            return reorderLegacyIndexedTscatter(
                readValueIds(r, size_t(info.num_operands) + size_t(imms.n1) + size_t(imms.n2)));
        case 0x04:
            return reorderLegacyIndexedTscatter(
                readValueIds(r, (((imms.optMask & 0x1U) != 0U) ? 1U : 0U) + (((imms.optMask & 0x2U) != 0U) ? 1U : 0U)));
        default:
            throw std::runtime_error("unknown operand_mode");
    }
}

static llvm::SmallVector<mlir::Value, kOperandInlineCapacity> materializeOperands(
    BuildCtx& bc, llvm::ArrayRef<uint64_t> operandIds)
{
...
static mlir::Operation* buildGenericOpFromReader(
    BuildCtx& bc, Reader& r, mlir::Block& block, uint64_t opId, uint64_t attrId)
{
...
static void addImmediateAttrs(
    BuildCtx& bc, mlir::OperationState& state, const ptobc::v0::OpInfo& info, const KnownOpImmediates& imms)
{
    switch (info.imm_kind) {
        case 0x01:
            state.addAttribute(
                "predicate", mlir::arith::CmpIPredicateAttr::get(bc.ctx, mlir::arith::CmpIPredicate(imms.cmpPred)));
            return;
        case 0x02:
            state.addAttribute("src_op", mlir::pto::SyncOpTypeAttr::get(bc.ctx, mlir::pto::SyncOpType(imms.evA)));
            state.addAttribute("dst_op", mlir::pto::SyncOpTypeAttr::get(bc.ctx, mlir::pto::SyncOpType(imms.evB)));
            state.addAttribute("event_id", mlir::pto::EventAttr::get(bc.ctx, mlir::pto::EVENT(imms.evC)));
            return;
        case 0x05:
...
static llvm::SmallVector<mlir::Type, kResultTypeInlineCapacity> readKnownResultTypes(
    BuildCtx& bc, Reader& r, const ptobc::v0::OpInfo& info, uint64_t& numResults)
{
    llvm::SmallVector<mlir::Type, kResultTypeInlineCapacity> resultTypes;
...
static mlir::Operation* buildKnownOpFromReader(
    BuildCtx& bc, Reader& r, mlir::Block& block, uint64_t opId, uint16_t opcode, uint64_t attrId)
{
    ...
    std::string opName = ptobc::v0::fullNameFromOpcodeVariant(opcode, variant) ?
...
static uint64_t readModuleHeader(Reader& r, bool dbg)
{
    ...
    if (dbg) {
        llvm::errs() << "[ptobc] module: profile=" << unsigned(profile) << " indexWidth=" << unsigned(indexWidth)
                     << "\n";
    }
    ...
    if (dbg) {
        llvm::errs() << "[ptobc] module: moduleAttrId=" << moduleAttrId << " globals=" << globalCount << "\n";
    }
    if (globalCount != 0)
...
static std::vector<FuncDecl> readFunctionDecls(BuildCtx& bc, Reader& r, bool dbg)
{
    ...
    for (uint64_t i = 0; i < funcCount; ++i) {
        ...
        if (dbg) {
            llvm::errs() << "[ptobc] func[" << i << "]: nameSid=" << nameSid << " ftypeId=" << ftypeId
                         << " flags=" << unsigned(flags) << " fattrId=" << fattrId << "\n";
        }
        ...
        decls.push_back({bc.strings->at(nameSid), funcType, getAttrDict(bc, fattrId), flags});
    }
    return decls;
}
...
static void buildFunctionBody(
    BuildCtx& bc, Reader& r, mlir::func::FuncOp fn, uint8_t flags, bool dbg,
    std::vector<std::vector<mlir::Operation*>>* opsByFuncOut)
{
    ...
    if (dbg) {
        llvm::errs() << "[ptobc] func body built ok: values=" << bc.values.size() << " ops=" << bc.opsById.size()
                     << "\n";
    }
    if (opsByFuncOut)
...
static mlir::ModuleOp decodeToModule(
    mlir::MLIRContext& ctx, const std::vector<std::string>& strings, const std::vector<TypeEntry>& types,
    const std::vector<AttrEntry>& attrs, const std::vector<uint8_t>& constPool, const std::vector<uint8_t>& moduleBytes,
    std::vector<std::vector<mlir::Operation*>>* opsByFuncOut)
{
    const bool dbg = debugEnabled();
    ...
    for (const auto& decl : decls) {
        ...
        auto fn = mlir::func::FuncOp::create(mlir::UnknownLoc::get(&ctx), decl.name, decl.type);
        if (dbg)
...
static std::pair<uint8_t, std::vector<uint8_t>> readSection(Reader& r, bool dbg)
{
    ...
    if (dbg)
        llvm::errs() << "[ptobc] section id=" << unsigned(sid) << " len=" << sectionLen << "\n";
    return {sid, bytes};
}

static void applyDebugLocations(
    mlir::MLIRContext& ctx, const std::vector<std::string>& strings, const DebugInfo& dbgInfo,
    const std::vector<std::vector<mlir::Operation*>>& opsByFunc)
{
    for (const auto& location : dbgInfo.locations) {
        ...
        if (file.pathSid >= strings.size())
            continue;
        op->setLoc(
            mlir::FileLineColLoc::get(&ctx, strings[file.pathSid], unsigned(location.sl), unsigned(location.sc)));
    }
}
...
mlir::OwningOpRef<mlir::ModuleOp> decodePTOBCToModule(llvm::ArrayRef<uint8_t> fileBytes, mlir::MLIRContext& ctx)
{
    ...
    Reader versionReader{fileBytes.data() + kPTOBCVersionOffset, fileBytes.data() + fileBytes.size()};
    uint16_t ver = versionReader.readU16LE();
    if (ver != kVersionV0)
        throw std::runtime_error("unsupported version");

    Reader payloadLengthReader{fileBytes.data() + kPTOBCPayloadLengthOffset, fileBytes.data() + fileBytes.size()};
    ...
    Reader r{fileBytes.data() + kPTOBCHeaderSize, fileBytes.data() + fileBytes.size()};
    ...
    if (s1 != kSectionStrings || s2 != kSectionTypes || s3 != kSectionAttrs || s4 != kSectionConstPool ||
        s6 != kSectionModule) {
    ...
    if (dbg) {
        llvm::errs() << "[ptobc] strings=" << strings.size() << " types=" << types.size() << " attrs=" << attrs.size()
                     << " moduleBytes=" << d6.size() << "\n";
    }
