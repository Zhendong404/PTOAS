...
using SyncMap =
    llvm::MapVector<mlir::pto::syncsolver::OperationBase*, std::deque<std::unique_ptr<mlir::pto::syncsolver::SyncOp>>>;
using SyncBeforeAfterMap = std::pair<SyncMap, SyncMap>;

namespace mlir::pto {
inline llvm::raw_ostream& operator<<(llvm::raw_ostream& os, TCoreType coreType)
{
    switch (coreType) {
...
struct SyncSolverOptions {
    ...
    SyncSolverOptions(SyncMode syncMode, bool isMemBasedArch, bool isRegBasedArch)
        : syncMode(syncMode), isMemBasedArch(isMemBasedArch), isRegBasedArch(isRegBasedArch)
    {
        decomposeMmadl1Op = false;
        alwaysUsePipeSAsWaitingPipe = !isTestMode() && isCrossCoreMode() && isMemBasedArch;
    ...
    {
        return syncMode == SyncMode::CROSS_CORE_SYNC || syncMode == SyncMode::TEST_CROSS_CORE_MODE;
    }

    bool isIntraCoreMode() const
    {
        return syncMode == SyncMode::INTRA_CORE_SYNC || syncMode == SyncMode::TEST_INTRA_CORE_MODE;
    }

    bool isTestMode() const
    {
        return syncMode == SyncMode::TEST_INTRA_CORE_MODE || syncMode == SyncMode::TEST_CROSS_CORE_MODE;
    }
};
...
struct ProcessingOrder {
    ...
    ProcessingOrder(Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2, bool isUseless)
        : occ1(occ1), occ2(occ2), rwOp1(rwOp1), rwOp2(rwOp2), isUseless(isUseless) {};
};
...
public:
    ...
    void merge(const UnitFlagInfo& other, Occurrence* occ1, Occurrence* occ2, bool asSet = true, bool asWait = true)
    {
...
struct Occurrence {
    ...
    Occurrence(OperationBase* op, Occurrence* parentOcc, int depth, int startIndex, int endIdx)
        : op(op), parentOcc(parentOcc), depth(depth), startIndex(startIndex), endIndex(endIdx)
    {}
    ...
    static Occurrence* getParentWithOp(Occurrence* occ, const Operation* op, bool assertExists = true);
    static Occurrence* getParentWithOp(Occurrence* occ, const OperationBase* op, bool assertExists = true);
    ...
    static std::pair<Occurrence*, Occurrence*> getLCAPair(Occurrence* occ1, Occurrence* occ2);

    template <typename OpTy>
    Occurrence* getParentOfType() const
    {
...
struct ConflictPair {
    ...
    ConflictPair(
        RWOperation* op1, RWOperation* op2, OperationBase* setOp, OperationBase* waitOp, Occurrence* setOcc,
        Occurrence* waitOcc, CorePipeInfo setCorePipeInfo, CorePipeInfo waitCorePipeInfo, int startIndex, int endIndex)
        : id(globalIdCounter++),
          op1(op1),
          op2(op2),
          setOp(setOp),
          ...
          setOcc(setOcc),
          waitOcc(waitOcc),
          setCorePipeInfo(setCorePipeInfo),
          waitCorePipeInfo(waitCorePipeInfo),
    ...
    std::unique_ptr<ConflictPair> clone()
    {
        auto clonedConflictPair = std::make_unique<ConflictPair>(
            op1, op2, setOp, waitOp, setOcc, waitOcc, setCorePipeInfo, waitCorePipeInfo, startIndex, endIndex);
...
public:
    EventIdNode(ConflictPair* conflictPair, int64_t eventIdNum, bool reversePriority)
        : id(globalIdCounter++),
          initConflictPair(conflictPair),
    ...
    {
        auto clonedNode = std::make_unique<EventIdNode>(initConflictPair, eventIdNum, reversePriority);
        clonedNode->eventIds = eventIds;
        clonedNode->conflictPairs = conflictPairs;
        return clonedNode;
    }

    void insertConflictPair(const ConflictPair* conflictPair) { conflictPairs[conflictPair] += 1; }
    ...
    void setEventIds(const llvm::SmallVector<int64_t>& newEventIds) { eventIds = newEventIds; }
    ...
    std::string str(bool printConflictPairs = true)
    {
        ...
        if (printConflictPairs) {
            for (auto [conflictPair, frq] : conflictPairs) {
                ASSERT(frq > 0);
                ret += std::string(kConflictPairIndentWidth, ' ') + conflictPair->str() + "\n";
            }
        }
        ret.pop_back();
        return ret;
...
struct MmadL1SyncArgs {
    ...
    MmadL1SyncArgs(
        Value l0WaitL1AEvent, Value l0WaitL1BEvent, Value l1AWaitL0Event, Value l1BWaitL0Event, Value kLoopDBCond,
        Value bwdPipeMPipeMTE1Event0, Value bwdPipeMPipeMTE1Event1)
        : l0WaitL1AEvent(l0WaitL1AEvent),
          l0WaitL1BEvent(l0WaitL1BEvent),
          l1AWaitL0Event(l1AWaitL0Event),
          l1BWaitL0Event(l1BWaitL0Event),
...
int64_t getHWAvailableEventIdNum(
    SyncMode syncMode, pto::PIPE setPipe = pto::PIPE::PIPE_UNASSIGNED, pto::PIPE waitPipe = pto::PIPE::PIPE_UNASSIGNED);

llvm::SmallVector<int64_t> getHWAvailableEventIds(
    SyncMode syncMode, pto::PIPE setPipe = pto::PIPE::PIPE_UNASSIGNED, pto::PIPE waitPipe = pto::PIPE::PIPE_UNASSIGNED);

// Create a boolean Value that is true for the first iteration of `forOp`.
Value getIsFirstIterationValue(scf::ForOp forOp, Location loc, IRRewriter& rewriter);

// Create a boolean Value that is true for the last iteration of `forOp`.
Value getIsLastIterationValue(scf::ForOp forOp, Location loc, IRRewriter& rewriter);
