...
public:
    ...
    llvm::SmallVector<int64_t> getUnitFlagModesAsSet(bool = true) const { return {}; }
    llvm::SmallVector<int64_t> getUnitFlagModesAsWait(bool = true) const { return {}; }
};
...
public:
    ...
    // LCA.
    static std::pair<OperationBase*, OperationBase*> getLCAPair(OperationBase* op1, OperationBase* op2);

    template <typename TyOp>
    TyOp* getParentOfType() const
    {
...
public:
    PlaceHolder(Operation* op, OperationBase* parentOp) : OperationBase(OpType::PLACE_HOLDER, op, parentOp) {}

    static bool classof(const OperationBase* e) { return e->opType == OpType::PLACE_HOLDER; }
...
public:
    Scope(const OpType& opType = OpType::SCOPE, Operation* op = nullptr, OperationBase* parentOp = nullptr)
        : OperationBase(opType, op, parentOp)
    {}

    static bool classof(const OperationBase* e) { return e->opType >= OpType::SCOPE && e->opType < OpType::SCOPE_END; }
...
public:
    ...
    static bool classof(const OperationBase* e) { return e->opType == OpType::FUNCTION_BLOCK; }
};
...
public:
    ...
    static bool classof(const OperationBase* e) { return e->opType == OpType::FUNCTION; }
};
...
public:
    ...
    Loop(Operation* op, OperationBase* parentOp) : Scope(OpType::LOOP, op, parentOp) {}

    static bool classof(const OperationBase* e) { return e->opType >= OpType::LOOP && e->opType < OpType::LOOP_END; }
...
public:
    ...
    MmadL1LoopOp(Operation* op, OperationBase* parentOp) : Scope(OpType::MMAD_SCOPE, op, parentOp) {};

    static bool classof(const OperationBase* e) { return e->opType == OpType::MMAD_SCOPE; }
};
...
public:
    ...
    Condition(
        Operation* op, OperationBase* parentOp, std::unique_ptr<Scope> trueScope, std::unique_ptr<Scope> falseScope)
    ...
    static bool classof(const OperationBase* e) { return e->opType == OpType::CONDITION; }
...
public:
    RWOperation(
        Operation* op, OperationBase* parentOp, pto::TCoreType coreType, pto::PIPE pipeRead, pto::PIPE pipeWrite,
        const llvm::SmallVector<Value>& readMemVals, const llvm::SmallVector<Value>& writeMemVals,
        OpType opType = OpType::RW_OPERATION)
        : OperationBase(opType, op, parentOp),
          coreType(coreType),
          pipeRead(pipeRead),
          pipeWrite(pipeWrite),
          readMemVals(readMemVals),
    ...
    RWOperation(
        Operation* op, OperationBase* parentOp, pto::TCoreType coreType, pto::PIPE pipeRead, pto::PIPE pipeWrite,
        const llvm::SmallVector<llvm::SmallVector<int64_t>>& testReadMemVals,
        const llvm::SmallVector<llvm::SmallVector<int64_t>>& testWriteMemVals, OpType opType = OpType::RW_OPERATION)
        : OperationBase(opType, op, parentOp),
          coreType(coreType),
          pipeRead(pipeRead),
          pipeWrite(pipeWrite),
          testReadMemVals(testReadMemVals),
          testWriteMemVals(testWriteMemVals)
    {
    ...
    {
        return e->opType >= OpType::RW_OPERATION && e->opType < OpType::RW_OPERATION_END;
    }
};
...
public:
    LoadL0AOp(
        ...
        const llvm::SmallVector<Value>& readMemVals, const llvm::SmallVector<Value>& writeMemVals)
        : RWOperation(
              op, parentOp, coreType, pipeRead, pipeWrite, readMemVals, writeMemVals, OpType::MMAD_LOAD_L0A_OPERATION)
    {}

    static bool classof(const OperationBase* e) { return e->opType == OpType::MMAD_LOAD_L0A_OPERATION; }
};
...
public:
    LoadL0BOp(
        ...
        const llvm::SmallVector<Value>& readMemVals, const llvm::SmallVector<Value>& writeMemVals)
        : RWOperation(
              op, parentOp, coreType, pipeRead, pipeWrite, readMemVals, writeMemVals, OpType::MMAD_LOAD_L0B_OPERATION)
    {}

    static bool classof(const OperationBase* e) { return e->opType == OpType::MMAD_LOAD_L0B_OPERATION; }
};
...
public:
    LoadBiasOp(
        ...
        const llvm::SmallVector<Value>& readMemVals, const llvm::SmallVector<Value>& writeMemVals)
        : RWOperation(
              op, parentOp, coreType, pipeRead, pipeWrite, readMemVals, writeMemVals, OpType::MMAD_LOAD_BIAS_OPERATION)
    {}

    static bool classof(const OperationBase* e) { return e->opType == OpType::MMAD_LOAD_BIAS_OPERATION; }
};
...
public:
    MmadL0Operation(
        Operation* op, OperationBase* parentOp, pto::TCoreType coreType, pto::PIPE pipeRead, pto::PIPE pipeWrite,
        const llvm::SmallVector<Value>& readMemVals, const llvm::SmallVector<Value>& writeMemVals)
        : RWOperation(op, parentOp, coreType, pipeRead, pipeWrite, readMemVals, writeMemVals, OpType::MMAD_OPERATION)
    {}

    static bool classof(const OperationBase* e) { return e->opType == OpType::MMAD_OPERATION; }
};

class SyncOp : public OperationBase {
public:
    std::optional<int> debugId;
    SyncOp(const OpType& opType, Operation* op, OperationBase* parentOp) : OperationBase(opType, op, parentOp) {}
...
public:
    ...
    SetWaitOp(
        const OpType& opType, Operation* op, OperationBase* parentOp, const llvm::SmallVector<int64_t>& eventIds,
        pto::PIPE pipeSrc, pto::PIPE pipeDst)
        : SyncOp(opType, op, parentOp), eventIds(eventIds), pipeSrc(pipeSrc), pipeDst(pipeDst)
    {}

    static bool classof(const OperationBase* e)
    {
        return e->opType >= OpType::SW_FLAG_OP && e->opType < OpType::SW_FLAG_OP_END;
    }
};
...
public:
    SetFlagOp(
        Operation* op, OperationBase* parentOp, const llvm::SmallVector<int64_t>& eventIds, pto::PIPE pipeSrc,
        pto::PIPE pipeDst)
        : SetWaitOp(OpType::SET_FLAG_OP, op, parentOp, eventIds, pipeSrc, pipeDst)
    {}

    std::unique_ptr<SetFlagOp> clone() { return std::make_unique<SetFlagOp>(op, parentOp, eventIds, pipeSrc, pipeDst); }

    static bool classof(const OperationBase* e) { return e->opType == OpType::SET_FLAG_OP; }
...
public:
    WaitFlagOp(
        Operation* op, OperationBase* parentOp, const llvm::SmallVector<int64_t>& eventIds, pto::PIPE pipeSrc,
        pto::PIPE pipeDst)
        : SetWaitOp(OpType::WAIT_FLAG_OP, op, parentOp, eventIds, pipeSrc, pipeDst)
    {}

    std::unique_ptr<WaitFlagOp> clone()
    {
        return std::make_unique<WaitFlagOp>(op, parentOp, eventIds, pipeSrc, pipeDst);
    }

    static bool classof(const OperationBase* e) { return e->opType == OpType::WAIT_FLAG_OP; }
...
public:
    ...
    static bool classof(const OperationBase* e) { return e->opType == OpType::BARRIER_OP; }
