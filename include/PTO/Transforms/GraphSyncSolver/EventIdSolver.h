...
public:
    ...
    static bool classof(const Action* e) { return e->actionType == ACTION_TYPE::NONE; }
...
public:
    ...
    ActionAddNode(EventIdNode* node) : Action(ACTION_TYPE::ADD_NODE), node(node) { ASSERT(node != nullptr); }
    static bool classof(const Action* e) { return e->actionType == ACTION_TYPE::ADD_NODE; }
    std::string str() const override { return "ADD_NODE(" + std::to_string(node->id) + ")"; }
};
...
public:
    ...
    ActionAddEdge(EventIdNode* node1, EventIdNode* node2) : Action(ACTION_TYPE::ADD_EDGE), node1(node1), node2(node2)
    {
        ASSERT(node1 != nullptr);
        ASSERT(node2 != nullptr);
    }
    static bool classof(const Action* e) { return e->actionType == ACTION_TYPE::ADD_EDGE; }
    std::string str() const override
    {
        return "ADD_EDGE(" + std::to_string(node1->id) + ", " + std::to_string(node2->id) + ")";
    }
};
...
public:
    ...
    ActionInsertConflictPair(EventIdNode* node, ConflictPair* conflictPair)
        : Action(ACTION_TYPE::INSERT_CONFLICT_PAIR), node(node), conflictPair(conflictPair)
    {
        ASSERT(node != nullptr);
        ASSERT(conflictPair != nullptr);
    }
    static bool classof(const Action* e) { return e->actionType == ACTION_TYPE::INSERT_CONFLICT_PAIR; }
    std::string str() const override
    {
        return "INSERT_CONFLICT_PAIR(" + std::to_string(node->id) + ", " + std::to_string(conflictPair->id) + ")";
    }
};
...
public:
    ...
    ActionAssignEventIds(
        EventIdNode* node, const llvm::SmallVector<int64_t>& oldEventIds, const llvm::SmallVector<int64_t>& newEventIds)
        : Action(ACTION_TYPE::ASSIGN_EVENT_IDS), node(node), oldEventIds(oldEventIds), newEventIds(newEventIds)
    {
        ASSERT(node != nullptr);
    }
    static bool classof(const Action* e) { return e->actionType == ACTION_TYPE::ASSIGN_EVENT_IDS; }
    std::string str() const override
    {
        std::string ret;
...
public:
    ...
    ActionAssignNeedRecalc(bool oldValue, bool newValue)
        : Action(ACTION_TYPE::ASSIGN_NEED_RECALC), oldValue(oldValue), newValue(newValue)
    {}
    static bool classof(const Action* e) { return e->actionType == ACTION_TYPE::ASSIGN_NEED_RECALC; }
    std::string str() const override
    {
        return "ASSIGN_NEED_RECALC(" + std::to_string(oldValue) + ", " + std::to_string(newValue) + ")";
    }
};
...
public:
    ...
    EventIdNode* createNode(ConflictPair* conflictPair, int64_t eventIdNum = 1, bool reversePriority = false);

    void addConflicts(ConflictPair* conflictPairSrc, const std::vector<ConflictPair*>& conflictPairsDst);
...
private:
    ...
    // do-undo
    void assignEventIds(EventIdNode* node, const llvm::SmallVector<int64_t>& eventIds, bool pushAction = true);
    ...
    llvm::SmallVector<int64_t> getChosenEventIds(EventIdNode* node, int64_t eventIdMax);
};
} // namespace mlir::pto::syncsolver
