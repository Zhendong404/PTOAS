...
private:
    ...
    void FindUseEventID(unsigned int begin, unsigned int end, const SyncOperation* s, SmallVector<bool>& eventId);

    bool CheckSyncLifeCycleConflict(
    ...
    void UpdateBackwardMatchSync(const SyncOperation* setFlag, const SyncOperation* waitFlag, unsigned eventId);

    void SetUseEventID(unsigned int begin, unsigned int end, const SyncOperation* setFlag, unsigned int eventId);

    bool ExtendLifecycle(SmallVector<unsigned int>& syncLifeCycle, unsigned int beginNew, unsigned int endNew) const;
    ...
    SmallVector<bool> GetEventIdIdleStatus(SyncOperation* sync, size_t eventIdNum);
    ...
    SyncOperation* FindWidenSync(const SyncOperation* setSync, const SyncOperation* waitSync);
    void ClearEventId(const SyncOperation* sync);

    SmallVector<int> GetAvailableEventId(
    ...
    SmallVector<int> UpdateBlockAvailableEventId(
...
private:
    ...
    static const llvm::DenseMap<std::pair<PipelineType, PipelineType>, uint64_t> reservedEventIdNum;
