...
class SyncCodegen {
public:
    SyncCodegen(SyncIRs& syncIR, func::FuncOp func, SyncAnalysisMode syncAnalysisMode)
...
private:
    // --- 核心插入逻辑 ---
    void SyncInsert(IRRewriter& rewriter, Operation* op, SyncOperation* sync, bool beforeInsert);
    ...
    // --- 指令生成 ---
    void CreateBarrierOp(IRRewriter& rewriter, Operation* op, const SyncOperation* sync, bool beforeInsert);
    ...
    void CreateSetWaitOpForSingleBuffer(
        IRRewriter& rewriter, Operation* op, const SyncOperation* sync, bool beforeInsert) const;

    void CreateSetWaitOpForMultiBuffer(IRRewriter& rewriter, Operation* op, SyncOperation* sync, bool beforeInsert);

    void CreateBlockSyncBarrierOp(IRRewriter& rewriter, Operation* op, const SyncOperation* sync, bool beforeInsert);

    // --- 辅助函数 ---

    // 生成用于多缓冲 ID 选择的 Value (e.g., select(cond, id0, id1))
    Value GetBufferSelected(IRRewriter& rewriter, Operation* op, SyncOperation* sync);
