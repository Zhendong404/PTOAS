...
public:
    RemoveRedundantSync(
        ...
        : syncIR_(syncIR), syncOperations_(syncOperations), syncAnalysisMode_(syncAnalysisMode) {};
...
private:
    ...
    bool CheckBranchBetween(
        BranchInstanceElement* branchElement, SmallVector<bool> syncFinder, const SyncOperation* setFlag,
        unsigned endId, unsigned& i);

    /// 处理 Loop 结构中的冗余检查 (当前实现较为保守)
    bool CheckLoopBetween(LoopInstanceElement* loopElement, const SyncOperation* setFlag, unsigned& i) const;
