...
struct SyncRecord {
    ...
    std::array<bool, static_cast<unsigned>(PipelineType::PIPE_LAST) + 1U> alreadySync{false};
...
public:
    InsertSyncAnalysis(
        SyncIRs& syncIR, MemoryDependentAnalyzer& memDepAnalyzer, SyncOperations& syncOperations, func::FuncOp func,
...
private:
    ...
    void UpdateAlreadySync(
        const SyncOps& syncVector, SyncRecordList& syncRecordList, const PipelineType nowPipeValue) const;

    void UpdateSyncRecordInfo(const CompoundInstanceElement* frontCompound, SyncRecordList& syncRecordList) const;

    void UpdateSyncRecord(const SyncOperation* sync, SyncRecord& syncRecord, PipelineType nowPipeValue) const;

    void InsertLastPipeAll();

    /// 快速判断是否不需要插入同步 (Fast Path Pruning)
    /// isBackwardDep: 是否是跨循环迭代的依赖 (Back-edge)
    bool IsNoNeedToInsertSync(
        const CompoundInstanceElement* nowCompound, const CompoundInstanceElement* frontCompound,
    ...
    /// 判断两个操作是否构成GM的读写冲突
    bool IsGMHazard(const CompoundInstanceElement* nowCompound, const CompoundInstanceElement* frontCompound) const;
