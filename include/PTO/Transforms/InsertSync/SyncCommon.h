...
// 辅助函数：获取 Pipeline 数量
static constexpr unsigned getPipeNum() { return static_cast<unsigned>(PipelineType::PIPE_LAST); }

/// 核心类型定义：对应计算单元类型
enum class TCoreType { VECTOR, CUBE, CUBE_OR_VECTOR, CUBE_AND_VECTOR };

/// Meminfo of the target buffer
/// 用于追踪 Buffer 的别名和根节点
struct BaseMemInfo {
    BaseMemInfo(
        Value baseBuffer, Value rootBuffer, pto::AddressSpace scope, SmallVector<uint64_t> baseAddresses,
        uint64_t allocateSize)
        : baseBuffer(baseBuffer),
          rootBuffer(rootBuffer),
          scope(scope),
          baseAddresses(std::move(baseAddresses)),
    ...
    bool areVectorEqual(const SmallVector<uint64_t>& vec1, const SmallVector<uint64_t>& vec2) const
    {
    ...
    {
        return std::make_unique<BaseMemInfo>(baseBuffer, rootBuffer, scope, baseAddresses, allocateSize);
    }

    std::unique_ptr<BaseMemInfo> clone(Value cloneBaseBuffer) const
    {
        return std::make_unique<BaseMemInfo>(cloneBaseBuffer, rootBuffer, scope, baseAddresses, allocateSize);
    }
};

using DepBaseMemInfoPairVec = SmallVector<std::pair<const BaseMemInfo*, const BaseMemInfo*>>;
...
public:
    SyncOperation(
        TYPE type, pto::PipelineType srcPipe, pto::PipelineType dstPipe, unsigned kSyncIndex, unsigned syncIRIndex,
        ...
        : isCompensation(isComp),
          ...
          srcPipe_(srcPipe),
          dstPipe_(dstPipe),
          kSyncIndex_(kSyncIndex),
          syncIRIndex_(syncIRIndex),
...
SmallVector<const void*> canonicalizeSyncDepRoots(const SmallVector<Value>& roots);
...
public:
    ...
    PlaceHolderInstanceElement(unsigned index, unsigned parentScopeId)
        : InstanceElement(KindTy::PLACE_HOLDER, index), parentScopeId(parentScopeId) {};
...
public:
    LoopInstanceElement(unsigned index, unsigned beginId, unsigned endId, KindOfLoop loopKind = KindOfLoop::LOOP_BEGIN)
        : InstanceElement(KindTy::LOOP, index), beginId(beginId), endId(endId), kLoopKind(loopKind)
    {}
...
public:
    BranchInstanceElement(unsigned index, unsigned beginId, KindOfBranch branchKind = KindOfBranch::IF_BEGIN)
        : InstanceElement(KindTy::BRANCH, index), beginId(beginId), branchId(beginId), kBranchKind(branchKind)
    {}

    ~BranchInstanceElement() override = default;
    std::unique_ptr<BranchInstanceElement> CloneBranch(KindOfBranch branchKind) const;

    KindOfBranch getBranchKind() const { return kBranchKind; }
    static bool classof(const InstanceElement* e);

    BranchInstanceElement(
        unsigned index, unsigned beginId, unsigned branchId, unsigned endId,
        KindOfBranch branchKind = KindOfBranch::IF_BEGIN)
        : InstanceElement(KindTy::BRANCH, index),
          beginId(beginId),
          branchId(branchId),
          endId(endId),
          kBranchKind(branchKind)
    {}
...
public:
    CompoundInstanceElement(
        unsigned index, SmallVector<const BaseMemInfo*> defVec, SmallVector<const BaseMemInfo*> useVec,
        const pto::PipelineType PipeValue, OperationName opName)
        : InstanceElement(KindTy::COMPOUND, index),
          defVec(std::move(defVec)),
          useVec(std::move(useVec)),
          kPipeValue(PipeValue),
          opName(opName)
    {}
    ...
    std::optional<mlir::Value> getUnitFlagCond(Location loc, OpBuilder& rewriter) const;
};

using SyncIRs = SmallVector<std::unique_ptr<InstanceElement>>;
using SyncOperations = SmallVector<SmallVector<std::unique_ptr<SyncOperation>>>;
using Buffer2MemInfoMap = llvm::DenseMap<Value, llvm::SmallVector<std::unique_ptr<BaseMemInfo>>>;
