...
**Description**: Signal an FFTS cross-block event on a synchronization endpoint. The DSL lowers to `pto.set_cross_core`, which is normalized to `pto.sync.set` with `ffts_mode = 0` for the VPTO path.
...
**Description**: Wait for an FFTS cross-block event. The DSL lowers to `pto.wait_flag_dev`, which is normalized to `pto.sync.wait` with `ffts_mode = 0` for the VPTO path.
