...
and narrowing integer-to-floating conversion. Float-to-integer conversion is
always saturating. Some widening or same-format float conversions, and all
float conversions with `f32` destination, have no selectable saturation mode;
...
| `pto.abs` | `pto.absi` or `pto.absf` | category-specific integer or floating absolute value |
| `pto.select`, `pto.cast`, `pto.bitcast` | corresponding scalar operation | standard selection, numeric conversion, and bit reinterpretation |
