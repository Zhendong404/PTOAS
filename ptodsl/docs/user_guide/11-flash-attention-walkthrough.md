...
1. **`left_load`**: load Q tile from UB into LEFT scratch (`q_l0a`).
2. **`right_load`**: load K tile from UB into RIGHT scratch (`k_l0b`), with `transpose=True` for K^T.
3. **`mad`**: matrix multiply-accumulate — `s_acc = q_l0a @ k_l0b`.
4. **`acc_store_ub`**: write the accumulator result to the UB output tile `s_tile`.
...
| 6 | L3a | `qk_matmul` (left_load, right_load, mad, acc_store_ub) | Cube |
| 7 | L2 | `mem_bar(SYNC)` | — |
