...
| `alloc(split=0)` | Allocate the next FIFO slot. Returns an entry descriptor. |
| `push(entry, split=0)` | Push a filled entry to the consumer. Notifies the consumer side. |
...
| `init_cube()` | Initialise the pipe on the Cube side. |
| `init_simd()` | Initialise the pipe on the Vector (SIMD) side. |
| `pop(split=0, result_type=None)` | Pop the next entry from the producer. For global-entry pipes, `result_type` defaults to the pipe's `entry_type`. |
