...
> **Convenience wrappers**: `pto.dma_load(src, dst)` and `pto.dma_store(src, dst)` are Python-level shorthands that expand to `mte_gm_ub` / `mte_ub_gm` with inferred strides. The reference operations below are the full grouped DMA interfaces.
...
#### `pto.cube_load_frac(src: PtrType, dst: PtrType, mode: FractalMode, *, shape: tuple[int, int], src_layout: tuple[int, int], dst_group: tuple[int, int, int, int], ctrl: tuple[int, bool]) -> None`
...
#### `pto.right_load(src: PtrType, dst: PtrType, k: int, n: int, *, transpose: bool = False) -> None`
...
**Description**: MX-mode variants of `left_load` and `right_load` for MX-capable dtypes. Parameters same as their non-MX counterparts.
...
#### `pto.acc_store(src: PtrType, dst: PtrType, m: int, n: int, src_stride: int, dst_stride: int, *, mode: FractalMode = FractalMode.NZ2ND, loop0_src_stride: int | None = None, split: int | None = None, loop3: tuple[int, int, int] | None = None) -> None`
...
#### `pto.acc_store_gm(src: PtrType, dst: PtrType, m: int, n: int, src_stride: int, dst_stride: int, *, sid: int = 0, l2_cache_ctrl: int = 0, mode: FractalMode = FractalMode.NZ2ND, loop0_src_stride: int | None = None, split: int | None = None, loop3: tuple[int, int, int] | None = None) -> None`
...
#### `pto.acc_store_ub(src: PtrType, dst: PtrType, m: int, n: int, src_stride: int, dst_stride: int, *, dual_dst_mode: int = 0, sub_blockid: int = 0, mode: FractalMode = FractalMode.NZ2ND, loop0_src_stride: int | None = None, channel_split_en: int | None = None, loop3: tuple[int, int, int] | None = None) -> None`
...
|-----------|-----------|-----------|-----------|
| GM → L1 | `cube_load` | gm | l1 |
| GM → L1 (fractal) | `cube_load_frac` | gm | l1 |
| L1 → UB | `cube_store` | l1 | ub |
| L1 → L0A | `left_load` | l1 | l0a |
| L1 → L0B | `right_load` | l1 | l0b |
| L1 → L0A (MX) | `left_load_mx` | l1 | l0a |
| L1 → L0B (MX) | `right_load_mx` | l1 | l0b |
| L1 → Bias | `bias_load` | l1 | bt |
| L0C → L1 | `acc_store` | l0c | l1 |
| L0C → GM | `acc_store_gm` | l0c | gm |
| L0C → UB | `acc_store_ub` | l0c | ub |
...
def qk_matmul(q_tile, k_tile, q_l0a, k_l0b, s_acc, s_tile):
    ...
    pto.left_load(q_tile, q_l0a, m, k)          # UB tile → L0A
    pto.right_load(k_tile, k_l0b, k, n, transpose=True)  # UB tile → L0B
    pto.mad(q_l0a, k_l0b, s_acc)                # L0A × L0B → L0C
    pto.acc_store_ub(s_acc, s_tile, m, n)       # L0C → UB tile
```

The `left_load`/`right_load` operations take UB `Tile` references directly (not raw pointers) — the tile-to-cube-local transfer is implicit. `mad` performs the matrix multiply. `acc_store_ub` writes the result back to a UB tile.
