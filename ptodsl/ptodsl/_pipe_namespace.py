...
_PIPE_ID_COUNTER = count()


def _next_pipe_id():
    return next(_PIPE_ID_COUNTER)
...
class _PipeSurface:
    ...
    def push(self, entry, split=0):
        if self._descriptor.kind != "global":
            raise TypeError("push() is only available on global-entry pipes")
    ...
    def pop(self, split=0, result_type=None):
        if self._descriptor.kind != "global":
            raise TypeError("pop() is only available on global-entry pipes")
        split = _as_int(split, context="pipe.pop(..., split=...)")
        result_type = result_type or self.entry_type
        if result_type is None:
            raise TypeError("pop() requires result_type when entry_type is not known")
        if self._descriptor.direction == "c2v":
            op = _pto.TPopFromAicOp(result_type, split, id=self.id)
        else:
            op = _pto.TPopFromAivOp(result_type, split, id=self.id)
        return wrap_surface_value(op.result)

    def free(self, entry=None, split=0):
        if self._descriptor.kind != "global":
            raise TypeError("free() is only available on global-entry pipes")
