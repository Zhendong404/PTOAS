...
def _index_conversion_signedness(source_type, target_type):
    ...
    if IntegerType.isinstance(source_element) and IndexType.isinstance(target_element):
        # An index result has no authored signedness.  The unsigned variant is
        # only selected for index-to-integer conversion with an unsigned
        # integer destination.
        return "signed"
    raise TypeError(
...
def _emit_index_cast(value, target_type):
    source_type = value.type
    _validate_index_cast_types(source_type, target_type)
    signedness = _index_conversion_signedness(source_type, target_type)
    ...
    op = arith.IndexCastUIOp if signedness == "unsigned" else arith.IndexCastOp
    result = op(target, source).result
    return _restore_authored_integer_type(result, target_type)
