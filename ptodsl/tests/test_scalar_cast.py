...
def test_issue_1179_scalar_float_integer_casts():
    for _, probe, operation, signedness in ISSUE_1179_SIMT_PROBES + ISSUE_1179_ORDINARY_PROBES:
        ...
        assert "pto.index_cast" not in text


def test_index_integer_casts_use_standard_arith():
    index_to_i32_text = index_to_i32_probe.compile().mlir_text()
    assert "arith.index_cast" in index_to_i32_text
    assert "arith.index_castui" not in index_to_i32_text
    assert "pto.index_cast" not in index_to_i32_text

    index_to_ui32_text = index_to_ui32_probe.compile().mlir_text()
    assert "arith.index_castui" in index_to_ui32_text
    assert "pto.index_cast" not in index_to_ui32_text

    integer_to_index_text = integer_to_index_probe.compile().mlir_text()
    assert "arith.index_cast" in integer_to_index_text
    assert "pto.index_cast" not in integer_to_index_text
...
def main():
    ...
    test_index_integer_casts_use_standard_arith()
