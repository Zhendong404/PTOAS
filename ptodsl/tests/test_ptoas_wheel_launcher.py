...
class WheelLauncherTests(unittest.TestCase):
    ...
    def test_launcher_exports_runtime_contract_and_injects_default_paths(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.dict(_launcher.os.environ, {}, clear=True), mock.patch.object(
                ...
                load_entrypoint.assert_called_once_with(shared_module)
    ...
    def test_load_shared_entrypoint_configures_in_process_ctypes_call(self):
        ...
        with mock.patch.object(_launcher.ctypes, "CDLL", return_value=fake_library) as load_library:
            entrypoint = _launcher._load_shared_entrypoint(Path("/tmp/pto/ptoas.so"))

        load_library.assert_called_once()
        self.assertEqual(load_library.call_args.args[0], "/tmp/pto/ptoas.so")
        self.assertEqual(load_library.call_args.kwargs["mode"], getattr(_launcher.ctypes, "RTLD_GLOBAL", 0))
        self.assertIs(entrypoint, fake_library.ptoas_entrypoint)
    ...
    def test_launcher_falls_back_to_env_install_tree_for_editable_installs(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.dict(
                ...
                self.assertEqual(exc.exception.code, 0)
                load_entrypoint.assert_called_once_with(install_root / "lib" / "ptoas.so")
                entrypoint = load_entrypoint.return_value
                argc, c_argv = entrypoint.call_args.args
                self.assertEqual([
                    c_argv[i].decode("utf-8") for i in range(argc)
    ...
    def test_launcher_skips_empty_placeholder_shared_module(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(exc.exception.code, 0)
            load_entrypoint.assert_called_once_with(install_shared_module)
