// RUN: { ptoas %s --enable-op-fusion --pto-backend=vpto --pto-arch=a5 --print-ir-after-all --print-ir-after-all-func-filter=__pto_fused_group_9_9 -o /dev/null 2>&1 || true; } | awk '/IR Dump After PTOFusionLoadStoreElision/{found=1} found && !done {if ($0 ~ /^\/\/ -----\/\/ IR Dump After / && $0 !~ /PTOFusionLoadStoreElision/) {done=1; next} print}' | FileCheck %s
