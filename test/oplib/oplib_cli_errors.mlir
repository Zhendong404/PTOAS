// RUN: ! ptoas %S/softmax_chain.pto --enable-op-fusion --disable-oplib-lowering --pto-arch=a5 -o %t.dep.cpp > %t.dep.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.empty.cpp > %t.empty.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.core.cpp > %t.core.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.lanes.cpp > %t.lanes.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.simd_attr.cpp > %t.simd_attr.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.disallowed.cpp > %t.disallowed.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_vec.cpp > %t.bad_vec.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.legacy.cpp > %t.legacy.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.missing_knobs.cpp > %t.missing_knobs.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_prefix.cpp > %t.bad_prefix.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.missing_vld.cpp > %t.missing_vld.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.missing_vst.cpp > %t.missing_vst.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_vst_prefix.cpp > %t.bad_vst_prefix.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_sig.cpp > %t.bad_sig.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_arg_match.cpp > %t.bad_arg_match.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_scalar_pos.cpp > %t.bad_scalar_pos.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_is_binary.cpp > %t.bad_is_binary.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_math.cpp > %t.bad_math.log 2>&1
...
// RUN: ! ptoas %S/softmax_chain.pto --pto-arch=a5 -o %t.bad_vec_int.cpp > %t.bad_vec_int.log 2>&1
