module {
  func.func private @__pto_oplib_variant_tsels_scalar_mode_f32(
    ...
    %lhs = vector.maskedload %m0[%c0, %c0], %active, %passive {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<64xi1>, vector<64xf32> into vector<64xf32>
    %rhs = vector.maskedload %m1[%c0, %c0], %active, %passive {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<64xi1>, vector<64xf32> into vector<64xf32>
    %result = arith.select %active, %lhs, %rhs : vector<64xi1>, vector<64xf32>
    vector.maskedstore %md[%c0, %c0], %active, %result {pto.simd.vst_dist = "DIST_NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<64xi1>, vector<64xf32>
    return
  }
}
