...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tcolexpandadd.
#
# pto.tcolexpandadd: dst = src0 + expand_cols(src1), where src1 is a 1xcols
# tile broadcast down to dst rows.  Fully-valid f32/f16/i32/i16 tiles in PTODSL
...
def _tcolexpandadd_body(src0_ptr, src1_ptr, dst_ptr, *, rows, cols, dtype):
    ...
    pto.tile.colexpandadd(src0_tile, src1_tile, dst_tile)
    pto.tile.store(dst_tile, dst_view)


_tcolexpandadd_kernels = {}
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    _r, _c = _shape

    def _make(r=_r, c=_c, dtype=_dtype, kernel_name=f"tcolexpandadd_{_name}"):
        ...
        def _kernel(
            ...
            _tcolexpandadd_body(src0_ptr, src1_ptr, dst_ptr, rows=r, cols=c, dtype=dtype)

        return _kernel

    _tcolexpandadd_kernels[_name] = _make()
...
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    CASES.append(
        golden_output_case(
            "tcolexpandadd_" + _name,
            _tcolexpandadd_kernels[_name],
