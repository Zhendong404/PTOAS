...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tcolexpandmax.
#
# pto.tcolexpandmax: dst = max(src0, expand_cols(src1)), where src1 is a 1xcols
...
def _tcolexpandmax_body(src0_ptr, src1_ptr, dst_ptr, *, rows, cols, dtype):
    ...
    pto.tile.colexpandmax(src0_tile, src1_tile, dst_tile)
    pto.tile.store(dst_tile, dst_view)


_tcolexpandmax_kernels = {}
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    _r, _c = _shape

    def _make(r=_r, c=_c, dtype=_dtype, kernel_name=f"tcolexpandmax_{_name}"):
        ...
        def _kernel(
            ...
            _tcolexpandmax_body(src0_ptr, src1_ptr, dst_ptr, rows=r, cols=c, dtype=dtype)

        return _kernel

    _tcolexpandmax_kernels[_name] = _make()
...
def _make_expected(src0, src1, *, dtype):
    ...
    return np.maximum(src0, expanded_src1).astype(np_dtype)
...
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    CASES.append(
        golden_output_case(
            "tcolexpandmax_" + _name,
            _tcolexpandmax_kernels[_name],
