...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trowargmin.
#   tload(src) + trowargmin(src, tmp) -> dst(R, 1) + tstore(dst)
#
# Row argmin: dst[i, 0] = index of min over src[i, valid_cols].  The source
...
# the rowargmin dst tile itself is always (R, 8) valid (R, 1) in the legacy .pto.
CASE_SPECS = [
...
def _make_kernel(name: str, np_src_dtype, np_dst_dtype, shape, valid_shape, dst_cols):
    ...
    @pto.jit(name="trowargmin_" + name, target="a5")
    def _kernel(
...
def _make_expected(src, valid_shape, dst_cols, np_dst_dtype):
    ...
    golden[:, 0] = np.argmin(src[:valid_rows, :valid_cols], axis=1).astype(np_dst_dtype)
    return golden
