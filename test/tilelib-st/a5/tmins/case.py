...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tmins.
#   tload(src) + tmins(src, 5.0) -> dst + tstore(dst)
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="tmins_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.mins(src_tile, scalar_val, dst_tile)
...
def _make_expected(src, valid_shape, direction):
    ...
    if direction == "scalar_src":
        ...
        golden[:valid_rows, :valid_cols] = (np.minimum(src, src.dtype.type(5.0)))[:valid_rows, :valid_cols]
    return golden
