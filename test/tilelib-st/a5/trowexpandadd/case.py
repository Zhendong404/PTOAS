...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trowexpandadd.
#
# pto.trowexpandadd: dst = src0 + broadcast(src1) across columns.  src1 is a
...
# template parameter, not a tile valid_shape; pto.trowexpandadd requires
...
# column is used (src1Col=1 in the legacy launcher).
CASE_SPECS = [
...
def _trowexpandadd_body(src0_ptr, src1_ptr, dst_ptr, *, rows, cols, src1_cols, dtype):
    ...
    # pto.trowexpandadd requires row-major src1 valid_shape[1] == 32/sizeof(dtype).
    ...
    pto.tile.rowexpandadd(src0_tile, src1_tile, dst_tile)
    pto.tile.store(dst_tile, dst_view)


# One decorated kernel per case, each binding a static shape at definition time
# (mirroring the per-case funcs in trowexpandadd.pto).
_trowexpandadd_kernels = {}
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    _r, _c = _shape
    _src1_c = _src1_shape[1]

    def _make(r=_r, c=_c, src1_c=_src1_c, dtype=_dtype, kernel_name=f"trowexpandadd_{_name}"):
    ...
    _trowexpandadd_kernels[_name] = _make()
...
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    CASES.append(
        golden_output_case(
            "trowexpandadd_" + _name,
            _trowexpandadd_kernels[_name],
