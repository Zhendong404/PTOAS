...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tand.
#   tload(a) + tload(b) + tand(a, b) -> c + tstore(c)
...
def _make_kernel(name: str, rows: int, cols: int):
    @pto.jit(name="tand_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.bit_and(a_tile, b_tile, c_tile)
