...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tshl.
#   tload(a) + tload(b) + tshl(a, b) -> c + tstore(c)
...
def _make_kernel(name: str, rows: int, cols: int):
    @pto.jit(name="tshl_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.bit_shl(a_tile, b_tile, c_tile)
