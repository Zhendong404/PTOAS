...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tshls.
#   tload(src) + tshls(src, 2) -> dst + tstore(dst)
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="tshls_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.bit_shls(src_tile, scalar_val, dst_tile)
...
def _make_expected(src, valid_shape, direction):
    ...
    if direction == "scalar_src":
        ...
        golden[:valid_rows, :valid_cols] = (src << 2)[:valid_rows, :valid_cols]
    return golden
