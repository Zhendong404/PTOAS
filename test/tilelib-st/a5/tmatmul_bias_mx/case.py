...
- The split-M case (``bias_fp8_e4m3_200x192x95``) keeps the legacy two-chunk
  ...
  physical rows [128,208) (valid 72), both writing into separate GM output
  buffers exactly like the legacy launcher's ``c``/``c + 128*128`` pointers.
  The host passes the two packed A chunks and the two A-scale chunks as
  separate buffers (legacy ``LaunchTMATMUL_MX_bias_fp8_e4m3_200x192x95``
  computed ``a1 = a + 128*192`` and ``scale_a1 = scale_a + 768``).
...
def _make_split_m_data(spec):
    """Split-M variant: A and A-scale are passed as two independent GM chunks
    (a0@rows [0,128), a1@rows [128,208)) like the legacy launcher."""
    np.random.seed(zlib.crc32(spec["name"].encode("utf-8")) & 0xFFFFFFFF)
    x1_bin, x2_bin, x1_scale_gm, x2_scale_gm, bias, golden = gen_golden(spec)
    # x1_bin is the concatenation of chunk0 (128 physical rows) and chunk1
    # (80 physical rows); x1_scale_gm packs rows [0,128) into the first
    # 768 bytes and rows [128,208) into the following 480 bytes (legacy
    # kScaleAChunkBytes = 768).
    a0_bytes = 128 * 192
    a1_bytes = 80 * 192
    a0_bin = x1_bin[:a0_bytes].view(np.uint8)
    a1_bin = x1_bin[a0_bytes:a0_bytes + a1_bytes].view(np.uint8)
    a0_scale = x1_scale_gm[:768]
    a1_scale = x1_scale_gm[768:]
    return [a0_bin, x2_bin.view(np.uint8), a0_scale, x2_scale_gm, bias,
            a1_bin, a1_scale], golden
...
def _split_m_case(spec):
    """Custom case builder for the split-M kernel: chunk outputs are two
    separate GM buffers (c0 rows [0,128), c1 rows [128,208)) and the check
    concatenates them back into the [m_padded, n_padded] golden."""
    ...
    def make_case():
        ...
        return [*inputs, c0, c1], golden

    def check(device_inputs, golden):
        c0 = np.asarray(device_inputs[-2].cpu().numpy())
        c1 = np.asarray(device_inputs[-1].cpu().numpy())
