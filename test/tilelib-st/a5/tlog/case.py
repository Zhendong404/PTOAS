...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tlog.
#   tload(src) + tlog(src) -> dst + tstore(dst)
...
CASE_SPECS = [
    ...
    ("f32_16x64_hp", np.float32, (16, 64), (16, 64), 1e-05),
    ("f32_32x32_hp", np.float32, (32, 32), (32, 32), 1e-05),
    ("f16_16x64_hp", np.float16, (16, 64), (16, 64), 0.001),
    ("f16_32x32_hp", np.float16, (32, 32), (32, 32), 0.001),
]
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="tlog_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.log(src_tile, dst_tile)
...
def _make_inputs(name: str, np_dtype, shape, valid_shape):
    ...
    src = np.random.uniform(0.1, 10.0, size=shape).astype(np_dtype)
    return [src]
...
def _make_expected(src, valid_shape):
    ...
    golden[:valid_rows, :valid_cols] = np.log(src)[:valid_rows, :valid_cols]
    return golden
