...
@pto.jit(name="trandom_int32_4x256", target="a5")
def _kernel(
    ...
    # before scalar loads instead of adding a scalar conversion operation.
    ...
    output_tile = pto.alloc_tile(shape=[ROWS, COLS], dtype=pto.ui32, valid_shape=[ROWS, COLS])
    pto.tile.random(key0, key1, counter0, counter1, counter2, counter3, output_tile, rounds=10)
    pto.tile.store(
        output_tile,
        output_view,
        offsets=[0, 0, 0, 0, 0],
...
def _expected(key, counter):
    return _golden(key, counter)


CASES = [golden_output_case(
    "trandom_int32_4x256", _kernel, inputs=_inputs, expected=_expected,
    rtol=0.0, atol=0.0,
)]
