...
"""PTODSL port of the legacy A5 MAT-to-SCALING TMOV case.

The scale is intentionally only staged into SCALING, matching the legacy test:
the numerical result is the ordinary f16 matmul accumulated to f32.  The
MAT-to-SCALING ``pto.tile.mov`` is the behavior under test.

GM->MAT staging for the matmul operands and the ACC->GM numerical endpoint
use the public ``pto.tile.load`` / ``pto.tile.store`` surface (the MAT load
templates select ND2NZ from the rank-5 view metadata, mirroring tload_mat).

The [1, N] f32 scale row is the one residual explicit staging: the public MAT
load templates only express ND2NZ/DN2NZ fractal loads into ColMajor MAT
tiles, while the MAT->SCALING ``mov`` (template_tmov_m2s, ``mte_l1_fb``)
reads a contiguous L1 row.  The plain contiguous GM->L1 copy is therefore
kept as ``pto.mte_gm_l1`` (see the inline note at the call site).
"""
...
def _nd2nz_view_metadata(rows, cols):
    # Rank-5 ND view metadata consumed by the public MAT load template:
    # ND2NZ selects it from src_layout "ND" and strides[3] == cols, strides[4] == 1.
    return [1, 1, 1, rows, cols], [rows * cols, rows * cols, rows * cols, cols, 1]


@pto.jit(
    name="tmov2scale_f16_16x16x16",
    kernel_kind="cube",
    target="a5",
    mode="explicit",
    insert_sync=False,
...
def _kernel(
    ...
    scale_mat = pto.alloc_tile(
        ...
        addr=L1_SCALE, blayout="RowMajor", slayout="NoneBox",
    )
    ...
    scaling = pto.alloc_tile(
        shape=[1, N], dtype=pto.f32, memory_space=pto.MemorySpace.SCALING,
        addr=0, blayout="RowMajor", slayout="NoneBox",
    )
    ...
    # Residual explicit staging: the [1, N] f32 scale row must land in L1 as a
    # contiguous row for template_tmov_m2s (mte_l1_fb).  The public MAT load
    # templates only select ND2NZ/DN2NZ into ColMajor MAT tiles, so this plain
    # contiguous GM->L1 copy cannot be expressed on the tile surface.
    scale_l1 = pto.castptr(pto.ui64(L1_SCALE), pto.ptr(pto.f32, "mat"))
    pto.mte_gm_l1(scale_ptr, scale_l1, 64, nburst=(1, 0, 0))
    pto.set_flag(pto.Pipe.MTE2, pto.Pipe.MTE1, event_id=2)
    pto.wait_flag(pto.Pipe.MTE2, pto.Pipe.MTE1, event_id=2)
    pto.tile.mov(scale_mat, scaling)
    ...
    # ACC->GM numerical endpoint through the public tile.store surface
...
def _inputs():
    ...
    return [
        ...
        np.random.uniform(1.0, 4.0, (1, N)).astype(np.float32),
    ]


def _expected(a, b, _scale):
    return (a.astype(np.float32) @ b.astype(np.float32)).astype(np.float32)


CASES = [
    golden_output_case(
        "tmov2scale_f16_16x16x16",
        _kernel,
        inputs=_inputs,
        expected=_expected,
        rtol=1e-3,
