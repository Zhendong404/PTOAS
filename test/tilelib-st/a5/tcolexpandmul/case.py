...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tcolexpandmul.
#
# pto.tcolexpandmul: dst = src0 * expand_cols(src1), where src1 is a 1xcols
...
CASE_SPECS = [
    ("fp32_16_128_1_128", pto.f32, (16, 128), (1, 128), 1e-3),
    ("fp32_32_32_1_32",   pto.f32, (32, 32),  (1, 32),  1e-3),
...
def _tcolexpandmul_body(src0_ptr, src1_ptr, dst_ptr, *, rows, cols, dtype):
    ...
    pto.tile.colexpandmul(src0_tile, src1_tile, dst_tile)
    pto.tile.store(dst_tile, dst_view)


_tcolexpandmul_kernels = {}
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    _r, _c = _shape

    def _make(r=_r, c=_c, dtype=_dtype, kernel_name=f"tcolexpandmul_{_name}"):
        ...
        def _kernel(
            ...
            _tcolexpandmul_body(src0_ptr, src1_ptr, dst_ptr, rows=r, cols=c, dtype=dtype)

        return _kernel

    _tcolexpandmul_kernels[_name] = _make()


def _make_inputs(name, dtype, shape, src1_shape):
    # Mirrors st_common.setup_case_rng: per-case deterministic crc32 seed,
    # then gen_data.py's randint(1, 10) draw order (src0 then src1).
...
def _make_expected(src0, src1, *, rows, cols):
    # Mirrors gen_data.py: broadcast src1 down to dst rows, multiply, then
    # store into a dtype buffer (int dtypes stay exact, matching the legacy
    # golden).
    dtype = src0.dtype
...
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    ...
    CASES.append(
        golden_output_case(
            "tcolexpandmul_" + _name,
            _tcolexpandmul_kernels[_name],
