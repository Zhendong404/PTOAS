...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tshr.
#   tload(a) + tload(b) + tshr(a, b) -> c + tstore(c)
...
def _make_kernel(name: str, rows: int, cols: int):
    @pto.jit(name="tshr_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.bit_shr(a_tile, b_tile, c_tile)
