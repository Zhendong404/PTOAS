...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trowmax.
#   tload(src) + trowmax(src) -> dst(R, 1) + tstore(dst)
#
# Row reduction: dst[i, 0] = np.max over src[i, valid_cols].  The dst is a
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="trowmax_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.rowmax(src_tile, dst_tile)
...
def _make_expected(src, valid_shape):
    ...
    golden[:, 0] = np.max(src[:valid_rows, :valid_cols], axis=1).astype(src.dtype)
    return golden
