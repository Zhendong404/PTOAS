...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/txors.
#   tload(src) + txors(src, 3) -> dst + tstore(dst)
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="txors_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.bit_xors(src_tile, scalar_val, dst_tile, dst_tile)
