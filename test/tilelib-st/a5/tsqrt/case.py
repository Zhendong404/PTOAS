...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tsqrt.
#   tload(src) + tsqrt(src) -> dst + tstore(dst)
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="tsqrt_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.sqrt(src_tile, dst_tile)
...
def _make_inputs(name: str, np_dtype, shape, valid_shape):
    ...
    src = np.random.uniform(0.001, 1.0, size=shape).astype(np_dtype)
    return [src]
...
def _make_expected(src, valid_shape):
    ...
    golden[:valid_rows, :valid_cols] = np.sqrt(src)[:valid_rows, :valid_cols]
    return golden
