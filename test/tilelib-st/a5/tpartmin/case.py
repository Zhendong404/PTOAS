...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tpartmin.
#   tload(src0) + tload(src1) + pto.tile.partmin(src0, src1) -> dst + tstore(dst)
#
# tpartmin semantics (matches the legacy gen_data.py golden and the PTODSL
# template_tpartmin lowering): dst is first padded with the dtype maximum,
# then src0's valid region is copied into dst, then dst = min(dst, src1) over
# src1's valid region.  The TileLib partmin template accepts arbitrary src0/
# src1 valid regions nested inside the dst valid region (the "complex" cases
# where neither operand covers dst), like partmax.
#
...
# at the tile origin and participate in the partial min.
CASE_SPECS = [
...
def _make_kernel(name, np_dtype, shape, src0_valid, src1_valid, dst_valid):
    ...
    (mirroring the per-case funcs in tpartmin.pto)."""
    rows, cols = shape
    ...
    @pto.jit(name="tpartmin_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.partmin(a_tile, b_tile, c_tile)
...
def _max_val(np_dtype):
    """Dtype maximum used as the tpartmin padding value (template_tpartmin's
    pad_max / the legacy gen_data.py corner fill)."""
...
def _make_expected(a, b, src0_valid, src1_valid, dst_valid):
    """Reproduce gen_data.py tpartmin golden semantics exactly:

    dst = Max, dst[0:src0_vr, 0:src0_vc] = src0, then
    dst[0:src1_vr, 0:src1_vc] = min(dst, src1).  The corner not covered by
    either valid region keeps the Max padding (dtype maximum).
    """
    src0_vr, src0_vc = src0_valid
    src1_vr, src1_vc = src1_valid
    dst_vr, dst_vc = dst_valid
    ...
    if src0_eq_dst and src1_eq_dst:
        # Full min: both src0 and src1 cover the entire dst
        golden[:dst_vr, :dst_vc] = np.minimum(
            a[:dst_vr, :dst_vc], b[:dst_vr, :dst_vc]
        ).astype(dtype, copy=False)
    elif src0_eq_dst:
        # src0 covers dst, src1 is partial: min in src1 region, src0 elsewhere
        ...
        # src1 covers dst, src0 is partial: min in src0 region, src1 elsewhere
        ...
        # Overlapping region: min of both operands
        ...
        # Corner covered by neither operand keeps the Max padding
        if src1_vr > src0_vr and src0_vc > src1_vc:
            golden[src0_vr:src1_vr, src1_vc:src0_vc] = _max_val(dtype)
