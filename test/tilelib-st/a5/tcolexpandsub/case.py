...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tcolexpandsub.
#
# pto.tcolexpandsub: dst = src0 - expand_cols(src1), where src1 is a 1xcols
...
CASE_SPECS = [
    ("fp32_6_128_1_128",   pto.f32, (6, 128),   (1, 128), 1e-6),
    ("fp32_18_32_1_32",    pto.f32, (18, 32),   (1, 32),  1e-6),
    ("fp16_10_256_1_256",  pto.f16, (10, 256),  (1, 256), 1e-3),
    ("fp16_12_64_1_64",    pto.f16, (12, 64),   (1, 64),  1e-3),
    ("int32_16_32_1_32",   pto.i32, (16, 32),   (1, 32),  0),
    ("int16_16_64_1_64",   pto.i16, (16, 64),   (1, 64),  0),
]


def _tcolexpandsub_body(src0_ptr, src1_ptr, dst_ptr, *, rows, cols, dtype):
    ...
    pto.tile.colexpandsub(src0_tile, src1_tile, dst_tile)
    pto.tile.store(dst_tile, dst_view)


_tcolexpandsub_kernels = {}
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    _r, _c = _shape

    def _make(r=_r, c=_c, dtype=_dtype, kernel_name=f"tcolexpandsub_{_name}"):
        ...
        def _kernel(
            ...
            _tcolexpandsub_body(src0_ptr, src1_ptr, dst_ptr, rows=r, cols=c, dtype=dtype)

        return _kernel

    _tcolexpandsub_kernels[_name] = _make()
...
def _make_expected(src0, src1, *, dtype):
    ...
    return (src0 - expanded_src1).astype(np_dtype)
...
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    CASES.append(
        golden_output_case(
            "tcolexpandsub_" + _name,
            _tcolexpandsub_kernels[_name],
