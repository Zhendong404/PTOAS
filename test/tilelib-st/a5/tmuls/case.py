...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tmuls.
#
# pto.tmuls: dst = src * scalar.  Fully-valid f32/f16/i32/i16 tiles in PTODSL
...
# Scalar multiplied into every element.  Matches gen_data.py SCALAR and
...
def _tmuls_body(src_ptr, dst_ptr, *, rows, cols, dtype, scalar):
    ...
    pto.tile.muls(src_tile, scalar, dst_tile)
...
for _name, _dtype, _shape, _eps in CASE_SPECS:
    ...
    def _make(r=_r, c=_c, dtype=_dtype, scalar=_scalar, kernel_name=f"tmuls_{_name}"):
        @pto.jit(name=kernel_name, target="a5")
        def _kernel(src_ptr: pto.ptr(dtype, "gm"), dst_ptr: pto.ptr(dtype, "gm")):
            _tmuls_body(src_ptr, dst_ptr, rows=r, cols=c, dtype=dtype, scalar=scalar)

        return _kernel

    _tmuls_kernels[_name] = _make()
