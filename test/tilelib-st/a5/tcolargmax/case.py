...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tcolargmax.
#   tload(src) + tcolargmax(src, tmp) -> dst(1, C, i32) + tstore(dst)
#
# Column argmax: dst[r=0, c] = index of max over src[valid_rows, c].  The
...
def _make_kernel(name: str, pto_dtype, shape, valid_shape, dst_shape):
    ...
    @pto.jit(name="tcolargmax_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.colargmax(src_tile, tmp_tile, dst_tile)
...
def _make_expected(src, valid_shape, dst_shape):
    ...
    golden[:1, :valid_cols] = np.argmax(src[:valid_rows, :valid_cols], axis=0, keepdims=True)
    return golden
