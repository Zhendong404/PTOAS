...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tor.
#   tload(a) + tload(b) + tor(a, b) -> c + tstore(c)
...
def _make_kernel(name: str, rows: int, cols: int):
    @pto.jit(name="tor_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.bit_or(a_tile, b_tile, c_tile)
