...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tpartadd.
#   tload(src0) + tload(src1) + pto.tile.partadd(src0, src1) -> dst + tstore(dst)
#
# tpartadd semantics (matches the legacy gen_data.py golden and the PTODSL
# template_tpartadd lowering): one operand is "full" (its valid_shape equals
# the dst valid region) and provides the base values over the whole dst; the
# other operand may be partial (row_less or col_less).  dst = full + partial
# over the partial valid region, and dst = full elsewhere.
#
...
# partial operand's valid region is what participates in the add.
CASE_SPECS = [
...
def _make_kernel(name, np_dtype, shape, src0_valid, src1_valid, dst_valid):
    ...
    (mirroring the per-case funcs in tpartadd.pto)."""
    rows, cols = shape
    ...
    @pto.jit(name="tpartadd_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.partadd(a_tile, b_tile, c_tile)
...
def _make_expected(a, b, src0_valid, src1_valid, dst_valid):
    """Reproduce gen_data.py tpartadd golden semantics exactly.

    If src0_valid == dst_valid, src0 is the full operand (base values come
    from src0, src1 is the possibly-partial addend).  Otherwise src1 is the
    full operand and the roles swap.
    """
    src0_vr, src0_vc = src0_valid
    src1_vr, src1_vc = src1_valid
    dst_vr, dst_vc = dst_valid
    ...
    if src0_eq_dst:
        ...
        if src1_eq_dst:
            ...
            # src1 col_less: copy src0 full, then add in the overlapping region
            ...
            # src1 row_less: add in the src1 region, copy src0 for remaining rows
            golden[:src1_vr, :src1_vc] = (a[:src1_vr, :src1_vc] + b[:src1_vr, :src1_vc]).astype(dtype, copy=False)
        ...
        if src0_vc < dst_vc:
            # src0 col_less: copy src1 full, then add in the overlapping region
            ...
            # src0 row_less: add in the src0 region, copy src1 for remaining rows
            golden[:src0_vr, :src0_vc] = (a[:src0_vr, :src0_vc] + b[:src0_vr, :src0_vc]).astype(dtype, copy=False)
