...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tmin.
#
# pto.tmin: dst = min(a, b) element-wise over vec tiles, mirroring the legacy
# tload(a) + tload(b) + tmin(a, b) -> c + tstore(c) kernel.  Fully-valid and
# partially-valid (valid_shape smaller than the padded tile shape) f32 / i32 /
# i16 / f16 vec tiles in PTODSL auto mode; tile addresses, load/store
# partitions and sync are left to PTOAS.  Legacy dtype / shape / valid_shape /
# eps and the per-case crc32 RNG seed with the randint(1, 10) draw order are
# preserved; the partially-valid cases allocate the same padded tiles as the
# legacy .pto (e.g. tile_buf<vec, 64x64xf32, valid=60x60>).
...
# computation region, matching the legacy cases.py table.
CASE_SPECS = [
    ("f32_64x64",          pto.f32, np.float32, (64, 64),   (64, 64),   1e-6),
    ("i32_64x64",          pto.i32, np.int32,   (64, 64),   (64, 64),   0),
    ("i16_64x64",          pto.i16, np.int16,   (64, 64),   (64, 64),   0),
    ("f16_64x64",          pto.f16, np.float16, (64, 64),   (64, 64),   1e-3),
    ("f32_64x64_v60x60",   pto.f32, np.float32, (64, 64),   (60, 60),   1e-6),
    ("i32_64x64_v60x60",   pto.i32, np.int32,   (64, 64),   (60, 60),   0),
    ("f16_2x4096_v1x3600", pto.f16, np.float16, (2, 4096),  (1, 3600),  1e-3),
    ("i16_20x512_v16x200", pto.i16, np.int16,   (20, 512),  (16, 200),  0),
]


def _tmin_body(a_ptr, b_ptr, c_ptr, *, rows, cols, valid_rows, valid_cols, dtype):
    """Shared kernel body: tload(a) + tload(b) + tmin(a, b) -> c + tstore(c)."""
    ...
    # (same memory layout as the legacy .pto tensor views).
...
def _make_expected(input1, input2, shape, valid_shape):
    ...
    vr, vc = valid_shape
    golden[:vr, :vc] = np.minimum(input1[:vr, :vc], input2[:vr, :vc]).astype(
