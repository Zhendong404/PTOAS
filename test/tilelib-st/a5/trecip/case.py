...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trecip.
#   tload(src) + trecip(src) -> dst + tstore(dst)
...
CASE_SPECS = [
    ...
    ("f32_64x64_pad", np.float32, (66, 72), (64, 64), 1e-06),
    ("f32_58x70", np.float32, (66, 72), (58, 70), 1e-06),
]
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="trecip_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.recip(src_tile, dst_tile)
...
def _make_expected(src, valid_shape):
    ...
    golden[:valid_rows, :valid_cols] = np.reciprocal(src)[:valid_rows, :valid_cols]
    return golden
