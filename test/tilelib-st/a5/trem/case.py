...
# omitted.  The legacy suite uses positive operands, so np.remainder and the
# truncation-toward-zero remainder have the same result here.
...
CASE_SPECS = [
    ("f32_16x64", pto.f32, np.float32, (16, 64), 1e-6),
    ("f32_32x32", pto.f32, np.float32, (32, 32), 1e-6),
]
...
def _make_kernel(name, dtype, shape):
    ...
    def _kernel(
        ...
        pto.tile.rem(src0_tile, src1_tile, dst_tile)
...
def _make_inputs(name, np_dtype, shape):
    ...
    src1 = np.random.randint(1, 10, size=shape).astype(np_dtype)
    return [src0, src1]
