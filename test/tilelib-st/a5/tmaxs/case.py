...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tmaxs.
#   tload(src) + tmaxs(src, 5.0) -> dst + tstore(dst)
...
CASE_SPECS = [
    ("f32_32x64", np.float32, (32, 64), (32, 64), 1e-06, None),
    ("f16_63x64", np.float16, (63, 64), (63, 64), 0.001, None),
    ("i32_31x128", np.int32, (31, 128), (31, 128), 0.0, None),
    ("i16_15x192", np.int16, (15, 192), (15, 192), 0.0, None),
    ("f32_7x448", np.float32, (7, 448), (7, 448), 1e-06, None),
    ("f32_256x16", np.float32, (256, 16), (256, 16), 1e-06, None),
]
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="tmaxs_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.maxs(src_tile, scalar_val, dst_tile)
...
def _make_expected(src, valid_shape, direction):
    ...
    if direction == "scalar_src":
        ...
        golden[:valid_rows, :valid_cols] = (np.maximum(src, src.dtype.type(5.0)))[:valid_rows, :valid_cols]
    return golden
