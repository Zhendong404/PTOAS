...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tors.
#   tload(src) + tors(src, 3) -> dst + tstore(dst)
...
def _make_kernel(name: str, np_dtype, shape, valid_shape):
    ...
    @pto.jit(name="tors_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.bit_ors(src_tile, scalar_val, dst_tile)
