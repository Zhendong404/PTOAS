...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tcolmax.
#   tload(src) + tcolmax(src) -> dst(1, C) + tstore(dst)
#
# Column reduction: dst[r=0, c] = max over src[valid_rows, c].  The legacy
# cases use partially-valid tiles (valid_shape may be smaller than shape and
# dst_valid_shape == (1, valid_cols)); the kernel writes only the valid dst
# columns and the host golden keeps the invalid tail zeroed, so the full-array
...
CASE_SPECS = [
    ("f32_1x256",  pto.f32, np.float32, (1, 256),  (1, 255),  (1, 256),  1e-6),
    ("f32_16x128", pto.f32, np.float32, (16, 128), (16, 127), (1, 128),  1e-6),
    ("f32_16x256", pto.f32, np.float32, (16, 256), (15, 255), (1, 256),  1e-6),
    ("f16_1x256",  pto.f16, np.float16, (1, 256),  (1, 255),  (1, 256),  1e-3),
    ("f16_16x128", pto.f16, np.float16, (16, 128), (16, 127), (1, 128),  1e-3),
    ("f16_16x256", pto.f16, np.float16, (16, 256), (15, 255), (1, 256),  1e-3),
...
def _make_kernel(name: str, pto_dtype, shape, valid_shape, dst_shape):
    ...
    @pto.jit(name="tcolmax_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.colmax(src_tile, dst_tile)
...
def _make_expected(src, valid_shape, dst_shape):
    ...
    golden[:1, :valid_cols] = np.max(src[:valid_rows, :valid_cols], axis=0, keepdims=True)
    return golden
