...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trowexpandsub.
#
# pto.trowexpandsub: dst = src0 - broadcast(src1) across columns.  src1 is a
# per-row vector kept in a physically aligned tile (32/sizeof(dtype) columns,
# e.g. 8 for f32); the TileLib template loads src1[row, :] and vdup's the
# LOWEST lane, so only the first column of src1 takes part
# (dst = src0 - src1[:, 0:1]), exactly matching the legacy golden.  src1 is
# fully valid at its physical width (the legacy src1Col=1 was a launcher
# template parameter, not a tile valid_shape; pto.trowexpandsub requires
# row-major src1 valid_shape[1] == 32/sizeof(dtype)).  Fully valid src0/dst
# tiles in PTODSL auto mode; tile addresses, partitions and sync are left to
# PTOAS.
...
# src1 is fully valid at its physical width; semantically only the first
# column is used (src1Col=1 in the legacy launcher).  src0 and dst are fully
# valid at their shape (legacy src0eqdst true/false cases both have
# src0_shape == dst_shape).
CASE_SPECS = [
    ("f32_8x128",      pto.f32, (8, 128),   (8, 8),   1e-6),
    ("f32_24x32",      pto.f32, (24, 32),   (24, 8),  1e-6),
    ("f16_16x256",     pto.f16, (16, 256),  (16, 16), 1e-3),
    ("f16_32x64",      pto.f16, (32, 64),   (32, 16), 1e-3),
    ("f32_16x128_noeq", pto.f32, (16, 128), (16, 8),  1e-6),
    ("i32_16x32",      pto.i32, (16, 32),   (16, 8),  0),
    ("i16_16x64",      pto.i16, (16, 64),   (16, 16), 0),
]


def _trowexpandsub_body(src0_ptr, src1_ptr, dst_ptr, *, rows, cols, src1_cols, dtype):
    ...
    # src1 must stay fully valid at its physical width (32/sizeof(dtype) cols):
    # pto.trowexpandsub requires row-major src1 valid_shape[1] == 32/sizeof(dtype).
    ...
    pto.tile.rowexpandsub(src0_tile, src1_tile, dst_tile)
    pto.tile.store(dst_tile, dst_view)


# One decorated kernel per case, each binding a static shape at definition time
# (mirroring the per-case funcs in trowexpandsub.pto).
_trowexpandsub_kernels = {}
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    _r, _c = _shape
    _src1_c = _src1_shape[1]

    def _make(r=_r, c=_c, src1_c=_src1_c, dtype=_dtype, kernel_name=f"trowexpandsub_{_name}"):
    ...
    _trowexpandsub_kernels[_name] = _make()
...
def _make_expected(src0, src1):
    # dst = src0 - broadcast(src1[:, 0]) across columns (src1Col=1).
    return (src0 - src1[:, 0:1]).astype(src0.dtype)
...
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    CASES.append(
        golden_output_case(
            "trowexpandsub_" + _name,
            _trowexpandsub_kernels[_name],
