...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trowexpandmin.
#
# pto.trowexpandmin: dst = min(src0, broadcast(src1)) across columns.  src1 is
...
# (dst = min(src0, src1[:, 0:1])), exactly matching the legacy golden.  src1 is
...
def _trowexpandmin_body(src0_ptr, src1_ptr, dst_ptr, *, rows, cols, src1_cols, dtype):
    ...
    pto.tile.rowexpandmin(src0_tile, src1_tile, dst_tile)
    pto.tile.store(dst_tile, dst_view)


# One decorated kernel per case, each binding a static shape at definition time
# (mirroring the per-case funcs in trowexpandmin.pto).
_trowexpandmin_kernels = {}
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    _r, _c = _shape
    _src1_c = _src1_shape[1]

    def _make(r=_r, c=_c, src1_c=_src1_c, dtype=_dtype, kernel_name=f"trowexpandmin_{_name}"):
    ...
    _trowexpandmin_kernels[_name] = _make()
...
def _make_expected(src0, src1):
    # dst = min(src0, broadcast(src1[:, 0])) across columns (src1Col=1).
    return np.minimum(src0, src1[:, 0:1]).astype(src0.dtype)
...
for _name, _dtype, _shape, _src1_shape, _eps in CASE_SPECS:
    CASES.append(
        golden_output_case(
            "trowexpandmin_" + _name,
            _trowexpandmin_kernels[_name],
