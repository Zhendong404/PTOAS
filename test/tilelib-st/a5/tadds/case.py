...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tadds.
#
# pto.tadds: dst = src + scalar.  Fully-valid f32/f16/i32/i16 tiles in PTODSL
...
# Scalar added to every element.  Matches gen_data.py SCALAR and launch.cpp
# (the f16 launch scalar 0x4200 is exactly 3.0 in half precision).  Integer
# tiles receive the integer literal 3 so the constant materializes exactly.
SCALAR = 3.0


def _tadds_body(src_ptr, dst_ptr, *, rows, cols, dtype, scalar):
    ...
    pto.tile.adds(src_tile, scalar, dst_tile)
...
for _name, _dtype, _shape, _eps in CASE_SPECS:
    ...
    def _make(r=_r, c=_c, dtype=_dtype, scalar=_scalar, kernel_name=f"tadds_{_name}"):
        @pto.jit(name=kernel_name, target="a5")
        def _kernel(src_ptr: pto.ptr(dtype, "gm"), dst_ptr: pto.ptr(dtype, "gm")):
            _tadds_body(src_ptr, dst_ptr, rows=r, cols=c, dtype=dtype, scalar=scalar)

        return _kernel

    _tadds_kernels[_name] = _make()
