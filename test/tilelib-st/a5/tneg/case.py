...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/tneg.
#
# pto.tneg: dst = -src.  Fully-valid f32/f16/i32/i16 vec tiles in PTODSL auto
# mode; tile addresses, partitions and sync are left to PTOAS.  Every legacy
# case is fully valid (valid_shape == shape), so each case allocates a full 2D
# vec tile matching the legacy .pto tload(tneg)->tstore sequence.
...
CASE_SPECS = [
    ...
    ("i32_32x32", pto.i32, (32, 32), 0),
    ("i16_64x16", pto.i16, (64, 16), 0),
]


def _tneg_body(src_ptr, dst_ptr, *, rows, cols, dtype):
    ...
    pto.tile.neg(src_tile, dst_tile)
...
for _name, _dtype, _shape, _eps in CASE_SPECS:
    ...
    def _make(r=_r, c=_c, dtype=_dtype, kernel_name=f"tneg_{_name}"):
        @pto.jit(name=kernel_name, target="a5")
        def _kernel(src_ptr: pto.ptr(dtype, "gm"), dst_ptr: pto.ptr(dtype, "gm")):
            _tneg_body(src_ptr, dst_ptr, rows=r, cols=c, dtype=dtype)

        return _kernel

    _tneg_kernels[_name] = _make()
...
def _make_expected(src, dtype):
    ...
    return np.negative(src).astype(np_dtype)
