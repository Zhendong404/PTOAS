...
authoring conventionally uses `signed`. `pto.remi` uses the same signedness
clause, while `pto.remf` is the floating-point remainder operation.
...
- **Integer semantics:** the required `signed` or `unsigned` clause selects the
  ordering.
- **Index semantics:** `index` values use their normal ordered numeric
  comparison.
...
- **Semantics:** Integer and `index` operands require `signed` or `unsigned`.
  Signed values produce their absolute value; unsigned values are unchanged.
...
- **Cross-category conversion:** The required signedness clause selects signed
  ...
  integer conversion does not saturate; values outside the destination range
  and NaNs follow the ordinary numeric conversion contract. PTODSL authors
...
Floating truncation accepts `to_nearest_even`, `downward`, `upward`,
`toward_zero`, or `to_nearest_away`; omitting it uses the default rounding
mode. Floating extension/truncation may carry `fastmath<...>`.
