...
- **设计预留插点**：
  - `PTOPostFusionLoopUnroll` 计划插在 `PTOFusionLoadStoreElision` 与 `PTOFlattenFusionRegion` 之间；当前仓库主线尚未把该 pass 接到 `tools/ptoas/ptoas.cpp`。
- **非目标路径**：
...
```text
    ...
    5.11 融合区域内 Load/Store 消除
         (PTOFusionLoadStoreElision)
               ...
               ▼
    5.12 融合后循环展开（预留插点）
         (PTOPostFusionLoopUnroll)
               │
               ▼
    5.13 展平区域包装 (PTOFlattenFusionRegion)
               │
               ▼
    5.14 A5VM backend 发射
...
- **输入契约**：处理对象是 `pto.fusion_region` 内的 A5VM post-lowering loop body；在当前源/中间 IR 层，向量区间由 `pto.vecscope` 表示。
- **核心行为**：
...
### 5.12 融合后循环展开 (PTOPostFusionLoopUnroll，planned)

**设计动机 (Motivation)**
`PTOFusionLoadStoreElision` 之后，fusion-region 内常已收敛成单个较短的 A5VM carrier loop。若 loop body 只剩少量 `a5vm.*` 指令，硬件取指、loop branch 和乱序窗口利用率都会偏低。该 pass 的目标是在不改变 fusion-region 外部语义的前提下，针对 backend-ready loop 做保守 unroll，用更长的 steady-state body 提升 issue density，并尽量放大寄存器直传带来的收益。

**流水线位置 (Pipeline Position)**

- **Pre-conditions**：这是 `PTOFusionLoadStoreElision` 之后、`PTOFlattenFusionRegion` 之前的预留插点；处理对象仍是保留在 `pto.fusion_region` 内的 `scf.for + a5vm.*` 低层循环。
- **Post-conditions**：候选 loop 可能被改写成“unrolled 主循环 + 等价 tail 处理”的形式；`pto.fusion_region`、`pto.yield` 和后端发射依赖的低层边界保持有效。

**核心逻辑与约束 (Logic & Constraints)**

- **候选 loop 选择**：
  - 只考虑 fusion-region 内 header 已稳定的硬件前端 `scf.for`，不跨复杂 `scf.if` 或多出口控制流做激进结构改写。
  - 优先处理 body 过短、且已经通过 load/store 消除压缩出较干净数据通路的 carrier loop。
- **unroll 因子自动决策**：
  - 因子由 pass 自动选择，而不是由 tile fusion 前半段硬编码传入。
  - 决策信号至少包括：loop body 指令数、trip count 可证性、live value 数量、潜在寄存器压力，以及 tail/predicate 处理是否会被放大得过于昂贵。
  - 设计上优先采用小而稳的因子集合（如 `2/4`），避免为了增大 body 规模而破坏 A5 backend 的寄存器预算。
- **静态 / 动态 valid shape 支持**：
  - 当 `v_row` / `v_col` 或等价 trip count 可在编译期证明时，优先选择能整除主迭代空间的因子；若 trip count 很小且完全可证，可退化为小规模 full unroll。
  - 对动态 valid shape，不要求 trip count 在编译期常量化；只要上界和 tail 语义能在 IR 中显式表示，就允许做部分 unroll。
  - 当无法证明整除时，必须保留等价 tail 处理，通常体现为残余 loop 或沿用现有谓词机制，不能依赖“动态 shape 恰好整除”的隐式假设。
- **与前后 pass 的契约**：
  - 不重新引入中间 `store/load`，也不破坏 `PTOFusionPredicateElision` 已建立的谓词复用关系。
  - unroll 过程中新增的 index 计算、重复常量和死代码，预期继续交给后续 `Canonicalizer/CSE` 或 flatten 之后的常规清理去压缩。
- **现实边界**：
  - 若 unroll 会明显放大寄存器压力、地址计算复杂度或 tail 逻辑成本，则应保守放弃。
  - 当前目标是 backend-side 局部增益，不在此阶段引入跨 fusion-region 的软件流水、strip-mining 或更激进的 loop reshaping。

### 5.13 融合区域展平 (PTOFlattenFusionRegion)
...
- **Pre-conditions**：`PTOFusionLoadStoreElision` 已完成；若启用了 `PTOPostFusionLoopUnroll`，则其也已完成。此时 region 内剩余 op 已经是最终 backend-ready 形式。
...
### 5.14 A5VM backend 发射 (A5VM text / LLVM emission)
...
- `pto.vecscope` 是当前源/中间 IR 中的向量区间标记。
...
- **backend-side auto-unroll 预留点**：`PTOPostFusionLoopUnroll` 的目标是在 flatten 前基于 loop body 密度、trip count 与寄存器压力做保守自动展开；当前仍属于设计插点。
...
- **backend emitter 契约**：前面 pass 需要保持 `pto.vecscope` 等结构化边界不被破坏，否则后端发射会直接受影响。
- **unroll 与 backend 预算**：若后续接入 `PTOPostFusionLoopUnroll`，其决策必须同时服从寄存器压力、谓词 tail 复杂度和 A5VM 发射器可接受的低层结构边界。
...
- **结构指标**：
  ...
  - `pto.vecscope` 是否仍在源/中间 IR 中保持清晰边界。
- **低层指标**：
  - `a5vm.plt_*` 冗余是否下降。
  - region 内 `vlds/vsts` round-trip 是否减少。
  - 若启用 `PTOPostFusionLoopUnroll`，则同时观察 hardware-facing loop 的 body 指令数、unroll 因子和 tail 比例是否合理。
- **端到端样例**：
...
- **当前已落地边界**：
  ...
  - `PTOPostFusionLoopUnroll` 目前仍是设计中的 backend-side 预留插点，尚未接入当前默认 pipeline。
- **后续可扩展方向**：
