  ...
  func.func private @__pto_oplib_variant_tcolexpand_linear_bf16(
    ...
    pto.simd.vec_scope {
      ...
      scf.for %j = %c0 to %repeatTimes step %c1 {
        ...
        %lhs = vector.maskedload %m0[%c0, %cidx], %mask, %passive {pto.simd.vld_dist = "NORM"} : memref<?x?xbf16, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<128xi1>, vector<128xbf16> into vector<128xbf16>
  ...
  func.func private @__pto_oplib_variant_tcolexpand_linear_f16(
    ...
    pto.simd.vec_scope {
      ...
      scf.for %j = %c0 to %repeatTimes step %c1 {
        ...
        %active = arith.select %lt, %remain, %cLanes : index
        %mask = vector.create_mask %active : vector<128xi1>
        %lhs = vector.maskedload %m0[%c0, %cidx], %mask, %passive {pto.simd.vld_dist = "NORM"} : memref<?x?xf16, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<128xi1>, vector<128xf16> into vector<128xf16>
        scf.for %r = %c0 to %rows step %c1 {
  ...
  func.func private @__pto_oplib_variant_tcolexpand_linear_i16(
    ...
    pto.simd.vec_scope {
      ...
      scf.for %j = %c0 to %repeatTimes step %c1 {
        ...
        %active = arith.select %lt, %remain, %cLanes : index
        %mask = vector.create_mask %active : vector<128xi1>
        %lhs = vector.maskedload %m0[%c0, %cidx], %mask, %passive {pto.simd.vld_dist = "NORM"} : memref<?x?xi16, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<128xi1>, vector<128xi16> into vector<128xi16>
        scf.for %r = %c0 to %rows step %c1 {
  ...
  func.func private @__pto_oplib_variant_tcolexpand_linear_i8(
    ...
    pto.simd.vec_scope {
      ...
      scf.for %j = %c0 to %repeatTimes step %c1 {
        ...
        %lhs = vector.maskedload %m0[%c0, %cidx], %mask, %passive {pto.simd.vld_dist = "NORM"} : memref<?x?xi8, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<256xi1>, vector<256xi8> into vector<256xi8>
        scf.for %r = %c0 to %rows step %c1 {
