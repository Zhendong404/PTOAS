...
module {
    ...
    pto.simd.vec_scope {
      ...
      scf.for %r = %c0 to %rows step %c1 {
        scf.for %j = %c0 to %repeatTimes step %c1 {
          ...
          %result = arith.addf %scalarVec, %passive {pto.simd.exec_mode = "MODE_ZEROING"} : vector<128xbf16>
    ...
    pto.simd.vec_scope {
      ...
      scf.for %r = %c0 to %rows step %c1 {
        scf.for %j = %c0 to %repeatTimes step %c1 {
          ...
          %active = arith.select %lt, %remain, %cLanes : index
          %mask = vector.create_mask %active : vector<128xi1>
          %result = arith.addf %scalarVec, %passive {pto.simd.exec_mode = "MODE_ZEROING"} : vector<128xf16>
    ...
    pto.simd.vec_scope {
      ...
      scf.for %r = %c0 to %rows step %c1 {
        scf.for %j = %c0 to %repeatTimes step %c1 {
          ...
          %active = arith.select %lt, %remain, %cLanes : index
          %mask = vector.create_mask %active : vector<128xi1>
          %result = arith.addi %scalarVec, %passive : vector<128xi16>
  ...
  func.func private @__pto_oplib_variant_texpands_scalar_i8(
    ...
    pto.simd.vec_scope {
      ...
      %scalarVec = vector.splat %scalar : vector<256xi8>
      scf.for %r = %c0 to %rows step %c1 {
        scf.for %j = %c0 to %repeatTimes step %c1 {
          %cidx = arith.muli %j, %cLanes : index
          %remain = arith.subi %cols, %cidx : index
          ...
          %result = arith.addi %scalarVec, %passive : vector<256xi8>
