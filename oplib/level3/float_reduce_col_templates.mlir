module {
  func.func private @__pto_oplib_variant_tcolmax_f32(
    ...
    pto.simd.vec_scope {
      ...
      scf.for %cidx = %c0 to %cols step %c64 {
        ...
        vector.maskedstore %md[%c0, %cidx], %mask, %negInfVec {pto.simd.vst_dist = "DIST_NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32>
        scf.for %r = %c0 to %rows step %c1 {
          %acc = vector.maskedload %md[%c0, %cidx], %mask, %negInfVec {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32> into vector<32xf32>
          ...
          %next = arith.maximumf %acc, %a {pto.simd.exec_mode = "MODE_ZEROING"} : vector<32xf32>
  ...
  func.func private @__pto_oplib_variant_tcolmin_f32(
      %src: !pto.tile_buf<loc=vec, dtype=f32, rows=32, cols=32, v_row=?, v_col=?, blayout=row_major, slayout=none_box, fractal=512, pad=0>,
      %dst: !pto.tile_buf<loc=vec, dtype=f32, rows=32, cols=32, v_row=?, v_col=?, blayout=row_major, slayout=none_box, fractal=512, pad=0>)
      attributes {
        pto.oplib.kind = "l3_reduce_col_template",
    ...
    pto.simd.vec_scope {
      ...
      scf.for %cidx = %c0 to %cols step %c64 {
        ...
        %active = arith.select %lt, %remain, %c64 : index
        %mask = vector.create_mask %active : vector<32xi1>
        vector.maskedstore %md[%c0, %cidx], %mask, %posInfVec {pto.simd.vst_dist = "DIST_NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32>
        scf.for %r = %c0 to %rows step %c1 {
          %acc = vector.maskedload %md[%c0, %cidx], %mask, %posInfVec {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32> into vector<32xf32>
          ...
          %next = arith.minimumf %acc, %a {pto.simd.exec_mode = "MODE_ZEROING"} : vector<32xf32>
          vector.maskedstore %md[%c0, %cidx], %mask, %next {pto.simd.vst_dist = "DIST_NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32>
        }
      }
    }
