  ...
  func.func private @__pto_oplib_seed_l3_reduce_col_core_f32(
    ...
    pto.simd.vec_scope {
      ...
      scf.for %cidx = %c0 to %cols step %c64 {
        ...
        %seed = vector.maskedload %m0[%c0, %cidx], %mask, %zeroVec {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32> into vector<32xf32>
        vector.maskedstore %md[%c0, %cidx], %mask, %seed {pto.simd.vst_dist = "DIST_NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32>
        scf.for %r = %c1 to %rows step %c1 {
          %acc = vector.maskedload %md[%c0, %cidx], %mask, %seed {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32> into vector<32xf32>
          %a = vector.maskedload %m0[%r, %cidx], %mask, %seed {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<32xi1>, vector<32xf32> into vector<32xf32>
          %next = arith.maximumf %acc, %a {pto.simd.core_slot = "reduce_col_core", pto.simd.exec_mode = "MODE_ZEROING"} : vector<32xf32>
