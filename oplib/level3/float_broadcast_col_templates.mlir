module {
  func.func private @__pto_oplib_variant_tcolexpand_f32(
    ...
    pto.simd.vec_scope {
      ...
      scf.for %cidx = %c0 to %cols step %c64 {
        ...
        %colVec = vector.maskedload %m0[%c0, %cidx], %mask, %zeroVec {pto.simd.vld_dist = "NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<64xi1>, vector<64xf32> into vector<64xf32>
        scf.for %r = %c0 to %rows step %c1 {
          vector.maskedstore %md[%r, %cidx], %mask, %colVec {pto.simd.vst_dist = "DIST_NORM"} : memref<?x?xf32, strided<[32, 1], offset: 0>, #pto.address_space<vec>>, vector<64xi1>, vector<64xf32>
        }
      }
    }
    return
