module {
  func.func private @__pto_oplib_variant_tlrelu_f32(
      ...
      attributes {
        pto.oplib.kind = "l3_float_unary_scalar_template",
    ...
    pto.simd.vec_scope {
      scf.for %r = %c0 to %rows step %c1 {
        scf.for %cidx = %c0 to %cols step %c64 {
          ...
          %pred = arith.cmpf ogt, %a, %zero : vector<32xf32>
          %scaled = arith.mulf %scalarVec, %a {pto.simd.exec_mode = "MODE_ZEROING"} : vector<32xf32>
          %d = arith.select %pred, %a, %scaled : vector<32xi1>, vector<32xf32>
