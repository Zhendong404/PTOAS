...
static LogicalResult verifyFrontendTensorEntryMatchesInit(Operation* op, int32_t id, Type entryTy)
{
    ...
    if (!gmSlotTensor) {
        return op->emitOpError() << "expects 'id' = " << id
    ...
    if (slotTensorTy.getElementType() != entryViewTy.getElementType()) {
        return op->emitOpError() << "expects pipe entry element type to match gm_slot_tensor element type";
    }
    if (slotTensorTy.getRank() != entryViewTy.getRank()) {
        return op->emitOpError() << "expects pipe entry rank to match gm_slot_tensor rank";
    }
    ...
    for (auto [idx, entryDim] : llvm::enumerate(entryShape)) {
        ...
        if (slotDim == ShapedType::kDynamic || entryDim == ShapedType::kDynamic || slotDim == entryDim)
            continue;
        return op->emitOpError() << "expects pipe entry dimension " << idx << " to match gm_slot_tensor dimension "
                                 << slotDim;
    }
    return success();
}
...
static LogicalResult verifyFrontendPopOp(
    FrontendPopOpT op, FunctionKernelKind expected, StringRef kernelName, bool expectC2V)
{
    if (failed(verifyFrontendSplitOp(op.getOperation(), expected, kernelName, op.getId(), op.getSplit())))
        return failure();
    if (failed(verifyFrontendDataOpDirection(op.getOperation(), op.getId(), expectC2V)))
        return failure();
    if (failed(verifyFrontendTensorEntryMatchesInit(op.getOperation(), op.getId(), op.getTile().getType())))
        return failure();
    ...
    if (hasValidRow != hasValidCol)
        return op.emitOpError("expects valid_row and valid_col operands to be provided together");
    if (!hasValidRow)
        return success();

    if (isa<TensorViewType>(op.getTile().getType()))
        return op.emitOpError("does not accept valid_row/valid_col when result is !pto.tensor_view");

    auto tileTy = dyn_cast<TileBufType>(op.getTile().getType());
    if (!tileTy)
        return op.emitOpError("expects tile result to be !pto.tile_buf when valid_row/valid_col operands are provided");
...
static LogicalResult verifyPipeShape(
    Operation* op, int8_t dirMask, int32_t slotSize, int32_t slotNum, std::optional<int32_t> flagBase)
{
    ...
    if (dirMask != kPTOFrontendDirMaskC2V && dirMask != kPTOFrontendDirMaskV2C &&
    ...
    if (slotNum != kPTOFrontendMinSlotNum && slotNum != kPTOFrontendMaxSlotNum)
    ...
    if (flagBase) {
        int32_t flagWidth = dirMask == kPTOFrontendDirMaskBidirectional ? kPTOFrontendBidirectionalFlagWidth :
                                                                          kPTOFrontendUnidirectionalFlagWidth;
        if (*flagBase + flagWidth > kMaxHardwareFlagIds) {
            return op->emitOpError() << "requires 'flag_base' and dir_mask to fit within " << kMaxHardwareFlagIds
                                     << " hardware flag ids";
        }
    }
...
static LogicalResult verifyPipeHandleProducer(Operation* op, Value pipeHandle)
{
    ...
    if (!pipeHandle.getDefiningOp<InitializeL2LPipeOp>() && !pipeHandle.getDefiningOp<InitializeL2G2LPipeOp>()) {
        return op->emitOpError("pipe_handle must be produced by pto.initialize_l2l_pipe or "
                               "pto.initialize_l2g2l_pipe");
    }
    return success();
}

static bool getTensorLikeElementAndShape(Type ty, Type& elementType, ArrayRef<int64_t>& shape)
{
...
static FailureOr<InitializeL2G2LPipeOp> verifyTensorEntryInternalPipeHandle(Operation* op, Value pipeHandle)
{
    auto initOp = pipeHandle.getDefiningOp<InitializeL2G2LPipeOp>();
    if (!initOp) {
        op->emitOpError() << "expects !pto.tensor_view pipe entry to use a pipe produced by "
                             "pto.initialize_l2g2l_pipe";
        return failure();
    }
    if (initOp.getLocalAddr()) {
        op->emitOpError() << "expects !pto.tensor_view pipe entry to use global-only "
...
static LogicalResult verifyTensorEntrySlotType(Operation* op, TensorViewType entryViewTy, InitializeL2G2LPipeOp initOp)
{
    Type slotElementType;
    ArrayRef<int64_t> slotShape;
    if (!getTensorLikeElementAndShape(initOp.getGmAddr().getType(), slotElementType, slotShape)) {
        return op->emitOpError() << "expects !pto.tensor_view pipe entry to use "
                                    "pto.initialize_l2g2l_pipe gm_addr with tensor/memref slot type";
    }
    if (slotElementType != entryViewTy.getElementType()) {
        return op->emitOpError() << "expects pipe entry element type to match initialize_l2g2l_pipe "
                                    "gm_addr element type";
    }
    if (slotShape.size() != static_cast<size_t>(entryViewTy.getRank())) {
        return op->emitOpError() << "expects pipe entry rank to match initialize_l2g2l_pipe gm_addr "
                                    "rank";
    }
    ...
    for (auto [idx, entryDim] : llvm::enumerate(entryShape)) {
        ...
        return op->emitOpError() << "expects pipe entry dimension " << idx
