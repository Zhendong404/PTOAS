...
static LogicalResult verifyTensorEntryByteSize(Operation* op, TensorViewType entryViewTy, InitializeL2G2LPipeOp initOp)
{
    ...
    bool byteSizeMatches = entryBytes == slotBytes || (isSplitEntry && entryBytes * 2 == slotBytes);
    if (!byteSizeMatches) {
        return op->emitOpError() << "expects pipe entry byte size to match initialize_l2g2l_pipe "
                                    "slot_size"
                                 << (isSplitEntry ? " or half slot_size for split entries" : "")
                                 << " (got entry byte size = " << entryBytes << ", slot_size = " << initOp.getSlotSize()
                                 << ")";
    }
    return success();
}

static LogicalResult verifyTensorEntryMatchesInternalPipeInit(Operation* op, Value pipeHandle, Type entryTy)
{
    ...
    if (failed(initOp) || failed(verifyTensorEntrySlotType(op, entryViewTy, *initOp)) ||
...
static LogicalResult verifyAsyncSessionAttrs(BuildAsyncSessionOp op)
{
    ...
    if (auto syncIdAttr = op.getSyncIdAttr()) {
        ...
        if (syncId < kPTOAsyncSessionMinSyncId || syncId > kPTOAsyncSessionMaxSyncId)
...
static LogicalResult verifyCommTransferWithStaging(Operation* op, Value dst, Value src, Value ping, Value pong)
{
    if (shouldBypassDecodedMemrefVerifier(op))
        return success();
    if (failed(verifyCommGlobalLike(op, dst, "dst")) || failed(verifyCommGlobalLike(op, src, "src")) ||
...
static LogicalResult verifyRootedCommTileTransfer(
    Operation* op, Value src, OperandRange group, uint32_t root, Value ping, Value pong)
{
    if (shouldBypassDecodedMemrefVerifier(op))
        return success();
    if (failed(verifyCommGlobalLike(op, src, "src")) || failed(verifyCommStagingTileLike(op, ping, "ping")) ||
        failed(verifyCommPingPongSameType(op, ping, pong, "ping", "pong")) ||
...
static LogicalResult verifyFrontendPipeTileAccess(Operation* op, Value pipeHandle, int64_t split, Type tileTy)
{
    if (!isInsideSectionOrAttributedKernel(op))
        return op->emitOpError("must be inside pto.section.cube/vector or a kernel_kind function");
...
static void addOptionalPongWriteEffect(
    SmallVectorImpl<SideEffects::EffectInstance<MemoryEffects::Effect>>& effects, PongRangeT pongRange)
{
    if (auto it = pongRange.begin(); it != pongRange.end())
        addEffect(effects, &*it, MemoryEffects::Write::get());
}

LogicalResult TPutAsyncOp::verify() { return verifyAsyncTransferOp(getOperation(), getDst(), getSrc()); }

LogicalResult TGetAsyncOp::verify() { return verifyAsyncTransferOp(getOperation(), getDst(), getSrc()); }
