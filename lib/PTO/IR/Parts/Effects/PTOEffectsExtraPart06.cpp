...
{
    return verifyCommTransferWithStaging(getOperation(), getDst(), getSrc(), getPing(), getPong());
}
...
static LogicalResult verifySyncAllGmWorkspace(Operation* op, Value workspace, StringRef name)
{
    ...
    if (auto memTy = dyn_cast<MemRefType>(ty)) {
        ...
        if (!isGmAddressSpaceAttr(memTy.getMemorySpace()))
            return op->emitOpError() << "expects " << name << " to be in GM address space";
    }
...
static LogicalResult verifySyncAllTileWorkspace(
    Operation* op, Value workspace, StringRef name, pto::AddressSpace expectedSpace)
{
    Type ty = workspace.getType();
    if (!isa<pto::TileBufType, MemRefType>(ty))
        return op->emitOpError() << "expects " << name << " to be tile_buf or memref type";
    ...
    if (!as || *as != expectedSpace)
        return op->emitOpError() << "expects " << name << " to be in "
                                 << (expectedSpace == pto::AddressSpace::VEC ? "vec" : "mat") << " address space";
    ...
    if (shape.empty() || shape.size() > kPTORowColRank)
        return op->emitOpError() << "expects " << name << " to be rank-1 or rank-2";
    for (int64_t dim : shape) {
        if (dim != ShapedType::kDynamic && dim <= 0)
            return op->emitOpError() << "expects " << name << " shape to be positive";
    }
...
static LogicalResult verifySyncAllHardMode(SyncAllOp op, bool hasGm, bool hasUb, bool hasL1)
{
    if (hasGm || hasUb || hasL1 || op.getUsedCores()) {
        return op.emitOpError("expects hard syncall to have no workspace operands or used_cores");
    }
    return success();
}
...
static LogicalResult verifySyncAllSoftWorkspaces(SyncAllOp op, bool hasUb, bool hasL1)
{
    switch (op.getCoreType().getValue()) {
        case pto::SyncCoreType::AIVOnly:
            if (!hasUb || hasL1) {
                return op.emitOpError("expects soft AIV-only syncall to use gm_workspace + ub_workspace only");
            }
            return verifySyncAllTileWorkspace(
                op.getOperation(), op.getUbWorkspace(), "ub_workspace", pto::AddressSpace::VEC);
        case pto::SyncCoreType::AICOnly:
            if (hasUb || !hasL1) {
                return op.emitOpError("expects soft AIC-only syncall to use gm_workspace + l1_workspace only");
            }
            return verifySyncAllTileWorkspace(
                op.getOperation(), op.getL1Workspace(), "l1_workspace", pto::AddressSpace::MAT);
        case pto::SyncCoreType::Mix:
            if (!hasUb || !hasL1) {
                return op.emitOpError("expects soft mixed syncall to use gm_workspace + ub_workspace + l1_workspace");
            }
            if (failed(verifySyncAllTileWorkspace(
                    op.getOperation(), op.getUbWorkspace(), "ub_workspace", pto::AddressSpace::VEC))) {
                return failure();
            }
            return verifySyncAllTileWorkspace(
...
LogicalResult SyncAllOp::verify()
{
    ...
    if (failed(verifySyncAllGmWorkspace(getOperation(), getGmWorkspace(), "gm_workspace")) ||
        failed(verifySyncAllUsedCores(*this)) || failed(verifySyncAllSoftWorkspaces(*this, hasUb, hasL1)))
        return failure();
    return success();
}
...
{
    if (failed(verifyRootedCommTileTransfer(getOperation(), getSrc(), getGroup(), getRoot(), getPing(), getPong())))
        return failure();
...
LogicalResult CommTGatherOp::verify()
{
    ...
    if (failed(verifyCommGlobalLike(*this, getDst(), "dst")) ||
        ...
        failed(verifyCommPingPongSameType(*this, getPing(), getPong(), "ping", "pong")) ||
