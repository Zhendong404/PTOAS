...
LogicalResult TReduceOp::verify()
{
    ...
    if (failed(verifyCommGlobalLike(*this, getDst(), "dst")) ||
        ...
        failed(verifyCommPingPongSameType(*this, getRecvPing(), getRecvPong(), "recv_ping", "recv_pong")) ||
...
LogicalResult mlir::pto::detail::verifyFrontendInitPipeTrait(Operation* op)
{
    ...
    if (auto initOp = dyn_cast<AivInitializePipeOp>(op)) {
        return verifyFrontendInitCommon(initOp, FunctionKernelKind::Vector, "vector");
    }
...
{
    if (failed(verifyFrontendSplitOp(getOperation(), FunctionKernelKind::Cube, "cube", getId(), getSplit())))
        return failure();
    if (failed(verifyFrontendDataOpDirection(
            getOperation(), getId(),
            /*expectC2V=*/true)))
        return failure();
    return verifyFrontendTensorEntryMatchesInit(getOperation(), getId(), getEntry().getType());
}

LogicalResult TAllocToAicOp::verify()
{
    if (failed(verifyFrontendSplitOp(getOperation(), FunctionKernelKind::Vector, "vector", getId(), getSplit())))
        return failure();
    if (failed(verifyFrontendDataOpDirection(
            getOperation(), getId(),
            /*expectC2V=*/false)))
...
LogicalResult TPushToAivOp::verify()
{
    ...
    if (failed(verifyFrontendDataOpDirection(
            ...
            /*expectC2V=*/true)))
        return failure();
    return verifyFrontendTensorEntryMatchesInit(getOperation(), getId(), getTile().getType());
}
...
LogicalResult mlir::pto::detail::verifyFrontendPopTrait(Operation* op)
{
    if (auto popOp = dyn_cast<TPopFromAicOp>(op)) {
        return verifyFrontendPopOp(
            popOp, FunctionKernelKind::Vector, "vector",
            /*expectC2V=*/true);
    }
    if (auto popOp = dyn_cast<TPopFromAivOp>(op)) {
        return verifyFrontendPopOp(
            popOp, FunctionKernelKind::Cube, "cube",
            /*expectC2V=*/false);
    }
...
{
    if (failed(verifyPipeShape(
            getOperation(), getDirMask(), getSlotSize(), getSlotNum(),
            getFlagBaseAttr() ? std::optional<int32_t>(getFlagBaseAttr().getInt()) : std::nullopt)))
        return failure();
    ...
    if (!getLocalAddr()) {
        ...
        if (getLocalSlotNumAttr())
            return emitOpError("'local_slot_num' is only allowed when 'local_addr' is present");
        return success();
    }
    ...
    if (auto localSlotNumAttr = getLocalSlotNumAttr()) {
        ...
        if (static_cast<uint32_t>(localSlotNum) > getSlotNum())
            return emitOpError("expects 'local_slot_num' to be less than or equal to slot_num");
    }
...
{
    if (failed(verifyFrontendPipeTileAccess(getOperation(), getPipeHandle(), getSplit(), getTile().getType())))
        return failure();
    if (!isa<TensorViewType>(getTile().getType()) && getPipe() == pto::PIPE::PIPE_UNASSIGNED)
...
LogicalResult TAllocOp::verify()
{
    ...
    if (failed(verifyPipeHandleProducer(getOperation(), getPipeHandle())))
        return failure();
    if (failed(verifyTensorEntryMatchesInternalPipeInit(getOperation(), getPipeHandle(), getEntry().getType())))
        return failure();
...
LogicalResult TPopOp::verify()
{
    if (failed(verifyFrontendPipeTileAccess(getOperation(), getPipeHandle(), getSplit(), getTile().getType())))
        return failure();
    if (!isa<TensorViewType>(getTile().getType()) && getPipe() == pto::PIPE::PIPE_UNASSIGNED)
        return emitOpError("tile type and target arch must map to a supported consumer pipe");
    return success();
}
...
LogicalResult TFreeOp::verify()
{
    ...
    if (failed(verifyPipeHandleProducer(getOperation(), getPipeHandle())))
        return failure();
    if (getEntry() &&
        failed(verifyTensorEntryMatchesInternalPipeInit(getOperation(), getPipeHandle(), getEntry().getType())))
        return failure();
    return verifySplitAttr(getOperation(), getSplit());
}
