...
static LogicalResult verifyFrontendInitCommon(InitOpT op, FunctionKernelKind expected, StringRef kernelName)
{
    ...
    unsigned sameIdInitCount = countFrontendInitOpsWithSameId<InitOpT>(funcOp, op.getId());
    if (sameIdInitCount > 1) {
        return op.emitOpError("requires 'id' to be unique across frontend initialize_pipe ops in the function");
    }
    ...
    if (hasGlobalSlotTensor) {
        return verifyFrontendInitGlobalTensorForm(op, dirMask, hasC2vConsumerBuf, hasV2cConsumerBuf);
    }
    return verifyFrontendInitLocalPipeForm(op, dirMask, hasC2vConsumerBuf, hasV2cConsumerBuf);
}
...
LogicalResult ImportReservedBufferOp::verify()
{
    ...
    auto peerFunc = SymbolTable::lookupNearestSymbolFrom<func::FuncOp>(getOperation(), getPeerFuncAttr());
    if (!peerFunc)
    ...
    funcOp.walk([this, &sameImportCount](ImportReservedBufferOp importOp) {
        if (importOp.getName() == getName() && importOp.getPeerFuncAttr() == getPeerFuncAttr()) {
            ++sameImportCount;
        }
    });
    if (sameImportCount > 1) {
        return emitOpError("requires (name, peer_func) to be unique within the function");
    }
...
static FailureOr<Operation*> lookupFrontendInitOpById(Operation* op, func::FuncOp funcOp, int32_t id)
{
    ...
    if (matchedInitCount == 0) {
        op->emitOpError() << "expects 'id' = " << id << " to match a frontend initialize_pipe op in the same function";
        return failure();
    }
...
static LogicalResult verifyFrontendSplitOp(
    Operation* op, FunctionKernelKind expected, StringRef kernelName, int32_t id, int64_t split)
{
...
static FailureOr<int8_t> lookupFrontendInitDirMaskById(Operation* op, func::FuncOp funcOp, int32_t id)
{
...
static LogicalResult verifyFrontendDataOpDirection(Operation* op, int32_t id, bool expectC2V)
{
    ...
    if (expectC2V && dirMask != kPTOFrontendDirMaskC2V && dirMask != kPTOFrontendDirMaskBidirectional) {
        return op->emitOpError() << "expects 'id' = " << id << " to reference initialize_pipe with dir_mask = 1 or 3";
    }
    if (!expectC2V && dirMask != kPTOFrontendDirMaskV2C && dirMask != kPTOFrontendDirMaskBidirectional) {
