...
{
    if (failed(verifyAsyncFlatContiguous1DGMViewLike(getOperation(), getSrc(), "src")))
        return failure();
    return success();
}
...
ParseResult mlir::pto::SyncSetOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseSyncEventOpCommon(
        parser, result, SyncSetOp::getPipeAttrName(result.name), SyncSetOp::getEventIdAttrName(result.name));
}

void mlir::pto::SyncSetOp::print(OpAsmPrinter& p)
{
    printSyncEventOpCommon(
        p, getOperation(), getPipe(), getEventIdAttr(), getEventIdDyn(), getPipeAttrName().getValue(),
...
LogicalResult mlir::pto::SyncSetOp::verify()
{
    ...
    if (IntegerAttr fftsModeAttr = getFftsModeAttr()) {
        ...
        if (fftsMode < kPTOFftsModeMin || fftsMode > kPTOFftsModeMax)
            return emitOpError() << "requires ffts_mode in range [0, 2], but got " << fftsMode;
    }
    ...
    auto verifyA5 = [this]() -> LogicalResult {
        switch (getPipe().getPipe()) {
            ...
            case PIPE::PIPE_MTE3:
                return success();
            default:
                return emitOpError() << "A5 sync.set expects pipe to be one of <PIPE_FIX>, <PIPE_MTE3>";
        }
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}

ParseResult mlir::pto::SyncWaitOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseSyncEventOpCommon(
        parser, result, SyncWaitOp::getPipeAttrName(result.name), SyncWaitOp::getEventIdAttrName(result.name));
}
...
static ParseResult parseSyncAllOptionalOperands(
    OpAsmParser& parser, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& operands, SmallVectorImpl<Type>& operandTypes)
{
    if (parser.parseLParen())
        return failure();
    if (failed(parser.parseOptionalRParen())) {
        if (parser.parseOperandList(operands) || parser.parseColonTypeList(operandTypes) || parser.parseRParen())
            return failure();
...
static ParseResult parseSyncAllModeAndCoreType(
    OpAsmParser& parser, OperationState& result, SyncAllModeAttr& mode, SyncCoreTypeAttr& coreType)
{
    Attribute modeAttr;
    Attribute coreTypeAttr;
    if (parser.parseKeyword("mode") || parser.parseEqual() || parser.parseAttribute(modeAttr) || parser.parseComma() ||
        parser.parseKeyword("core_type") || parser.parseEqual() || parser.parseAttribute(coreTypeAttr))
        return failure();
    mode = dyn_cast<pto::SyncAllModeAttr>(modeAttr);
    if (!mode)
        return parser.emitError(parser.getCurrentLocation()) << "expects mode to be #pto.sync_all_mode<...>";
    coreType = dyn_cast<pto::SyncCoreTypeAttr>(coreTypeAttr);
    if (!coreType)
        return parser.emitError(parser.getCurrentLocation()) << "expects core_type to be #pto.sync_core_type<...>";
    result.addAttribute("mode", mode);
    result.addAttribute("core_type", coreType);
    return parser.parseOptionalAttrDict(result.attributes);
}

static void addSyncAllSegmentSizes(
    OpAsmParser& parser, OperationState& result, int32_t gm, int32_t ub, int32_t l1, int32_t used)
{
    result.addAttribute("operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr({gm, ub, l1, used}));
}

static ParseResult resolveSyncAllSoftOperands(
    OpAsmParser& parser, OperationState& result, ArrayRef<OpAsmParser::UnresolvedOperand> operands,
    ArrayRef<Type> operandTypes, int32_t gm, int32_t ub, int32_t l1)
{
    ...
    if (hasUsedCores && parser.resolveOperand(operands[required], operandTypes[required], result.operands))
        return failure();
    addSyncAllSegmentSizes(parser, result, gm, ub, l1, hasUsedCores ? 1 : 0);
    return success();
}

ParseResult mlir::pto::SyncAllOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    switch (mode.getValue()) {
        case pto::SyncAllMode::Hard:
            if (!operands.empty())
                return parser.emitError(parser.getCurrentLocation()) << "expects hard syncall to have no operands";
    ...
    switch (coreType.getValue()) {
        case pto::SyncCoreType::AIVOnly:
            ...
            return resolveSyncAllSoftOperands(parser, result, operands, operandTypes, 1, 1, 0);
        case pto::SyncCoreType::AICOnly:
            if (operands.size() != kNumber2 && operands.size() != kNumber3)
                return parser.emitError(parser.getCurrentLocation())
                       << "expects soft AIC-only syncall to have gm_workspace, "
                          "l1_workspace, and optional used_cores";
            return resolveSyncAllSoftOperands(parser, result, operands, operandTypes, 1, 0, 1);
        case pto::SyncCoreType::Mix:
            ...
            return resolveSyncAllSoftOperands(parser, result, operands, operandTypes, 1, 1, 1);
    }
...
void mlir::pto::SyncAllOp::print(OpAsmPrinter& p)
{
    ...
    if (!operands.empty()) {
        ...
        p << " : ";
        llvm::interleaveComma(operands, p, [&p](Value operand) { p.printType(operand.getType()); });
    }
    p << ") mode = " << getMode() << ", core_type = " << getCoreType();
    p.printOptionalAttrDict(
        (*this)->getAttrs(),
        /*elidedAttrs=*/{"operandSegmentSizes", "mode", "core_type"});
}
