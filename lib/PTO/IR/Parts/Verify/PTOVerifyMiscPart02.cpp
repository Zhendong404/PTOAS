...
LogicalResult TMatmulMxOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        return verifyMatmulMxA2A3LikeOp(
            *this, getAScale().getType(), getBScale().getType(), getA().getType(), getB().getType(), getDst().getType(),
            []() -> LogicalResult { return success(); });
    };
    auto verifyA5 = [this, &verifyA2A3]() -> LogicalResult {
        return verifyMatmulMxA5LikeOp(*this, getA().getType(), getB().getType(), getDst().getType(), verifyA2A3);
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
LogicalResult TMatmulMxBiasOp::verify()
{
    auto verifyOperands = [this]() -> LogicalResult {
        if (failed(verifyMatTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
...
LogicalResult TGetValOp::verify()
{
    ...
    Attribute memSpace = isa<pto::TileBufType>(srcTy) ? cast<pto::TileBufType>(srcTy).getMemorySpace() :
                                                        cast<MemRefType>(srcTy).getMemorySpace();
    auto addrSpaceAttr = dyn_cast_or_null<pto::AddressSpaceAttr>(memSpace);
    if (!addrSpaceAttr || addrSpaceAttr.getAddressSpace() != pto::AddressSpace::VEC) {
        if (addrSpaceAttr && addrSpaceAttr.getAddressSpace() == pto::AddressSpace::MAT)
            return emitOpError("Ascend hardware does not support reading from Mat tile_buf to Scalar unit");
...
static LogicalResult verifyTHistogramShapes(THistogramOp op, Type srcTy, Type idxTy, Type dstTy)
{
    ...
    if (srcShape.size() != kPTORowColRank || idxShape.size() != kPTORowColRank || dstShape.size() != kPTORowColRank ||
        ...
        return op.emitOpError("expects src, idx, and dst to have rank-2 shape and valid_shape");
    }
    if (!hasCompatibleKnownExtent(srcShape[0], idxShape[0]) || !hasCompatibleKnownExtent(srcValid[0], idxValid[0])) {
        return op.emitOpError("expects idx rows and valid rows to match src");
    }
    if (!hasCompatibleKnownExtent(srcShape[0], dstShape[0]) || !hasCompatibleKnownExtent(srcValid[0], dstValid[0])) {
...
static LogicalResult verifyTHistogramA5(THistogramOp op)
{
    ...
    if (failed(verifyTileBufCommon(op, srcTy, "src")) || failed(verifyTileBufCommon(op, idxTy, "idx")) ||
    ...
    if (!hasTileBufLayout(idxTB, pto::BLayout::ColMajor, pto::SLayout::NoneBox)) {
        return op.emitOpError("expects idx to use DN layout (col_major + none_box)");
    }
...
{
    auto verifyA2A3 = [this]() -> LogicalResult { return emitOpError("thistogram is only supported on A5"); };
    auto verifyA5 = [this]() -> LogicalResult { return verifyTHistogramA5(*this); };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}

LogicalResult TGetScaleAddrOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult { return emitOpError("tget_scale_addr is only supported on A5"); };
