...
static FailureOr<IndexedTileTransferCommon> verifyIndexedTileTransferCommon(
    Operation* op, Value src, Value dst, Value indexRow, Value indexCol, bool includeIndexAndIntOpsInConstFold,
    bool isInsertOp, bool requireSameElementType)
{
    ...
    if (failed(verifyTileBufCommon(op, srcTy, "src")) || failed(verifyTileBufCommon(op, dstTy, "dst")) ||
        failed(verifyNonNegativeIndexRowCol(*op, indexRow, indexCol, includeIndexAndIntOpsInConstFold)) ||
        failed(verifyBounds(*op, indexRow, indexCol, srcTy, dstTy, includeIndexAndIntOpsInConstFold))) {
        return failure();
    }
    ...
    if (requireSameElementType && (!srcElem || !dstElem || srcElem != dstElem))
        return op->emitOpError("expects src and dst to have the same element type"), failure();

    return IndexedTileTransferCommon{
        srcTy, dstTy, srcTb, dstTb, srcElem, dstElem, getPTOMemorySpaceEnum(srcTy), getPTOMemorySpaceEnum(dstTy)};
}

static LogicalResult verifyTExtractA2A3(const IndexedTileTransferCommon& common, TExtractOp op)
{
    if (!isA2A3ExtractElemType(common.dstElem))
        return op.emitOpError("expects A2/A3 textract element type to be i8/f16/bf16/f32");
    if (common.srcSpace && common.dstSpace && *common.srcSpace == pto::AddressSpace::VEC &&
    ...
    if (!common.dstSpace ||
        (*common.dstSpace != pto::AddressSpace::LEFT && *common.dstSpace != pto::AddressSpace::RIGHT)) {
        return op.emitOpError("expects A2/A3 textract dst to use loc=left, loc=right, or loc=vec");
    }
    ...
    if (*common.dstSpace == pto::AddressSpace::LEFT &&
        !hasTileBufLayout(common.dstTb, pto::BLayout::RowMajor, pto::SLayout::RowMajor)) {
    ...
    if (*common.dstSpace == pto::AddressSpace::RIGHT &&
        !hasTileBufLayout(common.dstTb, pto::BLayout::RowMajor, pto::SLayout::ColMajor)) {
        return op.emitOpError("expects A2/A3 right dst to use row_major blayout and col_major slayout");
    }
    return success();
}

static LogicalResult verifyTExtractA5Pair(const IndexedTileTransferCommon& common, TExtractOp op)
{
    const bool okPair = (*common.srcSpace == pto::AddressSpace::MAT &&
                         (*common.dstSpace == pto::AddressSpace::LEFT || *common.dstSpace == pto::AddressSpace::RIGHT ||
                          *common.dstSpace == pto::AddressSpace::SCALING)) ||
                        (*common.srcSpace == pto::AddressSpace::VEC &&
                         (*common.dstSpace == pto::AddressSpace::MAT || *common.dstSpace == pto::AddressSpace::VEC));
    if (!okPair) {
...
static LogicalResult verifyTExtractA5Layouts(const IndexedTileTransferCommon& common, TExtractOp op)
{
    if (*common.srcSpace == pto::AddressSpace::MAT) {
        ...
        if (*common.dstSpace == pto::AddressSpace::LEFT &&
            !hasTileBufLayout(common.dstTb, pto::BLayout::ColMajor, pto::SLayout::RowMajor)) {
    ...
    if (*common.srcSpace == pto::AddressSpace::VEC && *common.dstSpace == pto::AddressSpace::VEC &&
        (!isRowMajorNoneBoxNDTileBuf(common.srcTb) || !isRowMajorNoneBoxNDTileBuf(common.dstTb))) {
        return op.emitOpError("expects A5 vec->vec textract src/dst to use ND layout "
                              "(blayout=row_major, slayout=none_box)");
    }
    return success();
}

static LogicalResult verifyTExtractA5(const IndexedTileTransferCommon& common, TExtractOp op)
{
    ...
    if (failed(verifyTExtractA5Pair(common, op)) || failed(verifyTExtractA5Layouts(common, op)))
        return failure();
    return success();
}
...
mlir::LogicalResult mlir::pto::TExtractOp::verify()
{
    ...
    auto verifyA2A3 = [this, &common]() -> LogicalResult { return verifyTExtractA2A3(*common, *this); };
    auto verifyA5 = [this, &common]() -> LogicalResult { return verifyTExtractA5(*common, *this); };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}

static LogicalResult verifyTInsertA2A3(const IndexedTileTransferCommon& common, TInsertOp op)
{
    if (common.srcSpace && common.dstSpace && *common.srcSpace == pto::AddressSpace::VEC &&
        *common.dstSpace == pto::AddressSpace::VEC) {
        if (common.srcElem != common.dstElem || !isA2A3VecInsertElemType(common.srcElem)) {
            return op.emitOpError("expects A2/A3 vec->vec tinsert src/dst to have same supported dtype "
                                  "(i8/f16/bf16/f32)");
        }
        return success();
    }
    if (!common.srcSpace || !common.dstSpace || *common.srcSpace != pto::AddressSpace::ACC ||
    ...
    if (!(common.srcElem.isF32() && (common.dstElem.isF16() || common.dstElem.isBF16()))) {
        return op.emitOpError("expects A2/A3 tinsert element types to be src=f32, dst=f16/bf16");
    }
    return success();
}
