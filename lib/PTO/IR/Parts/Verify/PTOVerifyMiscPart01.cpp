...
// ---- GetBufOp / RlsBufOp ----
static LogicalResult verifyBufSyncOp(Operation* op, Attribute opTypeAttr, IntegerAttr bufIdAttr, IntegerAttr modeAttr)
{
    if (!opTypeAttr)
    ...
    if (failed(opTypeOr)) {
        auto diag = op->emitOpError("expects 'op_type' to be pipe_event_type/sync_op_type, got ");
        diag << opTypeAttr;
        return failure();
    }
...
{
    return verifyBufSyncOp(getOperation(), getOpTypeAttr(), getBufIdAttr(), getModeAttr());
}
...
static LogicalResult verifyMatmulBiasLikeOp(
    Operation* op, Type aTy, Type bTy, Type biasTy, Type dstTy, bool useGemvOperands)
{
    if (useGemvOperands) {
    ...
    if (failed(verifyMatBiasTile(op, biasTy, dstTy)))
        return failure();
    if (failed(verifyMatmulTypeTriple(op, getElemTy(aTy), getElemTy(bTy), getElemTy(dstTy))))
        return failure();
    return verifyMatmulLike(op, aTy, bTy, dstTy);
}

template <typename ExtraVerifyFn>
static LogicalResult verifyMatmulMxA2A3LikeOp(
    Operation* op, Type aScaleTy, Type bScaleTy, Type aTy, Type bTy, Type dstTy, ExtraVerifyFn extraVerify)
{
    if (failed(verifyTileBufCommon(op, aScaleTy, "a_scale")) || failed(verifyTileBufCommon(op, bScaleTy, "b_scale")))
        return failure();
...
static LogicalResult verifyMatmulMxA5LikeOp(Operation* op, Type aTy, Type bTy, Type dstTy, VerifyBaseFn verifyBase)
{
...
LogicalResult TGemvBiasOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        return verifyMatmulBiasLikeOp(
            *this, getA().getType(), getB().getType(), getBias().getType(), getDst().getType(),
...
{
    auto verifyA2A3 = [this]() -> LogicalResult { return emitOpError("tgemv.mx is only supported on A5 targets"); };
    auto verifyA5 = [this]() -> LogicalResult {
        if (failed(verifyScaleTileMatchesOperand(*this, getAScale().getType(), getA().getType(), "a_scale", "a")) ||
            failed(verifyScaleTileMatchesOperand(*this, getBScale().getType(), getB().getType(), "b_scale", "b")) ||
            failed(verifyGemvTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())))
            return failure();
        if (failed(
                verifyA5MxTypeTriple(*this, getA().getType(), getB().getType(), getDst().getType(), "a", "b", "dst")))
            return failure();
...
{
    auto verifyA2A3 = [this]() -> LogicalResult { return emitOpError("tgemv.mx.acc is only supported on A5 targets"); };
    auto verifyA5 = [this]() -> LogicalResult {
        if (failed(verifyAccTileCommon(*this, getCIn().getType(), "c_in")) ||
            failed(verifyScaleTileMatchesOperand(*this, getAScale().getType(), getA().getType(), "a_scale", "a")) ||
            failed(verifyScaleTileMatchesOperand(*this, getBScale().getType(), getB().getType(), "b_scale", "b")) ||
            failed(verifyGemvTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())))
            return failure();
        if (failed(
                verifyA5MxTypeTriple(*this, getA().getType(), getB().getType(), getDst().getType(), "a", "b", "dst")))
            return failure();
        if (failed(verifyTileBufSameElemType(*this, getCIn().getType(), getDst().getType(), "c_in", "dst")) ||
            failed(verifyTileBufSameValidShape(*this, getCIn().getType(), getDst().getType(), "c_in", "dst")))
            return failure();
        return verifyMatmulLike(*this, getA().getType(), getB().getType(), getDst().getType());
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
...
LogicalResult TGemvMxBiasOp::verify()
{
    ...
    auto verifyA5 = [this]() -> LogicalResult {
        if (failed(verifyScaleTileMatchesOperand(*this, getAScale().getType(), getA().getType(), "a_scale", "a")) ||
            failed(verifyScaleTileMatchesOperand(*this, getBScale().getType(), getB().getType(), "b_scale", "b")) ||
            failed(verifyGemvTileOperands(*this, getA().getType(), getB().getType(), getDst().getType())) ||
            failed(verifyMatBiasTile(
                *this, getBias().getType(), getDst().getType(),
        ...
        if (biasShape.size() != kPTORowColRank || dstShape.size() != kPTORowColRank)
            return emitOpError("expects bias and dst to be rank-2 for tgemv.mx.bias");
        if (biasShape[1] != ShapedType::kDynamic && dstShape[1] != ShapedType::kDynamic && biasShape[1] != dstShape[1])
            return emitOpError("expects bias and dst to have the same column shape");
        if (failed(verifyTileBufSameValidShape(*this, getBias().getType(), getDst().getType(), "bias", "dst")))
            return failure();
        return verifyMatmulLike(*this, getA().getType(), getB().getType(), getDst().getType());
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
