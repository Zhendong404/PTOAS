...
LogicalResult MScatterOp::verify()
{
    ...
    if (getPTOTypeRank(srcTy) == -1 || getPTOTypeRank(idxTy) == -1 || getPTOTypeRank(memTy) == -1)
    ...
    if (!isSupportedMGatherMScatterPayloadElemType(getOperation(), srcElem))
        return emitOpError("expects src element type to be i8/ui8/i16/ui16/i32/ui32/f16/bf16/f32 "
    ...
    if (failed(verifyMGatherMScatterMemOperand(getOperation(), getMem(), srcElem, "src")))
        return failure();

    if (getScatterAtomicOp() != pto::ScatterAtomicOp::None || getScatterOob() != pto::ScatterOOB::Undefined) {
        if (!isTargetArchA5(getOperation()))
            return emitOpError("expects non-default scatterAtomicOp/scatterOob only on A5 targets");
    }

    if (!isSupportedMScatterAtomicPayloadElemType(srcElem, getScatterAtomicOp()))
        return emitOpError("expects scatterAtomicOp-compatible src element type: add supports "
...
LogicalResult MGatherOp::verify()
{
    ...
    if (getPTOTypeRank(memTy) == -1 || getPTOTypeRank(idxTy) == -1 || getPTOTypeRank(dstTy) == -1)
    ...
    if (!isSupportedMGatherMScatterPayloadElemType(getOperation(), dstElem))
        return emitOpError("expects dst element type to be i8/ui8/i16/ui16/i32/ui32/f16/bf16/f32 "
                           "(and on A5 targets also float8_e4m3/float8_e5m2 family types)");

    if (!isSupportedMGatherMScatterIndexElemType(idxElem))
        return emitOpError("expects idx element type to be signless i32");

    if (failed(verifyMGatherMScatterMemOperand(getOperation(), getMem(), dstElem, "dst")))
        return failure();

    if (getGatherOob() != pto::GatherOOB::Undefined && !isTargetArchA5(getOperation()))
        return emitOpError("expects non-default gatherOob only on A5 targets");
...
ParseResult mlir::pto::TCvtOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (auto satmode = attrs.get("satmode")) {
        ...
        if (attrs.get("sat_mode"))
            return parser.emitError(parser.getCurrentLocation(), "cannot specify both satmode and sat_mode");
        attrs.set("sat_mode", satmode);
    }
    result.attributes = attrs;
    if (parser.parseRParen() || parser.parseKeyword("outs") || parser.parseLParen() || parser.parseOperand(dst) ||
        parser.parseColonType(dstTy) || parser.parseRParen())
        return failure();

    if (parser.resolveOperand(src, srcTy, result.operands) || parser.resolveOperand(dst, dstTy, result.operands))
        return failure();
    return success();
}
...
void mlir::pto::TMrgSortOp::print(OpAsmPrinter& p)
{
    if (isFormat1()) {
        p << " ins(" << getSrc() << ", " << getBlockLen() << " : " << getSrc().getType() << ", "
          << getBlockLen().getType() << ") outs(" << getDst() << " : " << getDst().getType() << ")";
    } else if (isFormat2()) {
        p << " ins(";
        ...
        p << ", " << getTmp().getType();
        p << ") outs(" << getDst() << ", " << getExcuted() << " : " << getDst().getType() << ", "
          << getExcuted().getType() << ")";
    } else {
...
static ParseResult parseTMrgSortFormat1(
    OpAsmParser& parser, OperationState& result, OpAsmParser::UnresolvedOperand first,
    OpAsmParser::UnresolvedOperand second)
{
    Type srcTy, blockLenTy, dstTy;
    if (parser.parseType(srcTy) || parser.parseComma() || parser.parseType(blockLenTy) || parser.parseRParen() ||
        parser.parseKeyword("outs") || parser.parseLParen())
        return failure();
    OpAsmParser::UnresolvedOperand dstOp;
    if (parser.parseOperand(dstOp) || parser.parseColon() || parser.parseType(dstTy) || parser.parseRParen())
        return failure();
    result.addAttribute("operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr({1, 1, 1, 0, 0}));
...
static ParseResult parseTMrgSortFormat2Sources(
    OpAsmParser& parser, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& srcs, OpAsmParser::UnresolvedOperand& tmpOp)
{
    ...
    if (srcs.size() < kNumber3 || srcs.size() > kNumber5) {
        return parser.emitError(
            parser.getCurrentLocation(), "tmrgsort format2 expects 2 to 4 src operands plus one tmp operand");
    }
    tmpOp = srcs.pop_back_val();
    return success();
}

static ParseResult parseTMrgSortFormat2Exhausted(OpAsmParser& parser, bool& exhaustedVal)
{
