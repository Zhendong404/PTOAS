...
LogicalResult mlir::pto::PartitionViewOp::verify()
{
    ...
    if (srcTy.getElementType() != resTy.getElementType()) {
        return emitOpError() << "element type mismatch between source and result: src=" << srcTy.getElementType()
                             << " result=" << resTy.getElementType();
    }

    int64_t srcRank = srcTy.getRank();
    if (static_cast<int64_t>(getOffsets().size()) != srcRank)
        return emitOpError() << "offset count (" << getOffsets().size() << ") must match source rank (" << srcRank
                             << ")";
    if (static_cast<int64_t>(getSizes().size()) != srcRank)
        return emitOpError() << "size count (" << getSizes().size() << ") must match source rank (" << srcRank << ")";

    ArrayRef<int64_t> srcShape = srcTy.getShape();
    ArrayRef<int64_t> resShape = resTy.getShape();
    bool sameRank = resTy.getRank() == srcRank;
    for (int64_t i = 0; i < srcRank; ++i) {
        if (failed(verifyPartitionViewDimension(*this, i, srcShape, resShape, sameRank)))
            return failure();
    }
    return success();
}
...
static LogicalResult verifyPtrLikeForAddressCast(Operation* op, Type type, StringRef name)
{
    ...
    if (!memTy)
        return op->emitOpError() << "expects " << name << " to be !pto.ptr<...> or a GM memref";

    if (memTy.getRank() != 1)
        return op->emitOpError() << "expects lowered memref " << name << " to be rank-1";

    if (!isGmAddressSpaceAttr(memTy.getMemorySpace()))
        return op->emitOpError() << "expects lowered memref " << name << " to use GM address space";
...
LogicalResult mlir::pto::PtrToIntOp::verify()
{
    ...
    return verifyPtrLikeForAddressCast(getOperation(), getPtr().getType(), "ptr operand");
}
...
LogicalResult mlir::pto::IntToPtrOp::verify()
{
    ...
    if (failed(verifyPtrLikeForAddressCast(getOperation(), getResult().getType(), "result")))
        return failure();
...
static constexpr StringLiteral kEffectivePTOEntryAttrName = "pto.internal.entry";
