...
LogicalResult pto::TConcatidxOp::verify()
{
    ...
    auto verifyA2A3 = [this, &elemOr]() -> LogicalResult {
        ...
        return verifyConcatidxElementTypes(getOperation(), elemOr->first, elemOr->second);
    };
    auto verifyA5 = [this, &elemOr]() -> LogicalResult {
        ...
        if (!isRowMajorTileBuf(getSrc0().getType()) || !isRowMajorTileBuf(getSrc1().getType()) ||
            !isRowMajorTileBuf(getSrc0Idx().getType()) || !isRowMajorTileBuf(getSrc1Idx().getType()) ||
    ...
    return dispatchVerifierByArch(
        ...
        [this, elemTy]() -> LogicalResult {
            if (!isSupportedVecElemType(
                    elemTy, /*allowBf16=*/false,
                    /*allowInt8=*/false))
                return emitOpError() << "expects A2/A3 dst element type to be f16/f32/i16/i32/u16/u32";
            return success();
        },
        [this, elemTy]() -> LogicalResult {
            if (!isSupportedVecElemType(
                    elemTy, /*allowBf16=*/true,
                    /*allowInt8=*/true))
                return emitOpError() << "expects A5 dst element type to be f16/f32/bf16/i8/i16/i32/u8/u16/u32";
            return success();
        });
}
...
static LogicalResult verifyTCmpA2A3(TCmpOp op)
{
    ...
    if (failed(verifyVecTileStorage(op, src0Ty, "src0")) || failed(verifyVecTileStorage(op, src1Ty, "src1")) ||
    ...
    if (!(src0Elem.isInteger(kPTOI32BitWidth) || src0Elem.isF16() || src0Elem.isF32())) {
        return op.emitOpError("expects A2/A3 tcmp input element type to be i32/f16/f32");
    }
...
static LogicalResult verifyTCmpA5(TCmpOp op)
{
    ...
    if (info.src0Elem != info.src1Elem)
        return op.emitOpError("expects src0 and src1 to have the same element type");
    if (!(info.src0Elem.isF16() || info.src0Elem.isF32() || info.src0Elem.isBF16() ||
          info.src0Elem.isInteger(kPTOI8BitWidth) || info.src0Elem.isInteger(kPTOI16BitWidth) ||
          info.src0Elem.isInteger(kPTOI32BitWidth))) {
        return op.emitOpError("expects A5 tcmp input element type to be i8/i16/i32/f16/bf16/f32");
    }
    auto dstInt = dyn_cast<IntegerType>(info.dstElem);
    if (!dstInt || dstInt.getWidth() != kPTOI8BitWidth)
        return op.emitOpError("expects dst element type to be i8");
    if (getShapeVec(info.src0Ty) != getShapeVec(info.src1Ty) || getShapeVec(info.src0Ty) != getShapeVec(info.dstTy)) {
...
{
    auto verifyA2A3 = [this]() -> LogicalResult { return verifyTCmpA2A3(*this); };
...
static LogicalResult verifyTCmpSCommon(TCmpSOp op)
{
    ...
    Type dstTy = op.getDst().getType();
    if (failed(verifyVecTileStorage(op, srcTy, "src")) || failed(verifyVecTileStorage(op, dstTy, "dst"))) {
        return failure();
    }
