...
static ParseResult parseTColSumInsClause(
    OpAsmParser& parser, OperationState& result, OpAsmParser::UnresolvedOperand& src,
    OpAsmParser::UnresolvedOperand& tmp, Type& srcTy, Type& tmpTy, bool& hasTmp)
{
...
static ParseResult parseTColSumOutsClause(OpAsmParser& parser, OpAsmParser::UnresolvedOperand& dst, Type& dstTy)
{
...
static LogicalResult verifyTColSumCommon(
    TColSumOp op, bool requireNonZeroSrc, bool allowInt8, bool allowBf16, StringRef errorMessage)
{
    ...
    if (!(elem.isF16() || elem.isF32() || (allowBf16 && elem.isBF16()) || elem.isInteger(kPTOI16BitWidth) ||
          elem.isInteger(kPTOI32BitWidth) || (allowInt8 && elem.isInteger(kPTOI8BitWidth))))
...
LogicalResult pto::TColProdOp::verify()
{
    return verifyTColReductionOpWithArchDispatch(
        ...
        /*allowInt8OnA5=*/false, /*allowBf16OnA5=*/true, "expects A2/A3 tcolprod element type to be f16/f32/i16/i32",
...
llvm::LogicalResult mlir::pto::TCvtOp::verify()
{
    ...
    if (failed(verifyTileBufCommon(*this, srcTy, "src", /*allowLowPrecision=*/true)) ||
        ...
        return failure();
    if (failed(verifyTileBufSameLogicalExtent(
...
{
    auto verifyA2A3 = [this]() -> LogicalResult { return emitOpError("trandom is only supported for A5 targets"); };
    auto verifyA5 = [this]() -> LogicalResult {
        if (shouldBypassDecodedMemrefVerifier(getOperation()))
            return success();
        ...
        if (failed(checkWord(getKey0(), "key0")) || failed(checkWord(getKey1(), "key1")) ||
            failed(checkWord(getCounter0(), "counter0")) || failed(checkWord(getCounter1(), "counter1")) ||
            failed(checkWord(getCounter2(), "counter2")) || failed(checkWord(getCounter3(), "counter3")))
            return failure();
