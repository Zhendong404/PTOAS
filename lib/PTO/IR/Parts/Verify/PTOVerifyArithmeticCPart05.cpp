...
static LogicalResult verifyTileValidWidthMatchesCols(Operation* op, Type ty, StringRef operandName)
{
    auto validShape = getValidShapeVec(ty);
    auto shape = getShapeVec(ty);
    if (validShape.size() == kPTORowColRank && shape.size() == kPTORowColRank &&
        validShape[1] != ShapedType::kDynamic && shape[1] != ShapedType::kDynamic && validShape[1] != shape[1]) {
        return op->emitOpError() << "expects " << operandName << " valid_shape[1] to equal " << operandName << " cols";
    }
    return success();
}
...
static LogicalResult verifyGatherMaskForm(TGatherOp op, bool allowA5MaskTypes)
{
    ...
    if (!srcSpace || !dstSpace || *srcSpace != pto::AddressSpace::VEC || *dstSpace != pto::AddressSpace::VEC) {
    ...
    if (failed(verifyTileValidWidthMatchesCols(op.getOperation(), common->dstTy, "dst"))) {
        return failure();
    }

    if (allowA5MaskTypes) {
        if (!(srcElemBytes == kPTOByteSize || srcElemBytes == kPTOHalfWordBytes || srcElemBytes == kPTOWordBytes)) {
            return op.emitOpError("expects A5 mask-pattern gather element size to be 1, 2, or 4 bytes");
        }
        if (!isSupportedGatherElemTypeA5(common->srcElem) || !isSupportedGatherElemTypeA5(common->dstElem)) {
...
static FailureOr<GatherIndexCommon> verifyGatherIndexCommon(TGatherOp op)
{
    ...
    if (failed(verifyTileBufCommon(op, idxTy, "indices")) || failed(verifyTileBufCommon(op, tmpTy, "tmp"))) {
        return failure();
    }
...
static LogicalResult verifyGatherIndexForm(TGatherOp op, bool allow16BitIndices, bool allowA5ElemTypes)
{
    ...
    if (allowA5ElemTypes) {
        if (!isSupportedGatherElemTypeA5Index(common->base.srcElem) ||
            ...
            return op.emitOpError("expects A5 gather src/dst element type to be i8/i16/i32/f16/f32");
        }
    } else if (
        !isSupportedGatherElemTypeA2A3(common->base.srcElem) || !isSupportedGatherElemTypeA2A3(common->base.dstElem)) {
        return op.emitOpError("expects gather src/dst element type to be i16/i32/f16/f32");
    }

    unsigned width = common->idxElem.getWidth();
    if (!(width == kPTOI32BitWidth || (allow16BitIndices && width == kPTOI16BitWidth))) {
        return op.emitOpError() << "expects indices element type to be i32" << (allow16BitIndices ? " or i16" : "");
    }
    if (failed(verifyTileValidWidthMatchesCols(op.getOperation(), common->base.dstTy, "dst")) ||
        failed(verifyTileValidWidthMatchesCols(op.getOperation(), common->idxTy, "indices"))) {
        return failure();
    }
    if (!allowA5ElemTypes) {
        if (getElemTy(common->tmpTy) != common->idxElem)
            return op.emitOpError("expects tmp and indices to have the same element type");
        if (failed(verifyTileBufSameValidShape(op, common->idxTy, common->tmpTy, "indices", "tmp"))) {
            return failure();
        }
    }
    return success();
...
static FailureOr<GatherCompareCommon> verifyGatherCompareCommon(TGatherOp op)
{
    ...
    Type tmpTy = op.getTmp().getType();
    if (failed(verifyTileBufCommon(op, cdstTy, "cdst")) || failed(verifyTileBufCommon(op, tmpTy, "tmp"))) {
        return failure();
    }
    Type cdstElem = getElemTy(cdstTy);
    if (!cdstElem)
        return op.emitOpError("failed to get element type for src/dst/cdst"), failure();
    ...
    if (cdstElem != base->dstElem)
        return op.emitOpError("expects cdst to have the same element type as dst"), failure();
    if (op.getKValue().getType() != base->srcElem) {
        return op.emitOpError("expects kValue to have the same type as src element type"), failure();
    }
    ...
    if (cmpMode != pto::CmpMode::EQ && cmpMode != pto::CmpMode::GT) {
        return op.emitOpError("expects compare-form tgather cmpMode to be eq or gt"), failure();
    }
    return GatherCompareCommon{*base, cdstTy, tmpTy, cdstElem, cmpMode};
}

static LogicalResult verifyGatherCompareForm(TGatherOp op, bool allowA5SrcTypes)
{
    ...
    if (allowA5SrcTypes) {
        if (!(common->base.srcElem.isF16() || common->base.srcElem.isF32() ||
              common->base.srcElem.isInteger(kPTOI16BitWidth) || common->base.srcElem.isInteger(kPTOI32BitWidth))) {
            return op.emitOpError("expects A5 compare-form tgather src element type to be i16/i32/f16/f32");
        }
    } else if (!(common->base.srcElem.isF16() || common->base.srcElem.isF32() ||
                 (common->base.srcElem.isInteger(kPTOI32BitWidth) && common->cmpMode == pto::CmpMode::EQ))) {
        return op.emitOpError(
