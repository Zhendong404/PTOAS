...
static ParseResult parseTMrgSortFormat2Types(
    OpAsmParser& parser, MutableArrayRef<OpAsmParser::UnresolvedOperand> srcs, SmallVectorImpl<Type>& srcTypes,
    Type& tmpTy)
{
    ...
    if (srcTypes.size() != srcs.size() + 1 || parser.parseRParen() || parser.parseKeyword("outs") ||
        parser.parseLParen())
        return failure();
...
static ParseResult parseTMrgSortFormat2Outputs(
    OpAsmParser& parser, OpAsmParser::UnresolvedOperand& dstOp, OpAsmParser::UnresolvedOperand& excutedOp, Type& dstTy,
    Type& excutedTy)
{
    if (parser.parseOperand(dstOp) || parser.parseComma() || parser.parseOperand(excutedOp) || parser.parseColon() ||
        parser.parseType(dstTy) || parser.parseComma() || parser.parseType(excutedTy) || parser.parseRParen())
        return failure();
    return success();
}

static ParseResult resolveTMrgSortFormat2Operands(
    OpAsmParser& parser, OperationState& result, ArrayRef<OpAsmParser::UnresolvedOperand> srcs, ArrayRef<Type> srcTypes,
    OpAsmParser::UnresolvedOperand tmpOp, Type tmpTy, OpAsmParser::UnresolvedOperand dstOp, Type dstTy,
    OpAsmParser::UnresolvedOperand excutedOp, Type excutedTy, bool exhaustedVal)
{
    result.addAttribute(
        "operandSegmentSizes",
        parser.getBuilder().getDenseI32ArrayAttr({static_cast<int32_t>(srcs.size()), 0, 1, 1, 1}));
    if (parser.resolveOperands(srcs, srcTypes, parser.getCurrentLocation(), result.operands) ||
        parser.resolveOperand(dstOp, dstTy, result.operands) || parser.resolveOperand(tmpOp, tmpTy, result.operands) ||
    ...
    OpAsmParser::UnresolvedOperand second)
{
    ...
    if (failed(parseTMrgSortFormat2Outputs(parser, dstOp, excutedOp, dstTy, excutedTy)))
        return failure();
    return resolveTMrgSortFormat2Operands(
        parser, result, srcs, srcTypes, tmpOp, tmpTy, dstOp, dstTy, excutedOp, excutedTy, exhaustedVal);
}
...
static LogicalResult verifyTMrgSortSingleRowTile(Operation* op, Type ty, StringRef name)
{
    auto shape = getShapeVec(ty);
    if (shape.size() != kPTORowColRank)
        return op->emitOpError() << "format2 expects " << name << " to be rank-2 tile-shaped";
    if (shape[0] != mlir::ShapedType::kDynamic && shape[0] != 1)
        return op->emitOpError() << "format2 expects " << name << " rows == 1";
    return success();
}

static LogicalResult verifyTMrgSortFormat2Executed(Operation* op, Value excuted)
{
    auto excutedTy = mlir::dyn_cast<mlir::VectorType>(excuted.getType());
    if (!excutedTy || excutedTy.getRank() != 1 || excutedTy.getNumElements() != kNumber4 ||
        !excutedTy.getElementType().isInteger(kPTOI16BitWidth))
        return op->emitOpError() << "format2 excuted must be vector<4xi16>";
    return success();
}

static LogicalResult verifyTMrgSortFormat2Src(Operation* op, Value src, Type elemTy)
{
...
static LogicalResult verifyTMrgSortFormat2(TMrgSortOp op)
{
    ...
    if (op.getDsts().size() != 1u || !op.getTmp() || !op.getExcuted())
        return op.emitOpError() << "format2 expects ins(srcs..., tmp), outs(dst), and excuted=vector";
    Type dstTy = op.getDst().getType();
    ...
    if (failed(verifyTMrgSortSingleRowTile(op, dstTy, "dst")) || failed(verifyTMrgSortSingleRowTile(op, tmpTy, "tmp")))
        return failure();
    if (dstShape[1] != mlir::ShapedType::kDynamic && tmpShape[1] != mlir::ShapedType::kDynamic &&
