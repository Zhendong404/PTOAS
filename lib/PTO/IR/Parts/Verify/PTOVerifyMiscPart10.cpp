...
static LogicalResult verifyTRowExpandCommon(TRowExpandOp op)
{
    ...
    if (failed(verifyTileBufCommon(op, srcTy, "src")) || failed(verifyNDStyleVecTile(op, dstTy, "dst")))
        return failure();
    ...
    auto checkNonZero = [opBase](ArrayRef<int64_t> valid, StringRef name) -> LogicalResult {
        if (valid[0] != ShapedType::kDynamic && valid[0] == 0)
            return opBase->emitOpError() << "expects " << name << " valid_shape[0] to be non-zero";
        if (valid[1] != ShapedType::kDynamic && valid[1] == 0)
            return opBase->emitOpError() << "expects " << name << " valid_shape[1] to be non-zero";
        return success();
    };
    if (failed(checkNonZero(srcValid, "src")) || failed(checkNonZero(dstValid, "dst")))
        return failure();
    return success();
}
...
{
    auto verifyA2A3 = [this]() -> LogicalResult { return verifyTRowExpandCommon(*this); };
    auto verifyA5 = [this]() -> LogicalResult { return verifyTRowExpandCommon(*this); };
