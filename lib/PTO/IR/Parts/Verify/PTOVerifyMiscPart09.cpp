...
[[maybe_unused]] static bool isTileBufOrMemref(Type ty) { return mlir::isa<MemRefType, pto::TileBufType>(ty); }
...
static bool isLocallyBoundTileSource(Value value)
{
    ...
    if (isa<AllocTileOp, DeclareTileOp, BindTileOp, PointerCastOp, MaterializeTileOp>(value.getDefiningOp()))
        return true;
...
mlir::LogicalResult mlir::pto::SetValidShapeOp::verify()
{
    ...
    if (auto srcTy = llvm::dyn_cast<TileBufType>(getSource().getType())) {
        ...
        if (!isLocallyBoundTileSource(getSource()))
            return emitOpError("requires a locally bound tile source; function arguments/results "
                               "are unsupported");
    } else if (auto srcTy = llvm::dyn_cast<MemRefType>(getSource().getType())) {
        if (!(*this)->hasAttr(kLoweredSetValidShapeAttrName))
            return emitOpError("expects tile_buf source; memref source is only valid for the internal lowered form");
        if (srcTy.getRank() != kPTORowColRank)
    ...
    auto checkDim = [this, &shape](Value operand, unsigned dimIdx, StringRef dimName) -> LogicalResult {
        ...
        if (maxStatic != ShapedType::kDynamic && *constVal > maxStatic)
            return emitOpError() << "expects " << dimName << " operand <= shape dim (" << maxStatic << ")";
        return success();
    };
...
mlir::LogicalResult mlir::pto::TReshapeOp::verify()
{
    ...
    if (failed(verifyTileBufCommon(*this, ts, "src")) || failed(verifyTileBufCommon(*this, tr, "dst")))
        return failure();
    ...
    if (srcElemBytes.value() * srcNumel.value() != dstElemBytes.value() * dstNumel.value())
        return emitOpError("expects src and dst to have the same total byte size");

    bool srcBoxed = srcTb.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::NoneBox);
    bool dstBoxed = dstTb.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::NoneBox);
...
mlir::LogicalResult mlir::pto::BitcastOp::verify()
{
    ...
    if (srcTy.getElementType() == dstTy.getElementType())
        return emitOpError("expects src/result to have different element types; use "
