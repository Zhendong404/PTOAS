...
static FailureOr<TMovCommonInfo> verifyTMovCommon(TMovOp op)
{
    ...
    if (hasFp && hasPreQuantScalar) {
        return op.emitOpError("expects fp and preQuantScalar forms to be mutually exclusive"), failure();
    }

    auto srcSpace = getPTOMemorySpaceEnum(srcTy);
    auto dstSpace = getPTOMemorySpaceEnum(dstTy);
    if (!srcSpace || !dstSpace)
        return op.emitOpError("expects src and dst to have explicit address spaces"), failure();

    TMovCommonInfo info{
        srcTy,
        dstTy,
        fp,
        dyn_cast<pto::TileBufType>(srcTy),
        dyn_cast<pto::TileBufType>(dstTy),
        srcSpace,
        dstSpace,
        hasFp,
        hasPreQuantScalar};
    info.isMatToTile = *srcSpace == pto::AddressSpace::MAT &&
                       (*dstSpace == pto::AddressSpace::LEFT || *dstSpace == pto::AddressSpace::RIGHT ||
                        *dstSpace == pto::AddressSpace::BIAS || *dstSpace == pto::AddressSpace::SCALING);
    info.isVecToVec = *srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::VEC;
    info.isVecToMat = *srcSpace == pto::AddressSpace::VEC && *dstSpace == pto::AddressSpace::MAT;
    info.isAccToMat = *srcSpace == pto::AddressSpace::ACC && *dstSpace == pto::AddressSpace::MAT;
    info.isAccToVec = *srcSpace == pto::AddressSpace::ACC && *dstSpace == pto::AddressSpace::VEC;
    return info;
}

static LogicalResult verifyTMovShapes(TMovOp op, const TMovCommonInfo& info, bool isA5)
{
...
static LogicalResult verifyTMovAddressSpacePair(TMovOp op, const TMovCommonInfo& info, bool isA5)
{
    bool okPair =
        info.isMatToTile || info.isVecToVec || info.isAccToMat || info.isAccToVec || (isA5 && info.isVecToMat);
    if (!okPair)
...
static LogicalResult verifyTMovDerivedForms(TMovOp op, const TMovCommonInfo& info)
{
    if (op.getReluPreMode() != pto::ReluPreMode::NoRelu && !(info.isAccToMat || info.isAccToVec)) {
...
static LogicalResult verifyTMovAccToVecMode(TMovOp op, const TMovCommonInfo& info)
{
...
static LogicalResult verifyTMovLayouts(TMovOp op, const TMovCommonInfo& info, bool isA5)
{
    if (info.srcTb && *info.srcSpace == pto::AddressSpace::ACC &&
        (info.hasFp || op.getReluPreMode() != pto::ReluPreMode::NoRelu) && !isColMajorRowMajorNZTileBuf(info.srcTb)) {
        return op.emitOpError("expects acc-source fp/relu tmov src to use blayout=col_major and slayout=row_major");
    }
    if (info.srcTb && info.dstTb && info.isAccToMat && !isA5 && info.dstTb.getSFractalSizeI32() != kFractalSize512) {
...
mlir::LogicalResult mlir::pto::TMovOp::verify()
{
    auto verifyImpl = [this](bool isA5) -> LogicalResult {
        ...
        if (failed(verifyTMovShapes(*this, *common, isA5)) ||
            ...
            failed(verifyTMovDerivedForms(*this, *common)) || failed(verifyTMovAccToVecMode(*this, *common)) ||
            failed(verifyTMovLayouts(*this, *common, isA5))) {
            return failure();
        }
        return success();
    };
    auto verifyA2A3 = [&verifyImpl]() -> LogicalResult { return verifyImpl(/*isA5=*/false); };
    auto verifyA5 = [&verifyImpl]() -> LogicalResult { return verifyImpl(/*isA5=*/true); };
...
static FailureOr<TMovFpCommonInfo> verifyTMovFpCommon(TMovFPOp op)
{
    ...
    auto srcIntTy = dyn_cast<IntegerType>(srcElemTy);
    if (!(srcElemTy.isF32() || (srcIntTy && srcIntTy.getWidth() == kPTOI32BitWidth))) {
        return op.emitOpError("expects src to have element type f32, i32"), failure();
    }
    auto fpSpace = getPTOMemorySpaceEnum(fpTy);
    if (!fpSpace || *fpSpace != pto::AddressSpace::SCALING)
        return op.emitOpError("expects fp to be in the scaling address space"), failure();
...
static LogicalResult verifyTMovFpA2A3(const TMovFpCommonInfo& info, TMovFPOp op)
{
    ...
    if (info.srcTb && !isColMajorRowMajorNZTileBuf(info.srcTb))
        return op.emitOpError("expects src to use blayout=col_major and slayout=row_major");
    if (info.dstTb && !isColMajorRowMajorNZTileBuf(info.dstTb))
        return op.emitOpError("expects dst to use blayout=col_major and slayout=row_major");
