...
static LogicalResult verifyTPrefetchElemTypes(Operation* op, Type srcElem, Type dstElem, bool allowLowPrecision)
{
    ...
    if (!allowLowPrecision && (isPTOLowPrecisionType(srcElem) || isPTOLowPrecisionType(dstElem))) {
        return op->emitOpError("expects A2/A3 tprefetch low-precision element types to be unsupported");
    }
    if (allowLowPrecision && (!isA5TLoadStoreTransferElemType(srcElem) || !isA5TLoadStoreTransferElemType(dstElem))) {
...
static FailureOr<TStoreVerifyInfo> verifyTStoreCommon(Operation* op, Value src, Value dst, bool allowLowPrecision)
{
    auto srcTile = dyn_cast<pto::TileBufType>(src.getType());
    auto dstPart = dyn_cast<pto::PartitionTensorViewType>(dst.getType());
    if (!srcTile || !dstPart) {
        op->emitOpError("expects src to be !pto.tile_buf and dst to be !pto.partition_tensor_view");
        return failure();
    }
    if (failed(verifyTileBufCommon(op, srcTile, "src", allowLowPrecision)) ||
        ...
        failed(verifyPositiveStaticDims(op, srcTile.getValidShape(), "src", "valid_shape"))) {
        return failure();
    }
    ...
    if (dstElemCount && srcValidElemCount && *dstElemCount != *srcValidElemCount) {
        op->emitOpError() << "expects dst static element count (" << *dstElemCount
                          << ") to match src valid_shape static element count (" << *srcValidElemCount << ")";
        return failure();
    }
    return TStoreVerifyInfo{srcTile, dstPart, srcTile.getElementType(), dstPart.getElementType()};
}
...
static LogicalResult verifyTStoreModeSource(
    Operation* op, pto::AddressSpace srcSpace, bool hasPreQuant, pto::ReluPreMode reluMode)
{
...
static LogicalResult verifyTStoreA2A3AccDstType(Operation* op, Type srcElem, Type dstElem, bool hasPreQuant)
{
    if (hasPreQuant) {
    ...
    if (!(dstElem.isInteger(kPTOI32BitWidth) || dstElem.isF32() || dstElem.isF16() || dstElem.isBF16())) {
        return op->emitOpError("expects A2/A3 acc tstore dst element type to be i32/f32/f16/bf16");
    }
    return success();
}

static LogicalResult verifyTStoreA2A3(
    TStoreOp op, const TStoreVerifyInfo& info, bool hasPreQuant, pto::ReluPreMode reluMode)
{
    auto srcSpace = getPTOMemorySpaceEnum(info.srcTile);
    if (!srcSpace || (*srcSpace != pto::AddressSpace::VEC && *srcSpace != pto::AddressSpace::MAT &&
                      *srcSpace != pto::AddressSpace::ACC)) {
        return op.emitOpError("expects A2/A3 tstore src to use loc=vec, loc=mat, or loc=acc");
    }
    if (failed(verifyTStoreModeSource(op.getOperation(), *srcSpace, hasPreQuant, reluMode))) {
        return failure();
    }

    if (*srcSpace == pto::AddressSpace::VEC || *srcSpace == pto::AddressSpace::MAT) {
        if (isPTOLowPrecisionType(info.dstElem)) {
            return op.emitOpError("expects A2/A3 vec/mat tstore low-precision dst element types to be unsupported");
        }
        ...
        if (getElemByteSize(info.srcElem) != getElemByteSize(info.dstElem)) {
            return op.emitOpError("expects A2/A3 vec/mat tstore src and dst element types to have the same bitwidth");
        }
        return success();
    }
    ...
    if (failed(verifyTStoreA2A3AccDstType(op.getOperation(), info.srcElem, info.dstElem, hasPreQuant))) {
        return failure();
    }
    ...
    if (srcValid[kPTOColumnDim] != ShapedType::kDynamic &&
        ...
        return op.emitOpError("expects A2/A3 acc tstore src valid_shape[1] to be in [1, 4095]");
    }
    return success();
}

static LogicalResult verifyTStoreA5AccDstType(Operation* op, Type srcElem, Type dstElem, bool hasPreQuant)
{
    if (hasPreQuant) {
    ...
    if (!(dstElem.isInteger(kPTOI32BitWidth) || dstElem.isF32() || dstElem.isF16() || dstElem.isBF16())) {
        return op->emitOpError("expects A5 acc tstore dst element type to be i32/f32/f16/bf16");
    }
    return success();
}

static LogicalResult verifyTStoreA5(
    TStoreOp op, const TStoreVerifyInfo& info, bool hasPreQuant, pto::ReluPreMode reluMode)
{
    auto srcSpace = getPTOMemorySpaceEnum(info.srcTile);
    if (!srcSpace || (*srcSpace != pto::AddressSpace::VEC && *srcSpace != pto::AddressSpace::ACC)) {
    ...
    if (*srcSpace == pto::AddressSpace::VEC) {
        ...
        if (getElemByteSize(info.srcElem) != getElemByteSize(info.dstElem)) {
            return op.emitOpError("expects A5 vec tstore src and dst element types to have the same bitwidth");
        }
        return success();
    }
    ...
    return verifyTStoreA5AccDstType(op.getOperation(), info.srcElem, info.dstElem, hasPreQuant);
}
