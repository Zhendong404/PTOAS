...
mlir::LogicalResult mlir::pto::TMulOp::verify()
{
    return verifyArithmeticBinaryTileOpWithArchDispatch(
        ...
        /*allowInt8OnA5=*/false, /*allowBf16OnA5=*/false, "expects A2/A3 tmul element type to be i32/i16/f16/f32",
...
mlir::LogicalResult mlir::pto::TMulSOp::verify()
{
    return verifyArithmeticScalarTileOpWithArchDispatch(
        getOperation(), getSrc0().getType(), getDst().getType(), getScalar().getType(), /*allowInt8OnA5=*/true,
        /*allowBf16OnA5=*/true, "expects A2/A3 tmuls element type to be i32/i16/f16/f32",
...
mlir::LogicalResult mlir::pto::TShrSOp::verify()
{
    ...
    auto verifyA2A3 = [this, &verifyCommon]() -> LogicalResult {
        ...
        if (!it || (it.getWidth() != kPTOI16BitWidth && it.getWidth() != kPTOI32BitWidth))
            return emitOpError("expects A2/A3 tshrs src and dst element type to be i16/i32");
        return success();
    };
    ...
    auto verifyA5 = [this, &verifyCommon]() -> LogicalResult {
        ...
        if (!it ||
            ...
            return emitOpError("expects A5 tshrs src and dst element type to be i8/i16/i32");
        return success();
    };
...
static LogicalResult verifyTNegA2A3(TNegOp op)
{
    ...
    if (failed(elemOr) || failed(verifyTileBufSameValidShape(op, srcTy, dstTy, "src", "dst")))
        return failure();
    Type elemTy = *elemOr;
    if (!(elemTy.isInteger(kPTOI16BitWidth) || elemTy.isInteger(kPTOI32BitWidth) || elemTy.isF16() || elemTy.isF32())) {
        return op.emitOpError() << "expects A2/A3 tneg element type to be i16/i32/f16/f32";
    }
    return success();
}
...
static LogicalResult verifyTNegA5(TNegOp op)
{
    ...
    if (!(elemTy.isInteger(kPTOI8BitWidth) || elemTy.isInteger(kPTOI16BitWidth) || elemTy.isInteger(kPTOI32BitWidth) ||
        ...
        return op.emitOpError() << "expects A5 tneg element type to be i8/i16/i32/f16/f32/bf16";
    }
    return success();
}
