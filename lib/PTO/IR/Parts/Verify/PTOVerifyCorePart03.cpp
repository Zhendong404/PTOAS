...
static LogicalResult verifyRowReductionDstLayout(Operation* op, Type ty, StringRef name)
{
    ...
    auto verifyTileLayout = [op, ty, name](pto::TileBufType tb) -> LogicalResult {
        ...
        if (shape.size() == kPTORowColRank && shape[1] != ShapedType::kDynamic && shape[1] != 1) {
...
static LogicalResult verifyPositiveStaticDims(Operation* op, ArrayRef<int64_t> dims, StringRef name, StringRef kind)
{
    for (auto [idx, dim] : llvm::enumerate(dims)) {
        if (dim != ShapedType::kDynamic && dim <= 0) {
            return op->emitOpError() << "expects " << name << " " << kind << "[" << idx << "] to be positive";
        }
    }
    return success();
}

static LogicalResult verifyPositiveRankedMemrefShape(Operation* op, MemRefType mr, StringRef name)
{
    if (!mr.hasRank())
        return op->emitOpError() << "expects " << name << " memref to be ranked";
    for (int64_t dim : mr.getShape()) {
        if (dim != ShapedType::kDynamic && dim <= 0)
            return op->emitOpError() << "expects " << name << " memref shape to be positive";
    }
    return success();
}
...
static FailureOr<TLoadVerifyInfo> verifyTLoadCommon(Operation* op, Value src, Value dst, bool allowLowPrecision)
{
    auto srcPart = dyn_cast<pto::PartitionTensorViewType>(src.getType());
    auto dstTile = dyn_cast<pto::TileBufType>(dst.getType());
    if (!srcPart || !dstTile) {
        op->emitOpError("expects src to be !pto.partition_tensor_view and dst to be !pto.tile_buf");
        return failure();
    }
    if (failed(verifyTileBufCommon(op, dstTile, "dst", allowLowPrecision)) ||
        ...
        failed(verifyPositiveStaticDims(op, dstTile.getValidShape(), "dst", "valid_shape"))) {
        return failure();
    }
    return TLoadVerifyInfo{srcPart, dstTile, srcPart.getElementType(), dstTile.getElementType()};
}
...
static LogicalResult verifyTLoadA2A3(Operation* op, const TLoadVerifyInfo& info)
{
    if (isPTOLowPrecisionType(info.srcElem) || isPTOLowPrecisionType(info.dstElem))
        return op->emitOpError("expects A2/A3 tload low-precision element types to be unsupported");
    if (!isA2A3TLoadDstElemType(info.dstElem)) {
        return op->emitOpError("expects A2/A3 tload dst element type to be i8/i16/i32/i64/u64/f16/bf16/f32");
    }
    ...
    if (getElemByteSize(info.srcElem) != getElemByteSize(info.dstElem)) {
        return op->emitOpError("expects src and dst element types to have the same bitwidth");
    }
    return success();
}
...
static LogicalResult verifyTLoadA5(Operation* op, const TLoadVerifyInfo& info)
{
    ...
    if (!(dstBytes == kPTOByteSize || dstBytes == kPTOHalfWordBytes || dstBytes == kPTOWordBytes ||
          dstBytes == kPTODoubleWordBytes)) {
        return op->emitOpError("expects A5 tload dst element size to be 1, 2, 4, or 8 bytes");
    }
    if (!isA5TLoadStoreTransferElemType(info.srcElem)) {
        return op->emitOpError("expects A5 tload src element type to be i8/i16/i32/i64/f16/bf16/f32/f8/hif8/fp4");
    }
    if (!isA5TLoadStoreTransferElemType(info.dstElem)) {
        return op->emitOpError("expects A5 tload dst element type to be i8/i16/i32/i64/f16/bf16/f32/f8/hif8/fp4");
    }
    if (info.dstElem.isInteger(kPTOI64BitWidth)) {
        auto pad = info.dstTile.getPadValueI32();
        if (pad != static_cast<int32_t>(pto::PadValue::Null) && pad != static_cast<int32_t>(pto::PadValue::Zero)) {
            return op->emitOpError("expects A5 i64/u64 tload dst pad to be null or zero");
        }
    }
    return success();
}
...
static FailureOr<Type> verifyTPrefetchDstElemType(Operation* op, Type dstTy, bool allowLowPrecision)
{
    ...
    if (auto dstMr = dyn_cast<MemRefType>(dstTy)) {
        ...
        if (!dstSpace || (*dstSpace != pto::AddressSpace::VEC && *dstSpace != pto::AddressSpace::MAT)) {
            return op->emitOpError("expects dst memref to use loc=vec or loc=mat"), failure();
        }
