...
static LogicalResult verifyMGatherMScatterTileShape(Operation* op, Type dataTy, Type idxTy, StringRef dataName)
{
    ...
    const bool idxRowMajor = idxTile.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::RowMajor);
    const bool idxColMajor = idxTile.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::ColMajor);

    const bool rowCoalesce1xR =
        idxRowMajor && isKnownUnitExtent(idxValid[0]) && hasCompatibleKnownExtent(idxValid[1], dataValid[0]);
    const bool rowCoalesceRx1 =
        idxColMajor && hasCompatibleKnownExtent(idxValid[0], dataValid[0]) && isKnownUnitExtent(idxValid[1]);
    const bool elemCoalesce =
        hasCompatibleKnownExtent(idxValid[0], dataValid[0]) && hasCompatibleKnownExtent(idxValid[1], dataValid[1]);
    if (!(rowCoalesce1xR || rowCoalesceRx1 || elemCoalesce))
        return op->emitOpError() << "expects idx valid_shape to be [1, " << dataName << ".valid_row], [" << dataName
                                 << ".valid_row, 1], or match " << dataName << " valid_shape";

    return success();
}

static LogicalResult verifyMGatherMScatterIdxTile(Operation* op, Type ty, StringRef name)
{
    ...
    if (blayout != static_cast<int32_t>(pto::BLayout::RowMajor) &&
        ...
        return op->emitOpError() << "expects " << name << " to use row_major or col_major blayout";
...
static bool isA5TLoadStoreTransferElemType(Type ty)
{
    return ty.isInteger(kPTOI8BitWidth) || ty.isInteger(kPTOI16BitWidth) || ty.isInteger(kPTOI32BitWidth) ||
           ty.isInteger(kPTOI64BitWidth) || ty.isF16() || ty.isBF16() || ty.isF32() || isPTOLowPrecisionType(ty);
}
...
static bool isA5AccStorePreQuantDstType(Type srcElem, Type dstElem)
{
    ...
    if (!srcElem.isF32())
        return false;
    return dstElem.isInteger(kPTOI8BitWidth) || dstElem.isF16() || dstElem.isBF16() || dstElem.isF32() ||
           isPTOHiFloat8Type(dstElem) || dstElem.isFloat8E4M3() || dstElem.isFloat8E4M3FN() ||
...
static LogicalResult verifyTileBufCommon(Operation* op, Type ty, StringRef name, bool allowLowPrecision)
{
    auto tb = dyn_cast<pto::TileBufType>(ty);
    if (tb) {
        ...
        if (!allowLowPrecision && isPTOLowPrecisionType(elemTy))
            return op->emitOpError() << name << ": dtype " << elemTy << " is not supported by this op yet";
        ...
        if (!allowLowPrecision && isPTOLowPrecisionType(mr.getElementType()))
            return op->emitOpError() << name << ": dtype " << mr.getElementType() << " is not supported by this op yet";
    } else {
...
static LogicalResult verifyTileBufSameElemType(Operation* op, Type lhs, Type rhs, StringRef lhsName, StringRef rhsName)
{
    if (!isTileLikeType(lhs) || !isTileLikeType(rhs))
        return op->emitOpError() << "expects " << lhsName << " and " << rhsName << " to be !pto.tile_buf or memref";
    if (getElemTy(lhs) != getElemTy(rhs))
        return op->emitOpError() << "expects " << lhsName << " and " << rhsName << " to have the same element type";
    return success();
}
    ...
    Operation* op, Type lhs, Type rhs, StringRef lhsName, StringRef rhsName)
{
    ...
    for (size_t i = 0; i < lhsValid.size() && i < rhsValid.size(); ++i) {
        if (lhsValid[i] != ShapedType::kDynamic && rhsValid[i] != ShapedType::kDynamic && lhsValid[i] != rhsValid[i])
            return op->emitOpError() << "expects " << lhsName << " and " << rhsName << " to have the same valid_shape";
    }
    ...
    Operation* op, Type lhs, Type rhs, StringRef lhsName, StringRef rhsName, bool compareValidShape)
{
    ...
    auto emitMismatch = [compareValidShape, lhsName, op, rhsName]() -> LogicalResult {
        if (compareValidShape)
            return op->emitOpError() << "expects " << lhsName << " and " << rhsName << " to have the same valid_shape";
        return op->emitOpError() << "expects " << lhsName << " and " << rhsName << " to have compatible shapes";
    };
    if (lhsExtent.size() != rhsExtent.size())
        return emitMismatch();

    for (size_t i = 0, e = lhsExtent.size(); i < e; ++i) {
        if (lhsExtent[i] != ShapedType::kDynamic && rhsExtent[i] != ShapedType::kDynamic &&
            lhsExtent[i] != rhsExtent[i])
            return emitMismatch();
    }
    return success();
}

static LogicalResult verifyScaleTileMatchesOperand(
    Operation* op, Type scaleTy, Type operandTy, StringRef scaleName, StringRef operandName)
{
    ...
    if (!scaleSpace || *scaleSpace != pto::AddressSpace::SCALING)
        return op->emitOpError() << "expects " << scaleName << " to be in the scaling address space";

    auto scaleShape = getShapeVec(scaleTy);
    auto operandShape = getShapeVec(operandTy);
    if (scaleShape.size() != operandShape.size())
        return op->emitOpError() << "expects " << scaleName << " and " << operandName << " to have the same rank";
    for (size_t i = 0; i < scaleShape.size(); ++i) {
        if (scaleShape[i] != ShapedType::kDynamic && operandShape[i] != ShapedType::kDynamic &&
            scaleShape[i] != operandShape[i])
            return op->emitOpError() << "expects " << scaleName << " and " << operandName << " to have the same shape";
    }
    ...
    for (size_t i = 0; i < scaleValid.size(); ++i) {
        if (scaleValid[i] != ShapedType::kDynamic && operandValid[i] != ShapedType::kDynamic &&
