...
static LogicalResult verifyPositiveStaticShape(Operation* op, Type ty, StringRef name)
{
    ...
    for (int64_t dim : shape) {
        if (dim == ShapedType::kDynamic || dim <= 0)
            return op->emitOpError() << "expects " << name << " to have a positive static shape";
    }
    return success();
}

static LogicalResult verifyCommGlobalLike(Operation* op, Value value, StringRef name)
{
    Type ty = value.getType();
    if (!isCommGlobalLikeType(ty))
        return op->emitOpError() << "expects " << name << " to be a GM memref/tensor_view/partition_view";
    return verifyPositiveStaticShape(op, ty, name);
}

static LogicalResult verifyCommSignalLike(Operation* op, Value value, StringRef name)
{
...
static LogicalResult verifyCommStagingTileLike(Operation* op, Value value, StringRef name)
{
    Type ty = value.getType();
    if (!isa<pto::TileBufType, MemRefType>(ty))
        return op->emitOpError() << "expects " << name << " to be a tile_buf or memref tile";
    auto as = getPTOMemorySpaceEnum(ty);
    if (!as || *as != pto::AddressSpace::VEC)
        return op->emitOpError() << "expects " << name << " to be in vec address space";
    return verifyPositiveStaticShape(op, ty, name);
}

static LogicalResult verifyCommGlobalGroup(Operation* op, ValueRange group, StringRef name)
{
    if (group.empty())
        return op->emitOpError() << "expects at least one " << name << " operand";
    Type groupTy = group.front().getType();
    for (auto it : llvm::enumerate(group)) {
        if (failed(verifyCommGlobalLike(op, it.value(), (name + "[" + Twine(it.index()) + "]").str())))
            return failure();
        if (it.value().getType() != groupTy)
            return op->emitOpError() << "expects all " << name << " operands to have identical types";
    }
    return success();
}

static LogicalResult verifyCommPingPongSameType(
    Operation* op, Value ping, Value pong, StringRef pingName, StringRef pongName)
{
    if (!pong)
        return success();
    if (failed(verifyCommStagingTileLike(op, ping, pingName)) || failed(verifyCommStagingTileLike(op, pong, pongName)))
        return failure();
    if (ping.getType() != pong.getType())
        return op->emitOpError() << "expects " << pingName << " and " << pongName << " to have identical types";
    return success();
}
...
static bool isSupportedMGatherMScatterPayloadElemType(Operation* op, Type ty)
{
    ...
    if (!isTargetArchA5(op))
        return false;
    return ty.isFloat8E4M3() || ty.isFloat8E4M3FN() || ty.isFloat8E4M3FNUZ() || ty.isFloat8E4M3B11FNUZ() ||
           ty.isFloat8E5M2() || ty.isFloat8E5M2FNUZ();
}

static bool isSupportedMScatterAtomicPayloadElemType(Type ty, pto::ScatterAtomicOp atomic)
{
    ...
    switch (atomic) {
        ...
        case pto::ScatterAtomicOp::Add:
            return ty.isF16() || ty.isF32() || (intTy && intTy.getWidth() == kPTOI32BitWidth);
        case pto::ScatterAtomicOp::Max:
        case pto::ScatterAtomicOp::Min:
            return ty.isF32() || (intTy && intTy.getWidth() == kPTOI32BitWidth);
    }
    llvm_unreachable("Unknown ScatterAtomicOp");
}

static LogicalResult verifyMGatherMScatterMemOperand(
    Operation* op, Value memValue, Type dataElemTy, StringRef dataOperandLabel)
{
    Type memTy = memValue.getType();
    Type memElem = getElemTy(memTy);
    if (!memElem || memElem != dataElemTy)
        return op->emitOpError() << "expects mem element type to match " << dataOperandLabel << " element type";
    if (isa<pto::PartitionTensorViewType>(memTy)) {
        if (auto layout = getLogicalViewLayout(memValue)) {
            if (*layout != pto::Layout::ND)
                return op->emitOpError("expects mem partition view to use ND logical layout when layout "
                                       "can be inferred");
        }
        return success();
    }
    ...
    if (auto mr = dyn_cast<MemRefType>(memTy)) {
        ...
        if (!as || (*as != pto::AddressSpace::GM && *as != pto::AddressSpace::Zero))
            return op->emitOpError("expects mem memref to use GM or zero address space");
        if (mr.getRank() == kPTOPaddedTensorRank5D) {
            ...
            if (allStatic && (shape[0] != 1 || shape[1] != 1 || shape[2] != 1))
                return op->emitOpError("expects rank-5 GM memref leading dimensions to be [1,1,1,...] "
                                       "(GlobalTensor table shape)");
        }
        return success();
    }

    return op->emitOpError("expects mem to be !pto.partition_tensor_view or a GM/ZERO memref");
}
