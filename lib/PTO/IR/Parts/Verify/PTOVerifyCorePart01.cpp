...
void mlir::pto::PartitionViewOp::print(OpAsmPrinter& p)
{
    ...
    auto inferredResultType =
        inferPartitionViewResultTypeFromSizes(dyn_cast<mlir::pto::TensorViewType>(getSource().getType()), getSizes());
...
static LogicalResult verifyNoneBoxTileBufLayout(
    Operation* op, StringRef name, int32_t blayout, int64_t rows, int64_t cols, unsigned elemBytes)
{
    constexpr int64_t kAlignedBytes = 32;
    auto checkByteAlignment = [op, name, elemBytes](
                                  int64_t dim, StringRef layoutName, StringRef byteExpr) -> LogicalResult {
        if (dim == ShapedType::kDynamic)
            return success();
        int64_t bytes = dim * static_cast<int64_t>(elemBytes);
        if (bytes % kAlignedBytes == 0)
            return success();
        return op->emitOpError() << "expects " << name << " " << layoutName << " none_box tile " << byteExpr
                                 << " to be 32-byte aligned, but got " << bytes << " bytes";
    };
    if (blayout == static_cast<int32_t>(BLayout::RowMajor))
        return checkByteAlignment(cols, "row-major", "row byte size (cols * sizeof(dtype))");
    return checkByteAlignment(rows, "col-major", "column byte size (rows * sizeof(dtype))");
}
    ...
    int64_t& innerCols)
{
    constexpr int64_t kAlignedBytes = 32;
    if (elemBytes == 0)
        return op->emitOpError() << "expects " << name << " to have a non-zero element byte size";
    switch (fractal) {
