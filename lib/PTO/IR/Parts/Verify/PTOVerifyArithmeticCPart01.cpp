...
mlir::LogicalResult mlir::pto::TDivSOp::verify()
{
    auto isTileLike = [](Type ty) -> bool {
        return isa<mlir::pto::TileBufType, MemRefType, RankedTensorType, mlir::pto::PartitionTensorViewType>(ty);
    };
    auto isScalarLike = [](Type ty) -> bool { return mlir::isa<IntegerType, FloatType>(ty); };
    ...
    auto verifyByArch = [this, &isTileLike, &isScalarLike](PTOArch targetArch) -> LogicalResult {
        ...
        if (targetArch == PTOArch::A3 &&
            !(elem.isInteger(kPTOI32BitWidth) || elem.isInteger(kPTOI16BitWidth) || elem.isF16() || elem.isF32()))
            return emitOpError("expects A2/A3 tdivs element type to be i32/i16/f16/f32");
        if (targetArch == PTOArch::A5 && !(elem.isInteger(kPTOI32BitWidth) || elem.isInteger(kPTOI16BitWidth) ||
                                           elem.isInteger(kPTOI8BitWidth) || elem.isF16() || elem.isF32()))
...
mlir::LogicalResult mlir::pto::TExpOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        Type srcTy = getSrc().getType();
        Type dstTy = getDst().getType();
        if (failed(verifyVecTileUnaryOp(
                *this, srcTy, dstTy, "src", "dst",
...
static FailureOr<std::pair<Type, std::optional<pto::AddressSpace>>> verifyTExpandsCommon(TExpandsOp op)
{
    Type dstTy = op.getDst().getType();
    if (failed(verifyTileBufCommon(op, dstTy, "dst")))
        return failure();
    auto dstSpace = getPTOMemorySpaceEnum(dstTy);
    if (!dstSpace || (*dstSpace != pto::AddressSpace::VEC && *dstSpace != pto::AddressSpace::MAT)) {
        return op.emitOpError("expects dst to be in the vec or mat address space"), failure();
    }
...
static LogicalResult verifyTExpandsElemType(Operation* op, Type dstElem, StringRef error, bool allowI8)
{
    if (dstElem.isF16() || dstElem.isBF16() || dstElem.isF32())
        return success();
    if (auto it = mlir::dyn_cast<mlir::IntegerType>(dstElem)) {
        unsigned w = it.getWidth();
        if (w == kPTOI16BitWidth || w == kPTOI32BitWidth || (allowI8 && w == kPTOI8BitWidth))
            return success();
    }
...
mlir::LogicalResult mlir::pto::TExpandsOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        ...
        return verifyTExpandsElemType(
            getOperation(), dstElem, "expects A2/A3 texpands dst element type to be i16/i32/f16/bf16/f32",
            /*allowI8=*/false);
    };
    auto verifyA5 = [this]() -> LogicalResult {
        auto common = verifyTExpandsCommon(*this);
        ...
        (void)dstSpace;
        return verifyTExpandsElemType(
            getOperation(), dstElem, "expects A5 texpands dst element type to be i8/i16/i32/f16/bf16/f32",
            /*allowI8=*/true);
    };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
static bool hasTileBufLayout(pto::TileBufType ty, pto::BLayout bl, pto::SLayout sl)
{
    return ty.getBLayoutValueI32() == static_cast<int32_t>(bl) && ty.getSLayoutValueI32() == static_cast<int32_t>(sl);
}

static bool hasMatExtractSourceLayoutA2A3(pto::TileBufType srcTy)
{
    return srcTy.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::RowMajor) ||
           (srcTy.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::RowMajor) &&
            srcTy.getSLayoutValueI32() == static_cast<int32_t>(pto::SLayout::RowMajor));
}

static bool hasMatExtractSourceLayoutA5(pto::TileBufType srcTy, pto::AddressSpace dstSpace)
{
    const bool rowMajorSrc = srcTy.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::RowMajor);
    const bool colMajorView = srcTy.getSLayoutValueI32() == static_cast<int32_t>(pto::SLayout::ColMajor);
    const bool rowMajorView = srcTy.getSLayoutValueI32() == static_cast<int32_t>(pto::SLayout::RowMajor);
    if (dstSpace == pto::AddressSpace::LEFT)
        return (rowMajorSrc && colMajorView) || (!rowMajorSrc && rowMajorView) || rowMajorSrc;
