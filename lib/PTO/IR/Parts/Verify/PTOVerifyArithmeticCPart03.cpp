...
static LogicalResult verifyTInsertA5AccToMat(const IndexedTileTransferCommon& common, TInsertOp op)
{
    ...
    const bool okTypes =
        (common.srcElem.isF32() && (common.dstElem.isF16() || common.dstElem.isBF16() || common.dstElem.isF32())) ||
        (common.srcElem.isInteger(kPTOI32BitWidth) && common.dstElem.isInteger(kPTOI32BitWidth));
    if (!okTypes) {
        return op.emitOpError("expects A5 acc->mat tinsert element types to be "
                              "(src=f32,dst=f16/bf16/f32) or (src=i32,dst=i32)");
    }
    return success();
}

static LogicalResult verifyTInsertA5VecToMat(const IndexedTileTransferCommon& common, TInsertOp op)
{
    if (!isColMajorRowMajorNZTileBuf(common.dstTb)) {
    ...
    if (common.srcElem != common.dstElem || !isA5SupportedVecInsertElemType(common.srcElem)) {
        return op.emitOpError("expects A5 vec->mat tinsert src/dst to have same supported dtype "
                              "(fp8/f16/bf16/f32/i8/i32)");
    }
    return success();
}

static LogicalResult verifyTInsertA5VecToVec(const IndexedTileTransferCommon& common, TInsertOp op)
{
    if (!isRowMajorNoneBoxNDTileBuf(common.srcTb) || !isRowMajorNoneBoxNDTileBuf(common.dstTb)) {
        return op.emitOpError("expects A5 vec->vec tinsert src/dst to use ND layout "
                              "(blayout=row_major, slayout=none_box)");
    }
    if (common.srcElem != common.dstElem || !isA5SupportedVecInsertElemType(common.srcElem)) {
        return op.emitOpError("expects A5 vec->vec tinsert src/dst to have same supported dtype "
                              "(fp8/f16/bf16/f32/i8/i32)");
    }
    return success();
}

static LogicalResult verifyTInsertA5(const IndexedTileTransferCommon& common, TInsertOp op)
{
    if (!common.srcSpace || !common.dstSpace)
        return op.emitOpError("expects A5 tinsert src/dst to have explicit loc");

    if (*common.srcSpace == pto::AddressSpace::ACC && *common.dstSpace == pto::AddressSpace::MAT)
        return verifyTInsertA5AccToMat(common, op);

    if (*common.srcSpace == pto::AddressSpace::VEC && *common.dstSpace == pto::AddressSpace::MAT)
        return verifyTInsertA5VecToMat(common, op);

    if (*common.srcSpace == pto::AddressSpace::VEC && *common.dstSpace == pto::AddressSpace::VEC)
        return verifyTInsertA5VecToVec(common, op);

    return op.emitOpError("expects A5 tinsert to use a supported src/dst loc pair: "
...
mlir::LogicalResult mlir::pto::TInsertOp::verify()
{
    ...
    auto verifyA2A3 = [this, &common]() -> LogicalResult { return verifyTInsertA2A3(*common, *this); };
    auto verifyA5 = [this, &common]() -> LogicalResult { return verifyTInsertA5(*common, *this); };
...
static bool isA5MxInputType(Type ty) { return isA5Fp8LikeType(ty); }

static LogicalResult verifyA5MxTypeTriple(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, StringRef lhsName, StringRef rhsName, StringRef dstName)
{
    ...
    if (!isA5MxInputType(lhsElem) || !isA5MxInputType(rhsElem))
        return op->emitOpError() << "expects A5 mx operands " << lhsName << " and " << rhsName
                                 << " to use fp8 element types";

    if (!dstElem.isF32())
        return op->emitOpError() << "expects A5 mx result " << dstName << " to use f32 element type";
...
{
    if (srcElem.isF32())
        return dstElem.isInteger(kPTOI8BitWidth) || isA5Fp8LikeType(dstElem) || dstElem.isF16() || dstElem.isBF16() ||
               dstElem.isF32();
    if (srcElem.isInteger(kPTOI32BitWidth))
...
static FailureOr<std::tuple<
    Type, Type, Type, pto::TileBufType, pto::TileBufType, pto::TileBufType, pto::AddressSpace, pto::AddressSpace,
    pto::AddressSpace>>
verifyVectorPreQuantTransferCommon(
    Operation* op, Value src, Value fp, Value dst, Value indexRow, Value indexCol, bool isInsertOp)
{
    ...
    if (!srcTb || !fpTb || !dstTb)
        return op->emitOpError("expects src, fp, and dst to be !pto.tile_buf"), failure();
    auto verifyBounds = isInsertOp ? verifyInsertStaticBoundsCommon : verifyExtractStaticBoundsCommon;
    if (failed(verifyTileBufCommon(op, srcTy, "src")) || failed(verifyTileBufCommon(op, fpTy, "fp")) ||
    ...
    if (!srcSpace || !fpSpace || !dstSpace)
        return op->emitOpError("expects src, fp, and dst to have explicit loc"), failure();
    ...
    if (!isColMajorRowMajorNZTileBuf(srcTb))
        return op->emitOpError("expects src to use blayout=col_major and slayout=row_major"), failure();
    if (!isColMajorRowMajorNZTileBuf(dstTb))
        return op->emitOpError("expects dst to use blayout=col_major and slayout=row_major"), failure();
    return std::make_tuple(srcTy, fpTy, dstTy, srcTb, fpTb, dstTb, *srcSpace, *fpSpace, *dstSpace);
}
...
static LogicalResult verifyVectorPreQuantTransferOp(
    Operation* op, Value src, Value fp, Value dst, Value indexRow, Value indexCol, bool isInsertOp,
    bool requireDstFractal512, VectorPreQuantTypePairFn verifyTypePair, llvm::StringRef message)
{
    auto common = verifyVectorPreQuantTransferCommon(op, src, fp, dst, indexRow, indexCol, isInsertOp);
    if (failed(common))
        return failure();
    auto [srcTy, fpTy, dstTy, srcTb, fpTb, dstTb, srcSpace, fpSpace, dstSpace] = *common;
    (void)fpTy;
    (void)srcTb;
    (void)fpTb;
    (void)srcSpace;
