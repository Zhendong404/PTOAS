...
void mlir::pto::annotatePTOEntryFunctions(ModuleOp module)
{
    ...
    if (defs.empty())
        return;
    if (defs.size() == 1) {
        defs.front()->setAttr(kEffectivePTOEntryAttrName, BoolAttr::get(module.getContext(), true));
        return;
    }

    for (auto func : defs) {
        func->setAttr(kEffectivePTOEntryAttrName, BoolAttr::get(module.getContext(), hasExplicitPTOEntryAttr(func)));
    }
}
...
[[maybe_unused]] static LogicalResult verifyMemrefToTensorLoad(Operation* op, Value src, Value res)
{
    ...
    if (mr.getElementType() != rt.getElementType())
        return op->emitOpError() << "memref/tensor element type mismatch: memref=" << mr.getElementType()
                                 << " tensor=" << rt.getElementType();

    if (mr.getRank() != rt.getRank())
        return op->emitOpError() << "rank mismatch: memref rank=" << mr.getRank() << " tensor rank=" << rt.getRank();
...
LogicalResult AllocTileOp::verify()
{
    ...
    // （这条规则由下面两句自然实现）
    if (hasVR != needVR)
        return emitOpError() << "valid_row operand " << (needVR ? "is required" : "must be absent")
                             << " because result type v_row is " << (needVR ? "?" : std::to_string(vs[0]));

    if (hasVC != needVC)
        return emitOpError() << "valid_col operand " << (needVC ? "is required" : "must be absent")
                             << " because result type v_col is " << (needVC ? "?" : std::to_string(vs[1]));
...
LogicalResult MaterializeTileOp::verify()
{
    ...
    if (!isSubview && sourceTy.getShape() != resultTy.getShape())
        return emitOpError() << "source/result shape mismatch: source=" << sourceTy << " result=" << resultTy;

    if (sourceTy.getElementType() != resultTy.getElementType())
        return emitOpError() << "source/result element type mismatch: source=" << sourceTy.getElementType()
    ...
    for (unsigned i = 0; i < kPTORowColRank; ++i) {
        if (shape[i] != ShapedType::kDynamic && validShape[i] != ShapedType::kDynamic && validShape[i] > shape[i]) {
            return emitOpError() << "valid_shape[" << i << "] must be <= shape[" << i << "]";
        }
    }
...
LogicalResult TLoadOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        auto common = verifyTLoadCommon(*this, getSrc(), getDst(), /*allowLowPrecision=*/false);
        if (failed(common))
            return failure();
    ...
    auto verifyA5 = [this]() -> LogicalResult {
        auto common = verifyTLoadCommon(*this, getSrc(), getDst(), /*allowLowPrecision=*/true);
        if (failed(common))
            return failure();
...
LogicalResult TPrefetchOp::verify()
{
    auto verifyByArch = [this](bool allowLowPrecision) -> LogicalResult {
        ...
        if (failed(srcElem))
            return failure();
        auto dstElem = verifyTPrefetchDstElemType(*this, getDst().getType(), allowLowPrecision);
        if (failed(dstElem))
            return failure();
        return verifyTPrefetchElemTypes(*this, *srcElem, *dstElem, allowLowPrecision);
    };
    return dispatchVerifierByArch(
        getOperation(), [&verifyByArch]() { return verifyByArch(/*allowLowPrecision=*/false); },
        [&verifyByArch]() { return verifyByArch(/*allowLowPrecision=*/true); });
}
