...
mlir::LogicalResult mlir::pto::TReluOp::verify()
{
    auto verifyByArch = [this](StringRef errorMessage) -> LogicalResult {
        ...
        if (failed(verifyVecTileCommon(*this, srcTy, "src")) || failed(verifyVecTileCommon(*this, dstTy, "dst")))
            return failure();
...
static LogicalResult verifyTRemRowMajorTiles(Operation* op, Type src0Ty, Type src1Ty, Type tmpTy, Type dstTy)
{
    if (isRowMajorTileBuf(src0Ty) && isRowMajorTileBuf(src1Ty) && isRowMajorTileBuf(tmpTy) &&
        isRowMajorTileBuf(dstTy)) {
        return success();
    }
    return op->emitOpError("expects src0, src1, tmp, and dst to use row-major layout");
}

static LogicalResult verifyTRemTmpCoverage(Operation* op, Type tmpTy, Type dstTy)
{
    ...
    if (dstValid[1] != ShapedType::kDynamic && tmpValid[1] != ShapedType::kDynamic && tmpValid[1] < dstValid[1]) {
        return op->emitOpError("expects tmp valid columns to cover dst valid columns");
    }
    return success();
}

static FailureOr<Type> verifyTRemCommon(Operation* op, Type src0Ty, Type src1Ty, Type tmpTy, Type dstTy)
{
    if (failed(verifyTileBufCommon(op, src0Ty, "src0")) || failed(verifyTileBufCommon(op, src1Ty, "src1")) ||
        failed(verifyTileBufCommon(op, tmpTy, "tmp")) || failed(verifyTileBufCommon(op, dstTy, "dst")))
        return failure();
    ...
    if (getElemTy(tmpTy) != getElemTy(dstTy))
        return op->emitOpError("expects tmp and dst to have the same element type"), failure();
...
mlir::LogicalResult mlir::pto::TRemOp::verify()
{
    ...
    FailureOr<Type> elemOr =
        verifyTRemCommon(*this, getSrc0().getType(), getSrc1().getType(), getTmp().getType(), getDst().getType());
    if (failed(elemOr))
        return failure();
    Type elem = *elemOr;
...
mlir::LogicalResult mlir::pto::TFModOp::verify()
{
    return verifyArithmeticBinaryTileOpWithArchDispatch(
        ...
        /*allowInt8OnA5=*/false, /*allowBf16OnA5=*/false, "expects A2/A3 tfmod element type to be i32/i16/f16/f32",
        "expects A5 tfmod element type to be i32/i16/f16/f32");
}

static FailureOr<Type> verifyTRemScalarCommon(Operation* op, Type srcTy, Type tmpTy, Type dstTy, Type scalarTy)
{
    if (failed(verifyTileBufCommon(op, srcTy, "src")) || failed(verifyTileBufCommon(op, tmpTy, "tmp")) ||
    ...
    if (getElemTy(tmpTy) != getElemTy(dstTy))
        return op->emitOpError("expects tmp and dst to have the same element type"), failure();
    if (!isRowMajorTileBuf(srcTy) || !isRowMajorTileBuf(tmpTy) || !isRowMajorTileBuf(dstTy)) {
        return op->emitOpError("expects src, tmp, and dst to use row-major layout"), failure();
    }
    Type elem = getElemTy(srcTy);
    if (scalarTy != elem)
        return op->emitOpError("expects scalar type to match the tile element type"), failure();
    auto dstValid = getValidShapeVec(dstTy);
    auto tmpValid = getValidShapeVec(tmpTy);
    if (dstValid.size() != kPTORowColRank || tmpValid.size() != kPTORowColRank)
    ...
    if (dstValid[1] != ShapedType::kDynamic && tmpValid[1] != ShapedType::kDynamic && tmpValid[1] < dstValid[1]) {
        return op->emitOpError("expects tmp valid columns to cover dst valid columns"), failure();
    }
    return elem;
}
...
mlir::LogicalResult mlir::pto::TRemSOp::verify()
{
    if (shouldBypassDecodedMemrefVerifier(getOperation()))
        return success();
    FailureOr<Type> elemOr = verifyTRemScalarCommon(
        *this, getSrc().getType(), getTmp().getType(), getDst().getType(), getScalar().getType());
    if (failed(elemOr))
        return failure();
    Type elem = *elemOr;
    auto verifyA2A3 = [this, elem]() -> LogicalResult {
...
mlir::LogicalResult mlir::pto::TFModSOp::verify()
{
    ...
    if (failed(verifyTileBufCommon(*this, srcTy, "src")) || failed(verifyTileBufCommon(*this, dstTy, "dst")))
        return failure();
    if (failed(verifyTileBufSameElemType(*this, srcTy, dstTy, "src", "dst")) ||
        failed(verifyTileBufSameValidShape(*this, srcTy, dstTy, "src", "dst")))
        return failure();
