...
mlir::LogicalResult mlir::pto::TExtractFPOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        return verifyVectorPreQuantTransferOp(
            getOperation(), getSrc(), getFp(), getDst(), getIndexRow(), getIndexCol(),
            /*isInsertOp=*/false,
...
static LogicalResult verifyTFillPadMatHomogeneousConstraint(
    Operation* op, Type srcTy, Type dstTy, llvm::StringRef opName)
{
    if (opName != "tfillpad")
        return success();
    ...
    if (!(srcTb && dstTb && srcSpace && dstSpace && *srcSpace == mlir::pto::AddressSpace::MAT &&
          *dstSpace == mlir::pto::AddressSpace::MAT && srcTb != dstTb)) {
        return success();
    }

    auto dimToStr = [](int64_t dim) -> std::string { return dim == ShapedType::kDynamic ? "?" : std::to_string(dim); };
    ...
    if (srcValid.size() == kPTORowColRank && dstValid.size() == kPTORowColRank) {
        if (srcValid[0] != dstValid[0])
            mismatchFields.push_back("v_row (" + dimToStr(srcValid[0]) + " vs " + dimToStr(dstValid[0]) + ")");
        if (srcValid[1] != dstValid[1])
            mismatchFields.push_back("v_col (" + dimToStr(srcValid[1]) + " vs " + dimToStr(dstValid[1]) + ")");
    }
    if (srcTb.getPadValueI32() != dstTb.getPadValueI32()) {
        mismatchFields.push_back(
            "pad (" + std::to_string(srcTb.getPadValueI32()) + " vs " + std::to_string(dstTb.getPadValueI32()) + ")");
    }

    auto diag = op->emitError() << "expects src/dst tile types to be lowerable to TFILLPAD "
                                   "for loc=mat";
...
static LogicalResult verifyTFillPadDstPad(Operation* op, Type dstTy, llvm::StringRef opName)
{
...
static LogicalResult verifyTFillPadShapeCompatibility(
    Operation* op, ArrayRef<int64_t> srcShape, ArrayRef<int64_t> dstShape, bool allowDstExpand, llvm::StringRef opName)
{
    if (!allowDstExpand) {
        if (srcShape != dstShape) {
            return op->emitError() << "expects src and dst to have the same static shape for " << opName;
        }
        return mlir::success();
    }
    if (srcShape[0] > dstShape[0] || srcShape[1] > dstShape[1]) {
        return op->emitError() << "expects dst static shape to be >= src static shape for " << opName;
    }
    return mlir::success();
}

static mlir::LogicalResult verifyTFillPadLike(
    Operation* op, Type srcTy, Type dstTy, bool allowDstExpand, llvm::StringRef opName)
{
    ...
    if (!(srcB == static_cast<int64_t>(kPTOByteSize) || srcB == static_cast<int64_t>(kPTOHalfWordBytes) ||
    ...
    if (failed(verifyTFillPadMatHomogeneousConstraint(op, srcTy, dstTy, opName)) ||
        ...
        failed(verifyTFillPadShapeCompatibility(op, srcShape, dstShape, allowDstExpand, opName))) {
        return failure();
    }
    return mlir::success();
}
