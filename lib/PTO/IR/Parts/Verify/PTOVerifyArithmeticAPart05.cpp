...
static LogicalResult verifyArithmeticScalarTileOpWithArchDispatch(
    Operation* op, Type srcTy, Type dstTy, Type scalarTy, bool allowInt8OnA5, bool allowBf16OnA5, StringRef a2a3Error,
    StringRef a5Error, bool requireValidRowsEqualOnA2A3 = true, bool requireValidRowsEqualOnA5 = false)
{
    auto verifyByArch = [op, srcTy, dstTy, scalarTy, allowInt8OnA5, allowBf16OnA5, a2a3Error, a5Error](
                            PTOArch targetArch, bool requireValidRowsEqual) -> LogicalResult {
        FailureOr<Type> elemOr = verifyNumericScalarTileOpCommon(op, srcTy, dstTy, scalarTy, requireValidRowsEqual);
        if (failed(elemOr))
            return failure();
        return verifyArithmeticElemTypeForArch(
            op, *elemOr, targetArch, allowInt8OnA5, allowBf16OnA5, a2a3Error, a5Error);
    };
...
{
    bool ok =
        elemTy.isF16() || elemTy.isF32() || elemTy.isInteger(kPTOI16BitWidth) || elemTy.isInteger(kPTOI32BitWidth);
    if (targetArch == PTOArch::A5)
        ok = ok || (allowInt8OnA5 && elemTy.isInteger(kPTOI8BitWidth)) || (allowBf16OnA5 && elemTy.isBF16());
    if (ok)
        return success();
...
static LogicalResult verifyTColReductionOpWithArchDispatch(
    Operation* op, Type srcTy, Type dstTy, bool requireNonZeroSrcOnA2A3, bool requireNonZeroSrcOnA5, bool allowInt8OnA5,
    bool allowBf16OnA5, StringRef a2a3Error, StringRef a5Error)
{
    auto verifyByArch = [op, srcTy, dstTy, allowInt8OnA5, allowBf16OnA5, a2a3Error, a5Error](
                            PTOArch targetArch, bool requireNonZeroSrc) -> LogicalResult {
        if (failed(verifyNDStyleVecTile(op, srcTy, "src")) || failed(verifyNDStyleVecTile(op, dstTy, "dst")))
            return failure();
        ...
        return verifyTColReductionElemTypeForArch(
            op, elem, targetArch, allowInt8OnA5, allowBf16OnA5, a2a3Error, a5Error);
    };
...
static LogicalResult verifyTColArgReductionOpCommon(Operation* op, Type srcTy, Type tmpTy, Type dstTy)
{
    ...
    if (!(mlir::isa<IntegerType, FloatType>(srcElemTy) &&
          (srcElemBits == kPTOI8BitWidth || srcElemBits == kPTOI16BitWidth || srcElemBits == kPTOI32BitWidth)))
        return op->emitOpError("expects src/tmp element type to be 1, 2, or 4 bytes wide");
...
static bool isKnownUnitExtent(int64_t value) { return value == ShapedType::kDynamic || value == 1; }
...
static LogicalResult verifyVecTileUnaryOp(
    Operation* op, Type srcTy, Type dstTy, StringRef srcName, StringRef dstName, bool allowBf16, bool allowInt8)
{
    if (failed(verifyVecTileCommon(op, srcTy, srcName)) || failed(verifyVecTileCommon(op, dstTy, dstName)))
        return failure();
...
static LogicalResult verifyMatTileAddressSpaces(Operation* op, Type lhsTy, Type rhsTy, Type dstTy)
{
    ...
    if (*lhsSpace != pto::AddressSpace::LEFT || *rhsSpace != pto::AddressSpace::RIGHT ||
        *dstSpace != pto::AddressSpace::ACC) {
        return op->emitOpError("expects lhs, rhs, and dst to use the left, right, and acc address spaces");
    }
    return success();
}

static LogicalResult verifyMatTileLogicalShapes(Operation* op, Type lhsTy, Type rhsTy, Type dstTy)
{
    ...
    if (lhsShape[0] != dstShape[0] || rhsShape[1] != dstShape[1] || lhsShape[1] != rhsShape[0]) {
        return op->emitOpError("expects static matmul tile shapes lhs[M,K], rhs[K,N], and dst[M,N]");
    }
    return success();
}
