...
static LogicalResult verifyTPReluElemAndLayout(TPReluOp op, Type src0Ty, Type src1Ty, Type tmpTy, Type dstTy)
{
...
static LogicalResult verifyTPReluMatchingShapes(TPReluOp op, Type src0Ty, Type src1Ty, Type tmpTy, Type dstTy)
{
    if (failed(verifyTileBufSameValidShape(op, src0Ty, dstTy, "src0", "dst")) ||
        ...
        return failure();
    if (getShapeVec(src0Ty) != getShapeVec(src1Ty) || getShapeVec(src0Ty) != getShapeVec(tmpTy) ||
...
static LogicalResult verifyTPReluA2A3(TPReluOp op, const TPReluCommonInfo& common)
{
    ...
    if (auto arch = getVerifierArchName(op.getOperation()); arch && arch->equals_insensitive("a3")) {
        if (op.getSrc0() == op.getSrc1() || op.getSrc0() == op.getTmp() || op.getSrc0() == op.getDst() ||
            op.getSrc1() == op.getTmp() || op.getSrc1() == op.getDst() || op.getTmp() == op.getDst()) {
            return op.emitOpError("expects A3 src0, src1, tmp, and dst to use different storage");
        }
    }
    return success();
}
...
mlir::LogicalResult mlir::pto::TPReluOp::verify()
{
    ...
    if (failed(common))
        return failure();
    auto verifyA2A3 = [this, &common]() -> LogicalResult { return verifyTPReluA2A3(*this, *common); };
...
static LogicalResult verifyTQuantStructural(TQuantOp op)
{
    ...
    if (op.getQuantType() == mlir::pto::QuantType::INT8_SYM) {
        if (!dstIntTy || dstIntTy.getWidth() != kPTOI8BitWidth) {
            return op.emitOpError("expects dst element type i8/ui8 for INT8_SYM quantization");
        }
        if (op.getOffset()) {
            return op.emitOpError("INT8_SYM quantization must not have an offset operand");
        }
        return success();
    }

    if (!dstIntTy || dstIntTy.getWidth() != kPTOI8BitWidth) {
        return op.emitOpError("expects dst element type i8/ui8 for INT8_ASYM quantization");
    }
    if (!op.getOffset())
...
mlir::LogicalResult mlir::pto::TQuantOp::verify()
{
    ...
    auto verifyA2A3 = [this]() -> LogicalResult {
        if (failed(verifyTQuantCommon(*this)))
            return failure();
        if (!isRowMajorTileBuf(getSrc().getType()) || !isRowMajorTileBuf(getDst().getType())) {
...
mlir::LogicalResult mlir::pto::TDequantOp::verify()
{
    ...
    auto verifyStructural = [this]() -> LogicalResult {
        ...
        if (!srcIntTy || !(srcIntTy.getWidth() == kPTOI8BitWidth || srcIntTy.getWidth() == kPTOI16BitWidth))
            return emitOpError() << "expects src element type i8 or i16";
...
mlir::LogicalResult mlir::pto::TRecipOp::verify()
{
    ...
    if (auto arch = getVerifierArchName(getOperation()); arch && arch->equals_insensitive("a3") && getSrc() == getDst())
