    ...
    int32_t fractal)
{
    ...
    bool allowUnalignedRows = (loc && *loc == pto::AddressSpace::VEC) || fractal == kFractalSize32 || rows == 1;
...
static LogicalResult verifyTileBufLayoutConstraints(Operation* op, pto::TileBufType tb, StringRef name)
{
    ...
    return verifyBoxedTileBufLayout(op, tb, name, rows, cols, elemBytes, slayout, fractal);
}
...
[[maybe_unused]] static bool isSupportedLoadStoreElemTypeA2A3(Type ty)
{
    ...
    if (auto it = dyn_cast<IntegerType>(ty)) {
        ...
        return width == kPTOI8BitWidth || width == kPTOI16BitWidth || width == kPTOI32BitWidth ||
               width == kPTOI64BitWidth;
    }
    return false;
}
...
static std::optional<mlir::pto::Layout> inferLayout(
