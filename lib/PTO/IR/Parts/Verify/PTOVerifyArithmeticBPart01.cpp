...
LogicalResult pto::TAddOp::verify()
{
    return verifyArithmeticBinaryTileOpWithArchDispatch(
        ...
        /*allowInt8OnA5=*/true, /*allowBf16OnA5=*/true, "expects A2/A3 tadd element type to be i32/i16/f16/f32",
...
LogicalResult pto::TAddCOp::verify()
{
    ...
    if (!isPTOShapedLike(t0) || !isPTOShapedLike(t1) || !isPTOShapedLike(t2) || !isPTOShapedLike(td))
...
LogicalResult pto::TAddSOp::verify()
{
    return verifyArithmeticScalarTileOpWithArchDispatch(
        ...
        /*allowInt8OnA5=*/true, /*allowBf16OnA5=*/true, "expects A2/A3 tadds element type to be i32/i16/f16/f32",
...
static FailureOr<std::pair<Type, Type>> verifyTAxpyCommon(TAxpyOp op)
{
    ...
    if (failed(verifyVecTileCommon(op, srcTy, "src")) || failed(verifyVecTileCommon(op, dstTy, "dst")) ||
        failed(verifyTileBufSameValidShape(op, srcTy, dstTy, "src", "dst")))
        return failure();

    Type srcElem = getElemTy(srcTy);
    if (op.getScalar().getType() != srcElem)
        return op.emitOpError("expects scalar type to match src element type"), failure();
    if (getShapeVec(srcTy) != getShapeVec(dstTy))
        return op.emitOpError("expects src and dst to have the same shape"), failure();
    return std::make_pair(srcElem, getElemTy(dstTy));
}

static LogicalResult verifyTAxpyTypePair(Operation* op, Type srcElem, Type dstElem)
{
    bool sameType = srcElem == dstElem;
    bool widenF16ToF32 = srcElem.isF16() && dstElem.isF32();
    if (!(sameType || widenF16ToF32)) {
        return op->emitOpError("expects dst/src element types to match, or dst=f32 and src=f16");
    }
    return success();
}
...
static LogicalResult verifyArchIntegerWidthOp(
    Operation* op, VerifyCommonFn verifyCommon, StringRef a2a3Message, StringRef a5Message)
{
    auto verifyA2A3 = [&verifyCommon, op, a2a3Message]() -> LogicalResult {
...
static LogicalResult verifyRowMajorBinaryIntWidthOp(
    Operation* op, Type src0Ty, Type src1Ty, Type dstTy, StringRef a2a3Message, StringRef a5Message)
{
...
static LogicalResult verifyDistinctRowMajorUnaryIntWidthOp(
    Operation* op, Value src, Value dst, StringRef srcName, StringRef dstName, StringRef a2a3Message,
    StringRef a5Message)
{
    auto verifyCommon = [op, src, dst, srcName, dstName]() -> FailureOr<Type> {
        return verifyDistinctRowMajorUnaryTileOpCommon(op, src, dst, srcName, dstName);
    };
    return verifyArchIntegerWidthOp(op, verifyCommon, a2a3Message, a5Message);
}
...
static LogicalResult verifyTConcatValidRows(
    TConcatOp op, ArrayRef<int64_t> src0Valid, ArrayRef<int64_t> src1Valid, ArrayRef<int64_t> dstValid)
{
    ...
    if (src0Valid[0] != ShapedType::kDynamic && dstValid[0] != ShapedType::kDynamic && src0Valid[0] != dstValid[0]) {
