...
static LogicalResult verifyRowReductionValidRegion(Operation* op, Type srcTy, Type dstTy)
{
...
{
    return elem.isInteger(kPTOI16BitWidth) || elem.isInteger(kPTOI32BitWidth) || elem.isF16() || elem.isF32();
}

static LogicalResult verifyTRowReductionNoTmpCommon(Operation* op, Type srcTy, Type dstTy, StringRef elemTypeError)
{
    if (failed(verifyRowReductionSrcLayout(op, srcTy, "src")) || failed(verifyRowReductionDstLayout(op, dstTy, "dst")))
        return failure();
...
static LogicalResult verifyTRowArgReductionCommon(Operation* op, Type srcTy, Type tmpTy, Type dstTy)
{
    if (failed(verifyRowReductionSrcLayout(op, srcTy, "src")) || failed(verifyVecTileCommon(op, tmpTy, "tmp")) ||
        failed(verifyRowReductionDstLayout(op, dstTy, "dst")))
        return failure();
...
LogicalResult mlir::pto::MakeTensorViewOp::verify()
{
    ...
    if (static_cast<int64_t>(getShape().size()) != rank || static_cast<int64_t>(getStrides().size()) != rank)
        return emitOpError() << "shape/strides operand counts must match tensor_view rank=" << rank;

    // Detect dynamic shape/stride.
    bool hasDynamicShape = llvm::any_of(tvTy.getShape(), [](int64_t v) { return v == ShapedType::kDynamic; });
    bool hasDynamicStride = llvm::any_of(getStrides(), [](Value s) { return !getConstIndexValue(s).has_value(); });
    ...
    bool allStaticShape = llvm::none_of(tvTy.getShape(), [](int64_t v) { return v == ShapedType::kDynamic; });
    if (layoutAttr && allStaticShape && allStaticStride) {
        SmallVector<int64_t> shapeInts(tvTy.getShape().begin(), tvTy.getShape().end());
        if (auto inferred = inferLayout(shapeInts, strideInts, getElemByteSize(tvTy.getElementType()))) {
            (void)inferred;
        }
    }
...
static LogicalResult verifyPartitionViewDimension(
    PartitionViewOp op, int64_t dimIdx, ArrayRef<int64_t> srcShape, ArrayRef<int64_t> resShape, bool sameRank)
{
    auto offVal = getConstIndexValue(op.getOffsets()[dimIdx]);
    auto sizeVal = getConstIndexValue(op.getSizes()[dimIdx]);
    if (offVal && *offVal < 0)
        return op.emitOpError() << "offset at dim " << dimIdx << " must be non-negative, got " << *offVal;
    if (sizeVal && *sizeVal <= 0)
        return op.emitOpError() << "size at dim " << dimIdx << " must be positive, got " << *sizeVal;
    if (sameRank && sizeVal) {
        int64_t resDim = resShape[dimIdx];
        if (resDim != ShapedType::kDynamic && *sizeVal != resDim) {
            return op.emitOpError() << "size/result mismatch at dim " << dimIdx << ": size operand=" << *sizeVal
    ...
    if (sizeVal && *sizeVal > srcDim) {
        return op.emitOpError() << "size at dim " << dimIdx << " (" << *sizeVal << ") exceeds static source dim ("
                                << srcDim << ")";
    }
    if (offVal && sizeVal && (*offVal + *sizeVal > srcDim)) {
        return op.emitOpError() << "offset+size at dim " << dimIdx << " (" << (*offVal + *sizeVal)
