...
static LogicalResult verifyTConcatDstCols(
    TConcatOp op, Type dstTy, ArrayRef<int64_t> src0Valid, ArrayRef<int64_t> src1Valid)
{
    auto dstShape = getShapeVec(dstTy);
    if (dstShape.size() == kPTORowColRank && dstShape[1] != ShapedType::kDynamic &&
        src0Valid[1] != ShapedType::kDynamic && src1Valid[1] != ShapedType::kDynamic &&
...
static FailureOr<BinaryTileTypeInfo> verifyBinaryTileTypeInfo(
    Operation* op, Value src0, Value src1, Value dst, VerifyFn verifyOperand)
{
    Type src0Ty = src0.getType();
    Type src1Ty = src1.getType();
    Type dstTy = dst.getType();
    if (failed(verifyOperand(op, src0Ty, "src0")) || failed(verifyOperand(op, src1Ty, "src1")) ||
...
static FailureOr<Type> verifyTConcatCommon(TConcatOp op)
{
    ...
    auto infoOr = verifyBinaryTileTypeInfo(op, op.getSrc0(), op.getSrc1(), op.getDst(), verifyTileBufOperand);
    if (failed(infoOr))
        return failure();
    const auto& info = *infoOr;
    if (info.src0Elem != info.src1Elem || info.src0Elem != info.dstElem) {
        return op.emitOpError("expects src0, src1, and dst to have the same element type"), failure();
    }
...
static LogicalResult verifyTColExpandRowMajorLayout(Operation* op, Type ty, StringRef name)
{
    if (auto tileTy = dyn_cast<TileBufType>(ty); tileTy && tileTy.getBLayoutValueI32() != 0) {
        return op->emitOpError() << "expects " << name << " to use row-major layout";
    }
    return success();
}

static LogicalResult verifyTColExpandSrc1ValidCols(Operation* op, Type t1, Type td)
{
...
static LogicalResult verifyTColExpandShapeAndLayout(Operation* op, Type t0, Type t1, Type td)
{
    ...
    if (failed(verifyTileBufSameValidShape(op, t0, td, "src0", "dst")) ||
        ...
        failed(verifyTColExpandRowMajorLayout(op, td, "dst")) || failed(verifyTColExpandSrc1ValidCols(op, t1, td)))
        return failure();
    return success();
}
...
static LogicalResult verifyConcatidxElementTypes(Operation* op, Type dataElem, Type idxElem)
{
...
static LogicalResult verifyTConcatidxValidShapes(TConcatidxOp op)
{
    ...
    auto checkValidRow = [opBase, &dstValid](const auto& validShape, StringRef name) -> LogicalResult {
        if (validShape[0] != ShapedType::kDynamic && dstValid[0] != ShapedType::kDynamic &&
            validShape[0] != dstValid[0]) {
            opBase->emitOpError("expects ") << name << " valid row to match dst valid row";
            return failure();
        }
        return success();
    };
    if (failed(checkValidRow(src0Valid, "src0")) || failed(checkValidRow(src1Valid, "src1")) ||
        failed(checkValidRow(src0IdxValid, "src0Idx")) || failed(checkValidRow(src1IdxValid, "src1Idx"))) {
        return failure();
    }
...
static FailureOr<std::pair<Type, Type>> verifyTConcatidxCommon(TConcatidxOp op)
{
    ...
    if (failed(verifyTileBufCommon(op, src0Ty, "src0")) || failed(verifyTileBufCommon(op, src1Ty, "src1")) ||
        ...
        failed(verifyTileBufCommon(op, src1IdxTy, "src1Idx")) || failed(verifyTileBufCommon(op, dstTy, "dst"))) {
        return failure();
    }
    ...
    Type dstElem = getElemTy(dstTy);
    if (!src0Elem || !src1Elem || !dstElem)
        return op.emitOpError("failed to get element type for data operands"), failure();
    ...
    if (!src0IdxElem || !src1IdxElem) {
        return op.emitOpError("failed to get element type for index operands"), failure();
    }
    if (src0IdxElem != src1IdxElem) {
        return op.emitOpError("expects src0Idx and src1Idx to have the same element type"), failure();
    }
