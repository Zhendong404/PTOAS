...
static ParseResult parseKeywordedOperand(
    OpAsmParser& parser, StringRef keyword, OpAsmParser::UnresolvedOperand& operand)
{
    if (parser.parseKeyword(keyword) || parser.parseLParen() || parser.parseOperand(operand))
        return failure();
    return success();
}

static ParseResult parseRequiredCommaOperand(OpAsmParser& parser, OpAsmParser::UnresolvedOperand& operand)
{
...
static ParseResult parseOptionalCommaOperand(
    OpAsmParser& parser, OpAsmParser::UnresolvedOperand& operand, bool& isPresent)
{
...
static ParseResult parseOptionalCommaType(OpAsmParser& parser, bool isPresent, Type& type)
{
    if (!isPresent)
        return success();
...
static ParseResult parseOutsClauseWithOptionalAttrDict(
    OpAsmParser& parser, OperationState& result, OpAsmParser::UnresolvedOperand& dst, Type& dstTy)
{
...
static ParseResult resolveOptionalOperand(
    OpAsmParser& parser, bool isPresent, OpAsmParser::UnresolvedOperand& operand, Type type,
...
static void addOperandSegmentSizesAttr(OpAsmParser& parser, OperationState& result, ArrayRef<int32_t> sizes)
{
    result.addAttribute("operandSegmentSizes", parser.getBuilder().getDenseI32ArrayAttr(sizes));
}
...
ParseResult mlir::pto::TSort32Op::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parser.resolveOperand(src, srcTy, result.operands) || parser.resolveOperand(idx, idxTy, result.operands))
        return failure();
...
void mlir::pto::TSort32Op::print(OpAsmPrinter& p)
{
    ...
    if (getTmp()) {
        p << ", " << getTmp();
        p << " : " << getSrc().getType() << ", " << getIdx().getType() << ", " << getTmp().getType() << ")";
    } else {
...
static ParseResult parseTRowExpandBinaryLikeOp(OpAsmParser& parser, OperationState& result)
{
    OpAsmParser::UnresolvedOperand src0, src1, tmp, dst;
    Type src0Ty, src1Ty, tmpTy, dstTy;
    bool hasTmp = false;

    if (parseKeywordedOperand(parser, "ins", src0) || parseRequiredCommaOperand(parser, src1))
        return failure();
    if (parseOptionalCommaOperand(parser, tmp, hasTmp))
        return failure();
    ...
    if (parser.resolveOperand(src0, src0Ty, result.operands) || parser.resolveOperand(src1, src1Ty, result.operands))
        return failure();
    if (resolveOptionalOperand(parser, hasTmp, tmp, tmpTy, result.operands))
        return failure();
    if (parser.resolveOperand(dst, dstTy, result.operands))
...
static void printTRowExpandBinaryLikeOp(OpAsmPrinter& p, Operation* op, Value src0, Value src1, Value tmp, Value dst)
{
    p << " ins(" << src0 << ", " << src1;
    if (tmp) {
        p << ", " << tmp;
        p << " : " << src0.getType() << ", " << src1.getType() << ", " << tmp.getType() << ")";
    } else {
