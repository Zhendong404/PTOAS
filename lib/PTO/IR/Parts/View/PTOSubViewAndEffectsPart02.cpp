...
static LogicalResult computeInnerShape(
    TileBufConfigAttr cfg, Type elemTy, int64_t& innerRows, int64_t& innerCols, bool& boxed, int32_t& bl, int32_t& sl)
{
    bl = 0;
    sl = 0;
...
static LogicalResult verifySubviewExplicitValids(SubViewOp op, int64_t sizeR, int64_t sizeC)
{
    ...
    if (hasValidRow != hasValidCol) {
        return op.emitOpError("subview expects valid_row and valid_col to be both present or both absent");
    }
    if (!hasValidRow)
        return success();
...
static LogicalResult verifySubviewResultValidShape(SubViewOp op, const SubViewVerifyInfo& info)
{
...
{
    return shape.size() == kPTORowColRank && shape[0] != ShapedType::kDynamic && shape[1] != ShapedType::kDynamic;
}

static LogicalResult verifyBoxedSubviewMajorConstraint(
    SubViewOp op, const SubViewVerifyInfo& info, ArrayRef<int64_t> srcShape, int32_t bl)
{
    if (bl == 0) {
        if (info.sizeC != srcShape[1])
            return op.emitOpError("boxed RowMajor subview must keep full cols");
        if (!info.offCConst || info.offC != 0)
            return op.emitOpError("boxed RowMajor subview requires static col offset = 0");
        return success();
    }
    if (bl == 1) {
        ...
        if (!info.offRConst || info.offR != 0)
            return op.emitOpError("boxed ColMajor subview requires static row offset = 0");
    }
    return success();
}

static LogicalResult verifyBoxedSubviewLayout(SubViewOp op, const SubViewVerifyInfo& info)
{
    ...
    int32_t bl = 0;
    int32_t sl = 0;
    if (failed(computeInnerShape(cfg, info.srcTy.getElementType(), innerRows, innerCols, boxed, bl, sl))) {
