...
static bool extractStrideFromMulExpr(AffineExpr lhs, AffineExpr rhs, unsigned& position, int64_t& stride)
{
...
static bool extractStrideFromAffineTerm(AffineExpr term, unsigned& position, int64_t& stride)
{
    auto mul = llvm::dyn_cast<AffineBinaryOpExpr>(term);
    if (!mul || mul.getKind() != AffineExprKind::Mul)
        return false;
    return extractStrideFromMulExpr(mul.getLHS(), mul.getRHS(), position, stride) ||
           extractStrideFromMulExpr(mul.getRHS(), mul.getLHS(), position, stride);
}
...
static AffineMap buildStrictBitwiseAffineMap(MLIRContext* ctx, ArrayRef<int64_t> strides, bool isMultiDimSymbol)
{
...
[[maybe_unused]] static ParseResult parseStridedLayout(AsmParser& parser, Attribute& layout)
{
    ...
    if (isMultiDim && numSymbols != strides.size()) {
        return parser.emitError(parser.getCurrentLocation(), "Number of offset symbols must match rank");
    }
...
{
    if (parser.parseOperand(source) || parser.parseLSquare() || parser.parseOperandList(offsets) ||
        parser.parseRSquare() || parser.parseKeyword("sizes")) {
        return failure();
    }
    ArrayAttr sizesAttr;
...
static ParseResult parseSubViewValids(OpAsmParser& parser, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& valids)
{
    if (failed(parser.parseOptionalKeyword("valid")))
        return success();
    OpAsmParser::UnresolvedOperand rowValid;
    OpAsmParser::UnresolvedOperand colValid;
    if (parser.parseLSquare() || parser.parseOperand(rowValid) || parser.parseComma() ||
        parser.parseOperand(colValid) || parser.parseRSquare()) {
        return failure();
    }
...
static ParseResult resolveSubViewSourceAndIndices(
    ...
    bool& hasExplicitResultTy, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& offsets,
...
static ParseResult finalizeSubViewResultTypes(
    OpAsmParser& parser, OperationState& result, Type resultTy, bool hasExplicitResultTy)
{
    ...
    if (failed(SubViewOp::inferReturnTypes(
            parser.getContext(), std::nullopt, result.operands, attrs, nullptr, RegionRange(), inferredReturnTypes))) {
        return parser.emitError(parser.getCurrentLocation(), "failed to infer pto.subview result type");
    }
