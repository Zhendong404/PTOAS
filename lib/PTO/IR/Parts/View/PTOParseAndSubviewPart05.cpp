...
mlir::LogicalResult mlir::pto::TScatterOp::verify()
{
    ...
    if (hasIndexes == hasMaskPattern) {
        return emitOpError("expects exactly one of indexes operand or maskPattern attribute");
    }
...
static LogicalResult verifySelectElementType(
    Operation* op, Type elem, PTOArch targetArch, bool allowBf16, StringRef a2a3Message, StringRef a5Message)
{
...
static FailureOr<Type> verifyTSelSCommon(TSelSOp op)
{
    ...
    Type dstTy = op.getDst().getType();
    if (failed(verifyTileBufCommon(op, maskTy, "mask")) || failed(verifyTileBufCommon(op, srcTy, "src")) ||
...
mlir::LogicalResult mlir::pto::TShlOp::verify()
{
    auto verify = [this]() -> LogicalResult {
        FailureOr<Type> elemOr =
            verifyShiftLikeBinaryTileOpCommon(*this, getSrc0().getType(), getSrc1().getType(), getDst().getType());
        if (failed(elemOr))
            return failure();
        ...
        if (!it ||
            ...
            return emitOpError("expects tshl src0 and src1 element type to be i8/i16/i32");
        return success();
    };
...
mlir::LogicalResult mlir::pto::TShrOp::verify()
{
    auto verify = [this]() -> LogicalResult {
        ...
        if (!it ||
            (it.getWidth() != kPTOI8BitWidth && it.getWidth() != kPTOI16BitWidth && it.getWidth() != kPTOI32BitWidth))
            return emitOpError("expects tshr src0 and src1 element type to be i8/i16/i32");
        return success();
    };
    return dispatchVerifierByArch(getOperation(), verify, verify);
}
