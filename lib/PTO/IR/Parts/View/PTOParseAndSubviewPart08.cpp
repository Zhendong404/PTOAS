...
[[maybe_unused]] static LogicalResult verifyMatmulCommon(
    Operation* op, Value lhs, Value rhs, Value biasOpt, Type maybeDstElemTy, Type maybeResultElemTy)
{
    if (auto lhsTy = dyn_cast<ShapedType>(lhs.getType())) {
        return verifyMatmulShapedCommon(op, lhsTy, rhs, biasOpt, maybeDstElemTy, maybeResultElemTy);
    }
    ...
    return verifyMatmulTileCommon(op, lhsTile, rhs, biasOpt, maybeDstElemTy, maybeResultElemTy);
}

using VerifyMatTileOperandsFn = LogicalResult (*)(Operation*, Type, Type, Type);

static LogicalResult verifyMatmulLikeTileOp(
    Operation* op, Type lhsTy, Type rhsTy, Type dstTy, VerifyMatTileOperandsFn verifyOperands)
{
    if (failed(verifyOperands(op, lhsTy, rhsTy, dstTy)))
        return failure();
    if (failed(verifyMatmulTypeTriple(op, getElemTy(lhsTy), getElemTy(rhsTy), getElemTy(dstTy))))
        return failure();
...
LogicalResult mlir::pto::TMatmulOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        return verifyMatmulLikeTileOp(
            *this, getLhs().getType(), getRhs().getType(), getDst().getType(), verifyMatTileOperands);
    };
...
LogicalResult mlir::pto::TGemvOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        return verifyMatmulLikeTileOp(
            *this, getLhs().getType(), getRhs().getType(), getDst().getType(), verifyGemvTileOperands);
    };
    auto verifyA5 = [&verifyA2A3]() -> LogicalResult { return verifyA2A3(); };
    return dispatchVerifierByArch(getOperation(), verifyA2A3, verifyA5);
}
...
LogicalResult mlir::pto::TGemvAccOp::verify()
{
    if (shouldBypassDecodedMemrefVerifier(getOperation()))
        return success();
    if (failed(verifyAccTileCommon(*this, getAccIn().getType(), "acc_in")) ||
        failed(verifyGemvTileOperands(*this, getLhs().getType(), getRhs().getType(), getDst().getType())))
        return failure();
    return success();
}
...
[[maybe_unused]] static mlir::Type inferMatmulTileResult2DFromAB(MLIRContext* context, ValueRange operands)
{
    ...
    if (lhsShape.size() >= kPTORowColRank && rhsShape.size() >= kPTORowColRank) {
...
[[maybe_unused]] static RankedTensorType inferMatmulResult2DFromAB(ValueRange operands)
{
    ...
    if (lhsTy.getRank() >= kPTORowColRank && rhsTy.getRank() >= kPTORowColRank) {
...
static LogicalResult parseShapeAndElem(
    AsmParser& parser, SmallVectorImpl<int64_t>& shape, Type& elementType, bool allowDynamic)
{
    if (parser.parseLess())
        return failure();
...
Type PartitionTensorViewType::parse(AsmParser& odsParser)
{
    ...
    Type elemTy;
    if (failed(parseShapeAndElem(
            odsParser, shape, elemTy,
            /*allowDynamic=*/true)))
        return Type();
...
LogicalResult LocalArrayType::verify(
    llvm::function_ref<InFlightDiagnostic()> emitError, llvm::ArrayRef<int64_t> shape, Type elementType)
{
    if (shape.empty())
        return emitError() << "'!pto.local_array' requires at least one dimension";
    for (auto [i, d] : llvm::enumerate(shape)) {
        if (d <= 0)
            return emitError() << "'!pto.local_array' dimension " << i << " must be a positive static size, got " << d;
    }
    if (!elementType.isIntOrFloat())
        return emitError() << "'!pto.local_array' element type must be a scalar integer or "
                              "float, got "
                           << elementType;
    return success();
}
