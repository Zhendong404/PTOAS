...
static std::optional<ArrayAttr> getSubViewSizeAttr(DictionaryAttr attributes, OpaqueProperties properties)
{
    if (properties) {
...
static void decodeSubviewExplicitValidOperandsFromSegments(
    SubViewExplicitValidOperands& explicitValids, ValueRange operands, DictionaryAttr attributes)
{
    if (!attributes)
        return;
    ...
    if (srcSeg != 1 || offSeg < 0 || (vRowSeg != 0 && vRowSeg != 1) || (vColSeg != 0 && vColSeg != 1))
        return;
    ...
    ValueRange operands, DictionaryAttr attributes, int64_t rank)
{
    ...
    decodeSubviewExplicitValidOperandsFromSegments(explicitValids, operands, attributes);
...
static SmallVector<int64_t> inferSubviewValidShape(
    ArrayRef<int64_t> subviewShape, Value explicitVRow, Value explicitVCol)
{
...
LogicalResult SubViewOp::inferReturnTypes(
    MLIRContext* context, std::optional<Location> location, ValueRange operands, DictionaryAttr attributes,
    OpaqueProperties properties, RegionRange regions, SmallVectorImpl<Type>& inferredReturnTypes)
{
    (void)location;
    (void)regions;
    if (operands.empty())
    ...
    auto explicitValids =
        decodeSubviewExplicitValidOperands(operands, attributes, static_cast<int64_t>(subviewShape.size()));
    auto validShape = inferSubviewValidShape(subviewShape, explicitValids.row, explicitValids.col);
    ...
    inferredReturnTypes.push_back(TileBufType::get(
        context, subviewShape, sourceType.getElementType(), sourceType.getMemorySpace(), canonicalValidShape, cfg));
    return success();
}
...
static LogicalResult computeBoxedInnerShape(
    Type elemTy, int32_t fractalSize, int32_t slayout, int64_t& innerRows, int64_t& innerCols)
{
    ...
    if (fractalSize == kFractalSize512 && slayout == static_cast<int32_t>(SLayout::RowMajor)) {
        innerRows = kFractalSize16;
        innerCols = kFractalSize32 / elemBytes;
        return success();
    }
    if (fractalSize == kFractalSize512 && slayout == static_cast<int32_t>(SLayout::ColMajor)) {
