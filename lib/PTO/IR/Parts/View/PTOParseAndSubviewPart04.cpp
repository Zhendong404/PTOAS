...
mlir::LogicalResult mlir::pto::TRowExpandMinOp::verify()
{
    auto verifyA2A3 = [this]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            ...
            getTmp() ? getTmp().getType() : Type{}, static_cast<bool>(getTmp()), PTOArch::A3, "trowexpandmin",
            /*allowIntegerTypes=*/true);
    };
    auto verifyA5 = [this]() -> LogicalResult {
        return verifyTRowExpandReduceLikeOp(
            getOperation(), getSrc0().getType(), getSrc1().getType(), getDst().getType(),
            getTmp() ? getTmp().getType() : Type{}, static_cast<bool>(getTmp()), PTOArch::A5, "trowexpandmin",
            /*allowIntegerTypes=*/true);
    };
...
mlir::LogicalResult mlir::pto::TRowMaxOp::verify()
{
    auto verifyByArch = [this]() -> LogicalResult {
        return verifyTRowReductionNoTmpCommon(
            *this, getSrc().getType(), getDst().getType(), "expects element type to be i16/i32/f16/f32");
    };
...
{
    auto verifyByArch = [this]() -> LogicalResult {
        return verifyTRowArgReductionCommon(*this, getSrc().getType(), getTmp().getType(), getDst().getType());
    };
...
static LogicalResult verifyTScatterIndexedForm(TScatterOp op)
{
    ...
    if (failed(verifyVecTileStorage(op, srcTy, "src")) || failed(verifyVecTileStorage(op, indexTy, "indexes")) ||
    ...
    if (dataWidth != kPTOI8BitWidth && dataWidth != kPTOI16BitWidth && dataWidth != kPTOI32BitWidth)
...
static LogicalResult verifyTScatterMaskForm(TScatterOp op)
{
    ...
    if (srcValid[1] != ShapedType::kDynamic && dstValid[1] != ShapedType::kDynamic &&
        ...
        return op.emitOpError("expects src valid cols to equal dst valid cols times the mask expansion factor");
    }

    if (srcTile.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::RowMajor) ||
        dstTile.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::RowMajor)) {
