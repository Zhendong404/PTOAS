...
{
    printTRowExpandBinaryLikeOp(p, getOperation(), getSrc0(), getSrc1(), getTmp(), getDst());
}

ParseResult mlir::pto::TRowExpandExpdifOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseTRowExpandBinaryLikeOp(parser, result);
}
...
ParseResult mlir::pto::TRowExpandMaxOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseTRowExpandBinaryLikeOp(parser, result);
}
...
ParseResult mlir::pto::TRowExpandMinOp::parse(OpAsmParser& parser, OperationState& result)
{
    return parseTRowExpandBinaryLikeOp(parser, result);
}
...
static FailureOr<Type> verifyTRowExpandBinaryCore(
    Operation* op, Type src0Ty, Type src1Ty, Type dstTy, Type tmpTy, bool hasTmp)
{
...
static bool isTRowExpandBinaryElemSupported(Type elem, PTOArch targetArch, bool allowA2A3IntegerTypes)
{
    ...
    return allowA2A3IntegerTypes && (elem.isInteger(kPTOI16BitWidth) || elem.isInteger(kPTOI32BitWidth));
}

static LogicalResult verifyTRowExpandBinaryElemType(
    Operation* op, Type elem, PTOArch targetArch, bool allowA2A3IntegerTypes, StringRef a2a3Message,
    StringRef a5Message)
{
    if (isTRowExpandBinaryElemSupported(elem, targetArch, allowA2A3IntegerTypes))
        return success();
    if (targetArch == PTOArch::A5)
...
static LogicalResult verifyTRowExpandBinaryLikeOp(
    Operation* op, Type src0Ty, Type src1Ty, Type dstTy, Type tmpTy, bool hasTmp, PTOArch targetArch,
    bool allowA2A3IntegerTypes, StringRef a2a3Message, StringRef a5Message)
{
    FailureOr<Type> elemOr = verifyTRowExpandBinaryCore(op, src0Ty, src1Ty, dstTy, tmpTy, hasTmp);
    if (failed(elemOr))
        return failure();
    return verifyTRowExpandBinaryElemType(op, *elemOr, targetArch, allowA2A3IntegerTypes, a2a3Message, a5Message);
}

static LogicalResult verifyTRowExpandAddSrc1Shape(Operation* op, Type src1Ty, Type dstTy, Type elem)
{
    ...
    if (src1Valid[0] != ShapedType::kDynamic && dstValid[0] != ShapedType::kDynamic && src1Valid[0] != dstValid[0])
...
mlir::LogicalResult mlir::pto::TRowExpandDivOp::verify()
{
    auto verifyByArch = [this](PTOArch targetArch) -> LogicalResult {
        return verifyTRowExpandBinaryLikeOp(
            *this, getSrc0().getType(), getSrc1().getType(), getDst().getType(), getTmp() ? getTmp().getType() : Type{},
            static_cast<bool>(getTmp()), targetArch, /*allowA2A3IntegerTypes=*/false,
            "expects element type to be f16 or f32", "expects A5 trowexpanddiv element type to be i8/i16/i32/f16/f32");
    };
...
mlir::LogicalResult mlir::pto::TRowExpandMulOp::verify()
{
    auto verifyByArch = [this](PTOArch targetArch) -> LogicalResult {
        return verifyTRowExpandBinaryLikeOp(
            *this, getSrc0().getType(), getSrc1().getType(), getDst().getType(), getTmp() ? getTmp().getType() : Type{},
            static_cast<bool>(getTmp()), targetArch, /*allowA2A3IntegerTypes=*/true,
...
mlir::LogicalResult mlir::pto::TRowExpandAddOp::verify()
{
    auto verifyByArch = [this](PTOArch targetArch) -> LogicalResult {
        ...
        FailureOr<Type> elemOr = verifyTRowExpandBinaryCore(*this, src0Ty, src1Ty, dstTy, Type{}, false);
        if (failed(elemOr))
            return failure();
...
static LogicalResult verifyTRowExpandReduceTypes(
    Operation* op, Type src0Ty, Type src1Ty, Type dstTy, Type tmpTy, bool hasTmp)
{
    if (failed(verifyTileBufCommon(op, src0Ty, "src0")) || failed(verifyTileBufCommon(op, src1Ty, "src1")) ||
        failed(verifyTileBufCommon(op, dstTy, "dst")))
