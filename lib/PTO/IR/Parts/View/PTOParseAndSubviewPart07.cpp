...
static LogicalResult verifyTTransA2A3Constraints(TTransOp op, const TTransVerifyState& state)
{
    auto srcTile = dyn_cast<pto::TileBufType>(state.srcTy);
    if (!srcTile)
        return success();
    if (srcTile.getBLayoutValueI32() != static_cast<int32_t>(pto::BLayout::RowMajor)) {
        return op.emitOpError() << "expects A2/A3 transpose src to use the row_major blayout";
    }
    return success();
}

static LogicalResult verifyTTransA5MajorAlignment(TTransOp op, Type type, unsigned elemBytes, StringRef name)
{
    ...
    if (shape.size() != kPTORowColRank)
        return success();
    bool rowMajor = tile.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::RowMajor);
    int64_t major = rowMajor ? shape[1] : shape[0];
    if (major != ShapedType::kDynamic && (major * static_cast<int64_t>(elemBytes)) % kNumber32 != 0) {
...
mlir::LogicalResult mlir::pto::TTransOp::verify()
{
    ...
    auto verifyA5 = [this]() -> LogicalResult {
        FailureOr<TTransVerifyState> stateOr =
            verifyTTransCommon(*this, "expects src, tmp, and dst to have the same element type");
        if (failed(stateOr))
            return failure();
        if (failed(verifyTTransA5MajorAlignment(*this, stateOr->srcTy, stateOr->elemBytes, "src")) ||
            failed(verifyTTransA5MajorAlignment(*this, stateOr->dstTy, stateOr->elemBytes, "dst")))
            return failure();
        return success();
    };
...
mlir::LogicalResult mlir::pto::TXorOp::verify()
{
    ...
    auto verifyA2A3 = [this, &verifyBase]() -> LogicalResult {
        ...
        if (!it || (it.getWidth() != kPTOI8BitWidth && it.getWidth() != kPTOI16BitWidth))
            return emitOpError("expects A2/A3 txor src0, src1, tmp, and dst element type to be i8/i16");
        return success();
    };
    ...
    auto verifyA5 = [this, &verifyBase]() -> LogicalResult {
        ...
        if (!it ||
            (it.getWidth() != kPTOI8BitWidth && it.getWidth() != kPTOI16BitWidth && it.getWidth() != kPTOI32BitWidth))
            return emitOpError("expects A5 txor src0, src1, and dst element type to be i8/i16/i32");
        return success();
    };
...
mlir::LogicalResult mlir::pto::TXorSOp::verify()
{
    auto verifyCommon = [this]() -> FailureOr<Type> {
        return verifyDistinctRowMajorUnaryTileOpCommon(getOperation(), getSrc(), getDst(), "src", "dst");
    };
    return verifyArchIntegerWidthOp(
        getOperation(), verifyCommon, "expects A2/A3 txors src and dst element type to be i8/i16",
...
mlir::LogicalResult mlir::pto::TPrintOp::verify()
{
    ...
    if (auto tb = mlir::dyn_cast<mlir::pto::TileBufType>(srcType)) {
        ...
        if (!(elem.isF16() || elem.isF32() || elem.isInteger(kPTOI8BitWidth) || elem.isInteger(kPTOI16BitWidth) ||
              elem.isInteger(kPTOI32BitWidth)))
    ...
    if (mlir::dyn_cast<MemRefType>(srcType) || mlir::dyn_cast<mlir::pto::PartitionTensorViewType>(srcType))
        return mlir::success();
    return emitOpError() << "expects tile_buf, memref, or partition_tensor_view for src";
}

static LogicalResult verifyMatmulShapedCommon(
    Operation* op, ShapedType lhsTy, Value rhs, Value biasOpt, Type maybeDstElemTy, Type maybeResultElemTy)
{
    ...
    if (lhsTy.getElementType() != rhsTy.getElementType()) {
        return op->emitOpError() << "expects lhs and rhs to have the same element type, but got lhs="
    ...
    if (biasOpt) {
        ...
        if (biasTy.getElementType() != lhsTy.getElementType()) {
            return op->emitOpError() << "expects bias to have the same element type as lhs and rhs, but got bias="
                                     << biasTy.getElementType() << " vs " << lhsTy.getElementType();
        }
    }

    if (maybeDstElemTy && maybeDstElemTy != lhsTy.getElementType()) {
        return op->emitOpError() << "expects dst to have the same element type as lhs and rhs, but got dst="
                                 << maybeDstElemTy << " vs " << lhsTy.getElementType();
    }
    if (maybeResultElemTy && maybeResultElemTy != lhsTy.getElementType()) {
        return op->emitOpError() << "expects result to have the same element type as lhs and rhs, but got result="
                                 << maybeResultElemTy << " vs " << lhsTy.getElementType();
    }
    return success();
}

static LogicalResult verifyMatmulTileCommon(
    Operation* op, TileType lhsTile, Value rhs, Value biasOpt, Type maybeDstElemTy, Type maybeResultElemTy)
{
    ...
    if (lhsTile.getShape()[1] != rhsTile.getShape()[0]) {
        return op->emitOpError() << "expects lhs dim1 to equal rhs dim0, but got " << lhsTile.getShape()[1] << " vs "
                                 << rhsTile.getShape()[0];
    }
