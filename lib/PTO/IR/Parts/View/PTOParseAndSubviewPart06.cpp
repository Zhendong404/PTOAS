...
static bool shouldBypassTStoreFPVerifier(TStoreFPOp op)
{
    Value src = op.getSrc();
    Value fp = op.getFp();
    return isa<MemRefType>(src.getType()) || isa<MemRefType>(fp.getType()) || src.getDefiningOp<pto::BindTileOp>() ||
...
static LogicalResult verifyTStoreFPDstType(TStoreFPOp op)
{
    ...
    if (!isa<MemRefType, pto::PartitionTensorViewType>(dstTy))
        return op.emitOpError() << "expects dst to be a memref or !pto.partition_tensor_view";
    if (auto dstPart = dyn_cast<pto::PartitionTensorViewType>(dstTy)) {
        for (auto [idx, dim] : llvm::enumerate(dstPart.getShape())) {
            if (dim != ShapedType::kDynamic && dim <= 0) {
                return op.emitOpError() << "expects dst shape[" << idx << "] to be positive";
            }
        }
    }
    return success();
...
static LogicalResult verifyTStoreFPA2A3Constraints(TStoreFPOp op)
{
    ...
    if (srcValid[kPTOColumnDim] != ShapedType::kDynamic &&
        ...
        return op.emitOpError() << "expects src.valid_shape[1] to be in the range [1, 4095]";
    }
    return success();
}
...
mlir::LogicalResult mlir::pto::TStoreFPOp::verify()
{
    ...
    auto verifyA5 = [this]() -> LogicalResult { return verifyTStoreFPTileOperands(*this); };
...
mlir::LogicalResult mlir::pto::TSubOp::verify()
{
    return verifyArithmeticBinaryTileOpWithArchDispatch(
        ...
        /*allowInt8OnA5=*/true, /*allowBf16OnA5=*/false, "expects A2/A3 tsub element type to be i32/i16/f16/f32",
...
mlir::LogicalResult mlir::pto::TSubSOp::verify()
{
    return verifyArithmeticScalarTileOpWithArchDispatch(
        ...
        /*allowInt8OnA5=*/true, /*allowBf16OnA5=*/true, "expects A2/A3 tsubs element type to be i32/i16/f16/f32",
...
static FailureOr<TTransVerifyState> verifyTTransCommon(TTransOp op, StringRef mismatchMessage)
{
    Type srcTy = op.getSrc().getType();
    ...
    if (elemBytes != kPTOByteSize && elemBytes != kPTOHalfWordBytes && elemBytes != kPTOWordBytes) {
        op.emitOpError() << "expects transpose element size to be 1, 2, or 4 bytes";
        return failure();
    }
    if (!isSupportedTransposeElemType(srcElem, elemBytes)) {
        op.emitOpError() << "expects transpose element type to match the supported set for its width";
        return failure();
    }
