...
std::unordered_map<const MLIRContext*, PTOParserTargetArch> parserTargetArchByContext;
...
using TileBufValidShapeVector = SmallVector<int64_t, kTileBufValidShapeInlineCapacity>;
} // namespace

void mlir::pto::setPTOParserTargetArch(MLIRContext* context, PTOParserTargetArch arch)
{
    if (!context)
        return;
...
mlir::pto::ScopedPTOParserTargetArch::ScopedPTOParserTargetArch(MLIRContext* context, PTOParserTargetArch arch)
    : context(context), previousArch(getPTOParserTargetArch(context))
{
    setPTOParserTargetArch(context, arch);
}

mlir::pto::ScopedPTOParserTargetArch::~ScopedPTOParserTargetArch() { setPTOParserTargetArch(context, previousArch); }

static TileBufValidShapeVector canonicalizeTileBufValidShape(ArrayRef<int64_t> validShape)
{
...
static LogicalResult parseTileBufComma(AsmParser& parser) { return parser.parseComma(); }

static LogicalResult parseTileBufKeywordField(AsmParser& parser, StringRef key, std::string& value)
{
...
static LogicalResult parseTileBufTypeField(AsmParser& parser, StringRef key, Type& value)
{
    if (failed(parseTileBufKeyEq(parser, key)))
        return failure();
...
static LogicalResult parseTileBufIntegerField(AsmParser& parser, StringRef key, int64_t& value)
{
    if (failed(parseTileBufKeyEq(parser, key)))
        return failure();
...
static LogicalResult parseTileBufValidDim(AsmParser& parser, StringRef key, int64_t& value)
{
    if (failed(parseTileBufKeyEq(parser, key)))
        return failure();
    ...
    if (failed(parser.parseInteger(value)))
        return failure();
    if (value < -1) {
        parser.emitError(parser.getCurrentLocation(), key + " must be '?', -1, or a non-negative integer");
        return failure();
    }
    return success();
}

static LogicalResult parseTileBufValidShapeFields(AsmParser& parser, int64_t& vrow, int64_t& vcol)
{
...
static LogicalResult parseTileBufPadField(AsmParser& parser, uint32_t& padInt)
{
    ...
    if (parsedPad < 0 || parsedPad > std::numeric_limits<uint32_t>::max()) {
        parser.emitError(parser.getCurrentLocation(), "pad must be a non-negative 32-bit integer");
        return failure();
    }
...
static BLayout resolveTileBufBLayout(MLIRContext* context, AddressSpace memorySpace, BLayout parsedLayout)
{
...
bool TileBufType::hasNonDefaultConfig() const { return !getConfigAttr().isDefault(); }
...
mlir::Attribute TileBufType::getCompactModeAttr() const { return getConfigAttr().getCompactMode(); }
...
static LogicalResult parseTileBufUInt32Value(AsmParser& parser, StringRef key, uint32_t& value)
{
    int64_t parsedValue = 0;
    if (failed(parser.parseInteger(parsedValue)))
        return failure();
    if (parsedValue < 0 || parsedValue > std::numeric_limits<uint32_t>::max()) {
        parser.emitError(parser.getCurrentLocation()) << key << " must be a non-negative 32-bit integer";
        return failure();
    }
...
static LogicalResult parseLegacyTileBufFields(AsmParser& parser, ParsedTileBufFields& fields)
{
...
static LogicalResult parseCompactTileBufShapeAndType(AsmParser& parser, ParsedTileBufFields& fields)
{
    TileBufShape shape;
    if (failed(parser.parseDimensionList(shape, /*allowDynamic=*/false)) || failed(parser.parseType(fields.dtype))) {
        return failure();
    }
    if (shape.size() != kTileBufRank2D) {
        parser.emitError(parser.getCurrentLocation(), "tile_buf compact syntax expects exactly two shape dims");
        return failure();
    }
    fields.rows = shape[0];
    fields.cols = shape[1];
...
static LogicalResult initializeCompactTileBufDefaults(AsmParser& parser, ParsedTileBufFields& fields)
{
    ...
    auto defaultCompact = llvm::dyn_cast<CompactModeAttr>(defaultConfig.getCompactMode());
    if (!defaultBLayout || !defaultSLayout || !defaultPad || !defaultCompact) {
        parser.emitError(parser.getCurrentLocation(), "failed to load default tile_buf config");
        return failure();
    }
...
static LogicalResult markSeenCompactTileBufField(AsmParser& parser, StringRef key, bool& seen)
{
    if (!seen) {
        seen = true;
        return success();
    }
    parser.emitError(parser.getCurrentLocation(), "duplicate " + key + " in tile_buf compact syntax");
    return failure();
}

static LogicalResult parseCompactTileBufValidField(
    AsmParser& parser, ParsedTileBufFields& fields, SeenCompactTileBufKeys& seen)
{
    if (seen.seenValid) {
        parser.emitError(parser.getCurrentLocation(), "duplicate valid in tile_buf compact syntax");
        return failure();
    }
    seen.seenValid = true;
    TileBufShape validShape;
    if (failed(parser.parseDimensionList(
            validShape, /*allowDynamic=*/true,
            /*withTrailingX=*/false))) {
        return failure();
    }
    if (validShape.size() != kTileBufRank2D) {
        parser.emitError(parser.getCurrentLocation(), "tile_buf valid must have exactly two dims");
        return failure();
    }
...
static LogicalResult parseCompactTileBufField(
    AsmParser& parser, StringRef key, ParsedTileBufFields& fields, SeenCompactTileBufKeys& seen)
{
    if (key == "valid")
    ...
    parser.emitError(parser.getCurrentLocation(), "unknown key in tile_buf compact syntax: ") << key;
    return failure();
}

static LogicalResult parseCompactTileBufFields(AsmParser& parser, StringRef firstToken, ParsedTileBufFields& fields)
{
    fields.locStr = firstToken.str();

    if (failed(parser.parseComma()) || failed(parseCompactTileBufShapeAndType(parser, fields)) ||
...
static FailureOr<AddressSpace> parseTileBufMemorySpace(AsmParser& parser, StringRef locStr)
{
...
static FailureOr<TileBufConfigAttr> buildParsedTileBufConfig(
    AsmParser& parser, const ParsedTileBufFields& fields, AddressSpace memorySpace)
{
    ...
    if (!bl.has_value()) {
        parser.emitError(parser.getNameLoc(), "unknown blayout: ") << fields.blayoutStr;
        return failure();
    }
    if (!sl.has_value()) {
        parser.emitError(parser.getNameLoc(), "unknown slayout: ") << fields.slayoutStr;
        return failure();
    }
    if (!pv.has_value()) {
    ...
    if (!compact.has_value()) {
        parser.emitError(parser.getNameLoc(), "unknown compact: ") << fields.compactInt;
        return failure();
    }

    BLayout effectiveBLayout = resolveTileBufBLayout(parser.getContext(), memorySpace, bl.value());
    return TileBufConfigAttr::get(
        ctx, BLayoutAttr::get(ctx, effectiveBLayout), SLayoutAttr::get(ctx, sl.value()),
        IntegerAttr::get(IntegerType::get(ctx, kI32BitWidth), fields.fractal), PadValueAttr::get(ctx, pv.value()),
        CompactModeAttr::get(ctx, compact.value()));
}

static Type buildTileBufType(AsmParser& parser, const ParsedTileBufFields& fields)
{
    MLIRContext* ctx = parser.getContext();
...
static TileBufPrintInfo buildTileBufPrintInfo(const TileBufType& type)
{
    ...
    info.defaultBLayout = llvm::dyn_cast<BLayoutAttr>(info.defaultCfg.getBLayout());
    info.defaultSLayout = llvm::dyn_cast<SLayoutAttr>(info.defaultCfg.getSLayout());
    info.defaultPad = llvm::dyn_cast<PadValueAttr>(info.defaultCfg.getPad());
    info.defaultCompact = llvm::dyn_cast<CompactModeAttr>(info.defaultCfg.getCompactMode());
...
static void printOptionalTileBufFields(AsmPrinter& printer, const TileBufPrintInfo& info)
{
    ...
    if (info.blayout && info.defaultBLayout && info.blayout.getValue() != info.defaultBLayout.getValue()) {
        printer << ", blayout=" << stringifyBLayout(info.blayout.getValue());
    }
    if (info.slayout && info.defaultSLayout && info.slayout.getValue() != info.defaultSLayout.getValue()) {
        printer << ", slayout=" << stringifySLayout(info.slayout.getValue());
    }
    if (info.cfg.getSFractalSize().getInt() != info.defaultCfg.getSFractalSize().getInt()) {
        printer << ", fractal=" << info.cfg.getSFractalSize().getInt();
    }
    if (info.pad && info.defaultPad && info.pad.getValue() != info.defaultPad.getValue()) {
        printer << ", pad=" << stringifyLocFromPad(info.cfg.getPad());
    }
    if (info.compact && info.defaultCompact && info.compact.getValue() != info.defaultCompact.getValue()) {
