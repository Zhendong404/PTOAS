...
bool TileBufConfigAttr::isDefault() const
{
    ...
    return getBLayout() == d.getBLayout() && getSLayout() == d.getSLayout() &&
           getSFractalSize() == d.getSFractalSize() && getPad() == d.getPad() && getCompactMode() == d.getCompactMode();
}
...
static LogicalResult verifyTileBufConfigAttrKind(
    function_ref<InFlightDiagnostic()> emitError, StringRef attrName, bool isValid, StringRef expectedType)
{
    if (isValid)
        return success();
...
{
    return size == kFractalSize16 || size == kFractalSize32 || size == kFractalSize512 || size == kFractalSize1024;
}

static LogicalResult verifyTileBufConfigRange(
    function_ref<InFlightDiagnostic()> emitError, StringRef attrName, int32_t value, bool isValid)
{
    if (isValid)
        return success();
    return emitError() << "unsupported " << attrName << " value: " << value, failure();
}

static LogicalResult verifyTileBufConfigAttrKinds(
    function_ref<InFlightDiagnostic()> emitError, Attribute bLayout, Attribute sLayout, Attribute pad,
    Attribute compactMode)
{
    if (failed(verifyTileBufConfigAttrKind(
            emitError, "blayout", bLayout && (mlir::isa<BLayoutAttr>(bLayout) || mlir::isa<IntegerAttr>(bLayout)),
            "BLayoutAttr or i32 integer attr")))
        return failure();
    if (failed(verifyTileBufConfigAttrKind(
            emitError, "slayout", sLayout && (mlir::isa<SLayoutAttr>(sLayout) || mlir::isa<IntegerAttr>(sLayout)),
            "SLayoutAttr or i32 integer attr")))
        return failure();
    if (failed(verifyTileBufConfigAttrKind(
            emitError, "pad", pad && (mlir::isa<PadValueAttr>(pad) || mlir::isa<IntegerAttr>(pad)),
            "PadValueAttr or i32 integer attr")))
        return failure();
    return verifyTileBufConfigAttrKind(
        emitError, "compact_mode",
        compactMode && (mlir::isa<CompactModeAttr>(compactMode) || mlir::isa<IntegerAttr>(compactMode)),
        "CompactModeAttr or i32 integer attr");
}

static LogicalResult verifyTileBufConfigValues(
    function_ref<InFlightDiagnostic()> emitError, Attribute bLayout, Attribute sLayout, IntegerAttr sFractalSize,
    Attribute pad, Attribute compactMode)
{
    ...
    if (failed(verifyTileBufConfigRange(
            ...
            getLayoutInt(bLayout, -1) == kBLayoutRowMajor || getLayoutInt(bLayout, -1) == kBLayoutColMajor)))
        return failure();
    if (failed(verifyTileBufConfigRange(
            emitError, "slayout", getLayoutInt(sLayout, -1),
            getLayoutInt(sLayout, -1) >= kSLayoutNoneBox && getLayoutInt(sLayout, -1) <= kSLayoutColMajor)))
        return failure();
    if (failed(verifyTileBufConfigRange(
            emitError, "pad", getLayoutInt(pad, -1),
            getLayoutInt(pad, -1) >= kPadValueNull && getLayoutInt(pad, -1) <= kPadValueMin)))
        return failure();
    int32_t cmv = getLayoutInt(compactMode, -1);
    return verifyTileBufConfigRange(
        emitError, "compact_mode", cmv, cmv >= kCompactModeNull && cmv <= kCompactModeRowPlusOne);
}
...
LogicalResult TileBufConfigAttr::verify(
    ...
    Attribute pad, Attribute compactMode)
{
    if (failed(verifyTileBufConfigAttrKinds(emitError, bLayout, sLayout, pad, compactMode)))
        return failure();
    return verifyTileBufConfigValues(emitError, bLayout, sLayout, sFractalSize, pad, compactMode);
}
...
static ParseResult parseTileBufConfigAttrValue(
    AsmParser& p, MLIRContext* ctx, StringRef key, BLayoutAttr& bl, SLayoutAttr& sl, IntegerAttr& sz, PadValueAttr& pv,
...
Attribute TileBufConfigAttr::parse(AsmParser& odsParser, Type odsType)
{
    ...
    while (!parsedGreater) {
        ...
        if (odsParser.parseKeyword(&key) || odsParser.parseEqual() ||
            failed(parseTileBufConfigAttrValue(odsParser, ctx, key, bl, sl, sz, pv, compact)))
            return {};
