...
namespace mlir {
namespace pto {
static LogicalResult parseShapeAndElem(
    AsmParser& parser, SmallVectorImpl<int64_t>& shape, Type& elementType, bool allowDynamic = true);
static void printShapeAndElem(AsmPrinter& printer, ArrayRef<int64_t> shape, Type elementType);
} // namespace pto
} // namespace mlir
...
static bool isSupportedVecElemType(Type ty, bool allowBf16 = true, bool allowInt8 = true);
...
static ParseResult parseSyncEventOpCommon(
    OpAsmParser& parser, OperationState& result, StringAttr pipeAttrName, StringAttr eventIdAttrName);
static void printSyncEventOpCommon(
    OpAsmPrinter& p, Operation* op, PipeAttr pipeAttr, IntegerAttr eventAttr, Value eventDyn, StringRef pipeAttrName,
...
static LogicalResult verifyTileBufCommon(Operation* op, Type ty, StringRef name, bool allowLowPrecision = false);
static LogicalResult verifyTileBufSameElemType(Operation* op, Type lhs, Type rhs, StringRef lhsName, StringRef rhsName);
static LogicalResult verifyTileBufSameLogicalExtent(
    Operation* op, Type lhs, Type rhs, StringRef lhsName, StringRef rhsName, bool compareValidShape);

static LogicalResult verifyTileBufSameValidShape(
    Operation* op, Type lhs, Type rhs, StringRef lhsName, StringRef rhsName);
static LogicalResult verifyVecTileCommon(Operation* op, Type ty, StringRef name);
static LogicalResult verifyVecTileCommonA2A3(Operation* op, Type ty, StringRef name);
static LogicalResult verifyVecTileCommonA5(Operation* op, Type ty, StringRef name);
static LogicalResult verifyVecTileUnaryOp(
    Operation* op, Type srcTy, Type dstTy, StringRef srcName = "src", StringRef dstName = "dst", bool allowBf16 = true,
    bool allowInt8 = true);
static LogicalResult verifyAccTileCommon(Operation* op, Type ty, StringRef name);
static LogicalResult verifyAccTileCommonA2A3(Operation* op, Type ty, StringRef name);
static LogicalResult verifyAccTileCommonA5(Operation* op, Type ty, StringRef name);
static LogicalResult verifyMatTileOperands(Operation* op, Type lhsTy, Type rhsTy, Type dstTy);
static LogicalResult verifyMatTileOperandsA2A3(Operation* op, Type lhsTy, Type rhsTy, Type dstTy);
static LogicalResult verifyMatTileOperandsA5(Operation* op, Type lhsTy, Type rhsTy, Type dstTy);
static LogicalResult verifyGemvTileOperands(Operation* op, Type lhsTy, Type rhsTy, Type dstTy);
static LogicalResult verifyAsyncFlatContiguous1DGMViewLike(Operation* op, Value value, StringRef name);
static LogicalResult verifyGemvTileOperandsA2A3(Operation* op, Type lhsTy, Type rhsTy, Type dstTy);
static LogicalResult verifyGemvTileOperandsA5(Operation* op, Type lhsTy, Type rhsTy, Type dstTy);
static LogicalResult verifyMatBiasTile(Operation* op, Type biasTy, Type dstTy, bool requireFloatBias = false);
static LogicalResult verifyMatBiasTileA2A3(Operation* op, Type biasTy, Type dstTy, bool requireFloatBias = false);
static LogicalResult verifyMatBiasTileA5(Operation* op, Type biasTy, Type dstTy, bool requireFloatBias = false);
static LogicalResult verifyMatmulTypeTriple(Operation* op, Type lhsElemTy, Type rhsElemTy, Type dstElemTy);
...
static LogicalResult verifyPartialValidPattern(Operation* op, Type src0Ty, Type src1Ty, Type dstTy);
static Type getElemTy(Type ty);
static FailureOr<Type> verifyMatchingRowMajorBinaryTileOpCommon(Operation* op, Type src0Ty, Type src1Ty, Type dstTy);
static FailureOr<Type> verifyNumericScalarTileOpCommon(
    Operation* op, Type srcTy, Type dstTy, Type scalarTy, bool requireValidRowsEqual);
static FailureOr<Type> verifyShiftLikeBinaryTileOpCommon(Operation* op, Type src0Ty, Type src1Ty, Type dstTy);
static LogicalResult verifyArithmeticElemTypeForArch(
    Operation* op, Type elemTy, PTOArch targetArch, bool allowInt8OnA5, bool allowBf16OnA5, StringRef a2a3Error,
    StringRef a5Error);
...
[[maybe_unused]] static LogicalResult parseShapeAndElemStable(
    mlir::AsmParser& parser, llvm::SmallVectorImpl<int64_t>& shape, mlir::Type& elementType)
{
...
bool mlir::pto::isTargetArchA3(ModuleOp module) { return getTargetArch(module) == PTOArch::A3; }

bool mlir::pto::isTargetArchA5(ModuleOp module) { return getTargetArch(module) == PTOArch::A5; }

bool mlir::pto::isTargetArchA3(Operation* op) { return getTargetArch(op) == PTOArch::A3; }

bool mlir::pto::isTargetArchA5(Operation* op) { return getTargetArch(op) == PTOArch::A5; }

static llvm::TypeSize getOneByteTypeSize() { return llvm::TypeSize::getFixed(mlir::pto::kPTOByteBitWidth); }

llvm::TypeSize mlir::pto::HiF8Type::getTypeSizeInBits(const DataLayout& dataLayout, DataLayoutEntryListRef params) const
{
    (void)dataLayout;
    (void)params;
...
uint64_t mlir::pto::HiF8Type::getABIAlignment(const DataLayout& dataLayout, DataLayoutEntryListRef params) const
{
    (void)dataLayout;
    (void)params;
    return kPTOByteSize;
...
uint64_t mlir::pto::HiF8Type::getPreferredAlignment(const DataLayout& dataLayout, DataLayoutEntryListRef params) const
{
    (void)dataLayout;
    (void)params;
    return kPTOByteSize;
...
uint64_t mlir::pto::F4E1M2x2Type::getABIAlignment(const DataLayout& dataLayout, DataLayoutEntryListRef params) const
{
    (void)dataLayout;
    (void)params;
    return kPTOByteSize;
...
uint64_t mlir::pto::F4E2M1x2Type::getABIAlignment(const DataLayout& dataLayout, DataLayoutEntryListRef params) const
{
    (void)dataLayout;
    (void)params;
    return kPTOByteSize;
...
static VerifierTargetArch getVerifierTargetArch(Operation* op)
{
    if (auto archName = getVerifierArchName(op)) {
        return archName->equals_insensitive("a5") ? VerifierTargetArch::A5 : VerifierTargetArch::A2A3;
    }
...
static LogicalResult dispatchVerifierByArch(Operation* op, FnA2A3&& verifyA2A3, FnA5&& verifyA5)
{
    ...
    OpAsmParser& parser, OperationState& result, StringAttr pipeAttrName, StringAttr eventIdAttrName)
{
    ...
    if (succeeded(parser.parseOptionalLess())) {
        ...
        if (!pipeOr)
            return parser.emitError(parser.getCurrentLocation()) << "unknown pipe token: " << pipeTok;
    ...
    OptionalParseResult parseEventOperand = parser.parseOptionalOperand(eventOperand);
    if (parseEventOperand.has_value()) {
        if (failed(*parseEventOperand))
            return failure();
        if (parser.resolveOperand(eventOperand, parser.getBuilder().getIndexType(), result.operands))
            return failure();
    } else {
        IntegerAttr eventAttr;
        if (parser.parseAttribute(eventAttr, parser.getBuilder().getI32Type(), eventIdAttrName, result.attributes))
            return failure();
    }
...
static LogicalResult parseShapeElemTypeForPTOType(OpAsmParser& parser, SmallVectorImpl<int64_t>& shape, Type& elem)
{
    if (failed(parser.parseLess()))
        return failure();
    if (failed(parser.parseDimensionList(shape, /*allowDynamic=*/true)))
...
static Type parsePtrPTOTypeAllowNoBang(OpAsmParser& parser)
{
    ...
    if (succeeded(parser.parseOptionalComma())) {
        ...
        parser.emitError(parser.getCurrentLocation(), "!pto.ptr no longer accepts address space; use !pto.ptr<elem>");
        return Type();
    }
    if (failed(parser.parseGreater()))
        return Type();
...
mlir::Type TensorViewType::parse(::mlir::AsmParser& odsParser)
{
    SmallVec4<int64_t> shape;
    Type elementType;
    if (failed(parseShapeAndElem(odsParser, shape, elementType, /*allowDynamic=*/true)))
        return Type();
...
static ParseResult parseTDivSClauses(
    OpAsmParser& parser, OpAsmParser::UnresolvedOperand& op0, OpAsmParser::UnresolvedOperand& op1,
    OpAsmParser::UnresolvedOperand& dst, Type& ty0, Type& ty1, Type& dstTy)
{
    if (parser.parseKeyword("ins") || parser.parseLParen() || parser.parseOperand(op0) || parser.parseComma() ||
        ...
        parser.parseRParen() || parser.parseKeyword("outs") || parser.parseLParen() || parser.parseOperand(dst) ||
...
static ParseResult validateTDivSTypes(OpAsmParser& parser, Type ty0, Type ty1, Type dstTy)
{
...
static ParseResult resolveTDivSOperands(
    OpAsmParser& parser, OperationState& result, OpAsmParser::UnresolvedOperand& op0,
    OpAsmParser::UnresolvedOperand& op1, OpAsmParser::UnresolvedOperand& dst, Type ty0, Type ty1, Type dstTy)
{
...
static ParseResult parseMaskPatternInsClause(OpAsmParser& parser, OperationState& result, ParseStateT& state)
{
...
static ParseResult parseTGatherMaskInsClause(OpAsmParser& parser, OperationState& result, TGatherParseState& state)
{
    return parseMaskPatternInsClause(parser, result, state);
}

static ParseResult parseTGatherExtraInsClause(OpAsmParser& parser, TGatherParseState& state)
{
...
static ParseResult parseTGatherInsClause(OpAsmParser& parser, OperationState& result, TGatherParseState& state)
{
    if (parser.parseKeyword("ins") || parser.parseLParen() || parser.parseOperand(state.src)) {
        return failure();
    }
    if (!succeeded(parser.parseOptionalComma())) {
...
static ParseResult parseTGatherOutsClause(OpAsmParser& parser, TGatherParseState& state)
{
...
static ParseResult parseOptionalTGatherMaskPattern(
    OpAsmParser& parser, OperationState& result, TGatherParseState& state)
{
...
static ParseResult validateTGatherMaskForm(OpAsmParser& parser, const TGatherParseState& state)
{
...
static ParseResult validateTGatherCompareForm(OpAsmParser& parser, TGatherParseState& state)
{
    if (state.insOps.empty() || !(isa<IntegerType>(state.insTypes.front()) || isa<FloatType>(state.insTypes.front()))) {
...
static ParseResult validateTGatherIndexForm(OpAsmParser& parser, TGatherParseState& state)
{
...
static ParseResult validateTGatherOperands(OpAsmParser& parser, TGatherParseState& state)
{
    if (state.hasMask)
...
static ParseResult resolveTGatherOperands(OpAsmParser& parser, OperationState& result, const TGatherParseState& state)
{
...
static void addTGatherSegmentSizes(OpAsmParser& parser, OperationState& result, const TGatherParseState& state)
{
    result.addAttribute(
        "operandSegmentSizes",
...
static ParseResult parseTScatterMaskInsClause(OpAsmParser& parser, OperationState& result, TScatterParseState& state)
{
    return parseMaskPatternInsClause(parser, result, state);
}

static ParseResult parseTScatterIndexesInsClause(OpAsmParser& parser, TScatterParseState& state)
{
...
static ParseResult parseTScatterInsClause(OpAsmParser& parser, OperationState& result, TScatterParseState& state)
{
    if (parser.parseKeyword("ins") || parser.parseLParen() || parser.parseOperand(state.src)) {
        return failure();
    }
...
static ParseResult validateTScatterOperands(OpAsmParser& parser, const TScatterParseState& state)
{
...
static ParseResult parseCommCollectiveTypes(
    OpAsmParser& parser, SmallVectorImpl<Type>& fixedTypes, CommRecvClause& recvClause,
...
static ParseResult parseRequiredCommAttrs(
    ...
    SmallVectorImpl<OpAsmParser::UnresolvedOperand>& groupOps, SmallVectorImpl<Type>& groupTypes)
{
    ...
    return parser.resolveOperands(groupOps, groupTypes, parser.getCurrentLocation(), result.operands);
}

static void addCommCollectiveSegmentSizes(
    OpAsmParser& parser, OperationState& result, ArrayRef<int32_t> prefix, size_t groupCount)
{
...
static ParseResult parsePartitionViewHeader(
    OpAsmParser& parser, OperationState& result, OpAsmParser::UnresolvedOperand& source,
    SmallVectorImpl<OpAsmParser::UnresolvedOperand>& offsets, SmallVectorImpl<OpAsmParser::UnresolvedOperand>& sizes,
    Type& sourceTy)
{
    if (parser.parseOperand(source) || parser.parseComma() || parser.parseKeyword("offsets") || parser.parseEqual() ||
        parser.parseLSquare() || parser.parseOperandList(offsets) || parser.parseRSquare() || parser.parseComma() ||
        parser.parseKeyword("sizes") || parser.parseEqual() || parser.parseLSquare() ||
        parser.parseOperandList(sizes) || parser.parseRSquare() || parser.parseOptionalAttrDict(result.attributes) ||
...
static void setPartitionViewOperandSegments(OperationState& result, size_t offsetsSize, size_t sizesSize)
{
    auto& properties = result.getOrAddProperties<mlir::pto::PartitionViewOp::Properties>();
    llvm::copy(
        ArrayRef<int32_t>({1, static_cast<int32_t>(offsetsSize), static_cast<int32_t>(sizesSize)}),
...
ParseResult mlir::pto::TGatherOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parseTGatherInsClause(parser, result, state) || parseTGatherOutsClause(parser, state) ||
        parseOptionalTGatherMaskPattern(parser, result, state) || parser.parseOptionalAttrDict(result.attributes) ||
        validateTGatherOperands(parser, state) || resolveTGatherOperands(parser, result, state)) {
        return failure();
    }
...
void mlir::pto::TGatherOp::print(OpAsmPrinter& p)
{
    ...
    if (auto mp = getMaskPatternAttr()) {
        ...
        if (getTmp()) {
            p << ", " << getTmp();
            p << " : " << getSrc().getType() << ", " << getKValue().getType() << ", " << getTmp().getType();
        } else {
        ...
        if (getTmp()) {
            p << ", " << getTmp();
            p << " : " << getSrc().getType() << ", " << getIndices().getType() << ", " << getTmp().getType();
        } else {
...
ParseResult mlir::pto::TScatterOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parseTScatterInsClause(parser, result, state) || parser.parseKeyword("outs") || parser.parseLParen() ||
        parser.parseOperand(state.dst) || parser.parseColonType(state.dstTy) || parser.parseRParen() ||
        parser.parseOptionalAttrDict(result.attributes)) {
        return failure();
    }
    if (result.attributes.get("maskPattern"))
        state.hasMask = true;
    if (validateTScatterOperands(parser, state) || parser.resolveOperand(state.src, state.srcTy, result.operands) ||
        parser.resolveOperand(state.dst, state.dstTy, result.operands)) {
        return failure();
    }
    if (state.hasIndexes && parser.resolveOperand(state.indexes, state.idxTy, result.operands)) {
        return failure();
    }
    return success();
}
...
void mlir::pto::TScatterOp::print(OpAsmPrinter& p)
{
    ...
    if (getMaskPatternAttr()) {
        ...
        p << getIndexes() << " : " << getSrc().getType() << ", " << getIndexes().getType();
    }
    p << ") outs(" << getDst() << " : " << getDst().getType() << ")";
    p.printOptionalAttrDict(
        (*this)->getAttrs(),
...
static ParseResult parseCommRecvClause(OpAsmParser& parser, CommRecvClause& recvClause)
{
    if (parser.parseKeyword("recv") || parser.parseLParen() || parser.parseOperand(recvClause.ping))
        return failure();
    if (succeeded(parser.parseOptionalComma())) {
    ...
    ArrayRef<int32_t> operandSegmentsPrefix, ArrayRef<StringRef> requiredAttrs)
{
    if (parseCommGroupOperands(parser, groupOps) ||
        parseCommCollectiveTypes(parser, fixedTypes, recvClause, groupOps, groupTypes) ||
        parseRequiredCommAttrs(parser, result, requiredAttrs) ||
        resolveCommCollectiveOperands(parser, result, fixedOperands, fixedTypes, recvClause, groupOps, groupTypes)) {
        return failure();
    }
    addCommCollectiveSegmentSizes(parser, result, operandSegmentsPrefix, groupOps.size());
    return success();
}
...
ParseResult mlir::pto::TReduceOp::parse(OpAsmParser& parser, OperationState& result)
{
    OpAsmParser::UnresolvedOperand dst, acc;
    CommRecvClause recvClause;
    SmallVec4<OpAsmParser::UnresolvedOperand> groupOps;
    SmallVec4<Type> groupTypes;
    if (parser.parseLParen() || parser.parseOperand(dst) || parser.parseComma() || parser.parseOperand(acc) ||
        parser.parseComma())
        return failure();
    if (failed(parseCommRecvClause(parser, recvClause)))
        return failure();
    ...
    if (failed(parseCommCollectiveTail(
            parser, result, fixedOperands, fixedTypes, recvClause, groupOps, groupTypes,
            {1, 1, 1, recvClause.pong ? 1 : 0}, {"reduceOp", "root"})))
        return failure();
    return success();
}
...
void mlir::pto::TReduceOp::print(OpAsmPrinter& p)
{
    ...
    p << ", ";
    printCommGroupClause(p, getGroup());
    p << " : " << getDst().getType() << ", " << getAcc().getType() << ", " << getRecvPing().getType();
    if (getRecvPong())
...
ParseResult mlir::pto::MakeTensorViewOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    // , shape = [ ... ]
    if (parser.parseComma() || parser.parseKeyword("shape") || parser.parseEqual() || parser.parseLSquare() ||
        parser.parseOperandList(shapeOps) || parser.parseRSquare())
        return failure();

    // strides = [ ... ]
    if (parser.parseComma() || parser.parseKeyword("strides") || parser.parseEqual() || parser.parseLSquare() ||
    ...
    if (!tvTy)
        return parser.emitError(parser.getCurrentLocation(), "expected result type pto.tensor_view<...>");
...
static FailureOr<mlir::pto::PartitionTensorViewType> inferPartitionViewResultTypeFromSizes(
    mlir::pto::TensorViewType sourceType, ValueRange sizes)
{
    if (!sourceType)
        return failure();
    ...
    return mlir::pto::PartitionTensorViewType::get(sourceType.getContext(), shape, sourceType.getElementType());
}

static ParseResult parseOptionalArrowTypeAndResolveSource(
    OpAsmParser& parser, OperationState& result, OpAsmParser::UnresolvedOperand& source, Type sourceTy, Type& resultTy,
...
static ParseResult resolveIndexOperandsToResult(
    OpAsmParser& parser, ArrayRef<OpAsmParser::UnresolvedOperand> operands, OperationState& result)
{
    return parser.resolveOperands(operands, parser.getBuilder().getIndexType(), result.operands);
}
...
ParseResult mlir::pto::PartitionViewOp::parse(OpAsmParser& parser, OperationState& result)
{
    ...
    if (parsePartitionViewHeader(parser, result, source, offsets, sizes, sourceTy))
        return failure();
    if (parseOptionalArrowTypeAndResolveSource(parser, result, source, sourceTy, resultTy, hasExplicitResultTy))
        return failure();
    if (resolveIndexOperandsToResult(parser, offsets, result) || resolveIndexOperandsToResult(parser, sizes, result))
        return failure();
    ...
    ValueRange sizeOperands = allOperands.slice(1 + offsets.size(), sizes.size());
    auto inferredResultType =
        inferPartitionViewResultTypeFromSizes(dyn_cast<mlir::pto::TensorViewType>(sourceTy), sizeOperands);
