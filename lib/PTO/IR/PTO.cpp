...
LogicalResult mlir::pto::CastPtrOp::verify() {
  ...
  if (inputIsInteger && resultPtrType &&
    ...
    return emitOpError("integer address input must be i64");
  }

  if (inputPtrType && resultIsInteger &&
      cast<IntegerType>(resultType).getWidth() != 64) {
    return emitOpError("integer address result must be i64");
  }
