...
llvm::SmallVector<Value> IRTranslator::tracebackMemValsStep(Value val)
{
    ...
    if (auto blockArg = dyn_cast<BlockArgument>(val)) {
        if (auto forOp = dyn_cast_if_present<scf::ForOp>(blockArg.getOwner()->getParentOp())) {
...
llvm::SmallVector<Value> IRTranslator::tracebackMemVals(Value val)
{
    ...
    while (!queue.empty()) {
        ...
        if (!result)
            continue;
        Operation* defOp = result.getDefiningOp();
        if (isa<pto::PointerCastOp, pto::AllocTileOp, tensor::EmptyOp, memref::AllocOp>(defOp)) {
...
llvm::SmallVector<Value> IRTranslator::getMemoryOps(const SmallVector<Value>& vals)
{
...
std::pair<llvm::SmallVector<Value>, llvm::SmallVector<Value>> IRTranslator::getReadWriteMemoryOps(Operation* op)
{
    llvm::SmallVector<Value> reads;
    llvm::SmallVector<Value> writes;

    if (auto memEffect = dyn_cast<MemoryEffectOpInterface>(op)) {
        SmallVector<SideEffects::EffectInstance<MemoryEffects::Effect>, kMemoryEffectInlineCapacity> effects;
...
std::unique_ptr<OperationBase> IRTranslator::getLoadStoreOp(OP op, OperationBase* parentOp)
{
    auto pipe = pto::PIPE::PIPE_S;
    llvm::SmallVector<Value> reads;
    llvm::SmallVector<Value> writes;
    if constexpr (std::is_same_v<OP, memref::LoadOp> || std::is_same_v<OP, affine::AffineLoadOp>) {
        reads = getMemoryOps({op.getMemRef()});
    } else {
        writes = getMemoryOps({op.getMemRef()});
    }
    return std::make_unique<RWOperation>(
        op.getOperation(), parentOp, TCoreType::CUBE_OR_VECTOR, pipe, pipe, reads, writes);
}

std::unique_ptr<OperationBase> IRTranslator::getPipeInterfaceOp(pto::OpPipeInterface op, OperationBase* parentOp)
{
    auto [reads, writes] = getReadWriteMemoryOps(op.getOperation());
    pto::PIPE pipeRead = op.getPipe();
    pto::PIPE pipeWrite = op.getPipe();
    if (pipeRead == pto::PIPE::PIPE_UNASSIGNED || pipeWrite == pto::PIPE::PIPE_UNASSIGNED)
        return nullptr;
    return std::make_unique<RWOperation>(
        op.getOperation(), parentOp, TCoreType::CUBE_OR_VECTOR, pipeRead, pipeWrite, reads, writes);
}

std::unique_ptr<OperationBase> IRTranslator::getTensorExtractOp(tensor::ExtractOp extractOp, OperationBase* parentOp)
{
    auto reads = getMemoryOps({extractOp.getTensor()});
    return std::make_unique<RWOperation>(
        extractOp.getOperation(), parentOp, TCoreType::CUBE_OR_VECTOR, pto::PIPE::PIPE_S, pto::PIPE::PIPE_S, reads,
        llvm::SmallVector<Value>());
}

std::unique_ptr<OperationBase> IRTranslator::getCallOp(func::CallOp, const OperationBase* parentOp) const
{
    (void)parentOp;
    return nullptr;
}
...
{
    return condOp && condOp->op && condOp->op->hasAttrOfType<UnitAttr>("pto.unlikely_condition");
}

bool IRTranslator::isParallelLoop(const Loop* loopOp) const
{
    return loopOp && loopOp->op && loopOp->op->hasAttrOfType<UnitAttr>("pto.parallel_loop");
}

std::optional<int64_t> IRTranslator::getLoopMultibufferUnrollNum(Loop*) const { return {}; }

std::optional<int64_t> IRTranslator::getScopePreloadNum(Scope*) const { return {}; }

std::optional<int64_t> IRTranslator::getScopeMaxPreloadNum(Scope*) const { return {}; }

Scope* IRTranslator::prepareBlockScope(std::unique_ptr<Scope>& scopeOp, Block& block, bool isFunctionRegion) const
{
...
bool IRTranslator::handleIfLikeOp(Operation& op, Scope* parScope, bool skipEmptyScopes)
{
...
bool IRTranslator::handleLoopLikeOp(Operation& op, Scope* parScope, bool skipEmptyScopes)
{
...
std::unique_ptr<Scope> IRTranslator::funcIrBuilder(Region& region, OperationBase* parentOp, bool skipEmptyScopes)
{
    ...
    for (Block& block : region.getBlocks()) {
        ...
        for (Operation& op : block.getOperations()) {
            if (handleIfLikeOp(op, parScope, skipEmptyScopes) || handleLoopLikeOp(op, parScope, skipEmptyScopes) ||
...
bool IRTranslator::skipLaterIterations(Occurrence* occ1, Occurrence* occ2) const
{
...
void IRTranslator::generateProcessingOrders(const Scope* scopeOp, Occurrence* occ, bool isUseless)
{
    (void)scopeOp;
    generateProcessingOrders(occ->childOccs, isUseless);
}

void IRTranslator::generateProcessingOrders(const Loop* loopOp, Occurrence* occ, bool isUseless)
{
    (void)loopOp;
    ...
    SmallVector<Occurrence*> first(occ->childOccs.begin(), occ->childOccs.begin() + halfChildNum);
    SmallVector<Occurrence*> second(occ->childOccs.begin() + halfChildNum, occ->childOccs.end());
