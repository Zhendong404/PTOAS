...
struct UnitDistKeyInfo {
    ...
    {
        return {
            llvm::DenseMapInfo<int>::getEmptyKey(), llvm::DenseMapInfo<int>::getEmptyKey(),
    ...
    {
        return {
            llvm::DenseMapInfo<int>::getTombstoneKey(), llvm::DenseMapInfo<int>::getTombstoneKey(),
            CorePipeInfoKeyInfo::getTombstoneKey()};
    }

    static unsigned getHashValue(const UnitDistKey& val)
    {
        unsigned hash = llvm::DenseMapInfo<int>::getHashValue(std::get<kUnitDistSourceIndex>(val));
        hash = combineDenseHash(hash, llvm::DenseMapInfo<int>::getHashValue(std::get<kUnitDistTargetIndex>(val)));
        return combineDenseHash(hash, CorePipeInfoKeyInfo::getHashValue(std::get<kUnitDistPipeIndex>(val)));
    }

    static bool isEqual(const UnitDistKey& lhs, const UnitDistKey& rhs)
    {
        return llvm::DenseMapInfo<int>::isEqual(
                   std::get<kUnitDistSourceIndex>(lhs), std::get<kUnitDistSourceIndex>(rhs)) &&
               llvm::DenseMapInfo<int>::isEqual(
                   std::get<kUnitDistTargetIndex>(lhs), std::get<kUnitDistTargetIndex>(rhs)) &&
               CorePipeInfoKeyInfo::isEqual(std::get<kUnitDistPipeIndex>(lhs), std::get<kUnitDistPipeIndex>(rhs));
    }
};

using UnitDistMap = llvm::DenseMap<UnitDistKey, int, UnitDistKeyInfo>;
using UnitQueueItem = std::pair<int, UnitDistKey>;
using UnitPriorityQueue = std::priority_queue<UnitQueueItem, std::vector<UnitQueueItem>, std::greater<UnitQueueItem>>;

static bool hasBetterDistance(const UnitDistMap& distance, const UnitDistKey& distKey, int curIndex)
{
    return (distance.count(distKey) != 0) && distance.lookup(distKey) < curIndex;
}

static bool exceededSearchBound(const UnitDistMap& distance, const UnitDistKey& distKey, int endIndex)
{
    return (distance.count(distKey) != 0) && distance.lookup(distKey) > endIndex;
}

static bool reachedUnitFlagDestination(CorePipeInfo curCorePipe, pto::TCoreType coreDst, int startIndex, int curIndex)
{
    auto [curCore, curPipe] = curCorePipe;
    return curCore == coreDst &&
           ((curIndex != startIndex && curPipe == pto::PIPE::PIPE_S) || curPipe == pto::PIPE::PIPE_ALL);
}

static void relaxUnitFlagEdge(
    const GraphSolver::Edge& edge, const Occurrence* occ2, UnitDistMap& distance, UnitPriorityQueue& que)
{
    ASSERT(edge.conflictPair != nullptr);
    UnitDistKey nxtKey(edge.isUnitFlag, edge.conflictPair->waitOcc == occ2, edge.corePipeDst);
...
static std::optional<int> collectUnitFlagResult(const UnitDistMap& distance, CorePipeInfo corePipeDst)
{
    ...
    for (const UnitDistKey& key :
        ...
        if (it == distance.end())
            continue;
        retDist = retDist.has_value() ? std::min(retDist.value(), it->second) : it->second;
    }
    return retDist;
}
...
void GraphSolver::addPair(
    ConflictPair* conflictPair, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, int startIndex, int endIndex,
    bool isUnitFlag)
{
    Edge edge(conflictPair, corePipeSrc, corePipeDst, startIndex, endIndex, isUnitFlag);
...
void GraphSolver::addConflictPair(ConflictPair* conflictPair)
{
    ...
    if (conflictPair->isBarrier() && conflictPair->setCorePipeInfo.pipe == pto::PIPE::PIPE_ALL) {
        llvm::SmallVector<std::pair<pto::TCoreType, pto::TCoreType>> srcDstCores;
        if (options.isCrossCoreMode()) {
            srcDstCores.push_back(std::make_pair(pto::TCoreType::CUBE, pto::TCoreType::VECTOR));
            srcDstCores.push_back(std::make_pair(pto::TCoreType::VECTOR, pto::TCoreType::CUBE));
        } else {
            srcDstCores.push_back(std::make_pair(pto::TCoreType::CUBE_OR_VECTOR, pto::TCoreType::CUBE_OR_VECTOR));
        }
        for (auto [srcCore, dstCore] : srcDstCores) {
            for (int i = 0; i < static_cast<int>(pto::PIPE::PIPE_NUM); i++) {
                ...
                addPair(
                    conflictPair, CorePipeInfo(srcCore, setPipe), CorePipeInfo(dstCore, waitPipe), startIndex,
                    endIndex);
            }
        }
    } else if (!conflictPair->isBarrier() && conflictPair->waitCorePipeInfo.pipe == pto::PIPE::PIPE_S) {
        for (int i = 0; i < static_cast<int>(pto::PIPE::PIPE_NUM); i++) {
            auto coreDst = conflictPair->waitCorePipeInfo.coreType;
            auto waitPipe = static_cast<pto::PIPE>(i);
            addPair(
                conflictPair, conflictPair->setCorePipeInfo, CorePipeInfo(coreDst, waitPipe), conflictPair->startIndex,
        ...
        int startIndex = conflictPair->startIndex;
        int endIndex = conflictPair->endIndex;
        addPair(conflictPair, corePipeSrc, corePipeDst, startIndex, endIndex, conflictPair->replacedWithUnitFlag);
    }
}
...
void GraphSolver::optimizeAdjacencyList()
{
    for (auto& [corePipeSrc, dstMap] : adjacencyList) {
        for (auto& [corePipeDst, edges] : dstMap) {
            ...
            for (auto& edge : edges) {
                while (!optimizedEdges.empty() && optimizedEdges.rbegin()->endIndex >= edge.endIndex) {
...
std::optional<int> GraphSolver::runDijkstra(
    CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, int startIndex, int endIndex)
{
    CorePipeDenseMap<int> distance;
    std::priority_queue<
        std::pair<int, CorePipeInfo>, std::vector<std::pair<int, CorePipeInfo>>,
        std::greater<std::pair<int, CorePipeInfo>>>
        que;
    que.emplace(startIndex, corePipeSrc);
    auto [coreDst, pipeDst] = corePipeDst;

    LLVM_DEBUG(llvm::dbgs() << "dij-start-end-indices: " << startIndex << ' ' << endIndex << '\n');
    ...
    while (!que.empty()) {
        ...
        LLVM_DEBUG(llvm::dbgs() << "dij-step: " << curCore << ' ' << curPipe << ' ' << curIndex << '\n');

        if (curCorePipe == corePipeDst && (distance.count(corePipeDst) != 0)) {
            break;
        }

        if ((distance.count(curCorePipe) != 0) && distance[curCorePipe] < curIndex) {
            continue;
        }

        if ((distance.count(curCorePipe) != 0) && distance[curCorePipe] > endIndex) {
            break;
        }
        ...
        for (auto& [endCorePipe, edges] : adjacencyList[curCorePipe]) {
            ...
            for (; it != edges.end(); it++) {
                if (distance.count(endCorePipe) == 0 || (distance[endCorePipe] > (it->endIndex))) {
    ...
    return distance.count(corePipeDst) != 0 ? distance[corePipeDst] : std::optional<int>();
}

std::optional<int> GraphSolver::runDijkstraUnitFlagEnabled(
    const Occurrence* occ1, const Occurrence* occ2, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, int startIndex,
    int endIndex)
{
    ...
    while (!que.empty()) {
        ...
        LLVM_DEBUG(
            llvm::dbgs() << "dij-step: " << static_cast<int>(curCorePipe.coreType) << ' '
                         << static_cast<int>(curCorePipe.pipe) << ' ' << curIsUnitFlag << ' ' << curIsOccDst << ' '
                         << curIndex << '\n');
        if (hasBetterDistance(distance, curDistKey, curIndex))
            continue;
        if (exceededSearchBound(distance, curDistKey, endIndex))
            break;
        if (reachedUnitFlagDestination(curCorePipe, coreDst, startIndex, curIndex)) {
        ...
        for (auto& [endCorePipe, edges] : adjacencyList[curCorePipe]) {
            ...
            for (; it != edges.end(); ++it) {
                const auto& edge = *it;
                if (edge.isUnitFlag && curIndex == startIndex && edge.startIndex != startIndex)
                    continue;
