...
static llvm::SmallVector<pto::TCoreType> getBackwardSyncCoreTypes(const SyncSolverOptions& options)
{
...
void Solver::calcAllEventIds()
{
    for (auto& [pipes, eventIdSolver] : eventIdSolver) {
        ...
        [[maybe_unused]] auto result = eventIdSolver->shrinkEventIdMaxToEventIdNum();
...
void Solver::collectBackwardSyncEventIds()
{
    ...
    for (auto& conflictPair : chosenConflictedPairs) {
        if (!conflictPair->isUseless && conflictPair->isInnerBackward && conflictPair->eventIdNode != nullptr) {
            ...
            for (auto eventId : conflictPair->eventIdNode->getEventIds()) {
                auto& e = backwardSyncEvents[conflictPair->backwardSyncLoopOp]
                                            [{conflictPair->setCorePipeInfo, conflictPair->waitCorePipeInfo}][eventId];
...
void Solver::resetAndBuildSetWaitOpIndex(const SyncMap& syncMapBefore, const SyncMap& syncMapAfter)
{
...
std::set<std::pair<int64_t, SetWaitOp*>>& Solver::getSetWaitOpsIndexRef(
...
void Solver::collectSetWaitOpsIndexes(OperationBase* op, const SyncMap& syncMapBefore, const SyncMap& syncMapAfter)
{
    ASSERT(op != nullptr);
    auto collectSyncOpIndexesForMap = [this, op](const SyncMap& syncMap) {
        ...
        for (auto& syncOp : it->second) {
            if (auto* setWaitOp = llvm::dyn_cast<SetWaitOp>(syncOp.get())) {
                for (auto eventId : setWaitOp->eventIds) {
                    auto& index = getSetWaitOpsIndexRef(setWaitOp->pipeSrc, setWaitOp->pipeDst, eventId);
...
bool Solver::checkBackwardSyncEventsContains(
...
static bool isSetWaitIndexUsedInRange(const IndexT& index, int64_t startInclusive, int64_t endInclusive)
{
...
static bool isBackwardSyncChildUsable(
    OperationBase* childOp, const IndexT& index, int64_t childStartInclusive, int64_t childEnd,
    int64_t childEndInclusive, ContainsFn containsFn, ContainsAfterFn containsAfterFn, CorePipeInfo corePipeSrc,
    CorePipeInfo corePipeDst, int64_t eventId)
{
    ...
    bool usedAtleastOnce = it1 != index.end() && it1->first < childEndInclusive;
    if (!usedAtleastOnce)
        return true;
...
bool Solver::checkMergeable(
    Scope* scopeOp, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, int64_t eventId, bool shouldBeUsedAtleastOnce)
{
    auto& index = getSetWaitOpsIndexRef(corePipeSrc.pipe, corePipeDst.pipe, eventId);
    if (shouldBeUsedAtleastOnce) {
        if (!isSetWaitIndexUsedInRange(index, setWaitStartIndexInclusive[scopeOp], setWaitEndIndexInclusive[scopeOp])) {
            return false;
        }
    }
    ...
    bool usedAfter = it2 != index.end() && it2->first < setWaitEndIndexInclusive[scopeOp];
    if (usedBefore || usedAfter) {
        return false;
    }
    if (auto* conditionOp = llvm::dyn_cast<Condition>(scopeOp)) {
        return checkMergeableConditionScopes(conditionOp, corePipeSrc, corePipeDst, eventId);
    }
    if (auto* loopOp = llvm::dyn_cast<Loop>(scopeOp)) {
        return checkMergeableLoopScopes(loopOp, corePipeSrc, corePipeDst, eventId);
    }
    return checkMergeableScopeBody(scopeOp, index, corePipeSrc, corePipeDst, eventId);
}

bool Solver::checkMergeableConditionScopes(
    const Condition* conditionOp, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, int64_t eventId)
{
    if (!conditionOp->hasFalseScope())
        return false;
    return checkMergeable(conditionOp->getTrueScope(), corePipeSrc, corePipeDst, eventId, true) &&
           checkMergeable(conditionOp->getFalseScope(), corePipeSrc, corePipeDst, eventId, true);
}

bool Solver::checkMergeableLoopScopes(
    const Loop* loopOp, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, int64_t eventId)
{
    for (auto& childOp : loopOp->body) {
        ...
        if (childScopeOp && !checkMergeable(childScopeOp, corePipeSrc, corePipeDst, eventId, false)) {
            return false;
        }
    }
    for (auto& childOp : loopOp->body) {
        auto* childScopeOp = llvm::dyn_cast<Scope>(childOp.get());
        if (childScopeOp && checkMergeable(childScopeOp, corePipeSrc, corePipeDst, eventId, true)) {
            return true;
        }
    }
    return false;
    ...
    CorePipeInfo corePipeDst, int64_t eventId)
{
    for (auto& childOp : scopeOp->body) {
        if (!isBackwardSyncChildUsable(
                childOp.get(), index, setWaitStartIndexInclusive[childOp.get()], setWaitEndIndex[childOp.get()],
                setWaitEndIndexInclusive[childOp.get()],
                [this](OperationBase* targetOp, CorePipeInfo src, CorePipeInfo dst, int64_t targetEventId) {
                    return checkBackwardSyncEventsContains(targetOp, src, dst, targetEventId);
                },
                [this](OperationBase* targetOp, CorePipeInfo src, CorePipeInfo dst) {
                    return checkBackwardSyncEventsContainsAfterMerge(targetOp, src, dst);
                },
...
void Solver::mergeBackwardSyncPairs(SyncMap& syncMapBefore, SyncMap& syncMapAfter)
{
...
void Solver::collectMergeableBackwardSyncEvents(Scope* scopeOp, CorePipeEventDenseSet& toBeErased)
{
    llvm::SmallVector<pto::TCoreType> coreTypes = getBackwardSyncCoreTypes(options);
    int64_t eventIdMax = getHWAvailableEventIdNum(options.syncMode);
    for (int64_t eventId = 0; eventId < eventIdMax; ++eventId) {
        collectMergeableBackwardSyncEventsForEventId(scopeOp, coreTypes, eventId, toBeErased);
    }
}
    ...
    Scope* scopeOp, ArrayRef<pto::TCoreType> coreTypes, int64_t eventId, CorePipeEventDenseSet& toBeErased)
{
    for (auto coreSrc : coreTypes) {
        for (auto coreDst : coreTypes) {
            collectMergeableBackwardSyncEventsForCorePair(scopeOp, coreSrc, coreDst, eventId, toBeErased);
        }
    }
}

void Solver::collectMergeableBackwardSyncEventsForCorePair(
    Scope* scopeOp, pto::TCoreType coreSrc, pto::TCoreType coreDst, int64_t eventId, CorePipeEventDenseSet& toBeErased)
{
    ...
    for (size_t pipeSrcInt = 0; pipeSrcInt < pipeNumMax; ++pipeSrcInt) {
        for (size_t pipeDstInt = 0; pipeDstInt < pipeNumMax; ++pipeDstInt) {
            ...
            (void)tryCollectMergeableBackwardSyncEvent(scopeOp, corePipeSrc, corePipeDst, eventId, toBeErased);
        }
    }
}

void Solver::eraseMergedBackwardSyncEventsFromChildScope(Scope* childScopeOp, const CorePipeEventDenseSet& toBeErased)
{
    for (auto [corePipeSrc, corePipeDst, eventId] : toBeErased) {
        if (!checkBackwardSyncEventsContains(childScopeOp, corePipeSrc, corePipeDst, eventId)) {
            continue;
        }
...
void Solver::eraseMergedBackwardSyncEventsFromNestedScopes(Scope* scopeOp, const CorePipeEventDenseSet& toBeErased)
{
...
void Solver::appendSyncOpsForConflictPair(ConflictPair* conflictPair, SyncMap& syncMapBefore, SyncMap& syncMapAfter)
{
    ...
    if (conflictPair->isBarrier()) {
        auto barrierOp = std::make_unique<BarrierOp>(
            conflictPair->waitOp->op, conflictPair->waitOp->parentOp, conflictPair->waitCorePipeInfo.pipe);
    ...
    auto setOp = std::make_unique<SetFlagOp>(
        conflictPair->setOp->op, conflictPair->setOp->parentOp, conflictPair->eventIdNode->getEventIds(),
        conflictPair->setCorePipeInfo.pipe, conflictPair->waitCorePipeInfo.pipe);
    auto waitOp = std::make_unique<WaitFlagOp>(
        conflictPair->waitOp->op, conflictPair->waitOp->parentOp, conflictPair->eventIdNode->getEventIds(),
        conflictPair->setCorePipeInfo.pipe, conflictPair->waitCorePipeInfo.pipe);
    if (options.isCrossCoreMode()) {
...
void Solver::appendMergedBackwardSyncScopeOps(SyncMap& syncMapBefore, SyncMap& syncMapAfter)
{
    for (auto& [op, mp] : backwardSyncEvents) {
        ...
        for (auto [setWaitCorePipes, eventIdsMp] : mp) {
            ...
            auto setOp = std::make_unique<SetFlagOp>(
                scopeOp->op, scopeOp->parentOp, eventIds, corePipeSrc.pipe, corePipeDst.pipe);
            auto waitOp = std::make_unique<WaitFlagOp>(
                scopeOp->op, scopeOp->parentOp, eventIds, corePipeSrc.pipe, corePipeDst.pipe);
...
void Solver::processConflict(Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2, bool isUseless)
{
    for (auto [corePipeSrc, corePipeDst] : checkMemoryConflicts(rwOp1, rwOp2)) {
        ...
        auto eventIdInfo = getEventIdInfo(occ1, occ2, rwOp1, rwOp2, corePipeSrc, corePipeDst);
        handleConflict(occ1, occ2, rwOp1, rwOp2, corePipeSrc, corePipeDst, eventIdInfo, isUseless);
    }
}
...
void Solver::processOrders()
{
    for (auto& [occ1, occ2, rwOp1, rwOp2, isUseless] : processingOrders) {
        ...
        if (checkImpossibleOccPair(occ1, occ2) || checkAlreadySynced(occ1, occ2) ||
            skipMMad1DecomposedLoopOpt(occ1, occ2) || checkSkipParallelLoop(occ1, occ2) ||
        ...
        for (auto& corePipeInfoPair : st) {
            ...
            for (auto* scopeOcc : opAllOccurrences[scopeOp]) {
                ...
                auto conflictPair = std::make_unique<ConflictPair>(
                    nullptr, nullptr, nullptr, nullptr, setOcc, waitOcc, corePipeSrc, corePipeDst, startIndex,
                    endIndex);
                ...
                LLVM_DEBUG({
                    llvm::dbgs() << "consider-merged-backward-pair: " << scopeOp->str(0, false) << ' '
                                 << conflictPair->str() << "\n";
                });
...
bool Solver::pruneStaleMergedBackwardSyncPairs()
{
    ...
    for (auto& [scopeOp, st] : backwardSyncEventsAfterMerge) {
        ...
        for (auto& corePipeInfoPair : st) {
            if (!backwardSyncEvents.contains(scopeOp) || !backwardSyncEvents[scopeOp].contains(corePipeInfoPair)) {
...
bool Solver::insertOuterBackwardSyncPairs(const SmallVectorImpl<OperationBase*>& chosenOps)
{
...
llvm::LogicalResult Solver::reuseSyncPairToSaveEventIds()
{
    ...
    for (auto [corePipeSrc, corePipeDst] : barrierAllPairs) {
        if (reusePairs[{corePipeSrc, corePipeDst}] < maxReuseNum) {
            if (reusePairs[{corePipeSrc, corePipeDst}] <= reusedPairs[{corePipeSrc, corePipeDst}]) {
    ...
    DEBUG_WITH_TYPE("gss-sync-solver-reuse", {
        ...
        for (auto [pipeCorePairs, cnt] : reusePairs) {
            llvm::dbgs() << get<0>(pipeCorePairs).pipe << ' ' << get<1>(pipeCorePairs).pipe << ' ' << cnt << '\n';
        }
    });
...
{
    if (!options.disableMultiEventIdForBarrierAllPairs || barrierAllPairs.empty()) {
        return llvm::failure();
    }
    bool newPairIsInserted = false;
    ...
    LLVM_DEBUG({
        if (newPairIsInserted) {
            ...
            for (auto& [corePipeSrc, corePipeDst] : disabledMultiEventIdPairs) {
                llvm::dbgs() << corePipeSrc.coreType << ' ' << corePipeSrc.pipe << ' ' << corePipeDst.coreType << ' '
                             << corePipeDst.pipe << '\n';
            }
        }
    });
...
void Solver::solve()
{
    ...
    if (!options.isTestMode()) {
        ...
        if (llvm::succeeded(runSolver(/*enableOpts1=*/false, /*enableOpts2=*/false))) {
            return;
        }
    }
