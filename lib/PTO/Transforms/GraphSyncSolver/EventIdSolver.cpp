...
void EventIdSolver::insertConflictPair(EventIdNode* node, ConflictPair* conflictPair)
{
    ASSERT(node != nullptr && conflictPair != nullptr);
    actionsStack.push(std::make_unique<ActionInsertConflictPair>(node, conflictPair));
    conflictPair2Node[conflictPair] = node;
    node->insertConflictPair(conflictPair);
}

void EventIdSolver::eraseConflictPair(EventIdNode* node, ConflictPair* conflictPair)
{
    ASSERT(node != nullptr && conflictPair != nullptr);
...
void EventIdSolver::addEdge(EventIdNode* node1, EventIdNode* node2)
{
    ...
    LLVM_DEBUG({ llvm::dbgs() << "add-edge: " << node1->str(false) << ' ' << node2->str(false) << '\n'; });
    if (node1 == node2) {
        return;
    }
...
void EventIdSolver::assignEventIds(EventIdNode* node, const llvm::SmallVector<int64_t>& eventIds, bool pushAction)
{
    ASSERT(node != nullptr);
    ...
    if (pushAction) {
        actionsStack.push(std::make_unique<ActionAssignEventIds>(node, node->getEventIds(), eventIds));
    }
...
void EventIdSolver::assignNeedRecalc(bool newValue, bool pushAction)
{
    ...
    if (pushAction) {
        actionsStack.push(std::make_unique<ActionAssignNeedRecalc>(needRecalculateEventIds, newValue));
    }
...
EventIdNode* EventIdSolver::createNode(ConflictPair* conflictPair, int64_t eventIdNum, bool reversePriority)
{
    ASSERT(conflictPair != nullptr);
    ASSERT(eventIdNum > 0);
    auto node = std::make_unique<EventIdNode>(conflictPair, eventIdNum, reversePriority);
...
void EventIdSolver::addConflicts(ConflictPair* conflictPairSrc, const std::vector<ConflictPair*>& conflictPairsDst)
{
    ASSERT(conflictPairSrc != nullptr);
    EventIdNode* node1 = getNode(conflictPairSrc);
    for (auto* conflictPairDst : conflictPairsDst) {
        LLVM_DEBUG({ llvm::dbgs() << "add-conflict: " << conflictPairDst->str() << '\n'; });
        EventIdNode* node2 = getNode(conflictPairDst);
        addEdge(node1, node2);
    }
}

llvm::SmallVector<int64_t> EventIdSolver::getAdjNodesUsedEventIds(EventIdNode* node)
{
    llvm::SmallDenseSet<int64_t> usedEventIds;
    ...
    llvm::SmallVector<int64_t> usedEventIdsVec(usedEventIds.begin(), usedEventIds.end());
    llvm::sort(usedEventIdsVec);
    return usedEventIdsVec;
}

static llvm::SmallVector<int64_t> chooseForwardEventIds(ArrayRef<int64_t> usedEventIds, int64_t eventIdNum)
{
...
static llvm::SmallVector<int64_t> chooseReverseEventIds(
    ArrayRef<int64_t> usedEventIds, int64_t eventIdNum, int64_t eventIdMax, int64_t eventIdsNumMax)
{
    llvm::SmallVector<int64_t> chosenEventIds;
    int64_t curEventId = std::max(eventIdMax, eventIdsNumMax - 1);
    auto it = usedEventIds.rbegin();
    while ((curEventId >= 0) && (static_cast<int64_t>(chosenEventIds.size()) < eventIdNum)) {
    ...
    if (int64_t rem = eventIdNum - static_cast<int64_t>(chosenEventIds.size()); rem > 0) {
...
llvm::SmallVector<int64_t> EventIdSolver::getChosenEventIds(EventIdNode* node, int64_t eventIdMax)
{
    llvm::SmallVector<int64_t> usedEventIds = getAdjNodesUsedEventIds(node);
    auto chosenEventIds = node->reversePriority ?
                              chooseReverseEventIds(usedEventIds, node->eventIdNum, eventIdMax, this->eventIdsNumMax) :
...
{
    auto cmp = [](const std::pair<int64_t, EventIdNode*>& a, const std::pair<int64_t, EventIdNode*>& b) {
        if (a.first != b.first) {
...
{
    while (!actionsStack.empty() && (actionsStack.top()->actionType != ACTION_TYPE::NONE)) {
        ...
        llvm::TypeSwitch<Action*, void>(action.get())
            ...
            .Case([this](ActionAddEdge* action) { removeEdge(action->node1, action->node2); })
            .Case([this](ActionInsertConflictPair* action) { eraseConflictPair(action->node, action->conflictPair); })
            .Case([this](ActionAssignEventIds* action) {
                assignEventIds(
                    action->node, action->oldEventIds,
                    /*pushAction=*/false);
            })
            .Case([this](ActionAssignNeedRecalc* action) { assignNeedRecalc(action->oldValue, /*pushAction=*/false); })
            .Default([](Action*) { llvm_unreachable("EventIdSolver: unhandled action_type."); });
    }
