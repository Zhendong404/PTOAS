...
static PipeAttr makePipe(MLIRContext* ctx, PIPE pipe) { return PipeAttr::get(ctx, pipe); }
...
void CodeGenerator::setInsertionPoint(IRRewriter& rewriter, OperationBase* opBase, bool insertAfter)
{
    if (auto* ph = dyn_cast_or_null<PlaceHolder>(opBase)) {
        ...
        if (ph->scopeEnd && ph->block) {
            if (!ph->block->empty() && ph->block->back().hasTrait<OpTrait::IsTerminator>())
...
void CodeGenerator::emitSyncOp(IRRewriter& rewriter, SyncOp* syncOp)
{
    if (auto* barrier = dyn_cast<BarrierOp>(syncOp)) {
        rewriter.create<pto::BarrierOp>(resolveSyncLoc(barrier), makePipe(rewriter.getContext(), barrier->pipe));
        return;
    }
    ...
    ASSERT(!setWait->checkFirstIter && "checkFirstIter wrapping not implemented in codegen");
    ASSERT(!setWait->checkLastIter && "checkLastIter wrapping not implemented in codegen");
...
void CodeGenerator::emitSyncMap(IRRewriter& rewriter, SyncMap& syncMap, bool insertAfter)
{
