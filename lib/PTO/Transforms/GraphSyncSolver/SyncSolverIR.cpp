...
bool operator<(const SyncOp& op1, const SyncOp& op2) { return isa<WaitFlagOp>(&op1) && isa<SetFlagOp>(&op2); }
...
{
    std::string opStr = (op != nullptr ? op2str(op) : llvm::convertToCamelFromSnakeCase(getOpTypeStr(this->opType))) +
...
{
    std::string ret = std::string(indent, ' ') + llvm::convertToCamelFromSnakeCase(getOpTypeStr(this->opType)) +
                      std::to_string(this->id);
...
std::string Loop::str(int indent, bool recursive) const
{
    ...
    if (multibufferUnrollNum.has_value()) {
        ret += " multibuffer-unroll-num=" + std::to_string(multibufferUnrollNum.value());
    }
    if (recursive) {
        ret += " {\n";
        for (auto& op : body) {
    ...
    if (recursive) {
        ...
        for (auto& op : body) {
            if (op.get() == getTrueScope()) {
                ret += std::string(indent + kStep, ' ') + "(trueScope)\n";
            } else if (op.get() == getFalseScope()) {
                ret += std::string(indent + kStep, ' ') + "(falseScope)\n";
            }
            ret += op->str(indent + kStep, true) + "\n";
        }
        ret += std::string(indent, ' ') + "}";
...
std::string RWOperation::str(int indent, bool recursive) const
{
    ...
    if (this->pipeRead != this->pipeWrite) {
        pipesStr = "[<" + stringifyPIPE(this->pipeRead).str() + ">, <" + stringifyPIPE(this->pipeRead).str() + ">]";
    } else {
    ...
    if (!mergedUnitFlagInfo.disabledAsSet()) {
        ...
        ss << "unitFlagAsSet(";
        llvm::interleaveComma(mergedUnitFlagInfo.getUnitFlagModesAsSet(/*compress=*/true), ss);
        ss << ")";
        unitFlag += ss.str();
    }
    if (!mergedUnitFlagInfo.disabledAsWait()) {
        ...
        llvm::raw_string_ostream ss(iteratorsStr);
        ss << "unitFlagAsWait(";
        llvm::interleaveComma(mergedUnitFlagInfo.getUnitFlagModesAsWait(/*compress=*/true), ss);
        ss << ")";
        unitFlag += (!unitFlag.empty() ? " " : "") + ss.str();
    }
    ret += std::string(indent, ' ') + opStr + " " + coreTypeStr + " " + pipesStr + " " + unitFlag + "\n";
    if (indent != 0) {
...
{
    std::string ret;
    ret += std::string(indent, ' ') + llvm::convertToCamelFromSnakeCase(getOpTypeStr(this->opType)) +
           std::to_string(this->id);
    ...
    ret += stringifyPIPE(this->pipeSrc).str() + ">, <" + stringifyPIPE(this->pipeDst).str() + ">, (";
    Comma comma;
...
std::string ConflictPair::str() const
{
    ...
    ret += " (" + std::to_string(this->startIndex) + ", " + std::to_string(this->endIndex) + ")";
    if (this->isBarrier()) {
        ...
        ret += " [<" + stringifyPIPE(setCorePipeInfo.pipe).str() + ">, <" + stringifyPIPE(waitCorePipeInfo.pipe).str() +
               ">, (";
        Comma comma;
    ...
    ret += (replacedWithUnitFlag ? (comma.get() + "replaced-with-unit-flag") : "");
    ret += "}";
