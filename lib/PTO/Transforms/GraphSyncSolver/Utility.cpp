...
Occurrence* Occurrence::getParentWithOp(Occurrence* occ, const OperationBase* op, bool assertExists)
{
    ASSERT(op != nullptr);
    while (occ != nullptr) {
        if (occ->op == op) {
...
Occurrence* Occurrence::getParentWithOp(Occurrence* occ, const Operation* op, bool assertExists)
{
    ASSERT(op != nullptr);
    while (occ != nullptr) {
...
std::pair<Occurrence*, Occurrence*> Occurrence::getLCAPair(Occurrence* occ1, Occurrence* occ2)
{
...
Occurrence* Occurrence::getUnlikelyParentCondition(Occurrence* occ)
{
    ...
    if (auto* parentConditionOp = OperationBase::getUnlikelyParentCondition(occ->op)) {
...
bool OperationBase::sameScope(const OperationBase* op1, const OperationBase* op2)
{
...
std::pair<OperationBase*, OperationBase*> OperationBase::getLCAPair(OperationBase* op1, OperationBase* op2)
{
...
OperationBase* OperationBase::getUnlikelyParentCondition(OperationBase* op)
{
    ...
    while (cur != nullptr) {
        ...
        if (conditionOp->isUnlikely && conditionOp->getTrueScope()->isProperAncestor(op)) {
            return cur;
        }
...
bool checkRangesIntersect(int l1, int r1, int l2, int r2) { return r1 > l2 && r2 > l1; }
...
// ids.
int64_t getHWAvailableEventIdNum(SyncMode syncMode, pto::PIPE setPipe, pto::PIPE waitPipe)
{
...
SmallVector<int64_t> getHWAvailableEventIds(SyncMode syncMode, pto::PIPE setPipe, pto::PIPE waitPipe)
{
    if (syncMode == SyncMode::INTRA_CORE_SYNC) {
        const llvm::DenseMap<std::tuple<PIPE, PIPE>, int64_t> reservedEventIdNum = {
            {{pto::PIPE::PIPE_V, pto::PIPE::PIPE_S}, 1},
        ...
        std::iota(hwAvailableEventIds.begin(), hwAvailableEventIds.end(), static_cast<int64_t>(0));
        ...
        std::iota(availableEventIds.begin(), availableEventIds.end(), static_cast<int64_t>(0));
...
Value getIsFirstIterationValue(scf::ForOp forOp, Location loc, IRRewriter& rewriter)
{
    ...
    Value isFirstIter = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, lowerBound, currentInd);
    return isFirstIter;
}
...
Value getIsLastIterationValue(scf::ForOp forOp, Location loc, IRRewriter& rewriter)
{
    ...
    Value isLastIter = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::sge, nextInd, upperBound);
    return isLastIter;
}
...
Value getValueOrCreateCastToI64(IRRewriter& rewriter, Location loc, Value val)
{
    ...
    if (!val.getType().isInteger(kPTOI64BitWidth)) {
        if (val.getType().isIndex()) {
            val = rewriter.create<arith::IndexCastOp>(loc, rewriter.getIntegerType(kPTOI64BitWidth), val);
        } else if (val.getType().isInteger()) {
            val = rewriter.create<arith::ExtSIOp>(loc, rewriter.getIntegerType(kPTOI64BitWidth), val);
        } else {
