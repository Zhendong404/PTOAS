...
[[noreturn]] void reportSyncSolverInvariantFailure(const char* expr, const char* func, int line)
{
    llvm::report_fatal_error(
        llvm::Twine("SyncSolver invariant failed at ") + func + ":" + llvm::Twine(line) + ": " + expr);
}
} // namespace
...
Occurrence* Solver::getFirstIterOcc(Occurrence* occ, Occurrence* parOcc)
{
    ...
    if (parOcc->depth + 1 < occ->depth) {
        auto* newParOcc = getFirstIterOcc(Occurrence::getNthParent(occ, occ->depth - parOcc->depth - 1), parOcc);
        return getFirstIterOcc(occ, newParOcc);
    }
    auto* it = std::find_if(parOcc->childOccs.begin(), parOcc->childOccs.end(), [occ](const Occurrence* curOcc) {
...
Occurrence* Solver::getLastIterOcc(Occurrence* occ, Occurrence* parOcc)
{
    PTO_SYNC_SOLVER_CHECK(occ != nullptr && parOcc != nullptr);
    if (parOcc->depth + 1 < occ->depth) {
        auto* newParOcc = getLastIterOcc(Occurrence::getNthParent(occ, occ->depth - parOcc->depth - 1), parOcc);
        return getLastIterOcc(occ, newParOcc);
    }
    auto it = std::find_if(parOcc->childOccs.rbegin(), parOcc->childOccs.rend(), [occ](const Occurrence* curOcc) {
        return occ->op == curOcc->op;
    });
...
bool Solver::checkImpossibleOccPair(Occurrence* occ1, Occurrence* occ2) const
{
    ...
    auto [parOcc1, parOcc2] = Occurrence::getLCAPair(occ1, occ2);
    PTO_SYNC_SOLVER_CHECK(parOcc1 != nullptr && parOcc2 != nullptr);
    bool isIfElseSituation = parOcc1->parentOcc != nullptr && parOcc1->parentOcc == parOcc2->parentOcc &&
...
bool Solver::checkAlreadySyncedWithUnitFlag(Occurrence* occ1, const Occurrence* occ2)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
    ...
    DEBUG_WITH_TYPE("gss-sync-solver-check-unit-flag", {
        llvm::dbgs() << "unit-flag-step: " << occ1->syncIrIndex << ' ' << occ1->op->str(0, false) << "\n";
    });
    Occurrence* curOcc = occ1->unitFlagInfo.linkedElementAsSet;
    while (curOcc != nullptr) {
        DEBUG_WITH_TYPE("gss-sync-solver-check-unit-flag", {
            llvm::dbgs() << "unit-flag-step: " << curOcc->syncIrIndex << ' ' << curOcc->op->str(0, false) << "\n";
        });
...
bool Solver::ignoreMemoryConflict(
    const RWOperation* rwOp1, const RWOperation* rwOp2, const MemInfo& memInfo1, const MemInfo& memInfo2)
{
    (void)rwOp1;
    (void)rwOp2;
...
bool Solver::checkMemInfoConflict(
    RWOperation* rwOp1, RWOperation* rwOp2, const llvm::SmallVector<MemInfo>& memInfoList1,
    const llvm::SmallVector<MemInfo>& memInfoList2, std::optional<int64_t> lcmLen, std::optional<int64_t> eventIdNum)
{
    for (auto& memInfo1 : memInfoList1) {
        for (auto& memInfo2 : memInfoList2) {
            if (checkMemInfoConflict(rwOp1, rwOp2, memInfo1, memInfo2, lcmLen, eventIdNum)) {
                return true;
            }
        }
    }
...
// between two RW ops.
llvm::SmallVector<std::tuple<CorePipeInfo, CorePipeInfo>> Solver::checkMemoryConflicts(
    RWOperation* rwOp1, RWOperation* rwOp2)
{
    ...
    if (options.isCrossCoreMode()) {
        if (coreDst == pto::TCoreType::CUBE_AND_VECTOR) {
            coreDst = (coreSrc == pto::TCoreType::VECTOR) ? pto::TCoreType::CUBE : pto::TCoreType::VECTOR;
        }
        PTO_SYNC_SOLVER_CHECK(coreSrc == pto::TCoreType::VECTOR || coreSrc == pto::TCoreType::CUBE);
        PTO_SYNC_SOLVER_CHECK(coreDst == pto::TCoreType::VECTOR || coreDst == pto::TCoreType::CUBE);
    }
    CorePipePairDenseSet collectedConflictsSet;
    llvm::SmallVector<CorePipePairKey> collectedConflicts;
    auto addCollectedConflict = [&collectedConflictsSet, &collectedConflicts](CorePipeInfo src, CorePipeInfo dst) {
        CorePipePairKey key{src, dst};
        if (collectedConflictsSet.insert(key).second) {
            collectedConflicts.push_back(key);
        }
    };
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->readMemInfo, rwOp2->writeMemInfo)) {
        addCollectedConflict(CorePipeInfo(coreSrc, rwOp1->pipeRead), CorePipeInfo(coreDst, rwOp2->pipeWrite));
    }
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->readMemInfo)) {
        addCollectedConflict(CorePipeInfo(coreSrc, rwOp1->pipeWrite), CorePipeInfo(coreDst, rwOp2->pipeRead));
    }
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->writeMemInfo)) {
        addCollectedConflict(CorePipeInfo(coreSrc, rwOp1->pipeWrite), CorePipeInfo(coreDst, rwOp2->pipeWrite));
    }
    return it->second = collectedConflicts;
}

bool Solver::checkMemoryConflictBetweenOccExclusive(
    Occurrence* occ1, Occurrence* occ2, std::function<bool(RWOperation*)> filter)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
...
std::optional<LoopLikeOpInterface> Solver::getMultiBufferLoop(
    RWOperation* rwOp1, RWOperation* rwOp2, const llvm::SmallVector<MemInfo>& memInfoList1,
    ...
    for (auto& memInfo1 : memInfoList1) {
        for (auto& memInfo2 : memInfoList2) {
            if (checkMemInfoConflict(rwOp1, rwOp2, memInfo1, memInfo2)) {
                ...
                if (multibufferLoop1 == nullptr || multibufferLoop1 != multibufferLoop2) {
                    return {};
                }
                if (multibufferLoop.has_value() && multibufferLoop.value() != multibufferLoop1) {
                    return {};
                }
...
std::optional<LoopLikeOpInterface> Solver::getMultiBufferLoop(RWOperation* rwOp1, RWOperation* rwOp2)
{
    std::optional<LoopLikeOpInterface> multibufferLoop;
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->readMemInfo, rwOp2->writeMemInfo)) {
        auto curMultibufferLoop = getMultiBufferLoop(rwOp1, rwOp2, rwOp1->readMemInfo, rwOp2->writeMemInfo);
        if (multibufferLoop.has_value() && multibufferLoop.value() != curMultibufferLoop) {
            return {};
        }
        multibufferLoop = curMultibufferLoop;
    }
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->readMemInfo)) {
        auto curMultibufferLoop = getMultiBufferLoop(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->readMemInfo);
        if (multibufferLoop.has_value() && multibufferLoop.value() != curMultibufferLoop) {
            return {};
        }
        multibufferLoop = curMultibufferLoop;
    ...
    if (checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->writeMemInfo)) {
        auto curMultibufferLoop = getMultiBufferLoop(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->writeMemInfo);
        if (multibufferLoop.has_value() && multibufferLoop.value() != curMultibufferLoop) {
            return {};
        }
        multibufferLoop = curMultibufferLoop;
...
bool Solver::validateMultiBufferScope(
    Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2, LoopLikeOpInterface& multibufferLoop)
{
    ...
    if (options.isTestMode()) {
        ...
        if (!parLoop1 || parLoop1 != parLoop2)
            return false;
        return parLoop1->isProperAncestor(setOcc) && parLoop1->isProperAncestor(waitOcc);
    }
    ...
    return Occurrence::getParentWithOp(
               setOcc, multibufferLoop,
               /*assertExists=*/false) &&
           Occurrence::getParentWithOp(
               waitOcc, multibufferLoop,
               /*assertExists=*/false);
}

void Solver::updateMultiBufferConflictStats(
    RWOperation* rwOp1, RWOperation* rwOp2, const llvm::SmallVector<MemInfo>& lhs,
    const llvm::SmallVector<MemInfo>& rhs, bool includeMemInfo1Min, bool includeMemInfo2Min, int64_t& lcm,
...
std::optional<int64_t> Solver::findMultiBufferEventIdNum(
    RWOperation* rwOp1, RWOperation* rwOp2, int64_t lcm, int64_t minWriteSize)
{
    if (minWriteSize == LONG_MAX)
        return {};
    for (int64_t eventIdNum = minWriteSize; eventIdNum >= 1; --eventIdNum) {
        int64_t curLcm = std::lcm(lcm, eventIdNum);
        bool okRW = !checkMemInfoConflict(rwOp1, rwOp2, rwOp1->readMemInfo, rwOp2->writeMemInfo, curLcm, eventIdNum);
        bool okWR = !checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->readMemInfo, curLcm, eventIdNum);
        bool okWW = !checkMemInfoConflict(rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->writeMemInfo, curLcm, eventIdNum);
        if (okRW && okWR && okWW)
            return eventIdNum;
    }
    return {};
    ...
    Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2)
{
    ...
    updateMultiBufferConflictStats(
        rwOp1, rwOp2, rwOp1->readMemInfo, rwOp2->writeMemInfo, false, true, lcm, minWriteSize);
    updateMultiBufferConflictStats(
        rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->readMemInfo, true, false, lcm, minWriteSize);
    updateMultiBufferConflictStats(
        rwOp1, rwOp2, rwOp1->writeMemInfo, rwOp2->writeMemInfo, true, true, lcm, minWriteSize);
...
std::optional<EventIdInfo> Solver::checkMultiBufferEventIdInfo(
    Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2)
{
    PTO_SYNC_SOLVER_CHECK(rwOp1 != nullptr && rwOp2 != nullptr);
    if (!options.isTestMode()) {
        if (!checkAllParentLoopsAreForLoops(rwOp1->op) || !checkAllParentLoopsAreForLoops(rwOp2->op)) {
            return {};
        }
    }
...
std::optional<EventIdInfo> Solver::checkCVMultiBufferUnrollEventIdInfo(RWOperation* rwOp1, RWOperation* rwOp2)
{
    PTO_SYNC_SOLVER_CHECK(rwOp1 != nullptr && rwOp2 != nullptr);
    if (!options.isCrossCoreMode()) {
        return {};
    ...
    PTO_SYNC_SOLVER_CHECK(parentLoop1->multibufferUnrollNum.value() == parentLoop2->multibufferUnrollNum.value());
    EventIdInfo eventIdInfo;
    eventIdInfo.eventIdNum = parentLoop1->multibufferUnrollNum.value();
    eventIdInfo.multibufferUnrollLoop1 = cast<LoopLikeOpInterface>(parentLoop1->op);
    eventIdInfo.multibufferUnrollLoop2 = cast<LoopLikeOpInterface>(parentLoop2->op);
    return eventIdInfo;
}

std::optional<EventIdInfo> Solver::checkCVMultiBufferPreloadEventIdInfo(RWOperation* rwOp1, RWOperation* rwOp2)
{
    PTO_SYNC_SOLVER_CHECK(rwOp1 != nullptr && rwOp2 != nullptr);
    if (!options.isCrossCoreMode()) {
        return {};
    ...
    PTO_SYNC_SOLVER_CHECK(parentScope1->maxPreloadNum.value() == parentScope2->maxPreloadNum.value());
    ...
    eventIdInfo.preloadOffset1 = parentScope1->maxPreloadNum.value() - parentScope1->preloadNum.value() - 1;
    eventIdInfo.preloadOffset2 = parentScope2->maxPreloadNum.value() - parentScope2->preloadNum.value() - 1;
...
// occurrences.
EventIdInfo Solver::getEventIdInfo(
    Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2, CorePipeInfo, CorePipeInfo)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
    PTO_SYNC_SOLVER_CHECK(rwOp1 != nullptr && rwOp2 != nullptr);
    ...
    if (auto eventIdInfo = checkMultiBufferEventIdInfo(occ1, occ2, rwOp1, rwOp2)) {
        return eventIdInfo.value();
    }
    return singleEventId;
}
...
void Solver::addGraphConflictPair(
    GraphSolver& graphSolver, llvm::DenseSet<ConflictPair*>& visited, ConflictPair* conflictPair,
    EventIdInfo eventIdInfo, int startIndex, int endIndex,
    const llvm::SmallVector<ConflictPair*>& ignoreConflictPairs) const
{
    if (conflictPair->couldNotRun)
        return;
    if (conflictPair->endIndex < startIndex || conflictPair->startIndex > endIndex) {
        return;
    }
    if (conflictPair->isInnerBackward) {
        int64_t lhs = eventIdInfo.eventIdNum * eventIdInfo.eventIdRepeatNum;
        int64_t rhs = conflictPair->eventIdInfo.eventIdNum * conflictPair->eventIdInfo.eventIdRepeatNum;
        if (lhs < rhs)
            return;
    }
...
void Solver::addGraphConflictsFromScopes(
    const Occurrence* occ, GraphSolver& graphSolver, llvm::DenseSet<ConflictPair*>& visited, EventIdInfo eventIdInfo,
    int startIndex, int endIndex, const llvm::SmallVector<ConflictPair*>& ignoreConflictPairs, bool persistent)
{
    auto& conflictsMap = persistent ? persistentScopeOccChosenConflicts : scopeOccChosenConflicts;
    for (auto* parOcc : occ->getAllParents()) {
        if (!conflictsMap.contains(parOcc))
            continue;
        for (auto* conflictPair : conflictsMap[parOcc]) {
            addGraphConflictPair(
                graphSolver, visited, conflictPair, eventIdInfo, startIndex, endIndex, ignoreConflictPairs);
        }
    }
}

void Solver::addGraphConflictsFromScopePairs(
    Occurrence* occ1, Occurrence* occ2, GraphSolver& graphSolver, llvm::DenseSet<ConflictPair*>& visited,
    EventIdInfo eventIdInfo, int startIndex, int endIndex, const llvm::SmallVector<ConflictPair*>& ignoreConflictPairs)
{
    for (auto& [scopeOccPair, chosenConflicts] : scopeOccPairChosenConflicts) {
        auto [scopeOcc1, scopeOcc2] = scopeOccPair;
        if (!scopeOcc1->isProperAncestor(occ1) || !scopeOcc2->isProperAncestor(occ2)) {
            continue;
        }
...
bool Solver::checkGraphConflict(
    Occurrence* occ1, Occurrence* occ2, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, EventIdInfo eventIdInfo,
    ...
    const llvm::SmallVector<ConflictPair*>& ignoreConflictPairs)
{
    ...
    addGraphConflictsFromScopes(
        occ1, graphSolver, visited, eventIdInfo, *startIndex, *endIndex, ignoreConflictPairs,
        /*persistent=*/false);
    addGraphConflictsFromScopes(
        occ2, graphSolver, visited, eventIdInfo, *startIndex, *endIndex, ignoreConflictPairs,
        /*persistent=*/false);
    addGraphConflictsFromScopePairs(
        occ1, occ2, graphSolver, visited, eventIdInfo, *startIndex, *endIndex, ignoreConflictPairs);
    addGraphConflictsFromScopes(
        occ1, graphSolver, visited, eventIdInfo, *startIndex, *endIndex, ignoreConflictPairs,
        /*persistent=*/true);
    addGraphConflictsFromScopes(
    ...
    if (options.enableUnitFlagFeature) {
        mnDistance =
            graphSolver.runDijkstraUnitFlagEnabled(occ1, occ2, corePipeSrc, corePipeDst, *startIndex, *endIndex);
    } else {
        mnDistance = graphSolver.runDijkstra(corePipeSrc, corePipeDst, *startIndex, *endIndex);
    }
    return !mnDistance.has_value() || mnDistance.value() > *endIndex;
}

bool Solver::checkSetSyncOpsConflict(ConflictPair* conflictPair1, ConflictPair* conflictPair2)
{
    ...
    PTO_SYNC_SOLVER_CHECK(conflictPair1->setOcc != nullptr && conflictPair2->setOcc != nullptr);
    bool result = checkGraphConflict(
        conflictPair1->setOcc, conflictPair2->setOcc, corePipeSrc, corePipeDst, conflictPair1->eventIdInfo, startIndex,
        endIndex, {conflictPair1}, {conflictPair2});
    conflictPair1->startIndex -= 1;
    return result;
}

bool Solver::checkWaitSyncOpsConflict(ConflictPair* conflictPair1, ConflictPair* conflictPair2)
{
    ...
    PTO_SYNC_SOLVER_CHECK(conflictPair1->waitOcc != nullptr && conflictPair2->waitOcc != nullptr);
    bool result = checkGraphConflict(
        conflictPair1->waitOcc, conflictPair2->waitOcc, corePipeSrc, corePipeDst, conflictPair1->eventIdInfo,
        startIndex, endIndex, {conflictPair1}, {conflictPair2});
    conflictPair2->endIndex += 1;
    return result;
}

bool Solver::checkSyncOpsConflicts(ConflictPair* conflictPair1, ConflictPair* conflictPair2)
{
    ...
    if (conflictPair1->startIndex >= conflictPair2->startIndex || conflictPair1->endIndex >= conflictPair2->endIndex) {
        return true;
    }
    bool result =
        checkSetSyncOpsConflict(conflictPair1, conflictPair2) || checkWaitSyncOpsConflict(conflictPair1, conflictPair2);
...
bool Solver::checkIntersect(ConflictPair* conflictPair1, ConflictPair* conflictPair2)
{
    ...
    if (conflictPair1->dontCheckForConflict || conflictPair2->dontCheckForConflict) {
        return false;
    }
    if (options.isCrossCoreMode()) {
...
std::vector<ConflictPair*> Solver::getIntersectingConflictPairs(ConflictPair* conflictPair)
{
...
bool Solver::checkSkippable(bool reverseOrder, Occurrence* occ) { return skipOcc[reverseOrder].contains(occ); }
...
llvm::DenseSet<ConflictPair*> Solver::getMemorizedSyncedPairs(ConflictPair* conflictPair)
{
    auto key = std::make_tuple(
        conflictPair->backwardSyncLoopOp, conflictPair->op1, conflictPair->op2, conflictPair->setCorePipeInfo,
...
void Solver::memorizeReusedSyncedPair(ConflictPair* conflictPair, ConflictPair* reusedConflictPair)
{
    PTO_SYNC_SOLVER_CHECK(conflictPair != nullptr);
...
bool Solver::skipMMad1DecomposedLoopOpt(Occurrence* occ1, Occurrence* occ2) const
{
    ...
    if (parentLoopOp1 != nullptr && parentLoopOp2 != nullptr) {
        if (parentLoopOp1 != parentLoopOp2) {
            if (isa<MmadL1LoopOp>(parentLoopOp1) && isa<MmadL1LoopOp>(parentLoopOp2)) {
                return true;
            }
        }
    }
...
std::optional<std::pair<Occurrence*, Occurrence*>> Solver::checkAndApplyMmadl0LoopOpt(
    ConflictPair* conflictPair, Occurrence* occ1, Occurrence* occ2, Occurrence* parOcc1, Occurrence* parOcc2)
{
    ...
    if (occ1->parentOcc != nullptr && occ1->parentOcc->parentOcc != nullptr &&
        ...
        llvm::isa_and_present<syncsolver::LoadL0AOp, syncsolver::LoadL0BOp>(occ1->op) &&
        llvm::isa_and_present<syncsolver::MmadL1LoopOp>(occ1->parentOcc->parentOcc->op)) {
        conflictPair->setOnLastIterOnly = true;
        return std::make_pair(occ1, parOcc2);
    }
    if (!conflictPair->isInnerBackward && occ2->parentOcc != nullptr && occ2->parentOcc->parentOcc != nullptr &&
        occ2->parentOcc->parentOcc->parentOcc == parOcc2 &&
        llvm::isa_and_present<syncsolver::LoadL0AOp, syncsolver::LoadL0BOp>(occ2->op) &&
        llvm::isa_and_present<syncsolver::MmadL1LoopOp>(occ2->parentOcc->parentOcc->op)) {
...
std::optional<UnitFlagInfo> Solver::checkUnitFlagPatterns(Occurrence*, Occurrence*) const { return {}; }
...
std::pair<Occurrence*, Occurrence*> Solver::getSetWaitLCAPairOcc(Occurrence* occ1, Occurrence* occ2) const
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
    ...
    PTO_SYNC_SOLVER_CHECK(grandParOcc1->parentOcc != nullptr && grandParOcc2->parentOcc != nullptr);
    ...
    auto* setOcc = Occurrence::getNthParent(occ1, occ1->depth - parOcc1->depth - 1);
    auto* waitOcc = Occurrence::getNthParent(occ2, occ2->depth - parOcc2->depth - 1);
...
void Solver::adjustSetWaitForLoopBoundary(Occurrence* occ1, Occurrence*& setOcc, Occurrence*& waitOcc)
{
...
void Solver::adjustSetWaitForBackwardCondition(
    Occurrence* occ1, Occurrence* occ2, Occurrence*& setOcc, Occurrence*& waitOcc) const
{
    if (!isBackwardSync(occ1, occ2))
        return;
    if (setOcc->parentOcc && llvm::isa_and_present<Condition>(setOcc->parentOcc->op)) {
        setOcc = setOcc->parentOcc;
    }
    if (waitOcc->parentOcc && llvm::isa_and_present<Condition>(waitOcc->parentOcc->op)) {
        waitOcc = waitOcc->parentOcc;
    }
}

void Solver::adjustSetWaitForCrossCoreLoops(
    Occurrence* occ1, Occurrence* occ2, Occurrence*& setOcc, Occurrence*& waitOcc)
{
    if (!options.isCrossCoreMode())
        return;
    ...
    if (!forOp1 || !forOp2 || !forOp1->multibufferUnrollNum || !forOp2->multibufferUnrollNum) {
        return;
    }
    PTO_SYNC_SOLVER_CHECK(forOp1->multibufferUnrollNum == forOp2->multibufferUnrollNum);
    setOcc = Occurrence::getNthParent(occ1, occ1->depth - setOcc->depth - kCrossCoreParentDepthOffset);
    waitOcc = Occurrence::getNthParent(occ2, occ2->depth - waitOcc->depth - kCrossCoreParentDepthOffset);
}

void Solver::adjustSetWaitForCrossCoreScopes(
    Occurrence* occ1, Occurrence* occ2, Occurrence*& setOcc, Occurrence*& waitOcc)
{
    if (!options.isCrossCoreMode())
        return;
    ...
    if (!scopeOp1 || !scopeOp2 || !scopeOp1->maxPreloadNum || !scopeOp2->maxPreloadNum) {
        return;
    }
...
void Solver::adjustSetWaitForLoopPlaceholders(Occurrence*& setOcc, Occurrence*& waitOcc)
{
...
std::pair<Occurrence*, Occurrence*> Solver::getFixedSetWaitOcc(Occurrence* occ1, Occurrence* occ2)
{
...
std::optional<std::pair<Occurrence*, Occurrence*>> Solver::getFunctionBlockSetWaitOcc(
    const Occurrence* occ1, Occurrence* occ2)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
...
std::optional<std::pair<Occurrence*, Occurrence*>> Solver::getUnlikelyCondSetWaitOcc(Occurrence* occ1, Occurrence* occ2)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
    if (options.isCrossCoreMode() && isBackwardSync(occ1, occ2)) {
        return {};
    }
    if (auto* unlikelyParCondOcc1 = Occurrence::getUnlikelyParentCondition(occ1)) {
        if (!unlikelyParCondOcc1->isProperAncestor(occ2)) {
            ...
            if (parentLoopOcc == nullptr || parentLoopOcc->isProperAncestor(occ2)) {
                auto* placeHolderOcc = getScopeEndPlaceHolderOcc(
                    Occurrence::getNthParent(occ1, occ1->depth - unlikelyParCondOcc1->depth - 1));
                return std::make_pair(occ1, placeHolderOcc);
            }
        }
    }
    if (auto* unlikelyParCondOcc2 = Occurrence::getUnlikelyParentCondition(occ2)) {
        if (!unlikelyParCondOcc2->isProperAncestor(occ1)) {
            ...
            if (parentLoopOcc == nullptr || parentLoopOcc->isProperAncestor(occ1)) {
                auto* placeHolderOcc = getScopeBeginPlaceHolderOcc(
                    Occurrence::getNthParent(occ2, occ2->depth - unlikelyParCondOcc2->depth - 1));
                return std::make_pair(placeHolderOcc, occ2);
            }
        }
    }
...
std::pair<Occurrence*, Occurrence*> Solver::getSetWaitOcc(Occurrence* occ1, Occurrence* occ2)
{
...
void Solver::insertBarrierAllBeforeOcc(Occurrence* occ, bool isUseless, bool isPersistent)
{
    PTO_SYNC_SOLVER_CHECK(occ != nullptr);
    ...
    auto conflictPair = std::make_unique<ConflictPair>(
        nullptr, nullptr, rwOp, rwOp, occ, occ, CorePipeInfo(pto::TCoreType::CUBE_OR_VECTOR, pto::PIPE::PIPE_ALL),
        CorePipeInfo(pto::TCoreType::CUBE_OR_VECTOR, pto::PIPE::PIPE_ALL), occ->startIndex, occ->startIndex);
    conflictPair->isUseless = isUseless;
    auto* normScopeOcc = occ->parentOcc;
    PTO_SYNC_SOLVER_CHECK(normScopeOcc != nullptr);
    LLVM_DEBUG(llvm::dbgs() << (isPersistent ? "is-persistent " : "") << occ->op->str(0, false) << ' '
                            << conflictPair->str() << '\n';);
    if (isPersistent) {
...
void Solver::insertBarrierAllBeforeOp(const OperationBase* op, bool isUseless, bool isPersistent)
{
...
void Solver::pickAndInsertABarrierAll()
{
    ...
    insertBarrierAllBeforeOp(
        chosenOp, /*isUseless=*/false,
        /*isPersistent=*/true);
}
...
bool Solver::reuseCmp(const ConflictPair* conflictPair1, const ConflictPair* conflictPair2) const
{
    PTO_SYNC_SOLVER_CHECK(conflictPair1 != nullptr && conflictPair2 != nullptr);
...
ConflictPair* Solver::getReusableConflictPair(
    ConflictPair* conflictPair, const llvm::DenseSet<ConflictPair*>& conflictPairsSet)
{
    PTO_SYNC_SOLVER_CHECK(conflictPair != nullptr);
    ...
    for (auto* curConflictPair : conflictPairsSet) {
        ...
        if (curConflictPair->op1 != conflictPair->op1 || curConflictPair->op2 != conflictPair->op2 ||
        ...
        if (conflictPair->eventIdNode->eventIdNum < curConflictPair->eventIdNode->eventIdNum) {
            continue;
        }
        PTO_SYNC_SOLVER_CHECK(conflictPair->eventIdNode != nullptr);
        PTO_SYNC_SOLVER_CHECK(curConflictPair->eventIdNode != nullptr);
        if (conflictPair->eventIdNode->eventIdNum > curConflictPair->eventIdNode->eventIdNum) {
            if ((conflictPair->eventIdNode->eventIdNum % curConflictPair->eventIdNode->eventIdNum) != 0) {
                continue;
            }
        }
...
bool Solver::hasReusableConflictBudget(const ConflictPair* conflictPair, const ConflictPair* oldReusedConflictPair)
{
    ...
    if (!reusePairs.contains({corePipeSrc, corePipeDst}) ||
        reusePairs[{corePipeSrc, corePipeDst}] <= reusedPairs[{corePipeSrc, corePipeDst}]) {
        return false;
    }
    PTO_SYNC_SOLVER_CHECK(reusePairs.contains(std::make_tuple(corePipeSrc, corePipeDst)));
...
ConflictPair* Solver::findBestReusableConflictPair(
    ConflictPair* conflictPair, Occurrence* scopeOcc1, Occurrence* scopeOcc2)
{
    ConflictPair* reusableConflictPair = nullptr;
    auto updateReusableCandidate = [this, &reusableConflictPair](ConflictPair* candidate) {
        if (!candidate)
            return;
...
void Solver::applyReusableConflictPair(ConflictPair* conflictPair, ConflictPair* reusableConflictPair) const
{
...
bool Solver::reuseConflictPair(ConflictPair* conflictPair, Occurrence* scopeOcc1, Occurrence* scopeOcc2)
{
    if (conflictPair->isBarrier())
        return false;
    ...
    if (!hasReusableConflictBudget(conflictPair, oldReusedConflictPair))
        return false;
    ConflictPair* reusableConflictPair = findBestReusableConflictPair(conflictPair, scopeOcc1, scopeOcc2);
...
std::unique_ptr<EventIdSolver>& Solver::getEventIdSolverRef(pto::PIPE pipeSrc, pto::PIPE pipeDst)
{
    if (options.isCrossCoreMode()) {
    ...
    if (!eventIdSolver.contains(key)) {
        int64_t eventIdNumMax = getHWAvailableEventIdNum(options.syncMode, pipeSrc, pipeDst);
...
bool Solver::checkReuseMultiBufferFlagId(ConflictPair* conflictPair)
{
    ...
    if (!conflictPair->isInnerBackward || conflictPair->eventIdInfo.eventIdNum <= 1 || conflictPair->movedToOuterLoop) {
        return false;
    }
    auto [setOcc, waitOcc] = std::tie(conflictPair->setOcc, conflictPair->waitOcc);
...
std::unique_ptr<ConflictPair> Solver::createSetWaitConflictPair(
    Occurrence* occ1, Occurrence* occ2, RWOperation* rwOp1, RWOperation* rwOp2, CorePipeInfo corePipeSrc,
    CorePipeInfo corePipeDst, EventIdInfo eventIdInfo, bool isUseless, Occurrence*& setOcc, Occurrence*& waitOcc,
    Occurrence*& normScopeOcc1, Occurrence*& normScopeOcc2, OperationBase*& normScopeOp, Loop*& parentLCALoopOp,
    Occurrence*& parentLCALoopOcc, Occurrence*& parentLCALoopBeforePHOcc, Occurrence*& parentLCALoopAfterPHOcc)
{
    ...
    auto conflictPair = std::make_unique<ConflictPair>(
        rwOp1, rwOp2, setOcc->op, waitOcc->op, setOcc, waitOcc, corePipeSrc, corePipeDst, setOcc->endIndex,
        waitOcc->startIndex);
    ...
    PTO_SYNC_SOLVER_CHECK(parentLCALoopBeforePHOcc != nullptr && parentLCALoopAfterPHOcc != nullptr);
    return conflictPair;
}

void Solver::normalizeSetWaitConflictPair(
    ConflictPair* conflictPair, Occurrence* occ1, Occurrence* occ2, Occurrence*& setOcc, Occurrence*& waitOcc,
    CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst)
{
    if (auto setWaitOccs = checkAndApplyMmadl0LoopOpt(conflictPair, occ1, occ2, setOcc, waitOcc)) {
        std::tie(setOcc, waitOcc) = setWaitOccs.value();
        conflictPair->updateSetWaitOccs(setOcc, waitOcc);
    }
    if (!conflictPair->isInnerBackward || disabledMultiEventIdPairs.contains({corePipeSrc, corePipeDst})) {
        conflictPair->eventIdInfo = EventIdInfo(1);
    }
    if (!checkReuseMultiBufferFlagId(conflictPair))
        return;
    conflictPair->eventIdInfo.eventIdRepeatNum = conflictPair->eventIdInfo.eventIdNum;
    conflictPair->eventIdInfo.eventIdNum = 1;
}

bool Solver::prepareSetWaitConflictPairWithEventSolver(
    EventIdSolver& curEventIdSolver, ConflictPair* conflictPair, Occurrence* occ1, Occurrence* occ2,
    Occurrence* normScopeOcc1, Occurrence* normScopeOcc2, const OperationBase* normScopeOp, OperationBase* barrierOp,
    CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst)
{
    curEventIdSolver.pushActionNone();
    initializeConflictEventIdNode(conflictPair, curEventIdSolver, occ1, occ2, normScopeOp);
    if (options.reuseSyncPairToSaveEventIds && reuseConflictPair(conflictPair, normScopeOcc1, normScopeOcc2)) {
    ...
    return checkColorableOrConvertToBarrierAll(curEventIdSolver, conflictPair, barrierOp, corePipeSrc, corePipeDst);
}

bool Solver::checkColorableOrConvertToBarrierAll(
    EventIdSolver& curEventIdSolver, ConflictPair* conflictPair, OperationBase* barrierOp, CorePipeInfo corePipeSrc,
    CorePipeInfo corePipeDst)
{
    if (curEventIdSolver.isColorable())
        return true;
    LLVM_DEBUG(llvm::dbgs() << "will-be-converted-to-barrier-all " << conflictPair->str() << '\n';);
    insertBarrierAllBeforeOp(
        barrierOp, conflictPair->isUseless,
        /*isPersistent=*/false);
...
void Solver::initializeConflictEventIdNode(
    ConflictPair* conflictPair, EventIdSolver& curEventIdSolver, Occurrence* occ1, Occurrence* occ2,
    const OperationBase* normScopeOp)
{
    ...
    if (conflictPair->isInnerBackward && OperationBase::getParentloop(occ1->op) == normScopeOp->parentOp &&
        OperationBase::getParentloop(occ2->op) == normScopeOp->parentOp) {
        reversedPriority = true;
    }
    conflictPair->eventIdNode =
        curEventIdSolver.createNode(conflictPair, conflictPair->eventIdInfo.eventIdNum, reversedPriority);
}

bool Solver::insertExtraConflictPair(
    EventIdSolver& curEventIdSolver, ConflictPair* conflictPair, Occurrence* setOcc, Occurrence* waitOcc,
    Occurrence* parentScope, OperationBase* barrierOp, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst,
    ExtraConflictPairs& extraConflictPairs, bool couldNotRun)
{
    ...
    LLVM_DEBUG({ llvm::dbgs() << "extra-conflict-pair: " << extraConflictPair->str() << "\n"; });
    curEventIdSolver.insertConflictPair(conflictPair->eventIdNode, extraConflictPair.get());
    auto intersectingConflictPairs = getIntersectingConflictPairs(extraConflictPair.get());
    curEventIdSolver.addConflicts(extraConflictPair.get(), intersectingConflictPairs);
    if (!checkColorableOrConvertToBarrierAll(curEventIdSolver, conflictPair, barrierOp, corePipeSrc, corePipeDst)) {
        return false;
    }
    extraConflictPairs.push_back(std::make_pair(std::move(extraConflictPair), parentScope));
    return true;
}

bool Solver::insertOuterBackwardConflictPairIfNeeded(
    EventIdSolver& curEventIdSolver, ConflictPair* conflictPair, const Occurrence* setOcc, const Occurrence* waitOcc,
    const Loop* parentLCALoopOp, const Occurrence* parentLCALoopOcc, Occurrence* parentLCALoopBeforePHOcc,
    Occurrence* parentLCALoopAfterPHOcc, OperationBase* barrierOp, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst,
    ExtraConflictPairs& extraConflictPairs)
{
    ...
    if ((conflictPair->eventIdInfo.eventIdNum * conflictPair->eventIdInfo.eventIdRepeatNum) > 1) {
        ...
        if (setOcc->parentOcc == nullptr || setOcc->parentOcc->parentOcc == nullptr ||
            ...
            waitOcc->parentOcc == nullptr || waitOcc->parentOcc->parentOcc == nullptr ||
    ...
    return insertExtraConflictPair(
        curEventIdSolver, conflictPair, parentLCALoopBeforePHOcc, parentLCALoopAfterPHOcc, parentLCALoopOcc->parentOcc,
        barrierOp, corePipeSrc, corePipeDst, extraConflictPairs);
}

bool Solver::insertBoundaryBackwardConflictPairsIfNeeded(
    EventIdSolver& curEventIdSolver, ConflictPair* conflictPair, Occurrence* setOcc, Occurrence* waitOcc,
    Occurrence* normScopeOcc1, Occurrence* normScopeOcc2, Occurrence* parentLCALoopOcc,
    Occurrence* parentLCALoopBeforePHOcc, Occurrence* parentLCALoopAfterPHOcc, OperationBase* barrierOp,
    CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, ExtraConflictPairs& extraConflictPairs)
{
    if (!conflictPair->isInnerBackward || conflictPair->eventIdNode == nullptr)
        return true;
    auto* loopOpOcc1 = getFirstIterOcc(waitOcc, normScopeOcc1);
    auto* loopOpOcc2 = getLastIterOcc(setOcc, normScopeOcc2);
    if (!insertExtraConflictPair(
            curEventIdSolver, conflictPair, parentLCALoopBeforePHOcc, loopOpOcc1, parentLCALoopOcc, barrierOp,
            corePipeSrc, corePipeDst, extraConflictPairs,
            /*couldNotRun=*/true)) {
        return false;
    }
    return insertExtraConflictPair(
        curEventIdSolver, conflictPair, loopOpOcc2, parentLCALoopAfterPHOcc, parentLCALoopOcc, barrierOp, corePipeSrc,
        corePipeDst, extraConflictPairs, /*couldNotRun=*/true);
}

bool Solver::collectBackwardConflictExtras(
    EventIdSolver& curEventIdSolver, ConflictPair* conflictPair, Occurrence* setOcc, Occurrence* waitOcc,
    Occurrence* normScopeOcc1, Occurrence* normScopeOcc2, Loop* parentLCALoopOp, Occurrence* parentLCALoopOcc,
    Occurrence* parentLCALoopBeforePHOcc, Occurrence* parentLCALoopAfterPHOcc, OperationBase* barrierOp,
    CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, ExtraConflictPairs& extraConflictPairs)
{
    LLVM_DEBUG({
    ...
    if (!insertOuterBackwardConflictPairIfNeeded(
            curEventIdSolver, conflictPair, setOcc, waitOcc, parentLCALoopOp, parentLCALoopOcc,
            parentLCALoopBeforePHOcc, parentLCALoopAfterPHOcc, barrierOp, corePipeSrc, corePipeDst,
            extraConflictPairs)) {
        return false;
    }
    return insertBoundaryBackwardConflictPairsIfNeeded(
        curEventIdSolver, conflictPair, setOcc, waitOcc, normScopeOcc1, normScopeOcc2, parentLCALoopOcc,
        parentLCALoopBeforePHOcc, parentLCALoopAfterPHOcc, barrierOp, corePipeSrc, corePipeDst, extraConflictPairs);
}

void Solver::recordChosenConflictPair(
    ConflictPair* conflictPair, Occurrence* normScopeOcc1, Occurrence* normScopeOcc2,
    ...
    if (conflictPair->isInnerBackward && normScopeOcc1 != normScopeOcc2) {
        ...
        if (auto* conditionOp = llvm::dyn_cast_if_present<Condition>(parCond)) {
            if (parentLCALoopOcc->op->isProperAncestor(conditionOp)) {
                scopeOccPairChosenConflicts[{normScopeOcc1, normScopeOcc2}].insert(conflictPair);
                dontInsert = true;
            }
        }
    }
...
void Solver::handleSetWaitConflict(
    Occurrence* occ1, Occurrence* occ2, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, EventIdInfo eventIdInfo,
    bool isUseless)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
    ...
    auto conflictPair = createSetWaitConflictPair(
        occ1, occ2, rwOp1, rwOp2, corePipeSrc, corePipeDst, eventIdInfo, isUseless, setOcc, waitOcc, normScopeOcc1,
        normScopeOcc2, normScopeOp, parentLCALoopOp, parentLCALoopOcc, parentLCALoopBeforePHOcc,
        parentLCALoopAfterPHOcc);
    normalizeSetWaitConflictPair(conflictPair.get(), occ1, occ2, setOcc, waitOcc, corePipeSrc, corePipeDst);
    auto& curEventIdSolver =
        getEventIdSolverRef(conflictPair->setCorePipeInfo.pipe, conflictPair->waitCorePipeInfo.pipe);
    if (!prepareSetWaitConflictPairWithEventSolver(
            *curEventIdSolver, conflictPair.get(), occ1, occ2, normScopeOcc1, normScopeOcc2, normScopeOp, occ2->op,
            corePipeSrc, corePipeDst)) {
        return;
    }
    ExtraConflictPairs extraConflictPairs;
    if (!collectBackwardConflictExtras(
            *curEventIdSolver, conflictPair.get(), setOcc, waitOcc, normScopeOcc1, normScopeOcc2, parentLCALoopOp,
            parentLCALoopOcc, parentLCALoopBeforePHOcc, parentLCALoopAfterPHOcc, occ2->op, corePipeSrc, corePipeDst,
            extraConflictPairs)) {
        return;
    }
    recordChosenConflictPair(conflictPair.get(), normScopeOcc1, normScopeOcc2, parentLCALoopOcc);
...
void Solver::handleBarrierConflict(
    Occurrence* occ1, Occurrence* occ2, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, bool isUseless)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
    auto* rwOp1 = llvm::dyn_cast_if_present<RWOperation>(occ1->op);
    auto* rwOp2 = llvm::dyn_cast_if_present<RWOperation>(occ2->op);
    ...
    if (options.isRegBasedArch) {
        if (corePipeSrc.pipe == pto::PIPE::PIPE_V || corePipeSrc.pipe == pto::PIPE::PIPE_M) {
            return;
        }
    }
    ...
    auto conflictPair = std::make_unique<ConflictPair>(
        rwOp1, rwOp2, waitOcc->op, waitOcc->op, waitOcc, waitOcc, corePipeSrc, corePipeDst, waitOcc->startIndex,
        waitOcc->startIndex);
    conflictPair->isUseless = isUseless;
    PTO_SYNC_SOLVER_CHECK(conflictPair->startIndex <= conflictPair->endIndex);
...
void Solver::handleUnitFlagConflict(
    Occurrence* occ1, Occurrence* occ2, CorePipeInfo corePipeSrc, CorePipeInfo corePipeDst, UnitFlagInfo unitFlagInfo,
    bool isUseless)
{
    PTO_SYNC_SOLVER_CHECK(occ1 != nullptr && occ2 != nullptr);
    auto* rwOp1 = llvm::dyn_cast_if_present<RWOperation>(occ1->op);
    ...
    occ1->unitFlagInfo.merge(
        unitFlagInfo, occ1, occ2,
        /*asSet=*/true, /*asWait=*/false);
    occ2->unitFlagInfo.merge(
        unitFlagInfo, occ1, occ2,
        /*asSet=*/false, /*asWait=*/true);
    if (!isUseless) {
        rwOp1->mergedUnitFlagInfo.merge(
            unitFlagInfo, /*asSet=*/true,
            /*asWait=*/false);
        rwOp2->mergedUnitFlagInfo.merge(
            unitFlagInfo, /*asSet=*/false,
            /*asWait=*/true);
    }

    scopeOccPairChosenConflicts[{normScopeOcc1, normScopeOcc2}].insert(conflictPair.get());
    chosenConflictedPairs.push_back(std::move(conflictPair));
}

void Solver::handleConflict(
    Occurrence* occ1, Occurrence* occ2, const RWOperation* rwOp1, const RWOperation* rwOp2, CorePipeInfo corePipeSrc,
    CorePipeInfo corePipeDst, EventIdInfo eventIdInfo, bool isUseless)
{
    ...
    LLVM_DEBUG({
        llvm::dbgs() << "conflict found: " << "eventIdNum(" << eventIdInfo.eventIdNum << ")\n";
        llvm::dbgs() << occ1->syncIrIndex << ' ' << occ1->startIndex << ' ' << occ1->endIndex << ' '
                     << rwOp1->str(0, false) << '\n';
        llvm::dbgs() << occ2->syncIrIndex << ' ' << occ2->startIndex << ' ' << occ2->endIndex << ' '
                     << rwOp2->str(0, false) << '\n';
    });
    if (corePipeSrc == corePipeDst) {
        ...
        handleUnitFlagConflict(occ1, occ2, corePipeSrc, corePipeDst, unitFlagInfo.value(), isUseless);
    } else {
        handleSetWaitConflict(occ1, occ2, corePipeSrc, corePipeDst, eventIdInfo, isUseless);
    }
}
