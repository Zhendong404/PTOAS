...
static bool hasInterCoreSyncOp(func::FuncOp func) {
  ...
  func.walk([&](Operation *op) {
    if (isa<pto::SyncSetOp, pto::SyncWaitOp, pto::SetCrossCoreOp,
            pto::WaitFlagDevOp>(op)) {
      found = true;
      return WalkResult::interrupt();
    }
    return WalkResult::advance();
...
static void populatePTOToEmitCPatterns(RewritePatternSet &patterns,
  ...
  patterns.add<PTOCrossSyncToSync<pto::SetCrossCoreOp, pto::SyncSetOp>,
               PTOCrossSyncToSync<pto::WaitFlagDevOp, pto::SyncWaitOp>>(
      typeConverter, ctx);
  patterns.add<PTONamedIntraSyncToEmitC<pto::SetIntraBlockOp>,
               PTONamedIntraSyncToEmitC<pto::WaitIntraCoreOp>>(typeConverter,
                                                               ctx);
