...
static bool getStaticMemrefLayout(MemRefType mrTy, SmallVectorImpl<int64_t>& strides, int64_t& offset);
static std::string getGlobalTensorTypeStringFromShape(
    Type elemTy, ArrayRef<int64_t> shape, StringRef layoutEnum = "pto::Layout::ND");
static emitc::OpaqueType getGlobalTensorOpaqueTypeFromShape(
    MLIRContext* ctx, Type elemTy, ArrayRef<int64_t> shape, StringRef layoutEnum = "pto::Layout::ND");
...
[[maybe_unused]] static constexpr llvm::StringLiteral kLoweredSetValidShapeAttrName = "__pto.lowered_set_validshape";
...
[[maybe_unused]] static constexpr llvm::StringLiteral kGlobalTensorStridesAttrName = "__pto.globaltensor_strides";
...
static Value maybeWrapGlobalMemrefAsGlobalTensor(
    ConversionPatternRewriter& rewriter, Location loc, Value loweredValue, Type originalType, Operation* anchor);
...
static bool isKnownUnitExtentForMGather(int64_t value) { return value == ShapedType::kDynamic || value == 1; }
...
static std::optional<GatherScatterShapeLayoutInfo> getGatherScatterShapeLayoutInfo(Type ty)
{
...
static bool isRowCoalescedMGatherIndexType(Type dataTy, Type idxTy)
{
    ...
    const bool rowCoalesce1xR = idxInfo->rowMajor && isKnownUnitExtentForMGather(idxInfo->shape[0]) &&
                                hasCompatibleKnownExtentForMGather(idxInfo->shape[1], dataInfo->shape[0]);
    const bool rowCoalesceRx1 = idxInfo->colMajor &&
...
std::optional<mlir::pto::Layout> mlir::pto::resolveLayoutForGlobalTensor(Operation* anchor, Value basePtr)
{
...
{
    if (pto::isPTOFloat8Type(elemTy) && (elemTy.isFloat8E4M3() || elemTy.isFloat8E4M3FN() ||
                                         elemTy.isFloat8E4M3FNUZ() || elemTy.isFloat8E4M3B11FNUZ())) {
        return "float8_e4m3_t";
    }
    if (pto::isPTOFloat8Type(elemTy) && (elemTy.isFloat8E5M2() || elemTy.isFloat8E5M2FNUZ())) {
...
static emitc::PointerType getEmitCPointerType(MLIRContext* ctx, StringRef pointeeTypeStr)
{
    return emitc::PointerType::get(emitc::OpaqueType::get(ctx, pointeeTypeStr));
}

static emitc::PointerType getEmitCPointerType(MLIRContext* ctx, StringRef qualifier, StringRef elemTypeStr)
{
...
Value mlir::pto::scalePackedTileDynamicDim(
    ConversionPatternRewriter& rewriter, Location loc, Type elemTy, pto::BLayout blayout, Value emitted, int dimIdx)
{
...
Value mlir::pto::buildTileCtorDimValue(
    ConversionPatternRewriter& rewriter, Location loc, Value emitted, int64_t fallback)
{
    if (emitted)
        return emitted;
    return makeEmitCIntConstant(rewriter, loc, emitc::OpaqueType::get(rewriter.getContext(), "int32_t"), fallback);
}
...
std::optional<std::string> mlir::pto::getEmitCTileTypeString(pto::TileBufType type)
{
    ...
    std::string vrowTok = validShape[0] == ShapedType::kDynamic ? "-1" : std::to_string(render(validShape[0], 0));
    std::string vcolTok = validShape[1] == ShapedType::kDynamic ? "-1" : std::to_string(render(validShape[1], 1));

    int32_t fractal = kFractalSize512;
    if (auto frAttr = dyn_cast<IntegerAttr>(configAttr.getSFractalSize()))
        fractal = frAttr.getInt();
    return std::string("Tile<") + getTileRoleToken(type.getMemorySpace()) + ", " + getEmitCScalarTypeToken(elemTy) +
           ", " + std::to_string(render(rows, 0)) + ", " + std::to_string(render(cols, 1)) + ", " +
           getTileBufBLayoutToken(configAttr) + ", " + vrowTok + ", " + vcolTok + ", " +
           getTileBufSLayoutToken(configAttr) + ", " + std::to_string(fractal) + ", " + getTileBufPadToken(configAttr) +
...
private:
    ...
    {
        addConversion([Ctx](pto::HiF8Type) -> Type { return emitc::OpaqueType::get(Ctx, "hifloat8_t"); });
        addConversion([Ctx](pto::F4E1M2x2Type) -> Type { return emitc::OpaqueType::get(Ctx, "float4_e1m2x2_t"); });
        addConversion([Ctx](pto::F4E2M1x2Type) -> Type { return emitc::OpaqueType::get(Ctx, "float4_e2m1x2_t"); });
    ...
    void addPointerAndArrayConversions(MLIRContext* Ctx)
    {
        addConversion([this, Ctx](pto::PtrType type) -> std::optional<Type> {
            ...
            if (!opq) {
                llvm::errs() << "  [Error] PtrType elem type is not OpaqueType: " << newElemType << "\n";
                return std::nullopt;
            }
        ...
        addConversion([Ctx](pto::EventIdArrayType type) -> Type {
            return emitc::OpaqueType::get(Ctx, "PTOAS_EventIdArray<" + std::to_string(type.getSize()) + ">");
        });
    ...
    void addAsyncAndViewConversions(MLIRContext* Ctx)
    {
        ...
        addConversion([Ctx](pto::TensorViewType type) -> Type {
            return getGlobalTensorOpaqueTypeFromShape(Ctx, type.getElementType(), type.getShape());
        });
    ...
    void addMemRefAndFunctionConversions(MLIRContext* Ctx)
    {
        ...
        addConversion([this](FunctionType type) -> Type {
            ...
            if (failed(convertTypes(type.getInputs(), inputs)) || failed(convertTypes(type.getResults(), results))) {
                return Type{};
            }
    ...
    {
        auto materializeCast = [](OpBuilder& Builder, Type ResultType, ValueRange Inputs, Location Loc) -> Value {
            if (Inputs.size() != 1)
                return Value();
...
[[maybe_unused]] static constexpr unsigned kPTOIndexBitWidth = 32; // keep consistent with IndexType conversion

bool mlir::pto::isSetFFTsPointerLikeType(Type ty) { return isEmitCPointerLikeType(ty); }

bool mlir::pto::tileDataReturnsIntegralAddress(pto::AddressSpace as) { return as == pto::AddressSpace::BIAS; }

static Type getTileDataResultType(MLIRContext* ctx, pto::AddressSpace as, StringRef elemTok)
{
...
Value mlir::pto::materializeTileDataValue(
    ConversionPatternRewriter& rewriter, Location loc, Value tile, pto::AddressSpace as, StringRef elemTypeToken)
{
    auto rawTy = getTileDataResultType(rewriter.getContext(), as, elemTypeToken);
    return rewriter
        .create<emitc::CallOpaqueOp>(loc, rawTy, "PTOAS__TILE_DATA", ArrayAttr{}, ArrayAttr{}, ValueRange{tile})
        .getResult(0);
}

Value mlir::pto::materializeAddressAsPointer(
    ConversionPatternRewriter& rewriter, Location loc, Value addr, pto::AddressSpace as, StringRef elemTok)
{
    auto* ctx = rewriter.getContext();
    std::string ptrTyStr = std::string(addrSpaceQualifier(as)) + " " + elemTok.str() + "*";
    ...
    auto castTyAttr = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, ptrTyStr)});
    return rewriter
        .create<emitc::CallOpaqueOp>(loc, ptrTy, "reinterpret_cast", ArrayAttr{}, castTyAttr, ValueRange{addr})
        .getResult(0);
}
...
//===----------------------------------------------------------------------===//
Value mlir::pto::makeEmitCOpaqueConstant(
    ConversionPatternRewriter& rewriter, Location loc, Type type, llvm::StringRef literal)
{
    auto attr = emitc::OpaqueAttr::get(rewriter.getContext(), literal);
    return rewriter.create<emitc::ConstantOp>(loc, type, attr);
}

Value mlir::pto::makeEmitCIntConstant(ConversionPatternRewriter& rewriter, Location loc, Type type, int64_t value)
{
    return makeEmitCOpaqueConstant(rewriter, loc, type, std::to_string(value));
}
Value mlir::pto::emitCCast(ConversionPatternRewriter& rewriter, Location loc, Type dstType, Value src)
{
...
// unsigned type.
Value mlir::pto::castSignlessIntToUnsignedSameWidth(
    ConversionPatternRewriter& rewriter, Location loc, Value v, unsigned bitWidth)
{
...
struct PTOMGatherToMGATHER : public OpConversionPattern<pto::MGatherOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::MGatherOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto* ctx = rewriter.getContext();
        ...
        Value memArg =
            maybeWrapGlobalMemrefAsGlobalTensor(rewriter, op.getLoc(), mem, op.getMem().getType(), op.getOperation());
        ...
        const bool rowCoalesce = isRowCoalescedMGatherIndexType(op.getDst().getType(), op.getIdx().getType());
        templateArgVec.push_back(
            emitc::OpaqueAttr::get(ctx, rowCoalesce ? "pto::Coalesce::Row" : "pto::Coalesce::Elem"));
        if (op.getGatherOob() != pto::GatherOOB::Undefined) {
            templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, gatherOobTok(op.getGatherOob())));
        }
        ArrayAttr templateArgs = rewriter.getArrayAttr(templateArgVec);

        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "MGATHER", ArrayAttr{}, templateArgs, ValueRange{dst, memArg, idx});
...
{
    auto kernelKindAttr = funcOp->getAttrOfType<FunctionKernelKindAttr>(FunctionKernelKindAttr::name);
    if (!kernelKindAttr)
        return std::nullopt;
...
struct FuncToEmitC : public OpConversionPattern<func::FuncOp> {
    ...
    static void copyFunctionAttrs(func::FuncOp op, emitc::FuncOp emitcFunc)
    {
        for (const auto& namedAttr : op->getAttrs()) {
            ...
            if (name == op.getFunctionTypeAttrName() || name == SymbolTable::getSymbolAttrName() ||
                name == pto::kPTOEntryAttrName || name == pto::kLegacyHACCEntryAttrName ||
    ...
    static void setFunctionSpecifiers(func::FuncOp op, emitc::FuncOp emitcFunc, ConversionPatternRewriter& rewriter)
    {
        if (pto::isPTOEntryFunction(op)) {
            emitcFunc.setSpecifiersAttr(rewriter.getStrArrayAttr({"__global__ AICORE"}));
    ...
    static LogicalResult inlineAndConvertFuncBody(
        func::FuncOp op, emitc::FuncOp emitcFunc, FunctionType funcType, const TypeConverter& typeConverter,
        ConversionPatternRewriter& rewriter)
    {
        ...
        return rewriter.convertRegionTypes(&emitcFunc.getBody(), typeConverter, &entryConv);
    }

    static void emitFunctionPrologue(
        func::FuncOp op, emitc::FuncOp emitcFunc, std::optional<StringRef> kernelKindMacro, bool needsNoSplitGuard,
        ConversionPatternRewriter& rewriter)
    {
        ...
        if (needsNoSplitGuard)
            rewriter.create<emitc::VerbatimOp>(op.getLoc(), "if (get_subblockid() == 0) {");
    }

    static void emitFunctionEpilogue(
        func::FuncOp op, emitc::FuncOp emitcFunc, std::optional<StringRef> kernelKindMacro, bool needsNoSplitGuard,
        ConversionPatternRewriter& rewriter)
    {
        if (!kernelKindMacro)
    ...
    LogicalResult matchAndRewrite(
        func::FuncOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        auto emitcFunc = rewriter.create<emitc::FuncOp>(op.getLoc(), op.getName(), funcType);
        ...
        if (failed(inlineAndConvertFuncBody(op, emitcFunc, funcType, *getTypeConverter(), rewriter))) {
            return failure();
        }
        emitFunctionPrologue(op, emitcFunc, kernelKindMacro, needsNoSplitGuard, rewriter);
        emitFunctionEpilogue(op, emitcFunc, kernelKindMacro, needsNoSplitGuard, rewriter);
...
std::string mlir::pto::getElemTypeStringForGT(Type elemTy) { return getEmitCScalarTypeToken(elemTy); }

static bool hasStaticShape(MemRefType mrTy)
{
    return llvm::none_of(mrTy.getShape(), [](int64_t dim) { return dim == ShapedType::kDynamic; });
}
...
static bool getStaticMemrefLayout(MemRefType mrTy, SmallVectorImpl<int64_t>& strides, int64_t& offset)
{
    ...
    return offset != ShapedType::kDynamic &&
           llvm::none_of(strides, [](int64_t strideValue) { return strideValue == ShapedType::kDynamic; });
}

Value mlir::pto::applyStaticMemrefOffset(
    ConversionPatternRewriter& rewriter, Location loc, Value basePtr, int64_t offset)
{
    if (offset == 0)
        return basePtr;
    auto* ctx = rewriter.getContext();
    Type u32Ty = emitc::OpaqueType::get(ctx, "unsigned");
    auto offVal = rewriter.create<emitc::ConstantOp>(loc, u32Ty, emitc::OpaqueAttr::get(ctx, std::to_string(offset)));
    return rewriter.create<emitc::AddOp>(loc, basePtr.getType(), basePtr, offVal);
}

static int getGlobalTensorElementBytes(Type elemTy) { return static_cast<int>(getPTOStorageElemByteSize(elemTy)); }
...
void mlir::pto::buildGlobalTensorShapeAndStride(
    ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, SmallVectorImpl<int64_t>& shape5D,
    SmallVectorImpl<int64_t>& stride5D)
{
    ...
    for (int i = 0; i < rank && i < static_cast<int>(kPTOPaddedTensorRank5D); ++i) {
...
static std::string getGlobalTensorTypeStringFromShape(Type elemTy, ArrayRef<int64_t> shape, StringRef layoutEnum)
{
    SmallVector<int64_t> strides = buildRowMajorStrides(shape);
    return getGlobalTensorTypeStringFromShapeAndStrides(elemTy, shape, strides, layoutEnum);
}

std::string mlir::pto::getGlobalTensorTypeStringFromShapeAndStrides(
    Type elemTy, ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, StringRef layoutEnum)
{
    ...
    std::string strideType = "pto::Stride<" + joinIntTemplateParams(stride5D) + ">";
    return "GlobalTensor<" + elemTypeStr + ", " + shapeType + ", " + strideType + ", " + layoutEnum.str() + ">";
}

static emitc::OpaqueType getGlobalTensorOpaqueTypeFromShape(
    MLIRContext* ctx, Type elemTy, ArrayRef<int64_t> shape, StringRef layoutEnum)
{
    return emitc::OpaqueType::get(ctx, getGlobalTensorTypeStringFromShape(elemTy, shape, layoutEnum));
}

static std::string inferFallbackGlobalTensorLayout(ArrayRef<int64_t> shape5D, ArrayRef<int64_t> stride5D, Type elemTy)
{
    int elemBytes = getGlobalTensorElementBytes(elemTy);
    if (elemBytes == 0)
        return "pto::Layout::ND";
    if (shape5D[kPaddedShapeInnerRowDim2] == kFractalSize16 &&
        multiplyOrDynamic(shape5D[kPaddedShapeInnerRowDim2], shape5D[kPaddedShapeInnerColDim3]) * elemBytes ==
            kFractalSize512 &&
        stride5D[kPaddedShapeInnermostDim4] == kPaddedShapeUnitStride1 &&
        stride5D[kPaddedShapeInnerColDim3] == shape5D[kPaddedShapeInnermostDim4]) {
        return "pto::Layout::NZ";
    }
...
static std::string resolveGlobalTensorLayout(
    Operation* anchor, Value basePtr, ArrayRef<int64_t> shape5D, ArrayRef<int64_t> stride5D, Type elemTy)
{
...
{
    std::string suffix = "_" + std::to_string(static_cast<size_t>(llvm::hash_value(static_cast<const void*>(anchor))));
    return {
        "GTShape" + suffix,
        "GTStride" + suffix,
        "GT" + suffix,
...
static void emitGlobalTensorTypeAliases(
    ConversionPatternRewriter& rewriter, Location loc, const GlobalTensorTypeNames& names, ArrayRef<int64_t> shape5D,
    ArrayRef<int64_t> stride5D, StringRef elemTypeStr, StringRef layoutEnum)
{
    rewriter.create<emitc::VerbatimOp>(
        loc, "using " + names.shapeTypeName + " = pto::Shape<" + joinIntTemplateParams(shape5D) + ">;");
    rewriter.create<emitc::VerbatimOp>(
        loc, "using " + names.strideTypeName + " = pto::Stride<" + joinIntTemplateParams(stride5D) + ">;");
    rewriter.create<emitc::VerbatimOp>(
        loc, "constexpr pto::Layout " + names.layoutConstName + " = " + layoutEnum.str() + ";");
    rewriter.create<emitc::VerbatimOp>(
        loc, "using " + names.tensorTypeName + " = GlobalTensor<" + elemTypeStr.str() + ", " + names.shapeTypeName +
                 ", " + names.strideTypeName + ", " + names.layoutConstName + ">;");
}

static SmallVector<Value> buildGlobalTensorCtorArgs(
    ConversionPatternRewriter& rewriter, Location loc, const GlobalTensorTypeNames& names, Value ptr)
{
    auto* ctx = rewriter.getContext();
    ...
    Value shapeValue = rewriter
                           .create<emitc::CallOpaqueOp>(
                               loc, shapeTypeOpaque, names.shapeTypeName, ArrayAttr{}, ArrayAttr{}, ValueRange{})
                           .getResult(0);
    Value strideValue = rewriter
                            .create<emitc::CallOpaqueOp>(
                                loc, strideTypeOpaque, names.strideTypeName, ArrayAttr{}, ArrayAttr{}, ValueRange{})
                            .getResult(0);
    return {ptr, shapeValue, strideValue};
}

Value mlir::pto::buildGlobalTensorFromMemref(
    ConversionPatternRewriter& rewriter, Location loc, Value basePtr, MemRefType mrTy, Operation* anchor)
{
    auto* ctx = rewriter.getContext();
    ...
    std::string layoutEnum = resolveGlobalTensorLayout(anchor, basePtr, shape5D, stride5D, mrTy.getElementType());
    emitGlobalTensorTypeAliases(rewriter, loc, names, shape5D, stride5D, elemTypeStr, layoutEnum);
    ...
    auto gtInst = rewriter.create<emitc::CallOpaqueOp>(
        loc, gtType, names.tensorTypeName, ArrayAttr{}, ArrayAttr{}, ValueRange(gtArgs));
    ...
    ConversionPatternRewriter& rewriter, Location loc, Value loweredValue, Type originalType, Operation* anchor)
{
    ...
    bool isGlobal = true;
    if (auto asAttr = dyn_cast_or_null<pto::AddressSpaceAttr>(mrTy.getMemorySpace())) {
        auto as = asAttr.getAddressSpace();
        isGlobal = (as == pto::AddressSpace::GM || as == pto::AddressSpace::Zero);
    }
    if (!isGlobal)
        return loweredValue;
    if (Value gt = buildGlobalTensorFromMemref(rewriter, loc, loweredValue, mrTy, anchor))
        return gt;
    return loweredValue;
}

Value mlir::pto::castToGMBytePointer(ConversionPatternRewriter& rewriter, Location loc, Value value)
{
    auto* ctx = rewriter.getContext();
    auto targetTy = emitc::PointerType::get(emitc::OpaqueType::get(ctx, "__gm__ uint8_t"));
    if (value.getType() == targetTy)
        return value;

    auto castTyAttr = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, "__gm__ uint8_t*")});
    if (isSetFFTsPointerLikeType(value.getType())) {
        return rewriter
            .create<emitc::CallOpaqueOp>(loc, targetTy, "reinterpret_cast", ArrayAttr{}, castTyAttr, ValueRange{value})
            .getResult(0);
    }
...
Value mlir::pto::materializeTensorViewDataPointer(
    ConversionPatternRewriter& rewriter, Location loc, Value value, Type originalType)
{
    ...
    auto ptrTy = emitc::PointerType::get(emitc::OpaqueType::get(ctx, "__gm__ " + elemTypeStr));
    return rewriter
        .create<emitc::CallOpaqueOp>(
            loc, ptrTy, "PTOAS__GLOBAL_TENSOR_DATA", ArrayAttr{}, ArrayAttr{}, ValueRange{value})
        .getResult(0);
}
static pto::TileBufConfigAttr getScratchTileConfigAttr(Value originalScratch, MLIRContext* ctx)
{
...
static std::string buildScratchTileTypeString(
    Type elemTy, int64_t rows, int64_t cols, pto::TileBufConfigAttr configAttr)
{
    int32_t fractal = kFractalSize512;
    if (auto frAttr = dyn_cast<IntegerAttr>(configAttr.getSFractalSize()))
        fractal = frAttr.getInt();
    ...
    return "Tile<TileType::Vec, " + elemTypeStr + ", " + std::to_string(templateRows) + ", " +
           std::to_string(templateCols) + ", " + getTileBufBLayoutToken(configAttr) + ", " +
           std::to_string(templateRows) + ", " + std::to_string(templateCols) + ", " +
           getTileBufSLayoutToken(configAttr) + ", " + std::to_string(fractal) + ", " + getTileBufPadToken(configAttr) +
           ">";
}

Value mlir::pto::castAddressToU64(ConversionPatternRewriter& rewriter, Location loc, Value value)
{
    auto* ctx = rewriter.getContext();
    auto u64Ty = emitc::OpaqueType::get(ctx, "uint64_t");
    if (isSetFFTsPointerLikeType(value.getType())) {
        auto addrTyAttr = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, "uint64_t")});
        return rewriter
            .create<emitc::CallOpaqueOp>(loc, u64Ty, "reinterpret_cast", ArrayAttr{}, addrTyAttr, ValueRange{value})
            .getResult(0);
    }
...
FailureOr<Value> mlir::pto::buildAsyncScratchTileValue(
    ConversionPatternRewriter& rewriter, Location loc, Value originalScratch, Value emittedScratch)
{
    ...
    std::string tileTypeStr = buildScratchTileTypeString(elemTy, rows, cols, configAttr);

    Value tile =
        rewriter
            .create<emitc::VariableOp>(loc, emitc::OpaqueType::get(ctx, tileTypeStr), emitc::OpaqueAttr::get(ctx, ""))
            .getResult();
    Value scratchAddr = castAddressToU64(rewriter, loc, scratch);
    rewriter.create<emitc::CallOpaqueOp>(
        loc, TypeRange{}, "TASSIGN", ArrayAttr{}, ArrayAttr{}, ValueRange{tile, scratchAddr});
    return tile;
}
...
static ArrayAttr buildScatterTemplateArgs(ConversionPatternRewriter& rewriter, MLIRContext* ctx, pto::MScatterOp op)
{
    SmallVec3<Attribute> templateArgVec;
    const bool rowCoalesce = isRowCoalescedMGatherIndexType(op.getSrc().getType(), op.getIdx().getType());
    templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, rowCoalesce ? "pto::Coalesce::Row" : "pto::Coalesce::Elem"));
    if (op.getScatterAtomicOp() != pto::ScatterAtomicOp::None || op.getScatterOob() != pto::ScatterOOB::Undefined) {
        templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, scatterAtomicTok(op.getScatterAtomicOp())));
        if (op.getScatterOob() != pto::ScatterOOB::Undefined) {
            templateArgVec.push_back(emitc::OpaqueAttr::get(ctx, scatterOobTok(op.getScatterOob())));
        }
    }
...
struct PTOMScatterToMSCATTER : public OpConversionPattern<pto::MScatterOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::MScatterOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto* ctx = rewriter.getContext();
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "MSCATTER", ArrayAttr{}, templateArgs, ValueRange{memArg, src, idx});

        rewriter.eraseOp(op);
        return success();
    }
};
static void populatePTOToEmitCPatterns(
    RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
{
...
static constexpr llvm::StringLiteral kGlobalTensorDataHelper = R"cpp(
    ...
    static AICORE inline auto PTOAS__GLOBAL_TENSOR_DATA(Tensor& tensor) -> decltype(tensor.data())
    {
        return tensor.data();
    }
)cpp";
...
static constexpr llvm::StringLiteral kTRandomHelper = R"cpp(
    ...
    static AICORE inline void PTOAS__TRANDOM(
        DstTile& dst, uint32_t key0, uint32_t key1, uint32_t counter0, uint32_t counter1, uint32_t counter2,
        uint32_t counter3)
    {
...
struct EmitPTOManualPass : public PassWrapper<EmitPTOManualPass, OperationPass<ModuleOp>> {
    ...
    {
        registry.insert<
            emitc::EmitCDialect, func::FuncDialect, arith::ArithDialect, memref::MemRefDialect, affine::AffineDialect,
    ...
    static void emitCommIncludePreamble(OpBuilder& builder, Location loc)
    {
        ...
        builder.create<emitc::IncludeOp>(loc, "pto/comm/pto_comm_inst.hpp", /*is_standard_include=*/false);
    }

    static void emitGlobalTensorDataHelper(OpBuilder& builder, Location loc)
    {
        builder.create<emitc::VerbatimOp>(loc, builder.getStringAttr(kGlobalTensorDataHelper));
    }

    static void emitEventIdArrayHelper(OpBuilder& builder, Location loc)
    {
        builder.create<emitc::VerbatimOp>(loc, builder.getStringAttr(kEventIdArrayHelper));
    }
    ...
    {
        builder.create<emitc::VerbatimOp>(loc, builder.getStringAttr(kAutoSyncTailHelper));
    }
    ...
    static ModuleHelperNeeds analyzeModuleHelperNeeds(ModuleOp mop)
    {
        ...
        mop.walk([&needs](Operation* op) {
            needs.needsEventIdArrayHelper = needs.needsEventIdArrayHelper || isa<mlir::pto::DeclareEventIdArrayOp>(op);
            needs.needsTRandomHelper = needs.needsTRandomHelper || isa<mlir::pto::TRandomOp>(op);
            needs.needsGlobalTensorDataHelper =
                needs.needsGlobalTensorDataHelper || isa<mlir::pto::PartitionViewOp>(op);
            needs.needsCommInclude =
                needs.needsCommInclude ||
                isa<mlir::pto::BuildAsyncSessionOp, mlir::pto::TPutAsyncOp, mlir::pto::TGetAsyncOp,
                    mlir::pto::TPrefetchAsyncOp, mlir::pto::WaitAsyncEventOp, mlir::pto::TestAsyncEventOp,
                    mlir::pto::TPutOp, mlir::pto::TGetOp, mlir::pto::TNotifyOp, mlir::pto::TWaitOp, mlir::pto::TTestOp,
                    mlir::pto::TBroadcastOp, mlir::pto::CommTGatherOp, mlir::pto::CommTScatterOp, mlir::pto::TReduceOp>(
                    op);
            needs.needsBitcastHelper =
                needs.needsBitcastHelper || isa<arith::BitcastOp, arith::MaximumFOp, arith::MinimumFOp>(op);
        });
        return needs;
    }

    static void insertModulePreamble(ModuleOp mop, MLIRContext* ctx, const ModuleHelperNeeds& needs)
    {
        auto loc = mop->getLoc();
        OpBuilder builder(ctx);
        builder.setInsertionPointToStart(mop.getBody());
        builder.create<emitc::IncludeOp>(
            loc, "pto/pto-inst.hpp",
    ...
    static LogicalResult runSCFTypePreconversion(ModuleOp mop, PTOToEmitCTypeConverter& typeConverter)
    {
    ...
    static ConversionTarget buildMainConversionTarget(ModuleOp mop, PTOToEmitCTypeConverter& typeConverter)
    {
        ...
        target.addDynamicallyLegalOp<cf::BranchOp, cf::CondBranchOp>([&typeConverter](Operation* op) {
    ...
    static LogicalResult lowerUnrealizedCast(
        UnrealizedConversionCastOp cast, SmallVectorImpl<UnrealizedConversionCastOp>& castsToErase)
    {
        ...
        cast.emitError() << "cannot lower unrealized_conversion_cast(" << inTy << " -> " << outTy << ") to emitc.cast";
        return failure();
    }
    ...
    static LogicalResult cleanupUnrealizedCasts(ModuleOp mop)
    {
        ...
        mop.walk([&castCleanupFailed, &castsToErase](UnrealizedConversionCastOp cast) {
            if (castCleanupFailed)
                return;
            castCleanupFailed = failed(lowerUnrealizedCast(cast, castsToErase));
        });
...
std::unique_ptr<Pass> mlir::pto::createEmitPTOManualPass() { return std::make_unique<EmitPTOManualPass>(); }
