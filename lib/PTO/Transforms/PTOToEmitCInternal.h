...
inline emitc::OpaqueType getSignedIntOpaqueType(MLIRContext* ctx, unsigned bitWidth)
{
    switch (bitWidth) {
        case 1:
...
inline emitc::OpaqueType getUnsignedIntOpaqueType(MLIRContext* ctx, unsigned bitWidth)
{
    switch (bitWidth) {
        case 1:
        ...
        default:
            llvm::errs() << "[Debug] Unsupported unsigned integer bitwidth: " << bitWidth << "\n";
            return emitc::OpaqueType::get(ctx, "uint64_t");
    }
}

inline emitc::OpaqueType getWiderSignedIntOpaqueType(MLIRContext* ctx, unsigned bitWidth)
{
    switch (bitWidth) {
        case 1:
        case kPTOI8BitWidth:
...
inline emitc::OpaqueType getWiderUnsignedIntOpaqueType(MLIRContext* ctx, unsigned bitWidth)
{
    switch (bitWidth) {
        case 1:
        case kPTOI8BitWidth:
...
inline FailureOr<std::string> buildEmitCOpaqueConstantLiteral(Type targetType, Attribute valueAttr)
{
    ...
    if (opaqueTy.getValue() == "pto::MrgSortExecutedNumList") {
        ...
        if (!vecTy || vecTy.getRank() != kMrgSortExecutedNumRank ||
            vecTy.getNumElements() != kMrgSortExecutedNumCount || !vecTy.getElementType().isInteger(kPTOI16BitWidth))
            return failure();
...
Value scalePackedTileDynamicDim(
    ConversionPatternRewriter& rewriter, Location loc, Type elemTy, pto::BLayout blayout, Value emitted, int dimIdx);

Value buildTileCtorDimValue(ConversionPatternRewriter& rewriter, Location loc, Value emitted, int64_t fallback);

std::string getTileRoleToken(Attribute memorySpace);

inline std::string getTileBufBLayoutToken(pto::TileBufConfigAttr configAttr)
{
...
inline std::string getTileBufSLayoutToken(pto::TileBufConfigAttr configAttr)
{
...
inline std::string getTileBufPadToken(pto::TileBufConfigAttr configAttr)
{
...
Value castAddressToU64(ConversionPatternRewriter& rewriter, Location loc, Value value);
...
inline pto::BLayout getTileBufBLayoutValue(pto::TileBufConfigAttr configAttr)
{
    if (auto blAttr = dyn_cast<BLayoutAttr>(configAttr.getBLayout()))
...
inline int64_t renderTileTemplateDim(int64_t rawDim, Type elemTy, pto::BLayout blayout, int dimIdx)
{
    if (!(dimIdx >= 0 && dimIdx < kTileTemplateRank2D)) {
        llvm::report_fatal_error("renderTileTemplateDim expects a rank-2 rows/cols dimension index");
    }
...
std::string getGlobalTensorTypeStringFromShapeAndStrides(
    Type elemTy, ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, llvm::StringRef layoutEnum = "pto::Layout::ND");

std::string getElemTypeStringForGT(Type elemTy);

SmallVector<int64_t> buildRowMajorStrides(ArrayRef<int64_t> shape);

void buildGlobalTensorShapeAndStride(
...
Value buildGlobalTensorFromMemref(
...
Value materializeTensorViewDataPointer(
    ConversionPatternRewriter& rewriter, Location loc, Value value, Type originalType);

Value materializeAddressAsPointer(
    ConversionPatternRewriter& rewriter, Location loc, Value addr, pto::AddressSpace as, llvm::StringRef elemTok);
...
Value materializeTileDataValue(
    ConversionPatternRewriter& rewriter, Location loc, Value tile, pto::AddressSpace as, llvm::StringRef elemTypeToken);
