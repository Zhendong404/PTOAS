...
static constexpr llvm::StringLiteral kInferredLayoutAttrName = "pto.inferred_layout";
...
static unsigned elemByteSize(Type ty) { return getPTOStorageElemByteSize(ty); }

static bool isGlobalMemRef(MemRefType ty)
{
    if (auto asAttr = dyn_cast_or_null<pto::AddressSpaceAttr>(ty.getMemorySpace())) {
...
static bool isMinor2DLayout(Layout layout) { return layout == Layout::ND || layout == Layout::DN; }

static std::optional<ShapeStride5D> rightAlignTo5D(ArrayRef<int64_t> shape, ArrayRef<int64_t> stride)
{
...
static bool matchesNDMinor2D(int64_t rows, int64_t cols, int64_t rowStride, int64_t colStride)
{
...
static bool matchesDNMinor2D(int64_t rows, int64_t cols, int64_t rowStride, int64_t colStride)
{
...
static std::optional<Layout> inferMinor2DLayout(
    int64_t rows, int64_t cols, int64_t rowStride, int64_t colStride, std::optional<Layout> preferredMinor2D,
    bool* isMinor2DAmbiguous)
{
    ...
    if (nd && dn) {
        ...
        if (preferredMinor2D && (*preferredMinor2D == Layout::ND || *preferredMinor2D == Layout::DN)) {
...
static std::optional<Layout> inferNZLayout(ArrayRef<int64_t> shape, ArrayRef<int64_t> stride, unsigned elemBytes)
{
    int64_t sh3 = shape[2];
    int64_t sh4 = shape[3];
    int64_t sh5 = shape[4];
    ...
    bool alignMatch = (sh3 == kNZInnerRows) && (sh3 * sh4 * static_cast<int64_t>(elemBytes) == kFractalSize512);
...
static std::optional<Layout> inferLayout5D(
    ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, unsigned elemBytes,
    std::optional<Layout> preferredMinor2D = std::nullopt, bool* isMinor2DAmbiguous = nullptr)
{
    ...
    return inferMinor2DLayout(rows, cols, rowStride, colStride, preferredMinor2D, isMinor2DAmbiguous);
}
...
static bool isMinorColsOne(ArrayRef<int64_t> shape) { return !shape.empty() && shape.back() == 1; }
...
static bool getStaticShapeAndStride(
    MakeTensorViewOp op, SmallVectorImpl<int64_t>& shape, SmallVectorImpl<int64_t>& strides);
...
static void verifyOrSetLayoutAttr(
    Operation* op, std::optional<Layout> inferred, SignalFailureFn signalFailure, bool isMinor2DAmbiguous = false)
{
    auto existing = op->getAttrOfType<LayoutAttr>(kLayoutAttrName);
    if (existing) {
        if (inferred && existing.getLayout() != *inferred) {
            if (isMinor2DAmbiguous && isMinor2DLayout(existing.getLayout()) && isMinor2DLayout(*inferred)) {
                return;
            }
            op->emitError() << "layout mismatch: user-specified layout=" << stringifyLayout(existing.getLayout())
...
{
    if (!mrTy.hasStaticShape() || mrTy.getRank() == 0 || mrTy.getRank() > kPTOPaddedTensorRank5D)
        return std::nullopt;
    ...
    if (offset == ShapedType::kDynamic ||
        llvm::any_of(strideInts, [](int64_t s) { return s == ShapedType::kDynamic; })) {
        return std::nullopt;
    }
    return inferLayout5D(mrTy.getShape(), strideInts, elemByteSize(mrTy.getElementType()));
}

template <typename LoadStoreOp, typename ViewGetter, typename TileGetter>
static void maybeRepairMinor2DLoadStoreLayout(LoadStoreOp op, ViewGetter getView, TileGetter getTile)
{
    auto tilePref =
        isVectorTileType(getTile(op).getType()) ? tileBLayoutToGlobalLayout(getTile(op).getType()) : std::nullopt;
    if (!tilePref || (*tilePref != Layout::ND && *tilePref != Layout::DN))
        return;

    auto viewInfo = resolveLayoutFromViewValue(getView(op));
    if (!viewInfo.owner || !viewInfo.layout || !viewInfo.inferred || *viewInfo.layout == *tilePref) {
        return;
    }
    ...
    if (!getStaticShapeAndStride(tv, shape, strides))
        return;
    (void)inferLayout5D(
        shape, strides, elemByteSize(cast<TensorViewType>(tv.getResult().getType()).getElementType()), std::nullopt,
        &ambiguous);
...
template <typename LoadStoreOp, typename ViewGetter, typename TileGetter>
static void attachLoadStoreLayout(LoadStoreOp op, ViewGetter getView, TileGetter getTile)
{
    ...
    } else if (auto memTy = dyn_cast<MemRefType>(getView(op).getType()); memTy && isGlobalMemRef(memTy)) {
        setLayoutAttr(
            op.getOperation(), inferFromStaticMemRefTy(memTy).value_or(Layout::ND),
            /*inferred=*/true);
    }
...
static std::optional<Layout> inferMakeTensorViewLayout(
    MakeTensorViewOp op, ArrayRef<int64_t> shape, ArrayRef<int64_t> strides, bool& isAmbiguous)
{
    ...
    return inferLayout5D(
        shape, strides, elemByteSize(cast<TensorViewType>(op.getResult().getType()).getElementType()),
        preferredForAmbiguous, &isAmbiguous);
}

static void reconcileAmbiguousTensorViewLayout(MakeTensorViewOp op, ArrayRef<int64_t> shape)
{
    auto pref = collectPreferredLayoutFromConsumers(op.getResult());
...
template <typename SignalFailureFn>
static bool getConstFoldResults(
    ArrayRef<OpFoldResult> values, SmallVectorImpl<int64_t>& result, SignalFailureFn signalFailure, Operation* op)
{
    result.clear();
...
template <typename SignalFailureFn>
static void inferMakeTensorViewLayoutAttr(MakeTensorViewOp op, SignalFailureFn signalFailure)
{
    ...
    verifyOrSetLayoutAttr(op.getOperation(), inferred, signalFailure, isAmbiguous);
    if (isAmbiguous)
        reconcileAmbiguousTensorViewLayout(op, shape);
}

template <typename SignalFailureFn>
static void inferReinterpretCastLayoutAttr(memref::ReinterpretCastOp op, SignalFailureFn signalFailure)
{
    ...
    SmallVector<int64_t> shape;
    SmallVector<int64_t> strides;
    if (!getConstFoldResults(op.getMixedSizes(), shape, signalFailure, op.getOperation()) ||
    ...
    auto inferred =
        inferLayout5D(shape, strides, elemByteSize(mrTy.getElementType()), std::nullopt, &isMinor2DAmbiguous);
    verifyOrSetLayoutAttr(op.getOperation(), inferred, signalFailure, isMinor2DAmbiguous);
}
...
static void inferSubviewLayoutAttr(memref::SubViewOp op)
{
    ...
    SmallVector<int64_t> strideInts;
    int64_t offset = ShapedType::kDynamic;
    if (failed(getStridesAndOffset(srcTy, strideInts, offset)) || offset == ShapedType::kDynamic ||
        llvm::any_of(strideInts, [](int64_t s) { return s == ShapedType::kDynamic; })) {
        setLayoutAttr(op.getOperation(), Layout::ND, /*inferred=*/true);
        return;
    }

    auto inferred = inferLayout5D(srcTy.getShape(), strideInts, elemByteSize(srcTy.getElementType()));
    setLayoutAttr(
        op.getOperation(), inferred.value_or(Layout::ND),
        /*inferred=*/true);
}

template <typename SignalFailureFn>
static void inferMakeTensorLayouts(func::FuncOp func, SignalFailureFn signalFailure)
{
    func.walk([&signalFailure](MakeTensorViewOp op) { inferMakeTensorViewLayoutAttr(op, signalFailure); });
    func.walk([&signalFailure](memref::ReinterpretCastOp op) { inferReinterpretCastLayoutAttr(op, signalFailure); });
}
...
static void attachLoadStoreLayouts(func::FuncOp func)
{
    func.walk([](pto::TLoadOp op) {
        attachLoadStoreLayout(op, [](auto load) { return load.getSrc(); }, [](auto load) { return load.getDst(); });
    });
    func.walk([](pto::TStoreOp op) {
        attachLoadStoreLayout(op, [](auto store) { return store.getDst(); }, [](auto store) { return store.getSrc(); });
    });
}

struct InferPTOLayoutPass : public mlir::pto::impl::InferPTOLayoutBase<InferPTOLayoutPass> {
...
std::unique_ptr<Pass> mlir::pto::createInferPTOLayoutPass() { return std::make_unique<InferPTOLayoutPass>(); }
