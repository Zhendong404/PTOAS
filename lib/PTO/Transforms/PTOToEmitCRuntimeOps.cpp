...
static FailureOr<std::string> getTPipeDirectionToken(bool isL2G2L, int8_t dirMask, PTOArch targetArch)
{
...
static std::string buildTPipeToken(
    int32_t flagBase, llvm::StringRef dirTok, int32_t slotSize, int32_t slotNum, int32_t localSlotNum, bool nosplit)
{
    std::string token = "TPipe<" + std::to_string(flagBase) + ", " + dirTok.str() + ", " + std::to_string(slotSize) +
...
FailureOr<std::string> buildTPipeTokenFromInitOp(Operation* op, PTOArch targetArch)
{
    if (auto initOp = dyn_cast<pto::InitializeL2G2LPipeOp>(op)) {
        if (!initOp.getFlagBaseAttr())
            return failure();
        auto dirTok = getTPipeDirectionToken(/*isL2G2L=*/true, initOp.getDirMask(), targetArch);
        if (failed(dirTok))
            return failure();
        int32_t localSlotNum =
            initOp.getLocalSlotNumAttr() ? initOp.getLocalSlotNumAttr().getInt() : initOp.getSlotNum();
        return buildTPipeToken(
            initOp.getFlagBaseAttr().getInt(), *dirTok, initOp.getSlotSize(), initOp.getSlotNum(), localSlotNum,
    ...
    if (auto initOp = dyn_cast<pto::InitializeL2LPipeOp>(op)) {
        if (!initOp.getFlagBaseAttr())
            return failure();
        auto dirTok = getTPipeDirectionToken(/*isL2G2L=*/false, initOp.getDirMask(), targetArch);
        if (failed(dirTok))
            return failure();
        return buildTPipeToken(
            initOp.getFlagBaseAttr().getInt(), *dirTok, initOp.getSlotSize(), initOp.getSlotNum(),
            kPTOFrontendLocalSlotNum, initOp.getNosplitAttr() && initOp.getNosplitAttr().getValue());
    }
...
static FailureOr<std::string> getTPipeTokenFromValue(Value pipeHandle, PTOArch targetArch)
{
...
struct PTOTAllocToEmitC : public OpConversionPattern<mlir::pto::TAllocOp> {
    PTOTAllocToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::TAllocOp>(typeConverter, ctx), targetArch(targetArch)
    {}

    LogicalResult matchAndRewrite(
        mlir::pto::TAllocOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        std::string callee = "TALLOC<" + *pipeTok + ", " + *entryTok + ", " + *splitTok + ">";
...
struct PTOTPushToEmitC : public OpConversionPattern<mlir::pto::TPushOp> {
    PTOTPushToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::TPushOp>(typeConverter, ctx), targetArch(targetArch)
    {}
        ...
        PipeTileOp op, AdaptorT adaptor, PTOArch targetArch, StringRef calleeBase)
    {
        ...
        return PipeTileRuntimeCall{
            (Twine(calleeBase) + "<" + *pipeTok + ", " + *tileTok + ", " + *splitTok + ">").str(),
            peelUnrealized(adaptor.getPipeHandle()), convertedTile};
    }

    static LogicalResult lowerPipeTileRuntimeCall(
        Operation* op, const PipeTileRuntimeCall& call, ConversionPatternRewriter& rewriter)
    {
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
            op, TypeRange{}, call.callee, ArrayAttr{}, ArrayAttr{}, ValueRange{call.pipeHandle, call.tile});
        return success();
    }

    LogicalResult matchAndRewrite(
        mlir::pto::TPushOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct PTOTPopToEmitC : public OpConversionPattern<mlir::pto::TPopOp> {
    PTOTPopToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::TPopOp>(typeConverter, ctx), targetArch(targetArch)
    {}

    LogicalResult matchAndRewrite(
        mlir::pto::TPopOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto call = PTOTPushToEmitC::buildPipeTileRuntimeCall(op, adaptor, targetArch, "TPOP");
        if (failed(call))
            return rewriter.notifyMatchFailure(op, "failed to resolve pipe token");
...
struct PTOTFreeToEmitC : public OpConversionPattern<mlir::pto::TFreeOp> {
    PTOTFreeToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::TFreeOp>(typeConverter, ctx), targetArch(targetArch)
    {}

    LogicalResult matchAndRewrite(
        mlir::pto::TFreeOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto pipeTok = getTPipeTokenFromValue(op.getPipeHandle(), targetArch);
        if (failed(pipeTok))
            return rewriter.notifyMatchFailure(op, "failed to resolve pipe token");
        ...
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(op, TypeRange{}, callee, ArrayAttr{}, ArrayAttr{}, operands);
        return success();
    }
...
static std::string buildReinterpretCastTileTypeString(MemRefType resMrTy, Type elemTy, pto::AddressSpace as)
{
    ...
    int64_t templateRows = renderTileTemplateDim(rows, elemTy, pto::BLayout::RowMajor, 0);
    int64_t templateCols = renderTileTemplateDim(cols, elemTy, pto::BLayout::RowMajor, 1);

    return std::string("Tile<") + getReinterpretCastTileRoleToken(as) + ", " + elemTok + ", " +
           std::to_string(templateRows) + ", " + std::to_string(templateCols) + ", BLayout::RowMajor, " +
           std::to_string(templateRows) + ", " + std::to_string(templateCols) +
           ", SLayout::NoneBox, 512, PadValue::Null, CompactMode::Null>";
}

static Value buildReinterpretCastBaseAddr(
    ConversionPatternRewriter& rewriter, Location loc, Value source, pto::AddressSpace as, StringRef elemTok)
{
    Value rawPtr = source;
...
static LogicalResult rewriteGmReinterpretCast(
    memref::ReinterpretCastOp op, Value source, Value offsetVal, bool emitAddPtrTrace,
    ConversionPatternRewriter& rewriter, const TypeConverter* typeConverter)
{
    ...
    auto addOp = rewriter.create<emitc::AddOp>(op.getLoc(), resultType, source, offsetVal);
    if (emitAddPtrTrace) {
        rewriter.setInsertionPointAfter(addOp);
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "PTOAS__ADDPTR_TRACE", ArrayAttr{}, ArrayAttr{},
...
struct ReinterpretCastToEmitC : public OpConversionPattern<memref::ReinterpretCastOp> {
        ...
        memref::ReinterpretCastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (isGm)
            return rewriteGmReinterpretCast(op, source, offsetVal, emitAddPtrTrace, rewriter, getTypeConverter());
        ...
        std::string tileTypeStr = buildReinterpretCastTileTypeString(resMrTy, elemTy, as);

        auto tileType = emitc::OpaqueType::get(ctx, tileTypeStr);
        Value tile = rewriter.create<emitc::VariableOp>(loc, tileType, emitc::OpaqueAttr::get(ctx, "")).getResult();

        auto u64Ty = emitc::OpaqueType::get(ctx, "uint64_t");
        Value baseAddr = buildReinterpretCastBaseAddr(rewriter, loc, source, as, elemTok);
        Value addr = baseAddr;
        if (offsetVal) {
...
struct PTOTAddCToTADDC : public OpConversionPattern<pto::TAddCOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TAddCOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "TADD", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, src0, src1});
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "TADD", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, dst, src2});
...
struct PTOAddSToTADDS : public OpConversionPattern<pto::TAddSOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TAddSOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TADDS", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, src, scalar});
...
struct PTOAddSCToTADDSC : public OpConversionPattern<pto::TAddSCOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TAddSCOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        Value src0 = peelUnrealized(adaptor.getSrc0());
        Value scalar = peelUnrealized(adaptor.getScalar());
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "TADDS", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, src0, scalar});
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "TADD", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, dst, src1});
...
struct PTOPrintOpToEmitC : public OpConversionPattern<pto::PrintOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::PrintOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto* ctx = rewriter.getContext();
        ...
        Value scalar = peelUnrealized(adaptor.getScalar());
        auto argsAttr =
            rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, quoted), IntegerAttr::get(IndexType::get(ctx), 0)});
        rewriter.create<emitc::CallOpaqueOp>(
...
struct PTOTrapOpToEmitC : public OpConversionPattern<pto::TrapOp> {
    ...
    LogicalResult matchAndRewrite(pto::TrapOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "trap",
...
public:
    ...
    LogicalResult matchAndRewrite(
        arith::CmpIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        ...
        const bool isUnsignedPred =
            op.getPredicate() == arith::CmpIPredicate::ult || op.getPredicate() == arith::CmpIPredicate::ule ||
            op.getPredicate() == arith::CmpIPredicate::ugt || op.getPredicate() == arith::CmpIPredicate::uge;
...
bool needsA5NoSplitVectorGuard(Operation* op)
{
    ...
    if (auto func = dyn_cast<func::FuncOp>(op)) {
        if (auto kernelKindAttr = func->getAttrOfType<FunctionKernelKindAttr>(FunctionKernelKindAttr::name)) {
            isVectorScope = kernelKindAttr.getKernelKind() == FunctionKernelKind::Vector;
        }
    }
    if (!isVectorScope)
        return false;
...
void populatePTOToEmitCRuntimeOpPatterns(
