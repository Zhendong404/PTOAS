...
namespace {
static std::optional<memref::AllocOp> requireRootAlloc(Operation* op, Value value, StringRef valueName)
{
    auto alloc = tracebackMemRefToAlloc(value);
    if (!alloc.has_value())
        emitError(op->getLoc()) << "Cannot find root memref.alloc for " << valueName << " of this op.";
    return alloc;
}

static LogicalResult propagateAllocScope(
    Operation* op, Value value, StringRef valueName, const AddressSpaceAttr& targetScope,
    MemScopeInferAndPropagateHelper& helper)
{
    ...
    if (failed(helper.Run(*alloc, targetScope)))
        return op->emitOpError() << "Failed to infer/propagate memory scope for " << valueName;
    return success();
}

static bool hasMemRefResults(Operation* op)
{
    return llvm::any_of(op->getResults(), [](OpResult result) { return isa<MemRefType>(result.getType()); });
}

static LogicalResult propagateYieldScope(
    MemScopeInferAndPropagateHelper& helper, Value val, const AddressSpaceAttr& memrefScope, OpOperand& user)
{
    ...
    if (!yieldType || !valType || yieldType.getElementType() != valType.getElementType()) {
        return success();
    }
    return helper.Run(parentResult, memrefScope);
}

static LogicalResult propagateForScope(
    MemScopeInferAndPropagateHelper& helper, const AddressSpaceAttr& memrefScope, OpOperand& user)
{
    auto op = cast<scf::ForOp>(user.getOwner());
    auto result = op.getTiedLoopResult(&user);
    auto bbArg = op.getTiedLoopRegionIterArg(&user);
    return success(helper.Run(bbArg, memrefScope).succeeded() && helper.Run(result, memrefScope).succeeded());
}

static LogicalResult propagateViewLikeScope(
    MemScopeInferAndPropagateHelper& helper, const AddressSpaceAttr& memrefScope, Operation* op)
{
    auto result = op->getResult(0);
    return helper.Run(result, memrefScope);
}

static LogicalResult propagateMemScopeToUser(
    MemScopeInferAndPropagateHelper& helper, Value val, const AddressSpaceAttr& memrefScope, OpOperand& user)
{
    Operation* owner = user.getOwner();
    return TypeSwitch<Operation*, LogicalResult>(owner)
        .Case<scf::YieldOp>([&](scf::YieldOp) { return propagateYieldScope(helper, val, memrefScope, user); })
        .Case<scf::ForOp>([&](scf::ForOp) { return propagateForScope(helper, memrefScope, user); })
        .Case<
            memref::SubViewOp, memref::ViewOp, memref::ReinterpretCastOp, memref::CastOp, memref::CollapseShapeOp,
            memref::ExpandShapeOp, memref::ReshapeOp, memref::TransposeOp, memref::ExtractStridedMetadataOp>(
            [&](auto op) { return propagateViewLikeScope(helper, memrefScope, op); })
...
LogicalResult MemScopeInferAndPropagateHelper::propagateMemScopeToUsers(Value val)
{
...
LogicalResult MemScopeInferAndPropagateHelper::Run(Value operand, const AddressSpaceAttr& targetMemScope)
{
...
namespace {
struct InferPTOMemScopePass : public impl::InferPTOMemScopeBase<InferPTOMemScopePass> {
...
static AddressSpaceAttr getMemScopeAttr(MLIRContext* ctx, pto::AddressSpace scope)
{
    return AddressSpaceAttr::get(ctx, scope);
}

static LogicalResult propagateOperandScopes(
    Operation* op, ArrayRef<std::tuple<Value, StringRef, AddressSpaceAttr>> specs)
{
    MemScopeInferAndPropagateHelper helper;
...
LogicalResult pto::inferAndPropagateMemScopeForMovDps(pto::TMovOp op)
{
    ...
    auto l0aSpaceAttr = getMemScopeAttr(op->getContext(), pto::AddressSpace::LEFT);
    auto l0bSpaceAttr = getMemScopeAttr(op->getContext(), pto::AddressSpace::RIGHT);
    auto l0cSpaceAttr = getMemScopeAttr(op->getContext(), pto::AddressSpace::ACC);
    ...
    auto biasSpaceAttr = getMemScopeAttr(op->getContext(), pto::AddressSpace::BIAS);
    ...
    if (memSpace == l0aSpaceAttr || memSpace == l0bSpaceAttr || memSpace == biasSpaceAttr) {
...
LogicalResult pto::inferAndPropagateMemScopeForMatmulAccDps(pto::TMatmulAccOp op)
{
    ...
    return propagateOperandScopes(
        op, {{op.getAccIn(), "mAcc", getMemScopeAttr(op->getContext(), pto::AddressSpace::ACC)},
             {op.getLhs(), "mA", getMemScopeAttr(op->getContext(), pto::AddressSpace::LEFT)},
             {op.getRhs(), "mB", getMemScopeAttr(op->getContext(), pto::AddressSpace::RIGHT)},
             {op.getDst(), "mC", getMemScopeAttr(op->getContext(), pto::AddressSpace::ACC)}});
}
...
LogicalResult pto::inferAndPropagateMemScopeForMatmulBiasDps(pto::TMatmulBiasOp op)
{
    ...
    return propagateOperandScopes(
        op, {{op.getA(), "mA", getMemScopeAttr(op->getContext(), pto::AddressSpace::LEFT)},
             {op.getB(), "mB", getMemScopeAttr(op->getContext(), pto::AddressSpace::RIGHT)},
             {op.getDst(), "mC", getMemScopeAttr(op->getContext(), pto::AddressSpace::ACC)},
             {op.getBias(), "mD", getMemScopeAttr(op->getContext(), pto::AddressSpace::BIAS)}});
}
...
LogicalResult pto::inferAndPropagateMemScopeForMatmulDps(pto::TMatmulOp op)
{
    ...
    return propagateOperandScopes(
        op, {{op.getLhs(), "mA", getMemScopeAttr(op->getContext(), pto::AddressSpace::LEFT)},
             {op.getRhs(), "mB", getMemScopeAttr(op->getContext(), pto::AddressSpace::RIGHT)},
             {op.getDst(), "mC", getMemScopeAttr(op->getContext(), pto::AddressSpace::ACC)}});
}
...
LogicalResult InferPTOMemScopePass::fixDeviceCallSite(func::FuncOp op)
{
    ...
    for (SymbolTable::SymbolUse use : uses) {
        ...
        for (auto [idx, callOperand] : llvm::enumerate(call.getArgOperands())) {
            ...
            if (failed(helper.Run(tracebackMemRef(callOperand), getPTOAddressSpaceAttr(funcOperandType)))) {
                return op->emitOpError() << "Failed to propagate memory scope for operand " << callOperand;
            }
        ...
        for (auto [idx, returnValue] : llvm::enumerate(call->getResults())) {
            ...
            if (failed(helper.Run(returnValue, getPTOAddressSpaceAttr(funcReturnType)))) {
                return op->emitOpError() << "Failed to propagate memory scope for result " << returnValue;
            }
        }
    }
    return success();
...
[[maybe_unused]] LogicalResult InferPTOMemScopePass::fixHostFuncSignature(func::FuncOp op) const
{
    ...
    SmallVector<Type> newArgsType(
        llvm::map_to_vector(op.getArguments(), [](const BlockArgument& ba) { return ba.getType(); }));
    SmallVector<Type> newReturnType(
        llvm::map_to_vector(returnOp.getOperandTypes(), [](const Type& type) { return type; }));
...
static LogicalResult inferAndPropagateMemScopeForExternFunc(func::FuncOp op)
{
    ...
    auto gmSpaceAttr = AddressSpaceAttr::get(op->getContext(), pto::AddressSpace::GM);
    LDBG("Begin infer and propagate memory scope for extern func" << op.getSymName());
    ...
    for (auto arg : args) {
        ...
        if (op->hasAttr(pto::VectorFunctionAttr::name)) {
            if (failed(helper.Run(arg, ubSpaceAttr)))
                return op->emitOpError() << "Failed to propagate UB memory scope for argument # in VF"
                                         << arg.getArgNumber();
        } else if (failed(helper.Run(arg, gmSpaceAttr))) {
            return op->emitOpError() << "Failed to propagate memory scope for argument #" << arg.getArgNumber();
        }
    }
    if (!args.empty()) {
        auto newFt = op.getFunctionType().clone(op.getBody().front().getArgumentTypes(), op.getResultTypes());
        op.setFunctionType(newFt);
    }
    if (op->getNumResults() > 0)
        op.emitWarning() << "non-externl function has return value after bufferization!";
...
LogicalResult pto::inferAndPropagateUbufMemScope(memref::AllocOp allocOp)
{
    ...
    MemScopeInferAndPropagateHelper helper;
    auto ubSpaceAttr = AddressSpaceAttr::get(allocOp->getContext(), pto::AddressSpace::VEC);
    if (failed(helper.Run(allocOp, ubSpaceAttr))) {
        return allocOp->emitOpError("Failed to propagate memory scope ub for allocOp");
    }
    return success();
}
...
void InferPTOMemScopePass::runOnOperation()
{
    ...
    getOperation()->walk([&](gpu::GPUModuleOp gpuModule) {
        gpuModule->walk([&](gpu::GPUFuncOp gpuFunc) -> void { gpuFuncList.push_back(gpuFunc); });
    });
...
std::unique_ptr<Pass> mlir::pto::createInferPTOMemScopePass() { return std::make_unique<InferPTOMemScopePass>(); }
