...
static SignedDivAdjustOperands buildSignedDivAdjustOperands(
    ConversionPatternRewriter& rewriter, Location loc, Type dstTy, Value lhs, Value rhs, bool expectSameSigns)
{
    ...
    Value rNeZero = rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::ne, r, zero);
    Value lhsLt0 = rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, lhs, zero);
    Value rhsLt0 = rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, rhs, zero);
    auto pred = expectSameSigns ? emitc::CmpPredicate::eq : emitc::CmpPredicate::ne;
    Value signCond = rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), pred, lhsLt0, rhsLt0);
    Value adjust = rewriter.create<emitc::LogicalAndOp>(loc, rewriter.getI1Type(), rNeZero, signCond);
    return {q0, one, adjust};
}

static LogicalResult rewriteI1RightShiftApprox(
    Operation* op, Location loc, Type dstTy, Value lhs, Value rhs, ConversionPatternRewriter& rewriter)
{
    ...
    Value sh = rewriter.create<emitc::BitwiseRightShiftOp>(loc, u8Ty, lhsU8, rhsU8);
    Value masked = rewriter.create<emitc::BitwiseAndOp>(loc, u8Ty, sh, makeEmitCIntConstant(rewriter, loc, u8Ty, 1));
    rewriter.replaceOp(op, emitCCast(rewriter, loc, dstTy, masked));
    return success();
}

static LogicalResult getScalarIntegerOpInfo(
    Operation*, Type opTy, const TypeConverter* typeConverter, unsigned& bitWidth, Type& dstTy)
{
    ...
    bitWidth = intTy ? intTy.getWidth() : static_cast<unsigned>(kPTOIndexBitWidth);
...
static LogicalResult rewriteUnsignedBinaryIntLikeOp(
    Operation* op, Location loc, Type opTy, Value lhs, Value rhs, ConversionPatternRewriter& rewriter,
    const TypeConverter* typeConverter, std::optional<int64_t> i1MaskValue = std::nullopt)
{
    unsigned bitWidth = 0;
    Type dstTy;
    ...
    if (bitWidth == 1) {
        ...
        Value masked = rewriter.create<emitc::BitwiseAndOp>(
            loc, u8Ty, raw, makeEmitCIntConstant(rewriter, loc, u8Ty, *i1MaskValue));
        rewriter.replaceOp(op, emitCCast(rewriter, loc, dstTy, masked));
        return success();
    }

    auto uTy = getUnsignedIntOpaqueType(rewriter.getContext(), bitWidth);
    Value lhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, lhs, bitWidth);
    Value rhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, rhs, bitWidth);
    Value resU = rewriter.create<EmitCOp>(loc, uTy, lhsU, rhsU);
    rewriter.replaceOp(op, emitCCast(rewriter, loc, dstTy, resU));
    return success();
}

static LogicalResult rewriteDirectCastOp(Operation* op, Value in, Type dstTy, ConversionPatternRewriter& rewriter)
{
    rewriter.replaceOpWithNewOp<emitc::CastOp>(op, dstTy, in);
    return success();
}

static LogicalResult getScalarIntegerCastTypes(
    Operation*, Type dstSrcTy, Type inSrcTy, const TypeConverter* typeConverter, Type& dstTy, IntegerType& dstIntTy,
...
static FailureOr<ScalarIntegerCastInfo> getScalarIntegerCastInfo(
    Operation* op, Type dstSrcTy, Type inSrcTy, const TypeConverter* typeConverter)
{
    Type dstTy;
    IntegerType dstIntTy;
    IntegerType srcIntTy;
    if (failed(getScalarIntegerCastTypes(op, dstSrcTy, inSrcTy, typeConverter, dstTy, dstIntTy, srcIntTy))) {
        return failure();
    }
...
struct ArithSimpleBinaryToEmitC : public OpConversionPattern<ArithOp> {
    ...
    LogicalResult matchAndRewrite(
        ArithOp op, typename ArithOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        return rewriteSimpleBinary(op, adaptor, &rewriter);
    }

private:
    LogicalResult rewriteSimpleBinary(
        ArithOp op, typename ArithOp::Adaptor adaptor, ConversionPatternRewriter* rewriter) const
    {
...
struct ArithUnsignedBitwiseBinaryToEmitC : public OpConversionPattern<ArithOp> {
        ...
        ArithOp op, typename ArithOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        return rewriteUnsignedBinaryIntLikeOp<EmitCOp>(
            op, op.getLoc(), op.getType(), adaptor.getLhs(), adaptor.getRhs(), rewriter, this->getTypeConverter());
    }
};
...
struct ArithDivUIToEmitC : public OpConversionPattern<arith::DivUIOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::DivUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        return rewriteUnsignedBinaryIntLikeOp<emitc::DivOp>(
            op, op.getLoc(), op.getType(), adaptor.getLhs(), adaptor.getRhs(), rewriter, getTypeConverter());
    }
};
...
struct ArithRemUIToEmitC : public OpConversionPattern<arith::RemUIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::RemUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct ArithCeilDivUIToEmitC : public OpConversionPattern<arith::CeilDivUIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::CeilDivUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        unsigned bitWidth = 0;
        Type dstTy;
        if (failed(getScalarIntegerOpInfo(op, op.getType(), getTypeConverter(), bitWidth, dstTy)))
            return rewriter.notifyMatchFailure(op, "expected scalar integer or index type");

        auto uTy = getUnsignedIntOpaqueType(rewriter.getContext(), bitWidth);
        Value lhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getLhs(), bitWidth);
        Value rhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getRhs(), bitWidth);
...
struct ArithCeilDivSIToEmitC : public OpConversionPattern<arith::CeilDivSIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::CeilDivSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        unsigned bitWidth = 0;
        Type dstTy;
        if (failed(getScalarIntegerOpInfo(op, op.getType(), getTypeConverter(), bitWidth, dstTy)))
        ...
        auto data = buildSignedDivAdjustOperands(
            rewriter, op.getLoc(), dstTy, adaptor.getLhs(), adaptor.getRhs(),
            /*expectSameSigns=*/true);
        Value qPlusOne = rewriter.create<emitc::AddOp>(op.getLoc(), dstTy, data.q0, data.one);
        Value result = rewriter.create<emitc::ConditionalOp>(op.getLoc(), dstTy, data.adjust, qPlusOne, data.q0);
        rewriter.replaceOp(op, result);
        return success();
    }
};
...
struct ArithFloorDivSIToEmitC : public OpConversionPattern<arith::FloorDivSIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::FloorDivSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        unsigned bitWidth = 0;
        Type dstTy;
        if (failed(getScalarIntegerOpInfo(op, op.getType(), getTypeConverter(), bitWidth, dstTy)))
        ...
        Value qMinusOne = rewriter.create<emitc::SubOp>(op.getLoc(), dstTy, data.q0, data.one);
        Value result = rewriter.create<emitc::ConditionalOp>(op.getLoc(), dstTy, data.adjust, qMinusOne, data.q0);
        rewriter.replaceOp(op, result);
        return success();
    }
};
...
struct ArithShiftLeftToEmitC : public OpConversionPattern<arith::ShLIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::ShLIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct ArithShiftRightUIToEmitC : public OpConversionPattern<arith::ShRUIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::ShRUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        unsigned bitWidth = 0;
        Type dstTy;
        if (failed(getScalarIntegerOpInfo(op, op.getType(), getTypeConverter(), bitWidth, dstTy)))
            return rewriter.notifyMatchFailure(op, "expected scalar integer or index type");
        if (bitWidth == 1)
            return rewriteI1RightShiftApprox(op, op.getLoc(), dstTy, adaptor.getLhs(), adaptor.getRhs(), rewriter);
...
struct ArithShiftRightSIToEmitC : public OpConversionPattern<arith::ShRSIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::ShRSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        unsigned bitWidth = 0;
        Type dstTy;
        ...
        if (bitWidth == 1)
            return rewriteI1RightShiftApprox(op, loc, dstTy, adaptor.getLhs(), adaptor.getRhs(), rewriter);

        // Signed arithmetic shift; cast RHS to unsigned to interpret shift amount.
        Value rhsU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getRhs(), bitWidth);
        Value sh = rewriter.create<emitc::BitwiseRightShiftOp>(loc, dstTy, adaptor.getLhs(), rhsU);
...
struct ArithNegFToEmitC : public OpConversionPattern<arith::NegFOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::NegFOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct ArithRemFToEmitC : public OpConversionPattern<arith::RemFOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::RemFOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        Type dstTy = getTypeConverter()->convertType(op.getType());
        if (!dstTy)
...
struct ArithSelectToEmitC : public OpConversionPattern<arith::SelectOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::SelectOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (!op.getCondition().getType().isInteger(1))
            return rewriter.notifyMatchFailure(op, "only scalar i1 conditions supported for arith.select");
        ...
        auto cond = rewriter.create<emitc::ConditionalOp>(
            op.getLoc(), dstTy, adaptor.getCondition(), adaptor.getTrueValue(), adaptor.getFalseValue());
...
struct ArithExtUIToEmitC : public OpConversionPattern<arith::ExtUIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::ExtUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto castInfo = getScalarIntegerCastInfo(op, op.getType(), op.getIn().getType(), getTypeConverter());
        if (failed(castInfo))
        ...
        auto uDstTy = getUnsignedIntOpaqueType(rewriter.getContext(), castInfo->dstIntTy.getWidth());
        Value srcU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getIn(), castInfo->srcIntTy.getWidth());
...
struct ArithExtSIToEmitC : public OpConversionPattern<arith::ExtSIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::ExtSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        auto castInfo = getScalarIntegerCastInfo(op, op.getType(), op.getIn().getType(), getTypeConverter());
        if (failed(castInfo))
        ...
        if (castInfo->srcIntTy.getWidth() == 1) {
            ...
            Value neg = rewriter.create<emitc::SubOp>(loc, castInfo->dstTy, zero, asInt).getResult();
...
struct ArithCastToEmitC : public ConversionPattern {
    ArithCastToEmitC(const TypeConverter& typeConverter, MLIRContext* ctx)
        : ConversionPattern(
              typeConverter, CastOp::getOperationName(),
              /*benefit=*/1, ctx)
    {}

    LogicalResult matchAndRewrite(
        Operation* op, ArrayRef<Value> operands, ConversionPatternRewriter& builder) const override
    {
        return rewriteCast(op, operands, &builder);
    }

private:
    LogicalResult rewriteCast(Operation* rootOp, ArrayRef<Value> operands, ConversionPatternRewriter* builder) const
    {
...
struct ArithIndexCastUIToEmitC : public OpConversionPattern<arith::IndexCastUIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::IndexCastUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        Type dstTy = getTypeConverter()->convertType(op.getType());
        if (!dstTy)
...
struct ArithUIToFPToEmitC : public OpConversionPattern<arith::UIToFPOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::UIToFPOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        ...
        Value srcU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getIn(), srcIntTy.getWidth());
...
struct ArithFPToUIToEmitC : public OpConversionPattern<arith::FPToUIOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::FPToUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
...
struct ArithBitcastToEmitC : public OpConversionPattern<arith::BitcastOp> {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::BitcastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        Type dstTy = getTypeConverter()->convertType(op.getType());
        if (!dstTy)
        ...
        auto templateArgs =
            rewriter.getArrayAttr({emitc::OpaqueAttr::get(rewriter.getContext(), dstOpaque.getValue())});
        auto call = rewriter.create<emitc::CallOpaqueOp>(
...
struct ArithCmpFToEmitC : public OpConversionPattern<arith::CmpFOp> {
    ...
    static Value isNaN(ConversionPatternRewriter& rewriter, Location loc, Value v)
    {
        return rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::ne, v, v).getResult();
    }

    static Value isNotNaN(ConversionPatternRewriter& rewriter, Location loc, Value v)
    {
        return rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::eq, v, v).getResult();
    }

    static std::optional<Value> buildSpecialCmpFResult(
        arith::CmpFPredicate predicate, ConversionPatternRewriter& rewriter, Location loc, Type i1Ty, Value lhs,
        Value rhs)
    {
        switch (predicate) {
            ...
            case arith::CmpFPredicate::ORD:
                return rewriter
                    .create<emitc::LogicalAndOp>(loc, i1Ty, isNotNaN(rewriter, loc, lhs), isNotNaN(rewriter, loc, rhs))
                    .getResult();
            case arith::CmpFPredicate::UNO:
                return rewriter
                    .create<emitc::LogicalOrOp>(loc, i1Ty, isNaN(rewriter, loc, lhs), isNaN(rewriter, loc, rhs))
                    .getResult();
            default:
                return std::nullopt;
        }
    ...
    static std::optional<CmpFConfig> getCmpFConfig(arith::CmpFPredicate predicate)
    {
        switch (predicate) {
    ...
    static Value buildCmpFResult(
        const CmpFConfig& config, ConversionPatternRewriter& rewriter, Location loc, Type i1Ty, Value lhs, Value rhs)
    {
        Value cmp = rewriter.create<emitc::CmpOp>(loc, i1Ty, config.predicate, lhs, rhs).getResult();
        Value unord =
            rewriter.create<emitc::LogicalOrOp>(loc, i1Ty, isNaN(rewriter, loc, lhs), isNaN(rewriter, loc, rhs));
        if (config.unordered)
            return rewriter.create<emitc::LogicalOrOp>(loc, i1Ty, unord, cmp).getResult();
        Value ord =
            rewriter.create<emitc::LogicalAndOp>(loc, i1Ty, isNotNaN(rewriter, loc, lhs), isNotNaN(rewriter, loc, rhs));
        return rewriter.create<emitc::LogicalAndOp>(loc, i1Ty, ord, cmp).getResult();
    }

    LogicalResult matchAndRewrite(
        arith::CmpFOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto special =
                buildSpecialCmpFResult(op.getPredicate(), rewriter, loc, i1Ty, adaptor.getLhs(), adaptor.getRhs())) {
        ...
        rewriter.replaceOp(op, buildCmpFResult(*config, rewriter, loc, i1Ty, adaptor.getLhs(), adaptor.getRhs()));
        return success();
    }
};

struct ArithAddUIExtendedToEmitC : public OpConversionPattern<arith::AddUIExtendedOp> {
    using OpConversionPattern::OpConversionPattern;

    LogicalResult matchAndRewrite(
        arith::AddUIExtendedOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        ...
        const bool isIndex = isa<IndexType>(opTy);
        if (!intTy && !isIndex)
            return rewriter.notifyMatchFailure(op, "expected scalar integer or index operands");

        const unsigned bitWidth = intTy ? intTy.getWidth() : static_cast<unsigned>(kPTOIndexBitWidth);
        ...
        Value sumWide = rewriter.create<emitc::AddOp>(loc, wideTy, lhsWide, rhsWide).getResult();
        ...
        Value high = rewriter.create<emitc::BitwiseRightShiftOp>(loc, wideTy, sumWide, shiftAmt).getResult();
        Value zeroWide = makeEmitCIntConstant(rewriter, loc, wideTy, 0);
        Value overflow =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::ne, high, zeroWide)
                .getResult();
...
struct ArithMulExtendedToEmitC : public OpConversionPattern<ArithOp> {
        ...
        ArithOp op, typename ArithOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        SmallVector<Type> newResultTypes;
        if (failed(this->getTypeConverter()->convertTypes(op->getResultTypes(), newResultTypes)))
            return failure();
        if (newResultTypes.size() != kNumber2)
            return failure();
        ...
        Type wideTy = isUnsigned ? static_cast<Type>(getWiderUnsignedIntOpaqueType(rewriter.getContext(), bitWidth)) :
                                   static_cast<Type>(getWiderSignedIntOpaqueType(rewriter.getContext(), bitWidth));
        ...
        Value prodWide = rewriter.create<emitc::MulOp>(loc, wideTy, lhsWide, rhsWide).getResult();
        Value low = emitCCast(rewriter, loc, lowDstTy, prodWide);

        Value shiftAmt = makeEmitCIntConstant(rewriter, loc, wideTy, bitWidth);
        Value highWide = rewriter.create<emitc::BitwiseRightShiftOp>(loc, wideTy, prodWide, shiftAmt).getResult();
...
struct ArithMinMaxIToEmitCBase {
    static Value makeSelect(
        ConversionPatternRewriter& rewriter, Location loc, Type dstTy, Value cond, Value trueV, Value falseV)
    {
        return rewriter.create<emitc::ConditionalOp>(loc, dstTy, cond, trueV, falseV).getResult();
    }

    static Value makeLessThanCond(ConversionPatternRewriter& rewriter, Location loc, Value lhs, Value rhs)
    {
        return rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, lhs, rhs).getResult();
    }

    static LogicalResult rewriteSignedMinMax(
        Operation* op, Value lhs, Value rhs, bool chooseMax, ConversionPatternRewriter& rewriter,
        const TypeConverter* typeConverter)
    {
        ...
        Value res = makeSelect(rewriter, op->getLoc(), dstTy, cond, chooseMax ? rhs : lhs, chooseMax ? lhs : rhs);
        rewriter.replaceOp(op, res);
        return success();
    }

    static LogicalResult rewriteUnsignedMinMax(
        Operation* op, Value lhs, Value rhs, bool chooseMax, ConversionPatternRewriter& rewriter,
        const TypeConverter* typeConverter)
    {
        Type opTy = op->getResult(0).getType();
        ...
        Value lhsU = castSignlessIntToUnsignedSameWidth(rewriter, op->getLoc(), lhs, bitWidth);
        Value rhsU = castSignlessIntToUnsignedSameWidth(rewriter, op->getLoc(), rhs, bitWidth);
...
struct ArithMaxSIToEmitC : public OpConversionPattern<arith::MaxSIOp>, ArithMinMaxIToEmitCBase {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::MaxSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        return rewriteSignedMinMax(
            op, adaptor.getLhs(), adaptor.getRhs(),
            /*chooseMax=*/true, rewriter, getTypeConverter());
    }
};

struct ArithMinSIToEmitC : public OpConversionPattern<arith::MinSIOp>, ArithMinMaxIToEmitCBase {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::MinSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        return rewriteSignedMinMax(
            op, adaptor.getLhs(), adaptor.getRhs(),
            /*chooseMax=*/false, rewriter, getTypeConverter());
    }
};

struct ArithMaxUIToEmitC : public OpConversionPattern<arith::MaxUIOp>, ArithMinMaxIToEmitCBase {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::MaxUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        return rewriteUnsignedMinMax(
            op, adaptor.getLhs(), adaptor.getRhs(),
            /*chooseMax=*/true, rewriter, getTypeConverter());
    }
};

struct ArithMinUIToEmitC : public OpConversionPattern<arith::MinUIOp>, ArithMinMaxIToEmitCBase {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::MinUIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        return rewriteUnsignedMinMax(
            op, adaptor.getLhs(), adaptor.getRhs(),
            /*chooseMax=*/false, rewriter, getTypeConverter());
...
struct ArithFloatMinMaxToEmitCBase {
    ...
    static Value makeFZero(ConversionPatternRewriter& rewriter, Location loc, Type ty)
    {
        return makeEmitCOpaqueConstant(rewriter, loc, ty, "0.0f");
    }

    template <bool isMaximum>
    static LogicalResult rewriteMinMaxNumFOp(
        Operation* op, Location loc, Type dstTy, Value lhs, Value rhs, ConversionPatternRewriter& rewriter)
    {
        Value lhsNaN = isNaN(rewriter, loc, lhs);
        Value rhsNaN = isNaN(rewriter, loc, rhs);
        Value cmpLt =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, lhs, rhs).getResult();
        Value noNaNResult =
            rewriter.create<emitc::ConditionalOp>(loc, dstTy, cmpLt, isMaximum ? rhs : lhs, isMaximum ? lhs : rhs)
                .getResult();
        Value rhsOrResult = rewriter.create<emitc::ConditionalOp>(loc, dstTy, rhsNaN, lhs, noNaNResult).getResult();
        Value res = rewriter.create<emitc::ConditionalOp>(loc, dstTy, lhsNaN, rhs, rhsOrResult).getResult();
        rewriter.replaceOp(op, res);
        return success();
    }
};

struct ArithMaxNumFToEmitC : public OpConversionPattern<arith::MaxNumFOp>, ArithFloatMinMaxToEmitCBase {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::MaxNumFOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        Type dstTy = getTypeConverter()->convertType(op.getType());
        if (!dstTy)
            return failure();
        return rewriteMinMaxNumFOp<true>(op, loc, dstTy, adaptor.getLhs(), adaptor.getRhs(), rewriter);
    }
};

struct ArithMinNumFToEmitC : public OpConversionPattern<arith::MinNumFOp>, ArithFloatMinMaxToEmitCBase {
    using OpConversionPattern::OpConversionPattern;
    LogicalResult matchAndRewrite(
        arith::MinNumFOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        Type dstTy = getTypeConverter()->convertType(op.getType());
        if (!dstTy)
            return failure();
        return rewriteMinMaxNumFOp<false>(op, loc, dstTy, adaptor.getLhs(), adaptor.getRhs(), rewriter);
    }
};

template <typename ArithOp, bool isMaximum>
struct ArithMinMaxFPropagateNaNToEmitC : public OpConversionPattern<ArithOp>, ArithFloatMinMaxToEmitCBase {
    using OpConversionPattern<ArithOp>::OpConversionPattern;

    static Value buildPrimaryCandidate(
        ConversionPatternRewriter& rewriter, Location loc, Type dstTy, Value lhs, Value rhs)
    {
        Value cmpLt =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::lt, lhs, rhs).getResult();
        return rewriter.create<emitc::ConditionalOp>(loc, dstTy, cmpLt, isMaximum ? rhs : lhs, isMaximum ? lhs : rhs)
            .getResult();
    }

    static Value buildSignBitValue(ConversionPatternRewriter& rewriter, Location loc, Value lhs, FloatType floatTy)
    {
        auto bitsTy = getUnsignedIntOpaqueType(rewriter.getContext(), floatTy.getWidth());
        auto templateArgs = rewriter.getArrayAttr(
            {emitc::OpaqueAttr::get(rewriter.getContext(), cast<emitc::OpaqueType>(bitsTy).getValue())});
        Value lhsBits = rewriter
                            .create<emitc::CallOpaqueOp>(
                                loc, TypeRange{bitsTy}, "ptoas_bitcast", ValueRange{lhs}, ArrayAttr{}, templateArgs)
                            .getResult(0);
        Value oneBits = makeEmitCIntConstant(rewriter, loc, bitsTy, 1);
        Value shiftAmount = makeEmitCIntConstant(rewriter, loc, bitsTy, floatTy.getWidth() - 1);
        Value signMask = rewriter.create<emitc::BitwiseLeftShiftOp>(loc, bitsTy, oneBits, shiftAmount).getResult();
        return rewriter.create<emitc::BitwiseAndOp>(loc, bitsTy, lhsBits, signMask).getResult();
    }

    static Value buildSignedZeroCandidate(
        ConversionPatternRewriter& rewriter, Location loc, Type dstTy, Value lhs, Value rhs, FloatType floatTy)
    {
        Value zero = makeFZero(rewriter, loc, dstTy);
        Value equal =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::eq, lhs, rhs).getResult();
        Value lhsZero =
            rewriter.create<emitc::CmpOp>(loc, rewriter.getI1Type(), emitc::CmpPredicate::eq, lhs, zero).getResult();
        Value bothZero = rewriter.create<emitc::LogicalAndOp>(loc, rewriter.getI1Type(), equal, lhsZero).getResult();
        auto bitsTy = getUnsignedIntOpaqueType(rewriter.getContext(), floatTy.getWidth());
        Value zeroBits = makeEmitCIntConstant(rewriter, loc, bitsTy, 0);
        Value lhsIsNegZero = rewriter
                                 .create<emitc::CmpOp>(
                                     loc, rewriter.getI1Type(), emitc::CmpPredicate::ne,
                                     buildSignBitValue(rewriter, loc, lhs, floatTy), zeroBits)
                                 .getResult();
        Value tie =
            rewriter
                .create<emitc::ConditionalOp>(loc, dstTy, lhsIsNegZero, isMaximum ? rhs : lhs, isMaximum ? lhs : rhs)
                .getResult();
        return rewriter
            .create<emitc::ConditionalOp>(
                loc, dstTy, bothZero, tie, buildPrimaryCandidate(rewriter, loc, dstTy, lhs, rhs))
            .getResult();
    }

    static Value buildNaNPropagatingResult(
        ConversionPatternRewriter& rewriter, Location loc, Type dstTy, Value lhs, Value rhs, FloatType floatTy)
    {
        Value lhsNaN = isNaN(rewriter, loc, lhs);
        Value rhsNaN = isNaN(rewriter, loc, rhs);
        Value noNaN = buildSignedZeroCandidate(rewriter, loc, dstTy, lhs, rhs, floatTy);
        Value rhsOrNoNaN = rewriter.create<emitc::ConditionalOp>(loc, dstTy, rhsNaN, rhs, noNaN).getResult();
        return rewriter.create<emitc::ConditionalOp>(loc, dstTy, lhsNaN, lhs, rhsOrNoNaN).getResult();
    }
        ...
        ArithOp op, typename ArithOp::Adaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        rewriter.replaceOp(
            op, buildNaNPropagatingResult(rewriter, loc, dstTy, adaptor.getLhs(), adaptor.getRhs(), floatTy));
        return success();
    }
};
...
struct ArithMulIToEmitC : public OpConversionPattern<arith::MulIOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::MulIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct ArithAddIToEmitC : public OpConversionPattern<arith::AddIOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::AddIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct ArithCastOPToEmitC : public OpConversionPattern<arith::IndexCastOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::IndexCastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct ArithSubIToEmitC : public OpConversionPattern<arith::SubIOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::SubIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct ArithDivSIToEmitC : public OpConversionPattern<arith::DivSIOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::DivSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type dstTy = getTypeConverter()->convertType(op.getType());
        if (!dstTy)
            return failure();
        rewriter.replaceOpWithNewOp<emitc::DivOp>(op, dstTy, adaptor.getLhs(), adaptor.getRhs());
        return success();
    }
};
...
struct ArithRemSIToEmitC : public OpConversionPattern<arith::RemSIOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::RemSIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type dstTy = getTypeConverter()->convertType(op.getType());
        if (!dstTy)
            return failure();
        rewriter.replaceOpWithNewOp<emitc::RemOp>(op, dstTy, adaptor.getLhs(), adaptor.getRhs());
        return success();
    }
};
...
struct ArithTruncIToEmitC : public OpConversionPattern<arith::TruncIOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::TruncIOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        ...
        if (dstIntTy.getWidth() == 1) {
            ...
            auto uSrcTy = getUnsignedIntOpaqueType(rewriter.getContext(), srcIntTy.getWidth());
            Value inU = castSignlessIntToUnsignedSameWidth(rewriter, loc, adaptor.getIn(), srcIntTy.getWidth());
            Value one = makeEmitCIntConstant(rewriter, loc, uSrcTy, 1);
            Value masked = rewriter.create<emitc::BitwiseAndOp>(loc, uSrcTy, inU, one);
...
struct ArithConstantToEmitC : public OpConversionPattern<arith::ConstantOp> {
    ...
    LogicalResult matchAndRewrite(
        arith::ConstantOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (auto opaqueLiteral = buildEmitCOpaqueConstantLiteral(newType, valueAttr); succeeded(opaqueLiteral)) {
        ...
        if (auto floatAttr = dyn_cast_or_null<FloatAttr>(valueAttr)) {
            ...
            const bool hasFloatMarker = s.contains('.') || s.contains('e') || s.contains('E') || s.contains('p') ||
                                        s.contains('P') || s.starts_with("0x") || s.starts_with("0X") ||
                                        s.starts_with("nan") || s.starts_with("-nan") || s.starts_with("inf") ||
                                        s.starts_with("-inf");
            if (!hasFloatMarker)
                valStr.append(".0");
...
static void populatePTOToEmitCIntegerArithPatterns(
...
static void populatePTOToEmitCFloatArithPatterns(
    RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx)
{
...
static void populatePTOToEmitCCastPatterns(RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx)
{
...
void populatePTOToEmitCArithPatterns(RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx)
{
