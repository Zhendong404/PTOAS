...
using StoreTemplateTokenVector = SmallVector<std::string, kStoreTemplateTokenInlineCapacity>;
using TStoreOperandVector = SmallVector<Value, kTStoreOperandInlineCapacity>;

static Value materializeKernelSourceArg(
    ConversionPatternRewriter& rewriter, Location loc, Operation* anchor, Value originalSource, Value emittedSource)
{
...
struct PTOTLoadToTLOAD : public OpConversionPattern<pto::TLoadOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TLoadOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (!op.getDst())
        ...
        Value srcArg =
            materializeKernelSourceArg(rewriter, op.getLoc(), op.getOperation(), op.getSrc(), adaptor.getSrc());

        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TLOAD", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, srcArg});
...
struct PTOTPrefetchToTPREFETCH : public OpConversionPattern<pto::TPrefetchOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TPrefetchOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (!op.getDst())
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TPREFETCH", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, srcArg});
        rewriter.eraseOp(op);
        return success();
    }
};

struct PTOTPrefetchAsyncToEmitC : public OpConversionPattern<pto::TPrefetchAsyncOp> {
    using OpConversionPattern<pto::TPrefetchAsyncOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        pto::TPrefetchAsyncOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!isEmitCGlobalTensorLikeType(srcArg.getType())) {
            ...
            srcArg = buildGlobalTensorFromMemref(
                rewriter, op.getLoc(), src, srcMrTy,
                op.getSrc().getDefiningOp() ? op.getSrc().getDefiningOp() : op.getOperation());
        }
        if (!srcArg)
            return rewriter.notifyMatchFailure(op, "failed to build GlobalTensor src");

        Value prefetchCtx = peelUnrealized(adaptor.getCtx());

        Type eventTy = getTypeConverter()->convertType(op.getEvent().getType());
        if (!eventTy)
            return rewriter.notifyMatchFailure(op, "failed to convert tprefetch_async result type");

        Value event = rewriter
                          .create<emitc::CallOpaqueOp>(
                              op.getLoc(), TypeRange{eventTy}, "TPREFETCH_ASYNC", ArrayAttr{}, ArrayAttr{},
...
struct PTOMakePrefetchAsyncContextToEmitC : public OpConversionPattern<pto::MakePrefetchAsyncContextOp> {
    using OpConversionPattern<pto::MakePrefetchAsyncContextOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        pto::MakePrefetchAsyncContextOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type ctxTy = getTypeConverter()->convertType(op.getCtx().getType());
        if (!ctxTy)
            return rewriter.notifyMatchFailure(op, "failed to convert make_prefetch_async_context result type");
        ...
        Value ctx = rewriter
                        .create<emitc::CallOpaqueOp>(
                            op.getLoc(), TypeRange{ctxTy}, "pto::PrefetchAsyncContext", ArrayAttr{}, ArrayAttr{},
...
struct PTOGetPrefetchAsyncSessionToEmitC : public OpConversionPattern<pto::GetPrefetchAsyncSessionOp> {
    using OpConversionPattern<pto::GetPrefetchAsyncSessionOp>::OpConversionPattern;

    LogicalResult matchAndRewrite(
        pto::GetPrefetchAsyncSessionOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        Type sessionTy = getTypeConverter()->convertType(op.getSession().getType());
        if (!sessionTy)
            return rewriter.notifyMatchFailure(op, "failed to convert get_prefetch_async_session result type");

        Value ctx = peelUnrealized(adaptor.getCtx());
        Value session = rewriter
                            .create<emitc::CallOpaqueOp>(
                                op.getLoc(), TypeRange{sessionTy}, "PTOAS__PREFETCH_CTX_SESSION", ArrayAttr{},
...
struct PTOTStoreToTSTORE : public OpConversionPattern<pto::TStoreOp> {
    ...
    {
        switch (phase) {
            case pto::STPhase::Unspecified:
                return "STPhase::Unspecified";
            case pto::STPhase::Partial:
                return "STPhase::Partial";
            case pto::STPhase::Final:
                return "STPhase::Final";
        }
        return "STPhase::Unspecified";
    }
    ...
    static Value buildTStoreDstArg(ConversionPatternRewriter& rewriter, pto::TStoreOp op, Value dst)
    {
        ...
        if (!isGlobal)
            return dst;
        if (Value gt = buildGlobalTensorFromMemref(rewriter, op.getLoc(), dst, dstMrTy, op.getOperation()))
            return gt;
        return dst;
    }

    static FailureOr<std::string> getTStoreOpaqueTok(
        ConversionPatternRewriter& rewriter, pto::TStoreOp op, Value value, StringRef name)
    {
    ...
    static FailureOr<TStoreOpaqueTokPair> getTStoreSrcDstOpaqueToks(
        ConversionPatternRewriter& rewriter, pto::TStoreOp op, Value src, Value dstArg)
    {
    ...
    static ArrayAttr buildOpaqueTemplateArgs(ConversionPatternRewriter& rewriter, ArrayRef<std::string> tokens)
    {
    ...
    static FailureOr<ArrayAttr> buildTStoreTemplateArgs(
        ConversionPatternRewriter& rewriter, pto::TStoreOp op, Value src, Value dstArg, bool hasPreQuantScalar)
    {
        const auto phase = op.getStPhase();
        const auto atomicType = op.getAtomicType();
    ...
    LogicalResult matchAndRewrite(
        pto::TStoreOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (!op.getDst())
        ...
        FailureOr<ArrayAttr> targs = buildTStoreTemplateArgs(rewriter, op, src, dstArg, hasPreQuantScalar);
        if (failed(targs))
            return failure();
...
static ArrayAttr buildAccPhaseTemplateArgs(ConversionPatternRewriter& rewriter, pto::AccPhase phase)
{
    StringRef tmpl;
    switch (phase) {
    ...
    if (tmpl.empty())
        return ArrayAttr{};
    return rewriter.getArrayAttr({emitc::OpaqueAttr::get(rewriter.getContext(), tmpl)});
}
...
struct PTOTMatmulToTMATMUL : public OpConversionPattern<pto::TMatmulOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TMatmulOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        // 1. 获取操作数 (剥离 Cast)
        ...
        ArrayAttr templateArgs = buildAccPhaseTemplateArgs(rewriter, op.getAccPhase());

        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TMATMUL",
            /*args=*/ArrayAttr{}, /*template_args=*/templateArgs, ValueRange{dst, lhs, rhs});
...
struct PTOTGemvToTGEMV : public OpConversionPattern<pto::TGemvOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TGemvOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        // 1. 获取操作数 (剥离 Cast)
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TGEMV", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, lhs, rhs});
...
struct PTOTGemvAccToTGEMVACC : public OpConversionPattern<pto::TGemvAccOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TGemvAccOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (!op.getDst())
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, "TGEMV_ACC", ArrayAttr{}, ArrayAttr{}, ValueRange{dst, accIn, lhs, rhs});
...
struct PTOTMatmulAccToTMATMULACC : public OpConversionPattern<pto::TMatmulAccOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::TMatmulAccOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (!op.getDst())
        ...
        rewriter.create<emitc::CallOpaqueOp>(
            ...
            /*args=*/ArrayAttr{}, /*template_args=*/templateArgs, ValueRange{dst, accIn, lhs, rhs});
...
void populatePTOToEmitCKernelOpPatterns(RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx)
{
