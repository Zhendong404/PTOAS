...
static void wrapSingleBlockFuncBody(func::FuncOp funcOp)
{
    ...
    sectionBlock.getOperations().splice(sectionBlock.end(), entryBlock.getOperations(), entryBlock.begin(), sectionIt);
}
...
static LogicalResult rewriteFunction(func::FuncOp funcOp)
{
    ...
    if (!funcOp.getBody().hasOneBlock())
        return funcOp.emitOpError("requires a single-block body for kernel_kind wrapping");

    if (hasExistingSection(funcOp)) {
        return funcOp.emitOpError("already contains pto.section.cube or pto.section.vector");
    }
...
struct PTOWrapFunctionsInSectionsPass
    : public mlir::pto::impl::PTOWrapFunctionsInSectionsBase<PTOWrapFunctionsInSectionsPass> {
