...
enum class PlanStatus { PLAN_SUCCESS = 0, RESTART_NEW_PLAN, CONTINUE_PLAN, PLAN_FAILED };
...
struct OpInfo {
    OpInfo(Operation* operation, int index) : operation(operation), index(index) {}
    Operation* operation{nullptr};
    int index{0};
};
...
struct BufferLife {
    BufferLife(Value buffer, int64_t start, int64_t end) : buffer(buffer), allocTime(start), freeTime(end) {}
...
struct MemoryBound {
    MemoryBound(BufferLifeVec life, uint64_t o, uint64_t e, const StorageEntry* s)
        : bufferLifeVec(std::move(life)), offset(o), extent(e), lastStorageEntry(s)
    {}
...
struct OutlineSectionInfo {
    ...
    OutlineSectionInfo(MemBoundListConstIter& start, MemBoundListConstIter& end, uint64_t s, bool isDirectlyRollback)
        : mem_start(start), mem_end(end), size(s), isDirectlyRollback(isDirectlyRollback)
    {}
...
struct CompareBufferLife {
    bool operator()(const std::shared_ptr<BufferLife>& lhs, const std::shared_ptr<BufferLife>& rhs) const
    {
...
public:
    MemLivenessAnalysis(func::FuncOp func, MemPlanMode planMode) : func_(func), planMode(planMode) {}
...
private:
    ...
    std::optional<WalkResult> HandleRecursiveControlFlow(Operation* op, Liveness live);
    ...
    bool HandleGenericPtoDpsInitOp(Operation* op, OpInfo* curOpInfo, Liveness live);

    bool HandleGenericDestinationStyleOp(Operation* op, OpInfo* curOpInfo, Liveness live);

    bool HandleGenericAliasOrCallOp(Operation* op, OpInfo* curOpInfo, Liveness live);

    bool HandleKnownBufferTouchingGroup(Operation* op, OpInfo* curOpInfo, Liveness live);

    bool HandleGenericBufferTouchingOp(Operation* op, OpInfo* curOpInfo, Liveness live);
    ...
    BufferInfo GetBufferInfo(Operation* op, Value operand, pto::AddressSpace bufferScope) const;
    ...
    void UpdateBufferAlias(Value buffer, Value aliasBuffer, bool isIgnoreInplace = false);
    ...
    bool AllDeadAfter(Operation* op, SetVector<Value> aliasVec, Liveness live) const;
...
public:
    MemPlan(
        MemPlanMode planMode, bool enableGlobalReuse, bool enablePrintMemoryAllocatedSize, bool restrictInplaceAsISA)
        : planMode(planMode),
    ...
    /// Get buffer2Offsets
    inline DenseMap<Value, SmallVector<uint64_t>> GetBuffer2Offsets() { return buffer2Offsets; }

    inline void SetLinearOperation(SmallVector<std::unique_ptr<OpInfo>>& linearOp)
    {
        linearOperation = std::move(linearOp);
    };

    inline void SetBufferInfos(std::map<Value, BufferInfo, ValueComparator> bufsInfo) { bufferInfos = bufsInfo; }

    inline void SetBuffer2Life(DenseMap<Value, std::shared_ptr<BufferLife>> buf2Life) { buffer2Life = buf2Life; }

    inline void SetGenKillMap(DenseMap<const OpInfo*, GenKillEntry> gkMap) { genKillMap = gkMap; }

    inline void SetBuffer2MultiNum(DenseMap<Value, uint32_t> buf2MulBufNum) { buffer2MultiNum = buf2MulBufNum; }

    inline void SetInplacePairList(SmallVector<ValuePair> inplaceList) { inplacePairList = inplaceList; }
    ...
    inline void SetStableValueOrder(StableValueOrderMap valueOrder) { stableValueOrder = std::move(valueOrder); }
...
private:
    ...
    PlanStatus PlanSingleLocalBuffer(StorageEntry* rootStorageEntry, size_t align, size_t maxBits);

    /// Plan a reusable local buffer scope.
    PlanStatus PlanReusableLocalBuffer(StorageEntry* rootStorageEntry, size_t align, size_t maxBits);
    ...
    bool IsEnoughForBuffersNoReuse(StorageEntry* rootStorageEntry, size_t restBufferSize, size_t alignUnit);
    ...
    void PlanBuffersWithoutReuse(StorageEntry* rootStorageEntry, size_t alignUnit) const;
    ...
    LogicalResult MultiSpecPlan(SpecInfo& si, MemBoundList& outline, PlanRecHis& history, StorageEntry* entry);

    /// plan buffer in speculative ways.
    LogicalResult SpecAlloc(
        MemBoundList& outline, PlanRecHis& his, StorageEntry* e, const SpecInfo& si, int localLevel);

    /// spec_level == SPEC_LEVEL_2, mte2/3 do not reuse with vector.
    bool VerifyConflictStage2(
        PlanRecHis& his, const StorageEntry* e, int specLevel, MemBoundListConstIter& start,
        const MemBoundList& outline);

    /// spec_level == SPEC_LEVEL_1, pure single can reuse with db.
    bool VerifyConflictStage1(
        MemBoundList&, PlanRecHis& his, StorageEntry* e, const OutlineSectionInfo& outlineInfo, uint64_t& pongOffset);

    /// check if e1 and e2 has pipe conflict.
    bool PipeConflict(const StorageEntry* e1, const StorageEntry* e2, DenseMap<StorageEntryPair, bool>& conflictMap);
    ...
    bool VerifyDmaPipeConflict(
        const StorageEntry* e, int specLevel, MemBoundListConstIter& start, MemBoundListConstIter& end);

    /// Check if it matches the previous rollback result.
    bool IsSamePlanAsLastRollBack(uint64_t allocOffset, int curChildIdx, const SpecInfo& si) const;

    /// spec_level == SPEC_LEVEL_0, life time reuse.
    bool VerifyConflictStage0(StorageEntry* e, const std::shared_ptr<MemoryBound>& last) const;

    /// Update the outline information and record history
    void UpdateOutline(
        MemBoundList& outline, PlanRecHis& his, StorageEntry* e, const OutlineSectionInfo& outlineInfo,
    ...
    inline void MergeBufferLife(
        MemBoundList::const_iterator start, MemBoundList::const_iterator end, BufferLifeVec& newLife) const;
    ...
    /// plan failed.
    PlanStatus ApplyFailStrategy(StatusWrapper& statusWrapper, const size_t maxBits);
    ...
    void RollBackForAllocFailInner(StatusWrapper& statusWrapper, const size_t maxBits);
    ...
    DenseMap<ValuePair, BufferLife> GetOverlapBufferLife(const BufferLifeVec& b1, const BufferLifeVec& b2) const;

    bool HasSemanticConflict(const StorageEntry* entry, const BufferLifeVec& bufferLives) const;

    /// Reorder and make the storage entries of ping and pong continuous.
    void ReorderContinuousPingPongEntry(SmallVector<StorageEntry*>& storageEntryVec) const;
    ...
    bool IsBufferLifeVecConflict(PlanRecord& r, uint64_t offset, const StorageEntry* e) const;
    ...
    void SpecAllocRelationPongEntry(MemBoundList& outline, PlanRecHis& his, StorageEntry* e, uint64_t offset);

    /// Get relative pong storage entry when the current reuse bound storage entry
    /// is of type db.
    StorageEntry* GetMultiRelationPongEntry(const StorageEntry* reuseBoundStorageEntry);

    /// Get the innermost for loop of buffer definition.
    LoopLikeOpInterface GetBufferParentLoop(const SmallVector<Value>& buffers) const;
...
private:
    ...
    DenseMap<StorageEntry*, std::unique_ptr<StorageEntry>> pingEntry2RelationPongEntry;
