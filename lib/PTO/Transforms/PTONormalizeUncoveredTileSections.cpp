...
static bool isRawSectionCarrierOp(Operation *op) {
  ...
  return op &&
         isa<VectorMicroOpInterface, CubeMicroOpInterface, MteOpInterface,
             SetFlagOp, WaitFlagOp, SyncSetOp, SyncWaitOp, SetCrossCoreOp,
             WaitFlagDevOp, SetIntraBlockOp, WaitIntraCoreOp>(op);
}
...
classifyRawSectionCarrierOp(Operation *op) {
  ...
  if (auto crossSet = dyn_cast<SetCrossCoreOp>(op))
    return classifySyncPipe(crossSet.getPipe().getPipe());
  if (auto crossWait = dyn_cast<WaitFlagDevOp>(op))
  ...
  if (auto intraWait = dyn_cast<WaitIntraCoreOp>(op))
