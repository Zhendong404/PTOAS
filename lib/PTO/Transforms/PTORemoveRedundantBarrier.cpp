...
static Attribute getTrackedPipe(Operation* op, Attribute attrMTE2, Attribute attrMTE3, Attribute attrVec)
{
...
static bool shouldEraseBarrierOp(Block* block, Block::iterator it, llvm::DenseSet<Attribute>& intraPipeDirtySet)
{
...
{
    Attribute waitDst;
    return getWaitSyncDst(&*it, waitDst) && !isPipelineActiveFuture(block, std::next(it), waitDst);
}

static bool shouldEraseSetOp(Block* block, Block::iterator it, llvm::DenseSet<Attribute>& intraPipeDirtySet)
{
    Attribute setSrc;
    Attribute setDst;
...
static void collectRedundantBarriersInBlock(
    Block* block, Attribute attrMTE2, Attribute attrMTE3, Attribute attrVec, llvm::SmallVector<Operation*>& opsToErase)
{
    ...
    for (auto it = block->begin(); it != block->end(); ++it) {
        ...
        if (shouldEraseWaitOp(block, it) || shouldEraseSetOp(block, it, intraPipeDirtySet)) {
            opsToErase.push_back(op);
        }
    }
}
...
struct PTORemoveRedundantBarrierPass : public PassWrapper<PTORemoveRedundantBarrierPass, OperationPass<func::FuncOp>> {
    ...
    void runOnOperation() override
    {
        ...
        func.walk(
            [&](Block* block) { collectRedundantBarriersInBlock(block, attrMTE2, attrMTE3, attrVec, opsToErase); });
