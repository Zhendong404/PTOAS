...
static constexpr llvm::StringLiteral kAutoSyncTailPendingModeAttr = "__pto.auto_sync_tail_mode";
...
static Value castInterCoreEventIdToI32(ConversionPatternRewriter& rewriter, Location loc, Value eventId)
{
...
static Attribute getFFTSModeCodegenArg(ConversionPatternRewriter& rewriter, int64_t fftsMode)
{
...
static Value createFFTSMsg(ConversionPatternRewriter& rewriter, Location loc, Value eventI32, int64_t fftsMode)
{
    auto* ctx = rewriter.getContext();
...
static ArrayAttr buildPipeZeroArgs(ConversionPatternRewriter& rewriter, StringRef pipeTok)
{
    auto* ctx = rewriter.getContext();
...
static InterCoreSyncCallDesc buildInterCoreSyncSetCall(
    ConversionPatternRewriter& rewriter, Location loc, PTOArch targetArch, pto::PipeAttr pipeAttr,
    IntegerAttr eventIdAttr, int64_t fftsMode)
{
    auto* ctx = rewriter.getContext();
    std::string pipeTok = pipeTokFromPipeAttr(pipeAttr);
    if (targetArch == PTOArch::A3) {
        auto i32Ty = emitc::OpaqueType::get(ctx, "int32_t");
        Value eventVal = makeEmitCIntConstant(rewriter, loc, i32Ty, eventIdAttr.getInt());
    ...
    desc.args = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, pipeTok), eventIdAttr});
    return desc;
}

static InterCoreSyncCallDesc buildInterCoreSyncSetCallDyn(
    ConversionPatternRewriter& rewriter, Location loc, PTOArch targetArch, pto::PipeAttr pipeAttr, Value eventIdVal,
    int64_t fftsMode)
{
    std::string pipeTok = pipeTokFromPipeAttr(pipeAttr);
...
static InterCoreSyncCallDesc buildInterCoreSyncWaitCall(
    ConversionPatternRewriter& rewriter, PTOArch targetArch, pto::PipeAttr pipeAttr, IntegerAttr eventIdAttr)
{
    auto* ctx = rewriter.getContext();
    std::string pipeTok = pipeTokFromPipeAttr(pipeAttr);
...
static LogicalResult emitA5SyncSetCall(
    ConversionPatternRewriter& rewriter, Location loc, pto::SyncSetOp op, Value eventIdDyn, IntegerAttr eventIdAttr)
{
    auto* ctx = rewriter.getContext();
    ...
    auto emitSet = [&rewriter, ctx, loc, pipeTok](Value eventOperand, IntegerAttr eventLiteral, bool isDynamic) {
        if (isDynamic) {
    ...
    if (eventIdAttr) {
        ...
        if (needsMirrorPlus16) {
            auto plus16 = IntegerAttr::get(eventIdAttr.getType(), eventIdAttr.getInt() + 16);
    ...
    if (needsMirrorPlus16) {
        ...
        Value eventI32Plus16 = rewriter.create<emitc::AddOp>(loc, i32Ty, eventI32, c16).getResult();
        emitSet(eventI32Plus16, IntegerAttr{}, /*isDynamic=*/true);
    }
    return success();
}

static FailureOr<emitc::OpaqueType> buildSyncAllWorkspaceEmitType(
    ConversionPatternRewriter& rewriter, Value originalWorkspace)
{
    ...
    auto tileTy = pto::TileBufType::get(ctx, shape, memTy.getElementType(), memorySpace, validShape, configAttr);
...
static Value castSyncAllWorkspacePtrToU64(ConversionPatternRewriter& rewriter, Location loc, Value workspace)
{
    auto* ctx = rewriter.getContext();
    ...
    if (isSetFFTsPointerLikeType(workspace.getType())) {
        ...
        return rewriter
            .create<emitc::CallOpaqueOp>(loc, u64Ty, "reinterpret_cast", ArrayAttr{}, rcU64, ValueRange{workspace})
            .getResult(0);
    }
...
static FailureOr<Value> buildSyncAllWorkspaceTileValue(
    ConversionPatternRewriter& rewriter, Location loc, Value originalWorkspace, Value emittedWorkspace)
{
    ...
    FailureOr<emitc::OpaqueType> tileEmitTy = buildSyncAllWorkspaceEmitType(rewriter, originalWorkspace);
    if (failed(tileEmitTy))
        return failure();
    auto* ctx = rewriter.getContext();
    Value tile = rewriter.create<emitc::VariableOp>(loc, *tileEmitTy, emitc::OpaqueAttr::get(ctx, "")).getResult();

    Value rawPtr = castSyncAllWorkspacePtrToU64(rewriter, loc, workspace);
    rewriter.create<emitc::CallOpaqueOp>(
        loc, TypeRange{}, "TASSIGN", ArrayAttr{}, ArrayAttr{}, ValueRange{tile, rawPtr});
    return tile;
}

static FailureOr<Value> buildSyncAllGmWorkspace(
    ConversionPatternRewriter& rewriter, pto::SyncAllOp op, pto::SyncAllOp::Adaptor adaptor)
{
    ...
    Value gt = buildGlobalTensorFromMemref(
        ...
        op.getGmWorkspace().getDefiningOp() ? op.getGmWorkspace().getDefiningOp() : op.getOperation());
    if (!gt)
        return failure();
    return gt;
}

static Value buildSyncAllUsedCores(
    ConversionPatternRewriter& rewriter, pto::SyncAllOp op, pto::SyncAllOp::Adaptor adaptor)
{
    auto i32Ty = emitc::OpaqueType::get(rewriter.getContext(), "int32_t");
    Value usedCores = adaptor.getUsedCores() ? peelUnrealized(adaptor.getUsedCores()) :
                                               makeEmitCIntConstant(rewriter, op.getLoc(), i32Ty, 0);
    if (usedCores.getType() != i32Ty) {
        usedCores = rewriter.create<emitc::CastOp>(op.getLoc(), i32Ty, usedCores).getResult();
    }
    return usedCores;
}

static LogicalResult appendSyncAllWorkspaceOperands(
    ConversionPatternRewriter& rewriter, pto::SyncAllOp op, pto::SyncAllOp::Adaptor adaptor, pto::SyncCoreType coreType,
    SmallVectorImpl<Value>& operands)
{
    switch (coreType) {
        case pto::SyncCoreType::AIVOnly: {
            FailureOr<Value> ubWorkspace =
                buildSyncAllWorkspaceTileValue(rewriter, op.getLoc(), op.getUbWorkspace(), adaptor.getUbWorkspace());
            if (failed(ubWorkspace))
                return rewriter.notifyMatchFailure(op, "failed to materialize ub_workspace tile");
            operands.push_back(*ubWorkspace);
            return success();
        }
        case pto::SyncCoreType::AICOnly: {
            FailureOr<Value> l1Workspace =
                buildSyncAllWorkspaceTileValue(rewriter, op.getLoc(), op.getL1Workspace(), adaptor.getL1Workspace());
            if (failed(l1Workspace))
                return rewriter.notifyMatchFailure(op, "failed to materialize l1_workspace tile");
        ...
        case pto::SyncCoreType::Mix: {
            ...
            if (failed(ubWorkspace) || failed(l1Workspace)) {
                return rewriter.notifyMatchFailure(op, "failed to materialize mixed syncall workspace tiles");
            }
            operands.push_back(*ubWorkspace);
            operands.push_back(*l1Workspace);
            return success();
...
static constexpr llvm::StringLiteral kAutoSyncTailBarrierAttr = "pto.auto_sync_tail_barrier";
static constexpr llvm::StringLiteral kAutoSyncTailHintAttr = "pto.auto_sync_tail_hint";
static constexpr llvm::StringLiteral kAutoSyncTailPolicyBarrierAll = "barrier_all";
static constexpr llvm::StringLiteral kAutoSyncTailPolicyMte3ToSEvent0 = "setwait_mte3_to_s_event0";
static constexpr llvm::StringLiteral kAutoSyncTailModeBarrierAllToken = "PTOAutoSyncTailMode::kBarrierAll";
static constexpr llvm::StringLiteral kAutoSyncTailModeMte3ToSEvent0Token = "PTOAutoSyncTailMode::kSetWaitMte3ToSEvent0";
...
struct PTOBarrierToEmitC : public OpConversionPattern<pto::BarrierOp> {
    ...
    LogicalResult matchAndRewrite(
        pto::BarrierOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
static bool tryAssignSyncTokens(
    Attribute srcAttr, Attribute dstAttr, Attribute evtAttr, std::string& srcTok, std::string& dstTok,
    std::string& evtTok)
{
    std::string localSrc;
    std::string localDst;
    std::string localEvt;
    if (!tryConvertPipeAttrToToken(srcAttr, localSrc) || !tryConvertPipeAttrToToken(dstAttr, localDst) ||
...
static bool tryExtractSyncTokensFromNamedAttrs(
    Operation* op, StringRef srcName, StringRef dstName, StringRef evtName, std::string& srcTok, std::string& dstTok,
    std::string& evtTok)
{
    return tryAssignSyncTokens(
        op->getAttr(srcName), op->getAttr(dstName), op->getAttr(evtName), srcTok, dstTok, evtTok);
}

static bool tryExtractSyncTokensFromArrayAttr(
    Operation* op, StringRef attrName, std::string& srcTok, std::string& dstTok, std::string& evtTok)
{
    auto arrayAttr = op->getAttrOfType<ArrayAttr>(attrName);
    if (!arrayAttr || arrayAttr.size() < kNumber3)
        return false;
    return tryAssignSyncTokens(arrayAttr[0], arrayAttr[1], arrayAttr[2], srcTok, dstTok, evtTok);
}

static bool tryExtractFallbackSyncTokens(Operation* op, std::string& srcTok, std::string& dstTok, std::string& evtTok)
{
...
static LogicalResult extractSyncTripletTokens(
    Operation* op, std::string& srcTok, std::string& dstTok, std::string& evtTok, ConversionPatternRewriter& rewriter)
{
    if (tryExtractSyncTokensFromNamedAttrs(op, "src_pipe", "dst_pipe", "event_id", srcTok, dstTok, evtTok) ||
        tryExtractSyncTokensFromNamedAttrs(op, "srcPipe", "dstPipe", "eventId", srcTok, dstTok, evtTok) ||
        tryExtractSyncTokensFromNamedAttrs(op, "src", "dst", "event", srcTok, dstTok, evtTok)) {
        return success();
    }

    for (StringRef attrName : {"args", "pipes", "sync", "triplet", "attrs"}) {
        if (tryExtractSyncTokensFromArrayAttr(op, attrName, srcTok, dstTok, evtTok)) {
            return success();
        }
    }
...
static inline std::string pipeTokFromPipeEnum(mlir::pto::PIPE p) { return mlir::pto::stringifyPIPE(p).str(); }
...
static LogicalResult extractSyncTokens(
    SyncOpT op, std::string& srcTok, std::string& dstTok, std::string& evtTok, ConversionPatternRewriter& rewriter)
{
    if constexpr (HasGetSrcPipe<SyncOpT>::value && HasGetDstPipe<SyncOpT>::value && HasGetEventId<SyncOpT>::value) {
    ...
    if constexpr (
        HasGetSrcPipeAttr<SyncOpT>::value && HasGetDstPipeAttr<SyncOpT>::value && HasGetEventIdAttr<SyncOpT>::value) {
...
static ArrayAttr buildSyncTokenArgsAttr(
    ConversionPatternRewriter& rewriter, StringRef srcTok, StringRef dstTok, StringRef evtTok)
{
    auto* ctx = rewriter.getContext();
    return rewriter.getArrayAttr({
...
static LogicalResult lowerSyncFlagLikeOp(SyncFlagOp op, ConversionPatternRewriter& rewriter, StringRef callee)
{
...
struct PTOSetFlagToEmitC : public OpConversionPattern<mlir::pto::SetFlagOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::SetFlagOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
...
struct PTOWaitFlagToEmitC : public OpConversionPattern<mlir::pto::WaitFlagOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::WaitFlagOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
...
struct PTOSyncToEmitC : public OpConversionPattern<mlir::pto::TSyncOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::TSyncOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
...
struct PTOSyncAllToEmitC : public OpConversionPattern<mlir::pto::SyncAllOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::SyncAllOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (mode == pto::SyncAllMode::Hard) {
            ...
            rewriter.create<emitc::CallOpaqueOp>(
                op.getLoc(), TypeRange{}, callee, ArrayAttr{}, ArrayAttr{}, ValueRange{});
            rewriter.eraseOp(op);
            return success();
        }
        ...
        if (failed(gmWorkspace))
            return rewriter.notifyMatchFailure(op, "failed to build gm_workspace GlobalTensor");
        Value usedCores = buildSyncAllUsedCores(rewriter, op, adaptor);

        std::string callee = "SYNCALL<SyncAllMode::Soft, " + coreTypeTok(coreType).str() + ">";

        SmallVec4<Value> operands{*gmWorkspace};
        if (failed(appendSyncAllWorkspaceOperands(rewriter, op, adaptor, coreType, operands)))
            return failure();

        operands.push_back(usedCores);
        rewriter.create<emitc::CallOpaqueOp>(
            op.getLoc(), TypeRange{}, callee, ArrayAttr{}, ArrayAttr{}, ValueRange(operands));
        rewriter.eraseOp(op);
        return success();
    }
};
...
struct PTOSyncFlagDynToEmitC : public ConversionPattern {
    PTOSyncFlagDynToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, StringRef opName, StringRef callee)
        : ConversionPattern(typeConverter, opName, /*benefit=*/1, ctx), callee(callee.str())
    {}
        ...
        Operation* op, ArrayRef<Value> operands, ConversionPatternRewriter& rewriter) const override
    {
        ...
        eventVal = emitCCast(rewriter, op->getLoc(), emitc::OpaqueType::get(ctx, "event_t"), eventVal);
...
struct PTOGetBufToEmitC : public OpConversionPattern<mlir::pto::GetBufOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::GetBufOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        auto* ctx = rewriter.getContext();
...
struct PTORlsBufToEmitC : public OpConversionPattern<mlir::pto::RlsBufOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::RlsBufOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        (void)adaptor;
        auto* ctx = rewriter.getContext();
...
struct PTOSetFFTsToEmitC : public OpConversionPattern<mlir::pto::SetFFTsOp> {
    ...
    LogicalResult matchAndRewrite(
        mlir::pto::SetFFTsOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto* ctx = rewriter.getContext();
        auto loc = op.getLoc();
        ...
        if (isSetFFTsPointerLikeType(fftsAddr.getType())) {
            auto castTyAttr = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, "uint64_t")});
            fftsAddr = rewriter
                           .create<emitc::CallOpaqueOp>(
            ...
            fftsAddr = rewriter.create<emitc::CastOp>(loc, u64Ty, fftsAddr).getResult();
        }
...
struct PTOSyncSetToEmitC : public OpConversionPattern<mlir::pto::SyncSetOp> {
    PTOSyncSetToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::SyncSetOp>(typeConverter, ctx), targetArch(targetArch)
    {}

    LogicalResult matchAndRewrite(
        mlir::pto::SyncSetOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op->getLoc();
        ...
        InterCoreSyncCallDesc desc;
        if (eventIdAttr) {
            desc = buildInterCoreSyncSetCall(rewriter, loc, targetArch, op.getPipe(), eventIdAttr, fftsMode);
        } else {
            desc = buildInterCoreSyncSetCallDyn(rewriter, loc, targetArch, op.getPipe(), eventIdDyn, fftsMode);
        }
        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, desc.callee,
            /*args=*/desc.args,
            /*templateArgs=*/ArrayAttr{},
...
struct PTOSyncWaitToEmitC : public OpConversionPattern<mlir::pto::SyncWaitOp> {
    PTOSyncWaitToEmitC(TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
        : OpConversionPattern<mlir::pto::SyncWaitOp>(typeConverter, ctx), targetArch(targetArch)
    {}

    LogicalResult matchAndRewrite(
        mlir::pto::SyncWaitOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op->getLoc();
        IntegerAttr eventIdAttr = op.getEventIdAttr();
        Value eventIdDyn = adaptor.getEventIdDyn();
        ...
        InterCoreSyncCallDesc desc;
        if (eventIdAttr) {
            desc = buildInterCoreSyncWaitCall(rewriter, targetArch, op.getPipe(), eventIdAttr);
        } else {
            desc = buildInterCoreSyncWaitCallDyn(rewriter, loc, targetArch, op.getPipe(), eventIdDyn);
        }
        rewriter.create<emitc::CallOpaqueOp>(loc, TypeRange{}, desc.callee, desc.args, ArrayAttr{}, desc.operands);
...
void populatePTOToEmitCSyncPatterns(
    RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx, PTOArch targetArch)
{
    ...
    patterns.add<PTOSyncFlagDynToEmitC>(typeConverter, ctx, "pto.set_flag_dyn", "set_flag");
    patterns.add<PTOSyncFlagDynToEmitC>(typeConverter, ctx, "pto.wait_flag_dyn", "wait_flag");
    patterns.add<PTOSyncFlagDynToEmitC>(typeConverter, ctx, "pto.set_flag_d", "set_flag");
    patterns.add<PTOSyncFlagDynToEmitC>(typeConverter, ctx, "pto.wait_flag_d", "wait_flag");
