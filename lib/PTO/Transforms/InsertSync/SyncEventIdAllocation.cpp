...
static size_t getEventIdPoolSize(const SyncOperation* sync, uint64_t reservedBlockSyncEventIdNum)
{
    ...
    for (auto pipePair : reallocatedPipePair) {
        for (auto& e : syncIR_) {
            for (auto& sync : e->pipeAfter) {
                if (sync->isSyncSetType() && !sync->uselessSync && (ScopePair(sync) == pipePair)) {
...
size_t SyncEventIdAllocation::GetCompilerAvailableEventIdNum(const SyncOperation* sync) const
{
    if (sync->GetType() == SyncOperation::TYPE::SYNC_BLOCK_SET ||
        sync->GetType() == SyncOperation::TYPE::SYNC_BLOCK_WAIT) {
        return kBlockSyncSetWaitEventIdNum - reservedBlockSyncEventIdNum;
...
void SyncEventIdAllocation::SetEventId(SyncOperation* sync)
{
    ...
    SmallVector<int> canAllocaEventId =
        GetAvailableEventId(sync, eventIdLifetimeAvailableStatus, eventIdIdleStatus, poolSize);
    ...
    } else if ((reallocatedPipePair.count(ScopePair(sync)) != 0) && (canAllocaEventId.size() < idSize)) {
...
SmallVector<int> SyncEventIdAllocation::UpdateBlockAvailableEventId(
    const SyncOperation* sync, SmallVector<bool> eventIdLifetimeAvailableStatus, size_t eventIdNum) const
{
    SmallVector<int> canAllocaEventId;
    size_t idSize = static_cast<size_t>(sync->eventIdNum);
...
SmallVector<int> SyncEventIdAllocation::GetAvailableEventId(
    const SyncOperation* sync, SmallVector<bool> eventIdLifetimeAvailableStatus, SmallVector<bool> eventIdIdleStatus,
...
SmallVector<bool> SyncEventIdAllocation::GetEventIdIdleStatus(SyncOperation* sync, size_t eventIdNum)
{
...
SmallVector<bool> SyncEventIdAllocation::GetEventPool(const SyncOperation* sync, size_t eventIdNum)
{
    ...
    if (setFlag->GetForEndIndex().has_value()) {
        ...
        FindUseEventID(setFlag->GetSyncIRIndex(), waitFlag->GetSyncIRIndex(), setFlag, eventIdPool);
    }
    return eventIdPool;
}
...
{
    if (s->GetType() == SyncOperation::TYPE::SYNC_BLOCK_SET || s->GetType() == SyncOperation::TYPE::SYNC_BLOCK_WAIT) {
        return 0;
    }
...
void SyncEventIdAllocation::FindUseEventID(
    unsigned int begin, unsigned int end, const SyncOperation* s, SmallVector<bool>& eventId)
{
...
bool SyncEventIdAllocation::CheckSyncLifeCycleConflict(
    SmallVector<unsigned int>& syncLifeCycle, unsigned int begin, unsigned int end, SmallVector<bool>& eventId,
    unsigned i) const
{
...
void SyncEventIdAllocation::UpdateEventId(
    SmallVector<unsigned int>& syncLifeCycle, const unsigned int begin, const unsigned int end,
...
void SyncEventIdAllocation::SetEventPool(const SyncOperation* sync, unsigned eventId)
{
    assert(sync->GetSyncIndex() < syncOperations_.size());
    auto& syncPair = syncOperations_[sync->GetSyncIndex()];
    ...
    if (setFlag->GetForEndIndex().has_value()) {
        ...
        SetUseEventID(setFlag->GetSyncIRIndex(), waitFlag->GetSyncIRIndex(), setFlag.get(), eventId);
    }
...
void SyncEventIdAllocation::UpdateBackwardMatchSync(
    const SyncOperation* setFlag, const SyncOperation* waitFlag, unsigned eventId)
{
    // Creating new sync pair for backward match
    auto syncFront = std::make_unique<SyncOperation>(
        setFlag->GetType(), setFlag->GetSrcPipe(), setFlag->GetDstPipe(), static_cast<unsigned>(syncOperations_.size()),
...
void SyncEventIdAllocation::SetUseEventID(
    unsigned int begin, unsigned int end, const SyncOperation* setFlag, unsigned int eventId)
{
    assert(begin < end);
    int scopePair = ScopePair(setFlag);
    const size_t poolSize = getEventIdPoolSize(setFlag, reservedBlockSyncEventIdNum);
...
bool SyncEventIdAllocation::ExtendLifecycle(
    SmallVector<unsigned int>& syncLifeCycle, unsigned int beginNew, unsigned int endNew) const
{
    for (size_t j = 0; j < syncLifeCycle.size() / 2U; j++) {
        ...
        bool insertMiddleLifecycle =
            j < ((syncLifeCycle.size() / 2U) - 1) && beginOld >= endNew && beginNew >= syncLifeCycle[(j + 1) * 2U];
...
void SyncEventIdAllocation::ReallocatedEventId()
{
    ...
    for (auto& e : syncIR_) {
        for (auto& sync : e->pipeBefore) {
            if (!sync->isBarrierType() && reallocatedPipePair.count(ScopePair(sync)) != 0) {
                ClearEventId(sync);
                SetEventId(sync);
            }
        }
...
void SyncEventIdAllocation::ClearReallocatedBackwardMatchSync()
{
    ...
    for (auto& sync : syncIR_[0]->pipeBefore) {
        if (!sync->isSyncSetType() || reallocatedPipePair.count(ScopePair(sync)) == 0) {
            newPipeBefore.push_back(sync);
        }
    }
    ...
    for (auto& sync : syncIR_[syncIR_.size() - 1]->pipeAfter) {
        if (!sync->isSyncWaitType() || reallocatedPipePair.count(ScopePair(sync)) == 0) {
            newPipeAfter.push_back(sync);
        }
    }
...
llvm::LogicalResult SyncEventIdAllocation::ChangeNoEventIdSyncToPipeAll()
{
    for (auto& e : syncIR_) {
        for (auto& sync : e->pipeAfter) {
            if (sync->GetType() == SyncOperation::TYPE::WAIT_EVENT && sync->reallocatedLoopHeadTailSync) {
                MoveOutBackwardMatchSync(sync);
            }
            if (sync->GetType() == SyncOperation::TYPE::SET_EVENT && sync->eventIds.empty() && !sync->uselessSync) {
                // Fallback to PipeAll
                auto& syncPair = syncOperations_[sync->GetSyncIndex()];
                syncPair[0]->uselessSync = true;
                syncPair[1]->SetPipeAll();
            }
            if (sync->GetType() == SyncOperation::TYPE::SYNC_BLOCK_SET && sync->eventIds.empty() &&
                !sync->uselessSync) {
                return failure();
            }
        }
...
void SyncEventIdAllocation::MoveOutBackwardMatchSync(const SyncOperation* reallocatedSync)
{
    ...
    for (unsigned int i = 0; i <= syncIR_.size() - 1; i++) {
        ...
        for (auto& sync : syncIR_[i]->pipeBefore) {
            if (!sync->uselessSync && reallocatedSync->GetSyncIndex() != sync->GetSyncIndex() &&
...
void SyncEventIdAllocation::IgnoreBackHeadAndTailSync()
{
    ...
    for (auto& sync : syncIR_[0]->pipeBefore) {
        ...
        bool isPipeMTE1ToPipeMSync =
            sync->GetSrcPipe() == PipelineType::PIPE_M && sync->GetDstPipe() == PipelineType::PIPE_MTE1;
...
SyncOperation* SyncEventIdAllocation::FindWidenSync(const SyncOperation* setSync, const SyncOperation* waitSync)
{
    ...
    for (int loopId = static_cast<int>(setSync->GetSyncIRIndex()); loopId >= endIndex; loopId--) {
        ...
        for (auto& setSame : tmpIr->pipeAfter) {
            ...
            bool isSameTypeSync = (setSame != setSync) && (setSame->GetDstPipe() == setSync->GetDstPipe()) &&
                                  (setSame->GetSrcPipe() == setSync->GetSrcPipe());

            bool sameLoopScope = (setSame->GetForEndIndex() == setSync->GetForEndIndex());
            if (!isSameTypeSync || !sameLoopScope || setSame->uselessSync || setSame->eventIds.empty()) {
                continue;
            }
...
const llvm::DenseMap<std::pair<PipelineType, PipelineType>, uint64_t> SyncEventIdAllocation::reservedEventIdNum = {
