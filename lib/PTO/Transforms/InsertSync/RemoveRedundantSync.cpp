...
void RemoveRedundantSync::Run()
{
    ...
    std::sort(
        syncOps.begin(), syncOps.end(),
        [](std::pair<SyncOperation*, SyncOperation*> syncPair1, std::pair<SyncOperation*, SyncOperation*> syncPair2) {
...
bool RemoveRedundantSync::CheckAllSync(const SyncOperation* setFlag, const SyncOperation* waitFlag)
{
    ...
    if (begin < end) {
        ...
        auto* loopElement = dyn_cast<LoopInstanceElement>(syncIR_[forEndIndex.value()].get());
...
bool RemoveRedundantSync::CheckRepeatSync(
    unsigned int begin, unsigned int end, SmallVector<bool>& syncFinder, const SyncOperation* setFlag)
{
...
bool RemoveRedundantSync::CheckBranchBetween(
    BranchInstanceElement* branchElement, SmallVector<bool> syncFinder, const SyncOperation* setFlag, unsigned endId,
    unsigned& i)
{
    // 只处理 IF_BEGIN
    ...
    bool endIsInsideThenBranch =
        (!hasElseBranch && endId < branchElement->endId) || (hasElseBranch && endId < branchElement->branchId);
    if (endIsInsideThenBranch)
        return false;

    bool endIsInsideElseBranch = hasElseBranch && endId >= branchElement->branchId && endId < branchElement->endId;
...
bool RemoveRedundantSync::CheckLoopBetween(
    LoopInstanceElement* loopElement, const SyncOperation* setFlag, unsigned& i) const
{
    (void)setFlag;
...
bool RemoveRedundantSync::CanMatchedSync(
    SmallVector<bool>& syncFinder, const SyncOperation* relatedSync, const SyncOperation* setFlag) const
{
    ...
    if (syncAnalysisMode_ == SyncAnalysisMode::BLOCKSYNC) {
        isWait = isWait || (relatedSync->GetType() == SyncOperation::TYPE::SYNC_BLOCK_WAIT);
        isSet = isSet || (relatedSync->GetType() == SyncOperation::TYPE::SYNC_BLOCK_SET);
    }
