    ...
    for (auto& e : syncIR_) {
        if (auto* branchElement = dyn_cast<BranchInstanceElement>(e.get())) {
            ...
            if (branchElement->getBranchKind() == KindOfBranch::IF_BEGIN) {
                std::pair<unsigned, unsigned> bound = {branchElement->beginId, branchElement->endId};

                // 1. 遍历 THEN 分支内的指令
                for (unsigned i = branchElement->beginId + 1; i < branchElement->branchId; i++) {
                    PlanMoveOutBranchSync(syncIR_[i].get(), {branchElement->beginId, branchElement->branchId}, bound);
                }
                ...
                // 2. 遍历 ELSE 分支内的指令
                for (unsigned i = branchElement->branchId + 1; i < branchElement->endId; i++) {
                    PlanMoveOutBranchSync(syncIR_[i].get(), {branchElement->branchId, branchElement->endId}, bound);
                }
            }
        }
    }
...
void MoveSyncState::PlanMoveOutBranchSync(
    InstanceElement* e, std::pair<unsigned int, unsigned int> pair, std::pair<unsigned int, unsigned int> bound)
{
...
void MoveSyncState::PlanMoveOutIfWaitSync(
    SyncOps& newPipeBefore, SyncOperation* s, std::pair<unsigned int, unsigned int> pair,
    std::pair<unsigned int, unsigned int> bound)
{
    // 只处理 WaitEvent
    if (s->GetType() != SyncOperation::TYPE::WAIT_EVENT && s->GetType() != SyncOperation::TYPE::SYNC_BLOCK_WAIT) {
    ...
    if ((setSync->GetSyncIRIndex() >= pair.second) || (setSync->GetSyncIRIndex() <= pair.first)) {
...
void MoveSyncState::PlanMoveOutIfSetSync(
    SyncOps& newPipeAfter, SyncOperation* s, std::pair<unsigned int, unsigned int> pair,
    std::pair<unsigned int, unsigned int> bound)
{
    if (s->GetType() != SyncOperation::TYPE::SET_EVENT && s->GetType() != SyncOperation::TYPE::SYNC_BLOCK_SET) {
    ...
    if ((waitSync->GetSyncIRIndex() >= pair.second) || (waitSync->GetSyncIRIndex() <= pair.first)) {
        ...
        if (auto* forCompound = dyn_cast<LoopInstanceElement>(e.get())) {
            ...
            if (forCompound->getLoopKind() == KindOfLoop::LOOP_END) {
                ...
                for (unsigned i = forCompound->beginId + 1; i < forCompound->endId; i++)
                    MoveOutSync(syncIR_[i].get(), {forCompound->beginId, forCompound->endId});
            }
        }
    }
}

void MoveSyncState::MoveOutSync(InstanceElement* e, std::pair<unsigned int, unsigned int> pair)
{
    checkCondition(
    ...
    SyncOps& newPipeBefore, SyncOperation* s, std::pair<unsigned int, unsigned int> pair)
{
    ...
    if (s->GetForEndIndex().has_value() && static_cast<unsigned>(s->GetForEndIndex().value()) == pair.second) {
        newPipeBefore.push_back(s);
        return;
    }
...
void MoveSyncState::PlanMoveOutSetSync(
    SyncOps& newPipeAfter, SyncOperation* s, const std::pair<unsigned int, unsigned int> pair)
{
    if (s->GetType() != SyncOperation::TYPE::SET_EVENT && s->GetType() != SyncOperation::TYPE::SYNC_BLOCK_SET) {
        newPipeAfter.push_back(s);
        return;
