...
using MemoryEffectVector = SmallVector<SideEffects::EffectInstance<MemoryEffects::Effect>, kMemoryEffectInlineCapacity>;
...
{
    return shape.size() == kTileRank2D && llvm::none_of(shape, [](int64_t dim) { return dim == ShapedType::kDynamic; });
}
...
static int64_t getTileMajorStride(pto::TileBufType type)
{
    ...
    bool rowMajor = type.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::RowMajor);
...
static std::optional<SmallVector<uint64_t>> getPtoSubViewBaseAddresses(
    pto::SubViewOp op, pto::TileBufType sourceType, int64_t elemBytes)
{
    if (!isStaticRank2Shape(sourceType.getShape()))
        return std::nullopt;
    if (sourceType.getSLayoutValueI32() != static_cast<int32_t>(pto::SLayout::NoneBox))
        return std::nullopt;
    ...
    int64_t rowOffset = 0;
    int64_t colOffset = 0;
    if (!getConstIndexValue(op.getOffsets()[0], rowOffset) || !getConstIndexValue(op.getOffsets()[1], colOffset))
        return std::nullopt;
    ...
    bool rowMajor = sourceType.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::RowMajor);
...
static std::optional<SmallVector<uint64_t>> buildMemrefSubViewBaseAddresses(
    ArrayRef<int64_t> strides, int64_t rowOffset, int64_t colOffset, int64_t rowSize, int64_t colSize,
    int64_t elemBytes)
{
    SmallVector<uint64_t> addresses;
    if (strides[1] == 1) {
        addresses.reserve(static_cast<size_t>(rowSize));
        for (int64_t row = 0; row < rowSize; ++row) {
            int64_t elemOffset = (rowOffset + row) * strides[0] + colOffset * strides[1];
            addresses.push_back(static_cast<uint64_t>(elemOffset * elemBytes));
        }
        return addresses;
    }
    if (strides[0] == 1) {
        addresses.reserve(static_cast<size_t>(colSize));
        for (int64_t col = 0; col < colSize; ++col) {
            int64_t elemOffset = rowOffset * strides[0] + (colOffset + col) * strides[1];
            addresses.push_back(static_cast<uint64_t>(elemOffset * elemBytes));
        }
        return addresses;
    }
...
static bool parseStaticMemrefSubViewWindow(
    memref::SubViewOp op, int64_t& rowOffset, int64_t& colOffset, int64_t& rowSize, int64_t& colSize)
{
...
static std::optional<SmallVector<uint64_t>> getMemrefSubViewBaseAddresses(
    memref::SubViewOp op, MemRefType sourceType, int64_t elemBytes)
{
    ...
    if (failed(mlir::getStridesAndOffset(sourceType, strides, baseOffset)) || strides.size() != kTileRank2D ||
    ...
    int64_t rowSize = 0;
    int64_t colSize = 0;
    if (!parseStaticMemrefSubViewWindow(op, rowOffset, colOffset, rowSize, colSize))
        return std::nullopt;
    return buildMemrefSubViewBaseAddresses(strides, rowOffset, colOffset, rowSize, colSize, elemBytes);
}
...
static std::pair<int64_t, int64_t> getStaticOffsetAndSize(Operation* op, Value src)
{
    ...
    int64_t elemSize = static_cast<int64_t>(pto::getPTOStorageElemByteSize(srcType.getElementType()));
    if (elemSize == 0)
        return {-1, -1};
...
{
    auto result = region->walk<WalkOrder::PreOrder>([this](Operation* op) { return TranslateOperation(op); });
...
LogicalResult PTOIRTranslator::UpdateAllocTileOpMemInfo(pto::AllocTileOp op)
{
    ...
    if (tileType) {
        ...
        if (isStatic) {
            int64_t elemSize = static_cast<int64_t>(pto::getPTOStorageElemByteSize(tileType.getElementType()));
            if (elemSize == 0)
                return failure();
    ...
    auto newMemInfo = std::make_unique<BaseMemInfo>(
        res, res,
        space, // 使用解析出的 space
        SmallVector<uint64_t>{baseAddr}, sizeInBytes);
...
LogicalResult PTOIRTranslator::UpdatePointerCastOpMemInfo(pto::PointerCastOp op)
{
    ...
    if (memRefType.hasStaticShape()) {
        int64_t elemSize = static_cast<int64_t>(pto::getPTOStorageElemByteSize(memRefType.getElementType()));
        if (elemSize == 0)
            return failure();
        int64_t numElements = 1;
    ...
    auto newMemInfo = std::make_unique<BaseMemInfo>(res, rootSrc, space, SmallVector<uint64_t>{0}, sizeInBytes);

    buffer2MemInfoMap_[res].emplace_back(newMemInfo->clone());
    return success();
}

LogicalResult PTOIRTranslator::UpdateDeclareTileMemRefOpMemInfo(pto::DeclareTileMemRefOp op)
{
    Value res = op.getResult();
    auto memRefType = dyn_cast<MemRefType>(res.getType());
    if (!memRefType)
    ...
    auto newMemInfo = std::make_unique<BaseMemInfo>(res, res, space, SmallVector<uint64_t>{0}, sizeInBytes);
...
void PTOIRTranslator::UpdatePTOOpInfo(Operation* op)
{
    ...
    if (auto memEffect = dyn_cast<MemoryEffectOpInterface>(op)) {
        ...
        LLVM_DEBUG(llvm::dbgs() << "Warning: Op " << op->getName() << " has Pipe but no MemoryEffects interface.\n");
    }

    // 3. 构建 Compound Node
    auto compoundElement = std::make_unique<CompoundInstanceElement>(index, defVec, useVec, pipe, op->getName());
...
void PTOIRTranslator::UpdateConservativeAliasBufferInfo(Value result, Value source)
{
    if (!result || !source)
        return;
    if (!buffer2MemInfoMap_.contains(source))
...
void PTOIRTranslator::CloneSubviewAliasInfos(
    Value result, Value source, ArrayRef<uint64_t> subViewAddresses, uint64_t segmentSize)
{
    auto& resultMemInfoVec = buffer2MemInfoMap_[result];
    for (auto& parentInfo : buffer2MemInfoMap_[source]) {
        auto newInfo = parentInfo->clone(result);
...
void PTOIRTranslator::UpdateMemrefSubViewAliasBufferInfo(memref::SubViewOp op)
{
    ...
    auto subViewAddresses =
        elemBytes == 0 ? std::nullopt : getMemrefSubViewBaseAddresses(op, sourceType, static_cast<int64_t>(elemBytes));
    if (!subViewAddresses || subViewAddresses->empty()) {
        UpdateConservativeAliasBufferInfo(result, source);
        return;
    }

    auto getSegmentSize = [&op, sourceType, elemBytes]() -> std::optional<uint64_t> {
        SmallVector<int64_t> strides;
        int64_t baseOffset = ShapedType::kDynamic;
        if (failed(mlir::getStridesAndOffset(sourceType, strides, baseOffset)) || strides.size() != kTileRank2D) {
            return std::nullopt;
        ...
        ArrayRef<int64_t> staticSizes = op.getStaticSizes();
        if (strides[1] == 1) {
            return static_cast<uint64_t>(staticSizes[1] * static_cast<int64_t>(elemBytes));
        }
        if (strides[0] == 1) {
            return static_cast<uint64_t>(staticSizes[0] * static_cast<int64_t>(elemBytes));
        }
        return std::nullopt;
    };
...
void PTOIRTranslator::UpdateTileSubViewAliasBufferInfo(pto::SubViewOp op)
{
    ...
    unsigned elemBytes = pto::getPTOStorageElemByteSize(sourceType.getElementType());
    auto subViewAddresses =
        elemBytes == 0 ? std::nullopt : getPtoSubViewBaseAddresses(op, sourceType, static_cast<int64_t>(elemBytes));
    if (!subViewAddresses || subViewAddresses->empty()) {
        UpdateConservativeAliasBufferInfo(result, source);
        return;
    }
    ...
    int64_t colSize = cast<IntegerAttr>(sizesAttr[1]).getInt();
    bool rowMajor = sourceType.getBLayoutValueI32() == static_cast<int32_t>(pto::BLayout::RowMajor);
    uint64_t segmentSize = static_cast<uint64_t>((rowMajor ? colSize : rowSize) * static_cast<int64_t>(elemBytes));

    for (auto& parentInfo : buffer2MemInfoMap_[source]) {
        if (!parentInfo || parentInfo->baseAddresses.size() != 1 || parentInfo->allocateSize == 0) {
            UpdateConservativeAliasBufferInfo(result, source);
            return;
        }
    }
...
void PTOIRTranslator::printMemInfoList(
    llvm::raw_ostream& os, const SmallVector<const BaseMemInfo*>& list, AsmState& state) const
{
    os << "[";
    bool first = true;
