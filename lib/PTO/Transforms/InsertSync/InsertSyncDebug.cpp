...
static void dumpEventIds(llvm::raw_ostream& os, const SmallVector<int>& eventIds)
{
    os << "[";
...
static void dumpSyncOp(llvm::raw_ostream& os, const SyncOperation* op, bool showUselessSync)
{
    if (!op)
        return;
    ...
    os << " <" << getPipelineName(op->GetSrcPipe()) << " -> " << getPipelineName(op->GetDstPipe()) << ">";
...
static void dumpMemInfoList(
    llvm::raw_ostream& os, llvm::StringRef tag, const SmallVector<const BaseMemInfo*>& list, mlir::AsmState* state)
{
    os << tag << "=[";
...
static void adjustIndentBeforeElement(const InstanceElement* element, int& indent)
{
    ...
    if (auto* branch = dyn_cast<BranchInstanceElement>(element)) {
        if (branch->getBranchKind() == KindOfBranch::IF_END || branch->getBranchKind() == KindOfBranch::ELSE_BEGIN)
            indent = std::max(0, indent - 1);
    }
}

static void adjustIndentAfterElement(const InstanceElement* element, int& indent)
{
    if (auto* loop = dyn_cast<LoopInstanceElement>(element)) {
    ...
    if (auto* branch = dyn_cast<BranchInstanceElement>(element)) {
        if (branch->getBranchKind() == KindOfBranch::IF_BEGIN || branch->getBranchKind() == KindOfBranch::ELSE_BEGIN)
            indent += 1;
    }
}

static void dumpCompoundMemInfo(
    llvm::raw_ostream& os, const CompoundInstanceElement* comp, unsigned indent, mlir::AsmState* state)
{
    os.indent(indent);
...
static void dumpSyncIRElementHeader(llvm::raw_ostream& os, const InstanceElement* element)
{
    switch (element->GetKind()) {
        case InstanceElement::KindTy::COMPOUND: {
            auto* comp = cast<CompoundInstanceElement>(element);
            os << "COMPOUND " << comp->opName.getStringRef() << " [" << getPipelineName(comp->kPipeValue) << "]\n";
            break;
        }
        case InstanceElement::KindTy::LOOP: {
            auto* loop = cast<LoopInstanceElement>(element);
            os << "LOOP " << getLoopKindName(loop->getLoopKind()) << " (begin=" << loop->beginId
               << ", end=" << loop->endId << ")\n";
            break;
        }
        case InstanceElement::KindTy::BRANCH: {
            auto* branch = cast<BranchInstanceElement>(element);
            os << "BRANCH " << getBranchKindName(branch->getBranchKind()) << " (begin=" << branch->beginId
               << ", branch=" << branch->branchId << ", end=" << branch->endId << ")\n";
            break;
        }
...
static void dumpSyncOps(
    llvm::raw_ostream& os, llvm::StringRef prefix, const SyncOps& ops, unsigned indent, InsertSyncDumpOptions options)
{
...
static void dumpSyncIR(
    llvm::raw_ostream& os, const SyncIRs& syncIR, Operation* opForPrinting, InsertSyncDumpOptions options,
    bool showMemInfo)
{
    ...
    int indent = 0;
    auto indentBy = [&indent](int extra = 0) { return static_cast<unsigned>(std::max(0, indent) * 2 + extra); };
    ...
    for (const auto& e : syncIR) {
        ...
        if (showMemInfo && isa<CompoundInstanceElement>(e.get())) {
            dumpCompoundMemInfo(
                os, cast<CompoundInstanceElement>(e.get()), indentBy(kDebugDumpIndentSpaces),
                state ? &*state : nullptr);
        }
        dumpSyncOps(os, "PRE ", e->pipeBefore, indentBy(kDebugDumpIndentSpaces), options);
        dumpSyncOps(os, "POST", e->pipeAfter, indentBy(kDebugDumpIndentSpaces), options);
...
static void updateSyncOperationStats(const SyncOperation* op, SyncOperationStats& stats)
{
...
static SyncOperationStats collectSyncOperationStats(const SyncOperations& syncOperations)
{
...
static void dumpInsertSyncPhaseSummary(
    llvm::raw_ostream& os, llvm::StringRef phase, const SyncIRs& syncIR, const SyncOperations& syncOperations,
    const SyncOperationStats& stats)
{
    os << "\n// === [PTOInsertSync Debug] " << phase << " === //\n";
    os << llvm::formatv(
        "// nodes={0}, syncGroups={1}, activeOps={2} "
        "(set={3}, wait={4}, barrier={5}, blockSet={6}, "
        "blockWait={7}, blockAll={8})\n",
        syncIR.size(), syncOperations.size(), stats.activeOps, stats.setCnt, stats.waitCnt, stats.barrierCnt,
        stats.blockSetCnt, stats.blockWaitCnt, stats.blockAllCnt);
}

void mlir::pto::dumpInsertSyncPhase(
    llvm::StringRef phase, const SyncIRs& syncIR, const SyncOperations& syncOperations, Operation* opForPrinting,
    llvm::raw_ostream& os)
{
    ...
    const bool showMemInfo = level >= static_cast<unsigned>(InsertSyncDebugLevel::Trace);
