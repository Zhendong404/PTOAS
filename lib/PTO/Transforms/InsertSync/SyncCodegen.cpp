...
static bool IsSameSyncSignature(const SyncOperation* existing, const SyncOperation* candidate)
{
...
static Operation* resolveSyncInsertAnchor(Operation* op, const SyncOperation* sync)
{
...
static bool shouldInsertBefore(Operation* op, bool beforeInsert, const SyncOperation* sync)
{
    return beforeInsert || sync->isCompensation || op->hasTrait<OpTrait::IsTerminator>();
}

static void setSyncInsertionPoint(IRRewriter& rewriter, Operation* op, bool insertBefore)
{
    if (insertBefore) {
...
static bool hasNeighborBarrier(Block* block, Block::iterator ip, pto::PipeAttr pipeAttr, bool insertBefore)
{
    if (insertBefore) {
        if (ip == block->begin())
            return false;
...
static void createSetOrWaitFlagOp(
    IRRewriter& rewriter, Operation* op, const SyncOperation* sync, pto::PipeAttr srcPipe, pto::PipeAttr dstPipe,
...
void SyncCodegen::SyncInsert(IRRewriter& rewriter, Operation* op, SyncOperation* sync, bool beforeInsert)
{
    if (sync->uselessSync)
        return;
...
void SyncCodegen::CreateBarrierOp(IRRewriter& rewriter, Operation* op, const SyncOperation* sync, bool beforeInsert)
{
    ...
    if (isTargetArchA5(func_.getOperation()) && sync->GetActualSrcPipe() == PipelineType::PIPE_V) {
        return;
    }
...
void SyncCodegen::AppendAutoSyncTailBarrierIfNeeded(IRRewriter& rewriter)
{
    ...
    for (auto ret : returns) {
        ...
        if (auto hintAttr = func_->getAttrOfType<mlir::StringAttr>("pto.auto_sync_tail_hint")) {
...
void SyncCodegen::CreateSetWaitOpForSingleBuffer(
    IRRewriter& rewriter, Operation* op, const SyncOperation* sync, bool beforeInsert) const
{
    setSyncInsertionPoint(rewriter, op, beforeInsert || op->hasTrait<OpTrait::IsTerminator>());
...
void SyncCodegen::CreateSetWaitOpForMultiBuffer(
    IRRewriter& rewriter, Operation* op, SyncOperation* sync, bool beforeInsert)
{
...
Value SyncCodegen::GetBufferSelected(IRRewriter& rewriter, Operation* op, SyncOperation* sync)
{
    ...
    Value isZero = rewriter.create<arith::CmpIOp>(
        op->getLoc(), arith::CmpIPredicate::eq, counter, rewriter.create<arith::ConstantIndexOp>(op->getLoc(), 0));
