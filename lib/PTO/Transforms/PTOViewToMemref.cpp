...
static constexpr llvm::StringLiteral kLoweredSetValidShapeAttrName = "__pto.lowered_set_validshape";
static constexpr llvm::StringLiteral kForceDynamicValidShapeAttrName = "__pto.force_dynamic_valid_shape";

namespace {

static void markForceDynamicValidShape(Operation* op, bool force, MLIRContext* ctx);
...
constexpr int32_t kCompactModeRowPlusOne = static_cast<int32_t>(CompactMode::RowPlusOne);
...
static bool readBLayoutI32(Attribute attr, int32_t& out) { return readEnumAttrOrIntegerI32<BLayoutAttr>(attr, out); }

static bool readSLayoutI32(Attribute attr, int32_t& out) { return readEnumAttrOrIntegerI32<SLayoutAttr>(attr, out); }
...
static bool getConstIndexValue(Value value, int64_t& out)
{
    ...
    auto intAttr = constOp ? dyn_cast<IntegerAttr>(constOp.getValue()) : IntegerAttr();
    if (!intAttr)
        return false;
    out = intAttr.getInt();
    return true;
...
static bool computeBoxInnerShape(const TileLayoutConfig& config, Type elemTy, TileLayoutInfo& info)
{
...
static bool computeTilePointerStrides(const TileLayoutConfig& config, ArrayRef<int64_t> shape, TileLayoutInfo& info)
{
...
static bool computeTileLayoutInfo(
    mlir::pto::TileBufConfigAttr cfg, Type elemTy, ArrayRef<int64_t> shape, TileLayoutInfo& info)
{
    if (shape.size() != kTileRank2D || llvm::is_contained(shape, ShapedType::kDynamic))
        return false;

    TileLayoutConfig config = getTileLayoutConfig(cfg);
    return computeBoxInnerShape(config, elemTy, info) && computeTilePointerStrides(config, shape, info);
}

static void collectAffineAddTerms(AffineExpr root, SmallVectorImpl<AffineExpr>& terms)
{
...
static bool tryAssignAffineStride(AffineExpr expr, MutableArrayRef<int64_t> strides)
{
    ...
    auto assignStride = [&strides](AffineExpr dimExpr, AffineExpr constantExpr) -> bool {
    ...
    return assignStride(mulExpr.getLHS(), mulExpr.getRHS()) || assignStride(mulExpr.getRHS(), mulExpr.getLHS());
}

[[maybe_unused]] static void decomposeStridedLayout(AffineMap map, SmallVectorImpl<int64_t>& strides)
{
...
static Value makeIndexConstant(IRRewriter& rewriter, Location loc, int64_t value)
{
    return rewriter.create<arith::ConstantOp>(loc, rewriter.getIndexType(), rewriter.getIndexAttr(value));
}
...
static void materializeStaticValidDims(
    IRRewriter& rewriter, Location loc, mlir::pto::TileBufType tbTy, Value& vRow, Value& vCol)
{
    ...
    if (validShape.size() >= kTileRank2D && validShape[kColumnDimensionIndex] >= 0)
        vCol = makeIndexConstant(rewriter, loc, validShape[kColumnDimensionIndex]);
}

static bool checkMultipleOf(Operation* op, int64_t value, int64_t divisor, StringRef label)
{
    if (divisor <= 0) {
    ...
    if (value % divisor == 0)
        return true;
    op->emitError("boxed layout requires ") << label << " multiple of " << divisor << ", got " << value;
    return false;
}

// 确保 Value 是 Index 类型
static Value ensureIndex(IRRewriter& rewriter, Location loc, Value v, Operation* anchorOp)
{
...
static bool tryGetIndexAttrFromValue(IRRewriter& rewriter, Value v, IntegerAttr& constAttr)
{
...
static void appendMixedIndex(
    IRRewriter& rewriter, Location loc, Value v, Operation* anchorOp, SmallVectorImpl<OpFoldResult>& mixedVals)
{
...
static bool foldAddPtrChainIntoOffset(IRRewriter& rewriter, Location loc, Value& base, Value& totalOffset)
{
    bool folded = false;
    while (auto add = base.getDefiningOp<mlir::pto::AddPtrOp>()) {
        folded = true;
        Value off = ensureIndex(rewriter, loc, add.getOperand(1), add);
        totalOffset = totalOffset ? rewriter.create<arith::AddIOp>(loc, totalOffset, off) : off;
        base = add.getOperand(0);
    }
    return folded;
}

static Value clampSubViewValidDim(
    IRRewriter& rewriter, Location loc, Value explicitValid, int64_t size, Operation* anchorOp)
{
    ...
    Value lt = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::slt, v, sizeVal);
...
{
    TileLayoutInfo info;
    if (computeTileLayoutInfo(tbTy.getConfigAttr(), tbTy.getElementType(), tbTy.getShape(), info)) {
...
{
    auto layoutAttr = StridedLayoutAttr::get(tbTy.getContext(), ShapedType::kDynamic, buildTileMemRefStrides(tbTy));
    return MemRefType::get(tbTy.getShape(), tbTy.getElementType(), layoutAttr, tbTy.getMemorySpace());
}
...
static Type convertPTOTypeToMemRef(Type t)
{
    ...
    if (auto pty = dyn_cast<mlir::pto::PtrType>(t)) {
        return MemRefType::get({ShapedType::kDynamic}, pty.getElementType(), MemRefLayoutAttrInterface(), Attribute());
    }
    ...
    if (auto tvTy = dyn_cast<mlir::pto::TensorViewType>(t))
        return MemRefType::get(tvTy.getShape(), tvTy.getElementType(), MemRefLayoutAttrInterface(), Attribute());
    if (auto partTy = dyn_cast<mlir::pto::PartitionTensorViewType>(t))
        return MemRefType::get(partTy.getShape(), partTy.getElementType(), MemRefLayoutAttrInterface(), Attribute());
    // 其他类型透传
    return t;
}
...
static LogicalResult reconcileSCFIfResultTypes(func::FuncOp func)
{
    ...
    for (scf::IfOp ifOp : ifOps) {
        ...
        if (thenYield.getNumOperands() != ifOp.getNumResults() || elseYield.getNumOperands() != ifOp.getNumResults()) {
        ...
        for (unsigned i = 0; i < ifOp.getNumResults(); ++i) {
            ...
            if (thenTy != elseTy) {
                ifOp.emitError() << "scf.if branch yield type mismatch at result #" << i << ": then=" << thenTy
                                 << ", else=" << elseTy;
                return failure();
            }
...
static LogicalResult reconcileSCFForResultTypes(func::FuncOp func)
{
    ...
    for (scf::ForOp forOp : forOps) {
        ...
        if (yield.getNumOperands() != forOp.getNumResults() || forOp.getInitArgs().size() != forOp.getNumResults()) {
        ...
        for (unsigned i = 0; i < forOp.getNumResults(); ++i) {
            ...
            if (initTy != yieldTy) {
                forOp.emitError() << "scf.for init/yield type mismatch at result #" << i << ": init=" << initTy
                                  << ", yield=" << yieldTy;
                return failure();
            }
...
static LogicalResult markLoweredSetValidShapeOps(func::FuncOp func, MLIRContext* ctx)
{
    WalkResult result = func.walk([ctx](mlir::pto::SetValidShapeOp op) {
        if (isa<MemRefType>(op.getSource().getType())) {
            if (!lookupConfig(op.getSource())) {
                op.emitError("set_validshape requires a locally bound tile source; function "
...
static FailureOr<Value> materializePtrToIntAddPtrAddress(
    IRRewriter& rewriter, Location loc, mlir::pto::PtrToIntOp anchor, Value source)
{
    ...
    if (!baseMemTy) {
        anchor.emitOpError("pto.addptr source base could not be lowered to a GM memref");
        return failure();
    }

    Value byteAddress = rewriter.create<mlir::pto::PtrToIntOp>(loc, rewriter.getI64Type(), base);
    for (auto add : addPtrChain) {
        ...
        unsigned elemBytes = mlir::pto::getPTOStorageElemByteSize(addPtrTy.getElementType());
        if (elemBytes == 0) {
        ...
        if (elemBytes != kPTOByteSize) {
            Value elemBytesValue = rewriter.create<arith::ConstantIntOp>(loc, elemBytes, kPTOI64BitWidth);
            byteOffset = rewriter.create<arith::MulIOp>(loc, byteOffset, elemBytesValue).getResult();
        }
        byteAddress = rewriter.create<arith::AddIOp>(loc, byteAddress, byteOffset).getResult();
    }
...
static LogicalResult lowerIntToPtrOps(func::FuncOp func, MLIRContext* ctx)
{
    ...
    func.walk([&intToPtrs](mlir::pto::IntToPtrOp op) { intToPtrs.push_back(op); });

    for (auto op : intToPtrs) {
        if (!isa<mlir::pto::PtrType>(op.getResult().getType()))
            continue;

        auto targetTy = dyn_cast<MemRefType>(convertPTOTypeToMemRef(op.getResult().getType()));
        if (!targetTy) {
        ...
        auto lowered = rewriter.create<mlir::pto::IntToPtrOp>(op.getLoc(), targetTy, op.getAddr());
...
static LogicalResult lowerPtrToIntOps(func::FuncOp func, MLIRContext* ctx)
{
    ...
    for (auto op : ptrToInts) {
        ...
        if (source.getDefiningOp<mlir::pto::AddPtrOp>()) {
            IRRewriter rewriter(ctx);
            rewriter.setInsertionPoint(op);
            FailureOr<Value> byteAddress = materializePtrToIntAddPtrAddress(rewriter, op.getLoc(), op, source);
...
static bool collectMakeTensorViewBaseAndOffset(IRRewriter& rewriter, Location loc, Value& baseBuf, OpFoldResult& off0)
{
...
static MemRefType getDynamicTensorViewMemRefType(BaseMemRefType baseMr, size_t rank, MLIRContext* ctx)
{
    ...
    return MemRefType::get(dynShape, baseMr.getElementType(), layout, baseMr.getMemorySpace());
}

static SmallInlineVector<OpFoldResult> materializeIndexOperands(
    IRRewriter& rewriter, Location loc, ValueRange values, Operation* op)
{
...
static LogicalResult lowerOneMakeTensorViewOp(mlir::pto::MakeTensorViewOp op, MLIRContext* ctx)
{
    IRRewriter rewriter(ctx);
    rewriter.setInsertionPoint(op);
    ...
    bool foldedAddPtr = collectMakeTensorViewBaseAndOffset(rewriter, loc, baseBuf, off0);
    ...
    auto rc = rewriter.create<memref::ReinterpretCastOp>(loc, mrTy, baseBuf, off0, sizes, strides);
    if (foldedAddPtr)
...
[[maybe_unused]] static LogicalResult lowerMakeTensorViewOps(func::FuncOp func, MLIRContext* ctx)
{
    ...
    func.walk([&makeViews](mlir::pto::MakeTensorViewOp op) { makeViews.push_back(op); });
...
static LogicalResult rewriteLoadScalarAddPtrUses(func::FuncOp func, MLIRContext* ctx)
{
    DefaultInlineVector<mlir::pto::LoadScalarOp> loadScalars;
    func.walk([&loadScalars](mlir::pto::LoadScalarOp op) { loadScalars.push_back(op); });
    for (auto op : loadScalars) {
        ...
        if (!foldAddPtrChainIntoOffset(rewriter, loc, base, totalOffset))
            continue;
        auto newOp = rewriter.create<pto::LoadScalarOp>(loc, op.getValue().getType(), base, totalOffset);
        rewriter.replaceOp(op, newOp.getValue());
    }
    return success();
}

static LogicalResult rewriteStoreScalarAddPtrUses(func::FuncOp func, MLIRContext* ctx)
{
    DefaultInlineVector<mlir::pto::StoreScalarOp> storeScalars;
    func.walk([&storeScalars](mlir::pto::StoreScalarOp op) { storeScalars.push_back(op); });
...
static LogicalResult cleanupRemainingAddPtrOps(func::FuncOp func, llvm::StringRef errorMessage)
{
    DefaultInlineVector<Operation*> addPtrs;
    func.walk([&addPtrs](mlir::pto::AddPtrOp op) { addPtrs.push_back(op.getOperation()); });
    bool changed = true;
    while (changed) {
        changed = false;
        for (auto& op : addPtrs) {
...
{
    if (failed(rewriteLoadScalarAddPtrUses(func, ctx)) || failed(rewriteStoreScalarAddPtrUses(func, ctx)))
        return failure();
    return cleanupRemainingAddPtrOps(func, "addptr must feed make_tensor_view or load/store_scalar for lowering");
}

static void collectPartitionViewSizes(
    IRRewriter& rewriter, Location loc, mlir::pto::PartitionViewOp op, SmallVectorImpl<int64_t>& staticSizes,
...
static SmallVector<OpFoldResult> collectPartitionViewOffsets(
    IRRewriter& rewriter, Location loc, mlir::pto::PartitionViewOp op)
{
...
static LogicalResult lowerOnePartitionViewOp(mlir::pto::PartitionViewOp op, MLIRContext* ctx)
{
    IRRewriter rewriter(ctx);
    rewriter.setInsertionPoint(op);
    ...
    SmallVector<OpFoldResult> mixedStrides(srcMrTy.getRank(), rewriter.getIndexAttr(1));
    auto sv = rewriter.create<memref::SubViewOp>(op.getLoc(), resTy, src, mixedOffsets, mixedSizes, mixedStrides);
...
static LogicalResult lowerPartitionViewOps(func::FuncOp func, MLIRContext* ctx)
{
    ...
    func.walk([&partitionViews](mlir::pto::PartitionViewOp op) { partitionViews.push_back(op); });
...
static LogicalResult validateBoxedSubViewRowMajor(
    mlir::pto::SubViewOp op, ArrayRef<int64_t> staticSizes, ArrayRef<int64_t> srcShape, bool off1Const, int64_t off1)
{
...
static LogicalResult validateBoxedSubViewColMajor(
    mlir::pto::SubViewOp op, ArrayRef<int64_t> staticSizes, ArrayRef<int64_t> srcShape, bool off0Const, int64_t off0)
{
...
static LogicalResult validateBoxedSubViewOp(
    mlir::pto::SubViewOp op, TileBufConfigAttr configAttr, ArrayRef<int64_t> staticSizes,
    const TileLayoutInfo& layoutInfo, MemRefType srcMrTy)
{
    if (staticSizes.size() != kTileRank2D || op.getOffsets().size() != kTileRank2D) {
    ...
    if (off0Const && !checkMultipleOf(op, off0, layoutInfo.innerRows, "row offset")) {
        return failure();
    }
    if (off1Const && !checkMultipleOf(op, off1, layoutInfo.innerCols, "col offset")) {
        return failure();
    }
...
static SmallVector<OpFoldResult> collectSubviewOffsets(IRRewriter& rewriter, Location loc, mlir::pto::SubViewOp op)
{
    SmallVector<OpFoldResult> mixedOffsets;
    for (Value offset : op.getOffsets())
        appendMixedIndex(rewriter, loc, offset, op, mixedOffsets);
...
static SmallVector<int64_t> getSubviewStaticSizes(
    mlir::pto::SubViewOp op, SmallVectorImpl<OpFoldResult>& mixedSizes, IRRewriter& rewriter)
{
    SmallVector<int64_t> staticSizes;
...
static std::pair<MemRefType, MemRefType> getSubviewResultTypes(
    MLIRContext* ctx, MemRefType srcMrTy, ArrayRef<int64_t> staticSizes)
{
    ...
    auto resultMemRefType =
        MemRefType::get(srcMrTy.getShape(), srcMrTy.getElementType(), resultLayout, srcMrTy.getMemorySpace());
    auto subViewMemRefType =
        MemRefType::get(staticSizes, srcMrTy.getElementType(), resultLayout, srcMrTy.getMemorySpace());
    return {resultMemRefType, subViewMemRefType};
}

static std::pair<Value, Value> getClampedSubviewValidDims(
    IRRewriter& rewriter, mlir::pto::SubViewOp op, ArrayRef<int64_t> staticSizes)
{
    Value vRow;
    Value vCol;
    if (!staticSizes.empty())
        vRow = clampSubViewValidDim(rewriter, op.getLoc(), op.getValidRow(), staticSizes[0], op);
    if (staticSizes.size() > 1)
        vCol = clampSubViewValidDim(rewriter, op.getLoc(), op.getValidCol(), staticSizes[1], op);
    return {vRow, vCol};
}

static LogicalResult validateSubViewLayout(
    mlir::pto::SubViewOp op, TileBufConfigAttr configAttr, MemRefType srcMrTy, ArrayRef<int64_t> staticSizes,
    TileLayoutInfo& layoutInfo)
{
    if (!computeTileLayoutInfo(configAttr, srcMrTy.getElementType(), srcMrTy.getShape(), layoutInfo)) {
        op.emitError("unsupported tile layout for pto.subview");
        return failure();
    }
    if (!layoutInfo.boxed)
        return success();
    return validateBoxedSubViewOp(op, configAttr, staticSizes, layoutInfo, srcMrTy);
}

static LogicalResult lowerOneSubViewOp(mlir::pto::SubViewOp op, MLIRContext* ctx)
{
    IRRewriter rewriter(ctx);
    rewriter.setInsertionPoint(op);
    ...
    TileLayoutInfo layoutInfo;
    if (failed(validateSubViewLayout(op, configAttr, srcMrTy, staticSizes, layoutInfo))) {
        return failure();
    }

    auto [resultMemRefType, subViewMemRefType] = getSubviewResultTypes(ctx, srcMrTy, staticSizes);
    SmallVector<OpFoldResult> mixedStrides(staticSizes.size(), rewriter.getIndexAttr(1));
    auto sv =
        rewriter.create<memref::SubViewOp>(op.getLoc(), subViewMemRefType, src, mixedOffsets, mixedSizes, mixedStrides);

    auto [vRow, vCol] = getClampedSubviewValidDims(rewriter, op, staticSizes);
    auto bindOp = rewriter.create<pto::BindTileOp>(
        op.getLoc(), resultMemRefType, sv.getResult(), vRow ? vRow : Value(), vCol ? vCol : Value(), configAttr);
    markForceDynamicValidShape(bindOp, resultTileTy && resultTileTy.hasDynamicValid(), ctx);
...
static Value buildTileBufViewLikeValue(
    Operation* anchorOp, Value src, mlir::pto::TileBufType tbTy, StringRef viewSemantics, MLIRContext* ctx)
{
    ...
    auto bindOp = rewriter.create<pto::BindTileOp>(
        loc, targetType, src, vRow ? vRow : Value(), vCol ? vCol : Value(), configAttr);
...
static LogicalResult lowerTileBufViewLikeOps(func::FuncOp func, MLIRContext* ctx)
{
    ...
    for (auto op : reshapes) {
        ...
        Value lowered = buildTileBufViewLikeValue(op, op->getOperand(0), tbTy, "treshape", ctx);
        if (!lowered)
            return failure();
        IRRewriter rewriter(ctx);
    ...
    for (auto op : bitcasts) {
        ...
        Value lowered = buildTileBufViewLikeValue(op, op->getOperand(0), tbTy, "bitcast", ctx);
        if (!lowered)
            return failure();
        IRRewriter rewriter(ctx);
        rewriter.replaceOp(op, lowered);
...
static LogicalResult rewriteFunctionSignature(func::FuncOp func, MLIRContext* ctx)
{
...
static SmallVector<int64_t> computeAllocTileStrides(TileBufType tbTy, Type elemTy)
{
...
static LogicalResult lowerOneAllocTileOp(mlir::pto::AllocTileOp op, MLIRContext* ctx)
{
    IRRewriter rewriter(ctx);
    rewriter.setInsertionPoint(op);
    auto tbTy = dyn_cast<mlir::pto::TileBufType>(op.getResult().getType());
    ...
    auto targetLayout = StridedLayoutAttr::get(ctx, ShapedType::kDynamic, strides);
    ...
    if (Value addr = op.getAddr()) {
        auto pc = rewriter.create<pto::PointerCastOp>(
            op.getLoc(), targetType, ValueRange{addr}, vRow ? vRow : Value(), vCol ? vCol : Value(), configAttr);
        markForceDynamicValidShape(pc, tbTy.hasDynamicValid(), ctx);
        auto bindOp = rewriter.create<pto::BindTileOp>(
            op.getLoc(), targetType, pc.getResult(), vRow ? vRow : Value(), vCol ? vCol : Value(), configAttr);
        markForceDynamicValidShape(bindOp, tbTy.hasDynamicValid(), ctx);
        rewriter.replaceOp(op, bindOp.getResult());
        return success();
    }
    ...
    auto bindOp = rewriter.create<pto::BindTileOp>(
        op.getLoc(), targetType, alloc, vRow ? vRow : Value(), vCol ? vCol : Value(), configAttr);
    markForceDynamicValidShape(bindOp, tbTy.hasDynamicValid(), ctx);
    rewriter.replaceOp(op, bindOp.getResult());
    return success();
}
...
static LogicalResult lowerOneDeclareTileOp(mlir::pto::DeclareTileOp op, MLIRContext* ctx)
{
    IRRewriter rewriter(ctx);
    rewriter.setInsertionPoint(op);
    ...
    Value vCol;
    materializeStaticValidDims(rewriter, op.getLoc(), tbTy, vRow, vCol);
    auto declaredMemRef = rewriter.create<pto::DeclareTileMemRefOp>(op.getLoc(), targetType);
    auto bindOp = rewriter.create<pto::BindTileOp>(
        op.getLoc(), targetType, declaredMemRef.getResult(), vRow ? vRow : Value(), vCol ? vCol : Value(), configAttr);
    markForceDynamicValidShape(bindOp, tbTy.hasDynamicValid(), ctx);
    rewriter.replaceOp(op, bindOp.getResult());
    return success();
}
...
static LogicalResult lowerDeclareTileOps(func::FuncOp func, MLIRContext* ctx)
{
    ...
    func.walk([&declaredTiles](mlir::pto::DeclareTileOp op) { declaredTiles.push_back(op); });
...
static LogicalResult normalizeTAssignResultTypes(func::FuncOp func, MLIRContext* ctx)
{
    ...
    for (auto op : tassignOps) {
        ...
        IRRewriter rewriter(ctx);
        rewriter.setInsertionPoint(op);
        auto normalized = rewriter.create<pto::TAssignOp>(op.getLoc(), targetTy, op.getTile(), op.getAddr());
        rewriter.replaceOp(op, normalized.getResult());
    }
    return success();
}

static LogicalResult lowerPipeInitAddPtrUses(func::FuncOp func, MLIRContext* ctx)
{
    ...
    while (foldedPipeInitAddPtr) {
        ...
        for (auto op : addPtrsForPipeInit) {
            ...
            auto targetTy = MemRefType::get({dyn}, baseMrTy.getElementType(), layout, baseMrTy.getMemorySpace());
...
{
    if (failed(lowerViewToMemrefComputeOps(func, ctx)) || failed(reconcileSCFIfResultTypes(func)) ||
        failed(reconcileSCFForResultTypes(func)) || failed(markLoweredSetValidShapeOps(func, ctx))) {
        return failure();
    }
    return success();
}
...
{
    if (failed(lowerMakeTensorViewOps(func, ctx)) || failed(lowerTensorViewDimOps(func, ctx)) ||
        failed(lowerPartitionViewOps(func, ctx)) || failed(lowerSubViewOps(func, ctx)) ||
        failed(lowerTileBufViewLikeOps(func, ctx)) || failed(rewriteLoadScalarAddPtrUses(func, ctx)) ||
        failed(rewriteStoreScalarAddPtrUses(func, ctx)) || failed(lowerPipeInitAddPtrUses(func, ctx)) ||
        failed(cleanupRemainingAddPtrOps(
            func, "addptr must feed make_tensor_view,  initialize_l2g2l_pipe(gm_addr) "
...
static LogicalResult lowerFunctionViewToMemref(func::FuncOp func, MLIRContext* ctx)
{
    if (failed(validateIntToPtrUses(func)) || failed(rewriteFunctionSignature(func, ctx)) ||
        failed(lowerIntToPtrOps(func, ctx)) || failed(lowerPtrToIntOps(func, ctx)) ||
        failed(lowerAllocTileOps(func, ctx)) || failed(lowerDeclareTileOps(func, ctx)) ||
        failed(normalizeTAssignResultTypes(func, ctx)) || failed(lowerViewLikeStages(func, ctx)) ||
...
struct PTOViewToMemrefPass : public mlir::pto::impl::PTOViewToMemrefBase<PTOViewToMemrefPass> {
...
std::unique_ptr<Pass> createPTOViewToMemrefPass() { return std::make_unique<PTOViewToMemrefPass>(); }
