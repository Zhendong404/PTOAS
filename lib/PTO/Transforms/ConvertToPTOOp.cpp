...
std::optional<Value> getLeftPadNum(PatternRewriter& rewriter, std::optional<memref::AllocOp> maybeAlloc)
{
    if (!maybeAlloc.has_value())
        return std::nullopt;
    ...
    for (auto* user : maybeAlloc.value()->getUsers()) {
        if (auto subviewOp = llvm::dyn_cast<memref::SubViewOp>(user)) {
            ...
            Value offsetValue = offset.is<Value>() ? dyn_cast<Value>(offset) :
...
std::pair<std::optional<Operation*>, std::optional<Value>> getInitInfo(const Operation*, pto::TLoadOp)
{
    return {std::nullopt, std::nullopt};
}

std::pair<std::optional<Operation*>, std::optional<Value>> getUniqueInitInfo(
    PatternRewriter&, std::optional<memref::AllocOp> maybeAlloc, pto::TLoadOp loadOp)
{
    if (!maybeAlloc.has_value())
        return {std::nullopt, std::nullopt};
    ...
    for (auto* user : (*maybeAlloc)->getUsers()) {
        ...
        } else if (user->getDialect()->getNamespace() == PTODialect::getDialectNamespace()) {
...
LogicalResult replaceMemCopyByPTOLoadOp(memref::CopyOp copyOp, PatternRewriter& rewriter)
{
    ...
    auto loadOp = rewriter.create<pto::TLoadOp>(
        copyOp->getLoc(), TypeRange(), copyOp.getSource(), dst, nullptr, nullptr, nullptr, nullptr, false, nullptr);
    if (maybeLeftPadNum.has_value()) {
        loadOp.getLeftPaddingNumMutable().assign(maybeLeftPadNum.value());
    }
    if (maybePadValue.has_value()) {
        auto padModeAttr = rewriter.getAttr<pto::PadModeAttr>(pto::PadMode::PadValue);
        loadOp.setPadModeAttr(padModeAttr);
        loadOp.getPadValueMutable().assign(maybePadValue.value());
        auto [inlineInitOp, inlineInitCond] = getUniqueInitInfo(rewriter, maybeAlloc, loadOp);
...
struct MemrefCopyOpLowering : public OpRewritePattern<memref::CopyOp> {
    ...
    LogicalResult matchAndRewrite(memref::CopyOp copyOp, PatternRewriter& rewriter) const override
    {
        ...
        if (convertToStore) {
            rewriter.replaceOpWithNewOp<pto::TStoreOp>(copyOp, TypeRange(), src, dst, Value{});
            return success();
        }

        rewriter.replaceOpWithNewOp<pto::TMovOp>(
            copyOp, TypeRange(), src, dst, Value{}, Value{}, pto::AccToVecModeAttr{},
            pto::ReluPreModeAttr::get(rewriter.getContext(), pto::ReluPreMode::NoRelu));
        return success();
    }
};

struct BufferizeMaterializeOpLowering : public OpRewritePattern<bufferization::MaterializeInDestinationOp> {
    using OpRewritePattern<bufferization::MaterializeInDestinationOp>::OpRewritePattern;

    LogicalResult matchAndRewrite(
        bufferization::MaterializeInDestinationOp bufMIDOp, PatternRewriter& rewriter) const override
    {
        Value dst = bufMIDOp.getDest();
        bool convertToStore = isFromFunctionArg(dst);
        if (convertToStore) {
            rewriter.replaceOpWithNewOp<pto::TStoreOp>(bufMIDOp, TypeRange(), bufMIDOp.getSource(), dst, Value{});
            return success();
        }
        return failure();
    }
...
{
    patterns.add<MemrefCopyOpLowering, BufferizeMaterializeOpLowering>(patterns.getContext());
}

struct ConvertToPTOOpPass : public impl::ConvertToPTOOpBase<ConvertToPTOOpPass> {
...
std::unique_ptr<Pass> mlir::pto::createConvertToPTOOpPass() { return std::make_unique<ConvertToPTOOpPass>(); }
