...
static Value getPipeResult(InitOpT op)
{
    return op.getPipe();
}
    ...
    ArrayRef<PipeInitInfo*> component, std::optional<bool>& explicitNoSplit)
{
    for (PipeInitInfo* info : component) {
        if (info->usage == PipeSplitUsage::Mixed) {
            return info->op->emitOpError("cannot mix 'split = 0' with 'split = 1' or 'split = 2' on the "
                                         "same logical pipe");
        }
        if (!info->explicitNoSplit)
            continue;
        if (explicitNoSplit && *explicitNoSplit != *info->explicitNoSplit) {
            return info->op->emitOpError("conflicting explicit 'nosplit' across peer pipe init ops");
        }
    ...
    ArrayRef<PipeInitInfo*> component, std::optional<bool>& inferredNoSplit)
{
    for (PipeInitInfo* info : component) {
        ...
        if (inferredNoSplit && *inferredNoSplit != *usageNoSplit) {
            return info->op->emitOpError("conflicting pipe split usage across peer pipe init ops");
        }
...
static LogicalResult checkNoSplitConflictAgainstUsers(
    ArrayRef<PipeInitInfo*> component, std::optional<bool> explicitNoSplit, std::optional<bool> inferredNoSplit)
{
    ...
    for (PipeInitInfo* info : component) {
        if (!info->explicitNoSplit || *info->explicitNoSplit == *inferredNoSplit)
            continue;
        if (*info->explicitNoSplit) {
            return info->op->emitOpError("explicit 'nosplit = true' conflicts with downstream users that "
                                         "require split = 1 or split = 2");
        }
        return info->op->emitOpError("explicit 'nosplit = false' conflicts with downstream users that "
                                     "require split = 0");
    }
    return success();
}

static void applyNoSplitAttr(ArrayRef<PipeInitInfo*> component, BoolAttr noSplitAttr)
{
    for (PipeInitInfo* info : component) {
...
static LogicalResult resolveNoSplitComponent(ArrayRef<PipeInitInfo*> component, OpBuilder& builder)
{
    std::optional<bool> explicitNoSplit;
    ...
    if (failed(checkExplicitNoSplitConsistency(component, explicitNoSplit)) ||
        ...
        failed(checkNoSplitConflictAgainstUsers(component, explicitNoSplit, inferredNoSplit)))
        return failure();

    bool finalNoSplit = explicitNoSplit.value_or(inferredNoSplit.value_or(false));
    applyNoSplitAttr(component, builder.getBoolAttr(finalNoSplit));
    return success();
}

static void collectPeerAwareAdjacency(
    const PipeInitGroups& keyedInits, llvm::DenseMap<Operation*, SmallVector<Operation*>>& adjacency)
{
...
static SmallVector<PipeInitInfo*> collectPipeComponent(
    PipeInitInfo& rootInfo, llvm::DenseMap<Operation*, SmallVector<Operation*>>& adjacency,
...
struct PTOInferValidatePipeInitPass
    : public mlir::pto::impl::PTOInferValidatePipeInitBase<PTOInferValidatePipeInitPass> {
    void runOnOperation() override
    {
        ...
        for (PipeInitInfo& rootInfo : initInfos) {
            if (!visited.insert(rootInfo.op).second)
                continue;
            SmallVector<PipeInitInfo*> component = collectPipeComponent(rootInfo, adjacency, infoByOp, visited);
