...
static SmallVector<uint64_t> getAllocatedOffsets(
    memref::AllocOp op, BaseMemRefType memRefType, const DenseMap<Value, SmallVector<uint64_t>>& buffer2Offsets,
    uint64_t& fallbackNextOffset)
{
    ...
    if (offsets.empty()) {
        ...
        uint64_t stride =
            ((bytes + kDefaultAllocAlignmentBytes - 1) / kDefaultAllocAlignmentBytes) * kDefaultAllocAlignmentBytes;
        uint64_t off = fallbackNextOffset;
        fallbackNextOffset += std::max<uint64_t>(stride, kDefaultAllocAlignmentBytes);
...
LogicalResult MemrefAllocaOpToPointerCastOpPattern::matchAndRewrite(memref::AllocOp op, PatternRewriter& rewriter) const
{
    const auto& currentMemRefType = cast<BaseMemRefType>(op.getType());
    TileBufConfigAttr configAttr = inferBindTileConfig(op);
    SmallVector<uint64_t> offsets = getAllocatedOffsets(op, currentMemRefType, buffer2Offsets, fallbackNextOffset);
    SmallVector<Value> addrs;
    addrs.reserve(offsets.size());
    for (uint64_t offset : offsets) {
        auto constantIntOffsetOp = rewriter.create<arith::ConstantIntOp>(op->getLoc(), offset, kPTOI64BitWidth);
    ...
    auto ptoPointerCastOp = rewriter.create<pto::PointerCastOp>(
        op.getLoc(), currentMemRefType, ValueRange(addrs), vRow ? vRow : Value(), vCol ? vCol : Value(), configAttr);
