...
struct ReturnToEmitC : public OpConversionPattern<func::ReturnOp> {
    ...
    LogicalResult matchAndRewrite(
        func::ReturnOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (auto emitcFunc = op->getParentOfType<emitc::FuncOp>()) {
            if (auto modeAttr = emitcFunc->getAttrOfType<StringAttr>(kAutoSyncTailPendingModeAttr)) {
                auto* ctx = rewriter.getContext();
                rewriter.setInsertionPoint(op);
                auto args = rewriter.getArrayAttr({emitc::OpaqueAttr::get(ctx, modeAttr.getValue())});
                rewriter.create<emitc::CallOpaqueOp>(
                    op.getLoc(), TypeRange{}, "ptoas_auto_sync_tail", args, ArrayAttr{}, ValueRange{});
            }
        }
...
struct CallToEmitC : public OpConversionPattern<func::CallOp> {
    ...
    LogicalResult matchAndRewrite(
        func::CallOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        if (op.getNumResults() > 1)
            return rewriter.notifyMatchFailure(op, "EmitC cannot lower calls with multiple results");
        ...
        rewriter.replaceOpWithNewOp<emitc::CallOp>(op, op.getCalleeAttr(), resultTypes, adaptor.getOperands());
        return success();
    }
};
...
struct SectionToEmitC : public OpConversionPattern<SectionOpTy> {
    ...
    LogicalResult matchAndRewrite(
        SectionOpTy op, typename SectionOpTy::Adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (needsNoSplitGuard) {
            rewriter.create<emitc::VerbatimOp>(loc, "if (get_subblockid() == 0) {");
        }
...
static bool needsWholeFunctionSCFToCF(func::FuncOp func)
{
    ...
    func.walk([&](Operation* op) {
        ...
        if (auto exec = dyn_cast<scf::ExecuteRegionOp>(op)) {
            if (parentOp && parentOp->hasTrait<OpTrait::SingleBlock>() && !isTriviallyInlineableExecuteRegion(exec)) {
                needs = true;
...
struct SCFExecuteRegionInline : public OpRewritePattern<scf::ExecuteRegionOp> {
    ...
    LogicalResult matchAndRewrite(scf::ExecuteRegionOp op, PatternRewriter& rewriter) const override
    {
...
struct SCFExecuteRegionToCF : public OpRewritePattern<scf::ExecuteRegionOp> {
    ...
    LogicalResult matchAndRewrite(scf::ExecuteRegionOp op, PatternRewriter& rewriter) const override
    {
        ...
        // continuation block.
        rewriter.inlineRegionBefore(op.getRegion(), *parentRegion, continueBlock->getIterator());
...
struct SCFIndexSwitchToCF : public OpRewritePattern<scf::IndexSwitchOp> {
    ...
    static LogicalResult cloneYieldingBlockAndBranchTo(
        PatternRewriter& rewriter, Location loc, Block& srcBlock, Block* destBlock, Block* continueBlock)
    {
    ...
    static Block* splitBlockForContinuation(PatternRewriter& rewriter, scf::IndexSwitchOp op)
    {
        auto switchIt = Block::iterator(op.getOperation());
        return rewriter.splitBlock(op->getBlock(), std::next(switchIt));
    }

    static void addContinuationArguments(PatternRewriter&, scf::IndexSwitchOp op, Location loc, Block* continueBlock)
    {
        SmallVector<BlockArgument> contArgs;
        contArgs.reserve(op.getNumResults());
    ...
    static void createIndexSwitchBlocks(
        PatternRewriter& rewriter, Region* parentRegion, Region::iterator insertPt, unsigned numCases,
        SmallVectorImpl<Block*>& checkBlocks, Block*& defaultBlock, SmallVectorImpl<Block*>& caseBlocks)
    {
    ...
    static void populateIndexSwitchCheckBlocks(
        PatternRewriter& rewriter, Location loc, Value selector, ArrayRef<int64_t> cases, ArrayRef<Block*> checkBlocks,
        ArrayRef<Block*> caseBlocks, Block* defaultBlock)
    {
        for (unsigned i = 0; i < checkBlocks.size(); ++i) {
            ...
            Value cond = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, selector, caseVal);
            Block* falseDest = (i + 1 < checkBlocks.size()) ? checkBlocks[i + 1] : defaultBlock;
            rewriter.create<cf::CondBranchOp>(loc, cond, caseBlocks[i], ValueRange{}, falseDest, ValueRange{});
        }
    }
    ...
    LogicalResult matchAndRewrite(scf::IndexSwitchOp op, PatternRewriter& rewriter) const override
    {
        ...
        Operation* parentOp = op->getParentOp();
        if (parentOp && parentOp->hasTrait<OpTrait::SingleBlock>()) {
            return rewriter.notifyMatchFailure(op, "cannot lower scf.index_switch inside a single-block parent region");
        }
        ...
        createIndexSwitchBlocks(rewriter, parentRegion, insertPt, numCases, checkBlocks, defaultBlock, caseBlocks);

        Value selector = op.getArg();
        auto cases = op.getCases();
        populateIndexSwitchCheckBlocks(rewriter, loc, selector, cases, checkBlocks, caseBlocks, defaultBlock);

        // Fill case blocks and default block with cloned bodies + branch to cont.
        for (unsigned i = 0; i < numCases; ++i) {
            if (failed(cloneYieldingBlockAndBranchTo(rewriter, loc, op.getCaseBlock(i), caseBlocks[i], continueBlock)))
                return rewriter.notifyMatchFailure(op, "expected scf.yield terminator");
        }
        if (failed(cloneYieldingBlockAndBranchTo(rewriter, loc, op.getDefaultBlock(), defaultBlock, continueBlock)))
            return rewriter.notifyMatchFailure(op, "expected scf.yield terminator");
...
struct SCFWhileToCF : public OpRewritePattern<scf::WhileOp> {
    ...
    static Block* splitAfterWhileBlock(PatternRewriter& rewriter, scf::WhileOp op)
    {
        auto whileIt = Block::iterator(op.getOperation());
        return rewriter.splitBlock(op->getBlock(), std::next(whileIt));
    }

    static void addWhileExitArguments(PatternRewriter&, scf::WhileOp op, Location loc, Block* afterWhileBlock)
    {
    ...
    static Block* createWhileHeaderBlock(
        PatternRewriter& rewriter, scf::WhileOp op, Location loc, Block* afterWhileBlock)
    {
        ...
        return rewriter.createBlock(
            afterWhileBlock->getParent(), afterWhileBlock->getIterator(), headerArgTypes, headerArgLocs);
    }

    static Block* createWhileBodyBlock(PatternRewriter& rewriter, scf::WhileOp op, Location loc, Block* afterWhileBlock)
    {
        Block& afterRegionBlock = op.getAfter().front();
        SmallVector<Type> bodyArgTypes(
            afterRegionBlock.getArgumentTypes().begin(), afterRegionBlock.getArgumentTypes().end());
        SmallVector<Location> bodyArgLocs(bodyArgTypes.size(), loc);
        return rewriter.createBlock(
            afterWhileBlock->getParent(), afterWhileBlock->getIterator(), bodyArgTypes, bodyArgLocs);
    }

    static void rewriteWhileTerminators(
        PatternRewriter& rewriter, Location loc, Block* headerBlock, Block* bodyBlock, Block* afterWhileBlock)
    {
        auto condOp = cast<scf::ConditionOp>(headerBlock->getTerminator());
        rewriter.setInsertionPoint(condOp);
        rewriter.create<cf::CondBranchOp>(
            loc, condOp.getCondition(),
            /*trueDest=*/bodyBlock,
    ...
    LogicalResult matchAndRewrite(scf::WhileOp op, PatternRewriter& rewriter) const override
    {
        Operation* parentOp = op->getParentOp();
        if (parentOp && parentOp->hasTrait<OpTrait::SingleBlock>()) {
            return rewriter.notifyMatchFailure(op, "cannot lower scf.while inside a single-block parent region");
        }

        if (failed(validateWhileResultUses(op)))
            return rewriter.notifyMatchFailure(op, "unsupported: while results used outside the parent block");

        auto loc = op.getLoc();
        Block* afterWhileBlock = splitAfterWhileBlock(rewriter, op);
        addWhileExitArguments(rewriter, op, loc, afterWhileBlock);
        Block* headerBlock = createWhileHeaderBlock(rewriter, op, loc, afterWhileBlock);
        ...
        Block& afterRegionBlock = op.getAfter().front();
        rewriter.mergeBlocks(&op.getBefore().front(), headerBlock, headerBlock->getArguments());
        rewriter.mergeBlocks(&afterRegionBlock, bodyBlock, bodyBlock->getArguments());
        rewriteWhileTerminators(rewriter, loc, headerBlock, bodyBlock, afterWhileBlock);
...
struct CFSwitchToCondBr : public OpRewritePattern<cf::SwitchOp> {
    ...
    static SmallVector<SmallVector<Value>> collectSwitchCaseOperands(cf::SwitchOp op)
    {
    ...
    static SmallVector<Block*> createSwitchCheckBlocks(
        PatternRewriter& rewriter, Region* parentRegion, Block* curBlock, size_t numCases)
    {
    ...
    static LogicalResult populateSwitchCheckBlocks(
        PatternRewriter& rewriter, Location loc, Value flag, IntegerType flagTy, ArrayRef<APInt> caseValues,
        ArrayRef<Block*> caseDests, ArrayRef<SmallVector<Value>> caseOperands, Block* defaultDest,
        ValueRange defaultOperands, ArrayRef<Block*> checkBlocks, cf::SwitchOp op)
    {
        for (size_t i = 0; i < caseDests.size(); ++i) {
            rewriter.setInsertionPointToEnd(checkBlocks[i]);
            APInt caseVal = caseValues[i];
            if (caseVal.getBitWidth() != flagTy.getWidth()) {
                return rewriter.notifyMatchFailure(op, "case value bitwidth doesn't match flag type");
            }

            Value caseConst = rewriter.create<arith::ConstantOp>(loc, flagTy, rewriter.getIntegerAttr(flagTy, caseVal));
            Value cond = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, flag, caseConst);
            Block* falseDest = (i + 1 < checkBlocks.size()) ? checkBlocks[i + 1] : defaultDest;
            ValueRange falseOperands = (i + 1 < checkBlocks.size()) ? ValueRange{} : defaultOperands;
            rewriter.create<cf::CondBranchOp>(loc, cond, caseDests[i], caseOperands[i], falseDest, falseOperands);
        }
        return success();
    }

    LogicalResult matchAndRewrite(cf::SwitchOp op, PatternRewriter& rewriter) const override
    {
        Location loc = op.getLoc();
        Operation* parentOp = op->getParentOp();
        if (parentOp && parentOp->hasTrait<OpTrait::SingleBlock>()) {
            return rewriter.notifyMatchFailure(op, "cannot lower cf.switch inside a single-block parent region");
        }
        ...
        SmallVector<Value> defaultOperands(op.getDefaultOperands().begin(), op.getDefaultOperands().end());
        Block* defaultDest = op.getDefaultDestination();

        SmallVector<Block*> caseDests(op.getCaseDestinations().begin(), op.getCaseDestinations().end());
        ...
        SmallVector<Block*> checkBlocks = createSwitchCheckBlocks(rewriter, parentRegion, curBlock, caseDests.size());
        if (failed(populateSwitchCheckBlocks(
                rewriter, loc, flag, flagTy, caseValues, caseDests, caseOperands, defaultDest, defaultOperands,
                checkBlocks, op))) {
            return failure();
        }
        ...
        rewriter.setInsertionPoint(op);
        rewriter.replaceOpWithNewOp<cf::BranchOp>(op, checkBlocks.front(), ValueRange{});
        return success();
    }
};
...
LogicalResult runPTOToEmitCSCFPreLowering(ModuleOp mop, MLIRContext* ctx)
{
    ...
    if (needsAnySCFToCF) {
        ...
        scfToCfTarget.addIllegalOp<scf::ForallOp, scf::ForOp, scf::IfOp, scf::ParallelOp>();
    ...
    scfLoweringPatterns
        .add<SCFExecuteRegionInline, SCFExecuteRegionToCF, SCFIndexSwitchToCF, SCFWhileToCF, CFSwitchToCondBr>(ctx);
    ...
    mop.walk([&](Operation* op) {
        ...
        if (isa<cf::SwitchOp>(op)) {
            hasUnsupportedSCF = true;
            op->emitError() << "Unsupported CF op remained after pre-lowering: cf.switch";
            return WalkResult::interrupt();
        }
        return WalkResult::advance();
    });
...
void populatePTOToEmitCControlFlowPatterns(RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx)
{
