...
static std::optional<Role> inferMatmulLikeSubviewRole(MatmulLikeOp op, Value buffer)
{
    if (op.getLhs() == buffer)
        return Role::A;
    if (op.getRhs() == buffer)
...
struct SubviewToEmitCPattern : public OpConversionPattern<memref::SubViewOp> {
    ...
    static std::string elemTypeToString(Type elemTy) { return getEmitCScalarTypeToken(elemTy); }

    static Value makeU32Constant(ConversionPatternRewriter& rewriter, Location loc, Type u32Ty, int64_t value)
    {
        auto* ctx = rewriter.getContext();
        return rewriter.create<emitc::ConstantOp>(loc, u32Ty, emitc::OpaqueAttr::get(ctx, std::to_string(value)));
    }

    static Value convertOfrToU32Value(ConversionPatternRewriter& rewriter, Location loc, Type u32Ty, OpFoldResult ofr)
    {
        ...
        if (auto attr = ofr.dyn_cast<Attribute>()) {
            if (auto ia = dyn_cast<IntegerAttr>(attr))
                return makeU32Constant(rewriter, loc, u32Ty, ia.getValue().getSExtValue());
        }
        return makeU32Constant(rewriter, loc, u32Ty, 0);
    }

    SourceStrideInfo getSourceStrideInfo(
        memref::SubViewOp op, MemRefType srcType, ConversionPatternRewriter& rewriter) const
    {
    ...
    OffsetComputation buildSubviewOffset(
        memref::SubViewOp op, OpAdaptor adaptor, ArrayRef<OpFoldResult> sourceStrides,
        ConversionPatternRewriter& rewriter) const
    {
        ...
        for (int i = 0, rank = op.getSourceType().getRank(); i < rank; ++i) {
            Value offVal = staticOffsets[i] == ShapedType::kDynamic ?
                               rewriter.create<emitc::CastOp>(loc, u32Ty, dynamicOffsets[dynOffIdx++]).getResult() :
    ...
    static Value materializeSubviewSourcePointer(
        Value sourcePtr, Value tileCandidate, MemRefType srcType, Location loc, ConversionPatternRewriter& rewriter)
    {
        if (auto ot = dyn_cast<emitc::OpaqueType>(tileCandidate.getType())) {
            auto tyStr = ot.getValue();
            if (tyStr.find("Tile<") != std::string::npos || tyStr.find("ConvTile<") != std::string::npos) {
                std::string elemTok = elemTypeToString(srcType.getElementType());
                pto::AddressSpace as = getMemRefAddressSpace(srcType);
                sourcePtr = materializeTileDataValue(rewriter, loc, tileCandidate, as, elemTok);
                if (tileDataReturnsIntegralAddress(as))
                    sourcePtr = materializeAddressAsPointer(rewriter, loc, sourcePtr, as, elemTok);
            }
        }
        return sourcePtr;
    }

    static Value buildSubviewResultPointer(
        memref::SubViewOp op, MemRefType srcType, Value sourcePtr, Value totalOffset,
        ConversionPatternRewriter& rewriter)
    {
        ...
        if (!elemTy.isInteger(kPTOI16BitWidth))
            return rewriter.create<emitc::AddOp>(loc, sourcePtr.getType(), sourcePtr, totalOffset);

        std::string castElemTypeStr = cast<IntegerType>(elemTy).isUnsigned() ? "uint16_t" : "int16_t";
        ...
        auto typedPtrTy = emitc::PointerType::get(emitc::OpaqueType::get(ctx, qualifier + " " + castElemTypeStr));
        Value typedSourcePtr = rewriter.create<emitc::CastOp>(loc, typedPtrTy, sourcePtr);
        return rewriter.create<emitc::AddOp>(loc, typedPtrTy, typedSourcePtr, totalOffset);
    }

    Type convertSubviewResultType(memref::SubViewOp op) const { return getTypeConverter()->convertType(op.getType()); }

    LogicalResult replaceNonGlobalSubview(
        memref::SubViewOp op, MemRefType srcType, Value newPtr, ConversionPatternRewriter& rewriter) const
    {
        bool isGlobal = true;
        if (auto asAttr = dyn_cast_or_null<pto::AddressSpaceAttr>(srcType.getMemorySpace())) {
    ...
    SmallVector<Value> buildSubviewSizeValues(
        memref::SubViewOp op, Type u32Ty, ConversionPatternRewriter& rewriter) const
    {
        auto loc = op.getLoc();
        auto resTy = mlir::cast<MemRefType>(op.getResult().getType());
        ...
        for (int i = 0; i < resTy.getRank(); ++i) {
            if (i < static_cast<int>(mixedSizes.size())) {
                ...
                sizeValues.push_back(
                    makeU32Constant(rewriter, loc, u32Ty, resShape[i] == ShapedType::kDynamic ? 1 : resShape[i]));
            }
        }
        return sizeValues;
    }
        ...
        ConversionPatternRewriter& rewriter) const
    {
        ...
        for (int i = 0; i < rank; ++i) {
            OpFoldResult srcStrideOfr =
                i < static_cast<int>(sourceStrides.size()) ? sourceStrides[i] : rewriter.getIndexAttr(1);
            OpFoldResult stepOfr =
                i < static_cast<int>(subViewSteps.size()) ? subViewSteps[i] : rewriter.getIndexAttr(1);
            ...
            if (srcStatic && stepStatic) {
                ...
                result.strideValues.push_back(makeU32Constant(rewriter, loc, u32Ty, finalStride));
                continue;
            }
            ...
            if (stepStatic && *stepStatic == 1)
                ...
                result.strideValues.push_back(rewriter.create<emitc::MulOp>(loc, u32Ty, srcV, stepV));
        }
        return result;
    }

    static void finalizeSubviewStrideShapeInfo(
        SubviewStrideShapeInfo& result, Type u32Ty, ArrayRef<Value> sizeValues, ArrayRef<Value> strideValues,
        int64_t rank, ConversionPatternRewriter& rewriter, Location loc)
    {
        ...
        for (int i = 3; i >= 0; --i) {
            ...
            if (result.finalStride[i] != -1) {
                result.finalStrideValues[i] = makeU32Constant(rewriter, loc, u32Ty, result.finalStride[i]);
                continue;
            }
            if (result.finalShape[i + 1] == 1) {
                ...
                result.finalStrideValues[i] = rewriter.create<emitc::MulOp>(
                    loc, u32Ty, result.finalShapeValues[i + 1], result.finalStrideValues[i + 1]);
            }
        }
    }

    SubviewStrideShapeInfo buildSubviewStrideShapeInfo(
        memref::SubViewOp op, OpAdaptor, Type u32Ty, ArrayRef<OpFoldResult> sourceStrides,
        ConversionPatternRewriter& rewriter) const
    {
        auto loc = op.getLoc();
        auto resTy = mlir::cast<MemRefType>(op.getResult().getType());
        ...
        SubviewTemplateStrideInfo strideInfo = buildSubviewTemplateStrideInfo(op, u32Ty, sourceStrides, rewriter);
        buildGlobalTensorShapeAndStride(
            shapeParamsVec, strideInfo.strideTemplateVec, result.finalShape, result.finalStride);
        finalizeSubviewStrideShapeInfo(result, u32Ty, sizeValues, strideInfo.strideValues, rank, rewriter, loc);
        return result;
    }

    static std::string resolveSubviewLayout(
        memref::SubViewOp op, StringRef elemTypeStr, ArrayRef<int64_t> finalShape, ArrayRef<int64_t> finalStride)
    {
        if (auto layout = resolveLayoutForGlobalTensor(op, op.getSource()))
            return layoutToEmitCString(*layout);

        bool allStatic = llvm::all_of(finalShape, [](int64_t value) { return value != -1; }) &&
        ...
        if (allStatic) {
            if (finalShape[kNumber2] == kNumber16 && finalShape[2] * finalShape[3] * elemBytes == kFractalSize512 &&
                finalStride[4] == 1 && finalStride[3] == finalShape[4]) {
                layoutTag = kNumber2;
            } else {
                bool isCol = finalStride[0] == 1;
                for (int i = 0; i < static_cast<int>(kNumber4); ++i)
                    isCol = isCol && (finalStride[i + 1] == multiplyOrDynamic(finalStride[i], finalShape[i]));
                if (isCol)
                    layoutTag = 1;
            }
        }
    ...
    static Value buildSubviewShapeValue(
        ConversionPatternRewriter& rewriter, Location loc, Type shapeTypeOpaque, StringRef shapeCppType,
        ValueRange shapeArgs)
    {
        return rewriter
            .create<emitc::CallOpaqueOp>(loc, shapeTypeOpaque, shapeCppType, ArrayAttr{}, ArrayAttr{}, shapeArgs)
            .getResult(0);
    }

    static Value buildSubviewStrideValue(
        ConversionPatternRewriter& rewriter, Location loc, Type strideTypeOpaque, StringRef strideCppType,
        ValueRange strideCtorArgs)
    {
        return rewriter
            .create<emitc::CallOpaqueOp>(loc, strideTypeOpaque, strideCppType, ArrayAttr{}, ArrayAttr{}, strideCtorArgs)
            .getResult(0);
    }

    LogicalResult replaceGlobalSubview(
        memref::SubViewOp op, OpAdaptor adaptor, Value newPtr, const SubviewStrideShapeInfo& strideShapeInfo,
        StringRef elemTypeStr, ConversionPatternRewriter& rewriter) const
    {
        auto loc = op.getLoc();
        auto* ctx = rewriter.getContext();
        ...
        std::string layoutEnum =
            resolveSubviewLayout(op, elemTypeStr, strideShapeInfo.finalShape, strideShapeInfo.finalStride);
        ...
        Value shapeValue = buildSubviewShapeValue(rewriter, loc, shapeTypeOpaque, shapeCppType, shapeArgs);
        ...
        Value strideValue = buildSubviewStrideValue(rewriter, loc, strideTypeOpaque, strideCppType, strideCtorArgs);
        std::string gtCppType =
            "GlobalTensor<" + elemTypeStr.str() + ", " + shapeCppType + ", " + strideCppType + ", " + layoutEnum + ">";
        auto gtType = emitc::OpaqueType::get(ctx, gtCppType);
        rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
            op, gtType, gtCppType, ArrayAttr{}, ArrayAttr{}, ValueRange{newPtr, shapeValue, strideValue});
        return success();
    }
        ...
        memref::SubViewOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        OffsetComputation offsetInfo = buildSubviewOffset(op, adaptor, sourceStrideInfo.sourceStrides, rewriter);

        Value sourcePtr = adaptor.getSource();
        Value tileCandidate = peelCastLikeValue(sourcePtr);
        sourcePtr = materializeSubviewSourcePointer(sourcePtr, tileCandidate, srcType, loc, rewriter);
        Value newPtr = buildSubviewResultPointer(op, srcType, sourcePtr, offsetInfo.totalOffset, rewriter);
        ...
        SubviewStrideShapeInfo strideShapeInfo =
            buildSubviewStrideShapeInfo(op, adaptor, offsetInfo.u32Ty, sourceStrideInfo.sourceStrides, rewriter);
        return replaceGlobalSubview(op, adaptor, newPtr, strideShapeInfo, elemTypeStr, rewriter);
    }
};

} // namespace

void populatePTOToEmitCSubviewPatterns(RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx)
{
