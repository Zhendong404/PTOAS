...
struct PointerCastConversion : public OpConversionPattern<pto::PointerCastOp> {
    ...
    struct PointerCastConfigStrings {
        ...
        std::string extraParams = ", SLayout::NoneBox, 512, PadValue::Null, CompactMode::Null";
    };
    ...
    static std::string buildDimToken(
        int64_t dim, llvm::StringRef symbol, Type elemType, pto::BLayout blayout, int dimIdx)
    {
        if (dim == ShapedType::kDynamic)
            return symbol.str();
        return std::to_string(renderTileTemplateDim(dim, elemType, blayout, dimIdx));
    }

    static std::string buildDimString(TileRole role, ArrayRef<int64_t> shape, Type elemType, pto::BLayout blayout)
    {
    ...
    static std::string getPadToken(pto::TileBufConfigAttr config) { return getTileBufPadToken(config); }

    static std::string getCompactToken(pto::TileBufConfigAttr config) { return getTileBufCompactToken(config); }
    ...
    static PointerCastConfigStrings buildConfigStrings(pto::PointerCastOp op)
    {
        ...
        if (auto attr = dyn_cast<IntegerAttr>(config.getSFractalSize()))
            fractal = attr.getInt();
        result.extraParams =
            ", " + slStr + ", " + std::to_string(fractal) + ", " + getPadToken(config) + ", " + getCompactToken(config);
        return result;
    }

    static Value maybeScaleDynamicValid(
        ConversionPatternRewriter& rewriter, Location loc, Type elemType, pto::BLayout blayout, Value emitted,
        int dimIdx)
    {
        return scalePackedTileDynamicDim(rewriter, loc, elemType, blayout, emitted, dimIdx);
    }

    static ValidShapeInfo buildForcedDynamicValidShapeInfo(
        ConversionPatternRewriter& rewriter, Location loc, Type elemType, pto::BLayout blayout, ArrayRef<int64_t> shape,
        Value vRowEmitC, Value vColEmitC, bool rowIsConst, bool colIsConst, int64_t cRow, int64_t cCol)
    {
        ...
        result.constructorArgs.push_back(buildTileCtorDimValue(
            rewriter, loc, maybeScaleDynamicValid(rewriter, loc, elemType, blayout, vRowEmitC, 0),
            renderTileTemplateDim(rowIsConst ? cRow : shape[0], elemType, blayout, 0)));
        result.constructorArgs.push_back(buildTileCtorDimValue(
            rewriter, loc, maybeScaleDynamicValid(rewriter, loc, elemType, blayout, vColEmitC, 1),
            renderTileTemplateDim(colIsConst ? cCol : shape[1], elemType, blayout, 1)));
        return result;
    }

    static void assignRegularValidDim(
        ValidShapeInfo& result, std::string& targetTok, bool& isDynamic, Value originalValue, bool isConst,
        int64_t constValue, int64_t staticShapeValue, Type elemType, pto::BLayout blayout, int dimIdx)
    {
        if (isConst) {
            targetTok = std::to_string(renderTileTemplateDim(constValue, elemType, blayout, dimIdx));
            return;
        }
        if (originalValue) {
            targetTok = "-1";
        ...
        targetTok = std::to_string(renderTileTemplateDim(staticShapeValue, elemType, blayout, dimIdx));
    }

    static ValidShapeInfo buildValidShapeInfo(
        pto::PointerCastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter, Location loc, Type elemType,
        pto::BLayout blayout, ArrayRef<int64_t> shape)
    {
        ...
        if (forceDynamicValid)
            return buildForcedDynamicValidShapeInfo(
                rewriter, loc, elemType, blayout, shape, vRowEmitC, vColEmitC, rowIsConst, colIsConst, cRow, cCol);

        assignRegularValidDim(
            result, result.vrowTok, rowIsDynamic, vRow, rowIsConst, cRow, shape[0], elemType, blayout, 0);
        assignRegularValidDim(
            result, result.vcolTok, colIsDynamic, vCol, colIsConst, cCol, shape[1], elemType, blayout, 1);
        if (result.useConstructor) {
            if (rowIsDynamic && vRowEmitC)
                result.constructorArgs.push_back(
    ...
    static Value buildTileValue(
        ConversionPatternRewriter& rewriter, Location loc, MLIRContext* ctx, Type tileType, llvm::StringRef tileTypeStr,
        const ValidShapeInfo& validInfo)
    {
        if (validInfo.useConstructor) {
            return rewriter
                .create<emitc::CallOpaqueOp>(
                    loc, tileType, tileTypeStr, ArrayAttr{}, ArrayAttr{}, ValueRange(validInfo.constructorArgs))
                .getResult(0);
        }
        return rewriter.create<emitc::VariableOp>(loc, tileType, emitc::OpaqueAttr::get(ctx, "")).getResult();
    }

    static Value castAddressToIntegral(ConversionPatternRewriter& rewriter, Location loc, Value addr)
    {
    ...
    LogicalResult matchAndRewrite(
        pto::PointerCastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        auto loc = op.getLoc();
        ...
        std::string dimStr = buildDimString(role, shape, elemType, configStrings.blayout);
        ValidShapeInfo validInfo =
            buildValidShapeInfo(op, adaptor, rewriter, loc, elemType, configStrings.blayout, shape);
        std::string tileTypeStr = (llvm::Twine("Tile<") + getRoleToken(role) + ", " + elemTypeStr + ", " + dimStr +
                                   ", " + configStrings.layoutParams + ", " + validInfo.vrowTok + ", " +
                                   validInfo.vcolTok + configStrings.extraParams + ">")
                                      .str();

        auto tileType = emitc::OpaqueType::get(ctx, tileTypeStr);
        Value resultValue = buildTileValue(rewriter, loc, ctx, tileType, tileTypeStr, validInfo);
        Value addr = castAddressToIntegral(rewriter, loc, adaptor.getAddrs()[0]);

        rewriter.create<emitc::CallOpaqueOp>(
            loc, TypeRange{}, "TASSIGN", ArrayAttr{}, ArrayAttr{}, ValueRange{resultValue, addr});
...
static FailureOr<Value> buildGlobalTensorViewFromPointer(
    ConversionPatternRewriter& rewriter, Location loc, Value ptr, Type elemTy, ArrayRef<int64_t> shape,
    ArrayRef<int64_t> strides = {}, StringRef layoutEnum = "pto::Layout::ND")
{
    if (llvm::any_of(shape, [](int64_t dim) { return dim == ShapedType::kDynamic; }))
        return failure();
    ...
    auto shapeVal =
        rewriter
            .create<emitc::CallOpaqueOp>(
                loc, emitc::OpaqueType::get(ctx, shapeType), shapeType, ArrayAttr{}, ArrayAttr{}, ValueRange{})
            .getResult(0);
    auto strideVal =
        rewriter
            .create<emitc::CallOpaqueOp>(
                loc, emitc::OpaqueType::get(ctx, strideType), strideType, ArrayAttr{}, ArrayAttr{}, ValueRange{})
            .getResult(0);

    std::string gtTypeStr = getGlobalTensorTypeStringFromShapeAndStrides(elemTy, shape, effectiveStrides, layoutEnum);
    auto gtType = emitc::OpaqueType::get(ctx, gtTypeStr);
    auto gt = rewriter.create<emitc::CallOpaqueOp>(
        loc, gtType, gtTypeStr, ArrayAttr{}, ArrayAttr{}, ValueRange{ptr, shapeVal, strideVal});
    return gt.getResult(0);
}

static bool parseIntegerTemplateList(StringRef token, StringRef marker, SmallVectorImpl<int64_t>& values)
{
...
static LogicalResult getStaticTensorViewStrides(
    Value source, Value convertedSource, pto::TensorViewType sourceType, SmallVectorImpl<int64_t>& strides)
{
...
struct PTOPartitionViewToEmitC : public OpConversionPattern<mlir::pto::PartitionViewOp> {
    using OpConversionPattern<mlir::pto::PartitionViewOp>::OpConversionPattern;

    static LogicalResult verifyStaticPartitionView(
        mlir::pto::PartitionViewOp op, pto::TensorViewType srcTy, pto::PartitionTensorViewType resTy,
        ConversionPatternRewriter& rewriter)
    {
        ...
        for (auto [idx, value] : llvm::enumerate(op.getSizes())) {
            ...
            if (!cst) {
                return rewriter.notifyMatchFailure(op, "globaltensor partition_view requires static sizes");
            }
            int64_t resultDim = resTy.getShape()[idx];
            if (resultDim != ShapedType::kDynamic && resultDim != *cst) {
                return rewriter.notifyMatchFailure(op, "partition_view static size does not match result type");
            }
        }
        return success();
    }
    ...
    static FailureOr<Value> buildDynamicPartitionOffset(
        mlir::pto::PartitionViewOp op, ConversionPatternRewriter& rewriter, Value data, int64_t staticLinearOffset,
        ArrayRef<std::pair<Value, int64_t>> dynamicOffsetTerms)
    {
        ...
        for (auto [offsetValue, stride] : dynamicOffsetTerms) {
            ...
            if (stride != 1) {
                ...
                term = rewriter.create<emitc::MulOp>(op.getLoc(), u32Ty, term, strideValue).getResult();
            }
            totalOffset = rewriter.create<emitc::AddOp>(op.getLoc(), u32Ty, totalOffset, term).getResult();
        }
        return rewriter.create<emitc::AddOp>(op.getLoc(), data.getType(), data, totalOffset).getResult();
    }

    static FailureOr<Value> buildPartitionViewPointer(
        mlir::pto::PartitionViewOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter,
        ArrayRef<int64_t> srcStrides, pto::TensorViewType srcTy)
    {
        ...
        for (auto [idx, values] : llvm::enumerate(llvm::zip(op.getOffsets(), adaptor.getOffsets()))) {
            Value originalOffset = std::get<0>(values);
            Value convertedOffset = std::get<1>(values);
            int64_t stride = srcStrides[idx];
            if (stride == ShapedType::kDynamic) {
                return rewriter.notifyMatchFailure(op, "dynamic source stride is not supported");
            }
        ...
        Value data = rewriter
                         .create<emitc::CallOpaqueOp>(
                             op.getLoc(), ptrTy, "PTOAS__GLOBAL_TENSOR_DATA", ArrayAttr{}, ArrayAttr{}, ValueRange{src})
                         .getResult(0);
        if (dynamicOffsetTerms.empty())
            return applyStaticMemrefOffset(rewriter, op.getLoc(), data, staticLinearOffset);
        return buildDynamicPartitionOffset(op, rewriter, data, staticLinearOffset, dynamicOffsetTerms);
    }

    LogicalResult matchAndRewrite(
        mlir::pto::PartitionViewOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override
    {
        ...
        if (!srcTy || !resTy)
            return rewriter.notifyMatchFailure(op, "expected tensor_view source and partition_tensor_view result");
        if (failed(verifyStaticPartitionView(op, srcTy, resTy, rewriter)))
            return failure();

        SmallVector<int64_t> srcStrides;
        if (failed(getStaticTensorViewStrides(op.getSource(), adaptor.getSource(), srcTy, srcStrides)))
            return rewriter.notifyMatchFailure(op, "partition_view requires static source strides");
        FailureOr<Value> ptr = buildPartitionViewPointer(op, adaptor, rewriter, srcStrides, srcTy);
        if (failed(ptr))
            return failure();

        auto resultOr = buildGlobalTensorViewFromPointer(
            rewriter, op.getLoc(), *ptr, resTy.getElementType(), resTy.getShape(), srcStrides);
        if (failed(resultOr))
            return rewriter.notifyMatchFailure(op, "failed to materialize partition GlobalTensor");
...
void populatePTOToEmitCMemoryOpPatterns(RewritePatternSet& patterns, TypeConverter& typeConverter, MLIRContext* ctx)
{
