...
static int64_t ceilDivBitsToBytes(int64_t bits) { return (bits + kBitsPerByte - 1) / kBitsPerByte; }
...
static LocalMemSpec getLocalMemSpec(Operation* op, AddressSpace as)
{
    switch (as) {
        case AddressSpace::VEC:
            return isTargetArchA5(op) ? LocalMemSpec{kA5VecLocalMemBits, kLocalMemAlignmentBytes} :
...
static void collectStableValueOrder(
    Region& region, AsmState& asmState, DenseMap<Value, std::string>& stableValueKeys,
    ...
    for (Block& block : region) {
        ...
        for (Operation& op : block) {
            ...
            for (Region& nestedRegion : op.getRegions())
                collectStableValueOrder(nestedRegion, asmState, stableValueKeys, seenValues);
        }
    }
}
...
static uint32_t lookupStableValueOrder(Value value, const StableValueOrderMap& stableValueOrder)
{
...
static void sortValuesByStableOrder(SmallVectorImpl<Value>& values, const StableValueOrderMap& stableValueOrder)
{
...
static SmallVector<Value> getScratchBuffersFromEffects(
    Operation* op, ValueRange dpsInits, const StableValueOrderMap& stableValueOrder)
{
    ...
    SmallVector<SideEffects::EffectInstance<MemoryEffects::Effect>, kMemoryEffectReserveSize> effects;
...
static SmallVector<ValuePair> getScratchConflictPairsFromEffects(
    Operation* op, ValueRange dpsInits, const StableValueOrderMap& stableValueOrder)
{
    SmallVector<ValuePair> conflictPairs;
    SmallVector<Value> scratchBuffers = getScratchBuffersFromEffects(op, dpsInits, stableValueOrder);
...
static LogicalResult analyzeReserveBufferPlans(func::FuncOp funcOp, ReserveBufferPlans& plans)
{
    ...
    for (ReserveBufferOp reserveOp : reserveOps) {
        ...
        if (autoAlloc) {
            if (reserveOp.getBaseAttr()) {
                return reserveOp.emitOpError("expects 'base' to be absent when 'auto' is true");
            }
            continue;
        }
        ...
        if (baseBytes % spec.alignBytes != 0) {
            return reserveOp.emitOpError("expects 'base' to satisfy the address-space alignment");
        }
...
{
    llvm::sort(
        ranges, [](const OccupiedByteRange& lhs, const OccupiedByteRange& rhs) { return lhs.begin < rhs.begin; });
    ...
    const DenseMap<Value, SmallVector<uint64_t>>& buffer2Offsets)
{
    for (const auto& it : bufferInfos) {
        ...
        int64_t occupiedSizeBytes = alignUpBytes(ceilDivBitsToBytes(bufferInfo.constBits), /*align=*/1);
        for (uint64_t offsetBytes : offsetsIt->second) {
            occupiedByAddressSpace[bufferInfo.bufferScope].push_back(OccupiedByteRange{
                static_cast<int64_t>(offsetBytes), static_cast<int64_t>(offsetBytes) + occupiedSizeBytes});
        }
    }
}

static FailureOr<int64_t> chooseAutoReserveBufferBase(ReserveBufferPlan& plan, SmallVector<OccupiedByteRange>& occupied)
{
    ...
    if (candidateBase + plan.sizeBytes > plan.capacityBytes) {
        return plan.reserveOp.emitOpError("failed to allocate local memory hole for reserve_buffer");
    }
    return candidateBase;
}

static void assignResolvedReserveBufferBase(
    ReserveBufferPlan& plan, SmallVector<OccupiedByteRange>& occupied, int64_t candidateBase)
{
    plan.reserveOp->setAttr(
        "base", IntegerAttr::get(IntegerType::get(plan.reserveOp.getContext(), kI32BitWidth), candidateBase));
    occupied.push_back(OccupiedByteRange{candidateBase, candidateBase + plan.sizeBytes});
    normalizeOccupiedRanges(occupied);
}

static LogicalResult assignAutoReserveBufferBases(
    ReserveBufferPlans& plans, const std::map<Value, BufferInfo, ValueComparator>& bufferInfos,
    const DenseMap<Value, SmallVector<uint64_t>>& buffer2Offsets)
{
    std::map<AddressSpace, SmallVector<OccupiedByteRange>> occupiedByAddressSpace;
    ...
    for (ReserveBufferPlan& plan : plans) {
        ...
        SmallVector<OccupiedByteRange>& occupied = occupiedByAddressSpace[plan.addressSpace];
...
bool MemLivenessAnalysis::isLocalMemPlan() const { return planMode == MemPlanMode::LOCAL_MEM_PLAN; }

bool MemLivenessAnalysis::isGlobalWorkSpaceMemPlan() const { return planMode == MemPlanMode::GLOBAL_WORKSPACE_PLAN; }

std::optional<WalkResult> MemLivenessAnalysis::HandleRecursiveControlFlow(Operation* op, Liveness live)
{
...
bool MemLivenessAnalysis::HandleMetadataOnlyOp(Operation* op, OpInfo* curOpInfo, Liveness live)
{
...
bool MemLivenessAnalysis::HandleGenericStoreOp(Operation* op, OpInfo* curOpInfo, Liveness live)
{
...
bool MemLivenessAnalysis::HandleGenericPtoDpsInitOp(Operation* op, OpInfo* curOpInfo, Liveness live)
{
    auto ptoDpsOp = dyn_cast<pto::PTO_DpsInitOpInterface>(op);
    if (!ptoDpsOp)
        return false;
    SmallVector<Value> genBuffers = llvm::to_vector(ptoDpsOp.getDpsInits());
    auto scratchBuffers = getScratchBuffersFromEffects(op, ptoDpsOp.getDpsInits(), stableValueOrder);
    genBuffers.append(scratchBuffers.begin(), scratchBuffers.end());
    UpdateOpGenInfo(curOpInfo, genBuffers);
    for (const auto& conflictPair : getScratchConflictPairsFromEffects(op, ptoDpsOp.getDpsInits(), stableValueOrder)) {
...
bool MemLivenessAnalysis::HandleGenericDestinationStyleOp(Operation* op, OpInfo* curOpInfo, Liveness live)
{
...
bool MemLivenessAnalysis::HandleGenericAliasOrCallOp(Operation* op, OpInfo* curOpInfo, Liveness live)
{
...
bool MemLivenessAnalysis::HandleKnownBufferTouchingGroup(Operation* op, OpInfo* curOpInfo, Liveness live)
{
    if (isa<pto::TAllocOp, pto::TPushOp, pto::TFreeOp, pto::InitializeL2LPipeOp, pto::InitializeL2G2LPipeOp,
            pto::BuildAsyncSessionOp, pto::TPutAsyncOp, pto::TGetAsyncOp, pto::TPutOp, pto::TGetOp, pto::TNotifyOp,
            pto::TWaitOp, pto::TTestOp, pto::SyncAllOp, pto::TBroadcastOp, pto::CommTGatherOp, pto::CommTScatterOp,
            pto::TReduceOp>(op)) {
        UpdateOpGenInfo(curOpInfo, llvm::to_vector(op->getOperands()));
        OpKillHandle(curOpInfo, live, op->getBlock());
        return true;
...
bool MemLivenessAnalysis::HandleGenericBufferTouchingOp(Operation* op, OpInfo* curOpInfo, Liveness live)
{
    return HandleGenericStoreOp(op, curOpInfo, live) || HandleGenericPtoDpsInitOp(op, curOpInfo, live) ||
           HandleGenericDestinationStyleOp(op, curOpInfo, live) || HandleGenericAliasOrCallOp(op, curOpInfo, live) ||
...
void MemLivenessAnalysis::RecursionIR(Region* region, Liveness live)
{
    auto result = region->walk<WalkOrder::PreOrder>([this, live](Operation* op) {
        ...
        if (HandleMetadataOnlyOp(op, curOpInfo, live) || HandleGenericBufferTouchingOp(op, curOpInfo, live)) {
            return WalkResult::advance();
        }
...
void MemLivenessAnalysis::UpdateInitAndResAlias(DestinationStyleOpInterface dstStyleOp)
{
    auto results = dstStyleOp->getResults();
    if (results.empty()) {
        return;
    }
    for (auto [res, init] : llvm::zip(dstStyleOp->getResults(), dstStyleOp.getDpsInits())) {
...
void MemLivenessAnalysis::UpdateIfOpBufferAlias(scf::IfOp ifOp, scf::YieldOp yieldOp)
{
...
SmallVector<Value> MemLivenessAnalysis::GetLiveBuffersInLoop(scf::ForOp forOp, Liveness live)
{
    SmallVector<Value> allocBeforeLoopBuffers;
    const auto* liveBlockInfo = live.getLiveness(forOp->getBlock());
    auto currentLiveValues = liveBlockInfo->currentlyLiveValues(forOp.getOperation());
...
LogicalResult MemLivenessAnalysis::CheckLocalBufferAllocOp(Operation* op) const
{
...
LogicalResult MemLivenessAnalysis::CheckIfUnknownOpTouchBuffer(Operation* op) const
{
...
void MemLivenessAnalysis::UpdateBufferAlias(Value buffer, Value aliasBuffer, bool isIgnoreInplace)
{
    // union all alias buffers about `aliasBuffer` and `buffer`
    auto unionAliasSet = Union(GetAliasBuffers(aliasBuffer), GetAliasBuffers(buffer));
...
SetVector<Value> MemLivenessAnalysis::Union(SetVector<Value> set1, SetVector<Value> set2) const
{
...
void MemLivenessAnalysis::UpdateStoreOpInfo(OpInfo* opInfo, const Value storeValue, Liveness live)
{
...
void MemLivenessAnalysis::UpdateOpBufferInfo(Operation* op, const ValueRange& results)
{
...
void MemLivenessAnalysis::UpdateOpGenInfo(OpInfo* opInfo, const ValueRange& results)
{
    if (results.empty()) {
        return;
    }
...
void MemLivenessAnalysis::UpdateOperandGenInfo(const OpInfo* opInfo, Value operand)
{
...
void MemLivenessAnalysis::OpKillHandle(OpInfo* opInfo, Liveness live, Block* block)
{
    ...
    SmallVector<Value> liveValues(currentLiveValues.begin(), currentLiveValues.end());
...
void MemLivenessAnalysis::UpdateOpKillInfo(OpInfo* opInfo, Value operand, Liveness live)
{
    auto aliasBuffers = GetAliasBuffers(operand);
    aliasBuffers.insert(operand);
...
bool MemLivenessAnalysis::AllDeadAfter(Operation* op, SetVector<Value> aliasVec, Liveness live) const
{
...
BufferInfo MemLivenessAnalysis::GenerateBufferInfo(Operation* op, Value operand) const
{
...
BufferInfo MemLivenessAnalysis::GetBufferInfo(Operation* op, Value operand, pto::AddressSpace bufferScope) const
{
    ...
    std::optional<int64_t> totalStaticSize = getStaticTotalSize(memRefType.getShape());
    ...
    bufferInfo.constBits = totalStaticSize.value() * static_cast<int64_t>(elemBytes * kBitsPerByte);
    return bufferInfo;
}
...
std::shared_ptr<BufferLife> StorageEntry::GetBufferLifeByValue(const Value v) const
{
    auto find = std::find_if(bufferLifeVec.begin(), bufferLifeVec.end(), [v](std::shared_ptr<BufferLife> life) {
        return life->buffer == v;
    });
...
SmallVector<ValuePair> MemPlan::GenerateInplaceList()
{
    ...
    inplaceList.insert(inplaceList.end(), inplacePairList.begin(), inplacePairList.end());
    for (auto& operationSeq : linearOperation) {
        ...
        for (const Value& genBuffer : genBuffers) {
            ...
            for (const Value& killBuffer : killBuffers) {
                ...
                bool bufferSizeMatch = killBufferIter->second.constBits >= genBufferIter->second.constBits;
...
void MemPlan::EmitPlanMemoryFailureInfo()
{
    ...
    for (auto& iter : failApplyBufferInfo) {
        ...
        func_.emitError() << stringifyEnum(space) << " overflow, requires " << iter.second << " bits while "
...
bool MemPlan::RecordOverflowIfAny()
{
    ...
    if (planMode != MemPlanMode::LOCAL_MEM_PLAN || memscope2rootStorageEntry.empty()) {
        return false;
    }
    ...
    for (auto& it : memscope2rootStorageEntry) {
        ...
        auto bufferSpaceInfo = GetBufferSpaceInfo(rootStorageEntry->bufInfo->bufferScope);
        ...
        for (auto* child : rootStorageEntry->mergedChildren) {
            maxAllocBits = std::max(maxAllocBits, child->bitsOffset + child->alignedConstBits);
        }
...
bool MemPlan::HasSemanticConflict(const StorageEntry* entry, const BufferLifeVec& bufferLives) const
{
    if (!entry || semanticConflictPairs.empty() || bufferLives.empty())
        return false;

    auto containsPair = [this](Value lhs, Value rhs) {
        ValuePair pair = isLessValue(lhs, rhs) ? ValuePair(lhs, rhs) : ValuePair(rhs, lhs);
...
static bool hasPlannedAddressOverlap(const StorageEntry* lhs, const StorageEntry* rhs)
{
...
static SmallVector<const StorageEntry*> collectPlannedEntries(
    const SmallVector<std::unique_ptr<StorageEntry>>& storageEntryVec,
    const DenseMap<StorageEntry*, std::unique_ptr<StorageEntry>>& pingEntry2RelationPongEntry)
{
...
LogicalResult MemPlan::plan()
{
    ...
    // Plan memory address.
    PlanStatus as = planMode == MemPlanMode::LOCAL_MEM_PLAN ? PlanLocalMemAddress() : PlanWorkSpaceMemAddress();
    ...
    auto plannedEntries = collectPlannedEntries(StorageEntryVec, pingEntry2RelationPongEntry);
    for (size_t i = 0; i < plannedEntries.size(); ++i) {
        for (size_t j = i + 1; j < plannedEntries.size(); ++j) {
            ...
            if (!hasPlannedAddressOverlap(lhs, rhs))
                continue;
            bool lifeOverlap = !GetOverlapBufferLife(lhs->bufferLifeVec, rhs->bufferLifeVec).empty();
            bool semanticConflict = HasSemanticConflict(lhs, rhs->bufferLifeVec);
            if (!lifeOverlap && !semanticConflict)
                continue;
            return func_.emitError() << "PlanMemory produced overlapping local buffers in "
                                     << stringifyEnum(lhs->bufInfo->bufferScope) << " at offsets " << lhs->bitsOffset
                                     << " and " << rhs->bitsOffset;
        }
    }
...
void MemPlan::UpdateBuffer2Offsets()
{
    for (auto& e : StorageEntryVec) {
        for (Value& buffer : e->inplaceBuffers) {
            ...
            buffer2Offsets[buffer].push_back((e->bitsOffset + kBitsToByte - 1) / kBitsToByte);
        }
    }
...
void MemPlan::UpdateMultiBufferReuseExtraOffset()
{
    ...
    for (auto& relationEntry : pingEntry2RelationPongEntry) {
        for (Value& buffer : relationEntry.second->inplaceBuffers) {
            // MultiBuffer can cause multiple addrs.
            buffer2Offsets[buffer].push_back((relationEntry.second->bitsOffset + kBitsToByte - 1) / kBitsToByte);
        }
    }
}
...
void MemPlan::MergeInplaceSE()
{
    ...
    for (const auto& pairIter : inplaceList) {
        ...
        mergedBufferLifeVec.insert(mergedBufferLifeVec.end(), genSE->bufferLifeVec.begin(), genSE->bufferLifeVec.end());
        mergedBufferLifeVec.insert(
            mergedBufferLifeVec.end(), killSE->bufferLifeVec.begin(), killSE->bufferLifeVec.end());
        ...
        killSE->inplaceBuffers.insert(
            killSE->inplaceBuffers.begin(), genSE->inplaceBuffers.begin(), genSE->inplaceBuffers.end());

        // Take the maximum value for the inplace scene.
        killSE->multiBufferNum = std::max(genSE->multiBufferNum, killSE->multiBufferNum);
        ...
        auto e = std::find_if(
            StorageEntryVec.begin(), StorageEntryVec.end(),
            [genSE](std::unique_ptr<StorageEntry>& se) { return se.get() == genSE; });
...
PlanStatus MemPlan::PlanMemOffsetOfWholeWorkSpace()
{
    for (auto& it : workSpaceArg2rootStorageEntry) {
        ...
        outline.push_back(
            std::make_shared<MemoryBound>(BufferLifeVec(), 0, std::numeric_limits<uint64_t>::max(), nullptr));

        // The initial value is rootStorageEntry.
        StorageEntry* curEntry = rootStorageEntry;
        while (si.childIdx < childrenNum) {
            curEntry->alignedConstBits = static_cast<uint64_t>(curEntry->bufInfo->constBits);
...
bool MemPlan::IsEnoughForBuffersNoReuse(StorageEntry* rootStorageEntry, size_t restBufferSize, size_t alignUnit)
{
    auto iter = bufferScope2RequiredSize.find(rootStorageEntry->bufInfo->bufferScope);
...
void MemPlan::PlanBuffersWithoutReuse(StorageEntry* rootStorageEntry, size_t alignUnit) const
{
    uint offset = 0;
...
void MemPlan::MergeSameScopeSE()
{
    ...
    for (auto& iter : StorageEntryVec) {
        auto iter_scope = memscope2rootStorageEntry.find(iter->bufInfo->bufferScope);
    ...
    for (auto& rootStorageEntry : memscope2rootStorageEntry) {
        ...
        size_t accumulateSize = AlignUp(rootStorageEntry.second->bufInfo->constBits, bufferSpaceInfo.first);
        for (auto& childrenStorageEntry : rootStorageEntry.second->mergedChildren) {
            size_t curStorageSize = AlignUp(childrenStorageEntry->bufInfo->constBits, bufferSpaceInfo.first);
...
void MemPlan::PlanMemAddressForLevel0(StorageEntry* rootStorageEntry)
{
    ...
    int childrenNum = static_cast<int>(rootStorageEntry->mergedChildren.size());
    outline.push_back(std::make_shared<MemoryBound>(BufferLifeVec(), 0, maxBits, nullptr));
...
PlanStatus MemPlan::PlanSingleLocalBuffer(StorageEntry* rootStorageEntry, size_t align, size_t maxBits)
{
    uint64_t needAlignedBits = AlignUp(rootStorageEntry->bufInfo->constBits, align);
    if (needAlignedBits > maxBits) {
        failApplyBufferInfo[rootStorageEntry->bufInfo->bufferScope] = needAlignedBits;
        return PlanStatus::PLAN_FAILED;
    }
    rootStorageEntry->bitsOffset = 0;
...
PlanStatus MemPlan::PlanReusableLocalBuffer(StorageEntry* rootStorageEntry, size_t align, size_t maxBits)
{
    rootStorageEntry = GetReorderRootStorageEntry(rootStorageEntry);
    ...
    while (si.childIdx < childrenNum) {
        ...
        LogicalResult planResult = MultiSpecPlan(si, outline, history, curEntry);
        if (failed(planResult)) {
            StatusWrapper statusWrapper = {false, curEntry->alignedConstBits, &si, outline, history, rootStorageEntry};
            LDBG("\n");
...
bool MemPlan::IsSamePlanAsLastRollBack(uint64_t allocOffset, int curChildIdx, const SpecInfo& si) const
{
    return curChildIdx == si.rollbackIdx && allocOffset == si.rollbackAddr;
}

// spec_level == SPEC_LEVEL_0
bool MemPlan::VerifyConflictStage0(StorageEntry* e, const std::shared_ptr<MemoryBound>& last) const
{
    if (HasSemanticConflict(e, last->bufferLifeVec))
        return true;
    // level_0: offset = 0, offset means life distance
    DenseMap<ValuePair, BufferLife> intersection = GetOverlapBufferLife(e->bufferLifeVec, last->bufferLifeVec);
...
DenseMap<ValuePair, BufferLife> MemPlan::GetOverlapBufferLife(const BufferLifeVec& b1, const BufferLifeVec& b2) const
{
    ...
    while (i < b1Len && j < b2Len) {
        ...
        if (lo <= hi) {
            ...
            ValuePair key = lo == b1[i]->allocTime && hi == b2[j]->freeTime ?
                                std::make_pair(
                                    b1[i]->buffer,
...
PlanStatus MemPlan::ApplyFailStrategy(StatusWrapper& statusWrapper, const size_t maxBits)
{
    RollBackForAllocFail(statusWrapper, maxBits);
    // second class rollback, level 1 --> 0
    if (statusWrapper.si->specLevel > SPEC_LEVEL_0 && statusWrapper.si->childIdx >= 0) {
...
void MemPlan::ReportAllocatedEntryDebugInfo(StorageEntry* rootStorageEntry)
{
    auto printRecord = [this](const StorageEntry* entry) {
        uint64_t needByte = (entry->alignedConstBits + kBitsToByte - 1) / kBitsToByte;
...
void MemPlan::RollBackForAllocFail(StatusWrapper& statusWrapper, const size_t maxBits)
{
...
void MemPlan::RollBackForAllocFailInner(StatusWrapper& statusWrapper, const size_t maxBits)
{
    ...
    while (!statusWrapper.history.empty()) {
        PlanRecord r = RollbackOutline(statusWrapper.history, statusWrapper.outline);
        ...
        if (r.isDirectlyRollback || (r.entry->multiBufferNum == kDoubleBufferCount && !r.entry->relationPongEntry)) {
            continue;
        }
        ...
        if (si->specLevel > si->minLevel) {
            ...
            si->rollbackAddr =
                si->childIdx == -1 ? UINT64_MAX : statusWrapper.RootE->mergedChildren[si->childIdx]->bitsOffset;
            si->rollbackIdx = si->childIdx;
            if (statusWrapper.si->rollbackAddr + statusWrapper.alignedConstBits > maxBits) {
                continue;
            }
...
PlanRecord MemPlan::RollbackOutline(PlanRecHis& history, MemBoundList& outline) const
{
    auto r = history.back();
...
public:
    explicit PlanMemoryPass(const mlir::pto::PlanMemoryOptions& planMemoryOption) : PlanMemoryBase(planMemoryOption) {}

    void runOnOperation() override;

private:
    void populateBufferAddressToAllocOp(
        RewritePatternSet& patterns, DenseMap<Value, SmallVector<uint64_t>> buffer2Offsets)
    {
        if (this->memMode == MemPlanMode::LOCAL_MEM_PLAN) {
            patterns.add<MemrefAllocaOpToPointerCastOpPattern>(patterns.getContext(), buffer2Offsets);
        }
    }
};
} // namespace
...
void PlanMemoryPass::runOnOperation()
{
    ...
    for (auto funcOp : moduleOp.getOps<func::FuncOp>()) {
        ...
        if (this->memMode == MemPlanMode::LOCAL_MEM_PLAN && failed(analyzeReserveBufferPlans(funcOp, reservePlans))) {
        ...
        if (this->memMode == MemPlanMode::LOCAL_MEM_PLAN) {
            for (ReserveBufferPlan& reservePlan : reservePlans) {
                if (reservePlan.mode != ReserveBufferMode::Manual)
                    continue;
                reservePlan.reserveOp.emitOpError("pto.reserve_buffer with explicit 'base' (auto = false) is not "
        ...
        MemPlan memPlan(
            this->memMode, this->enableGlobalReuse, this->enablePrintMemoryAllocatedSize, this->restrictInplaceAsISA);
        ...
        if (this->memMode == MemPlanMode::LOCAL_MEM_PLAN &&
            failed(assignAutoReserveBufferBases(reservePlans, memLiveness.bufferInfos, memPlan.GetBuffer2Offsets()))) {
            return signalPassFailure();
        }
...
std::unique_ptr<Pass> mlir::pto::createPlanMemoryPass(const PlanMemoryOptions& planMemoryOption)
{
