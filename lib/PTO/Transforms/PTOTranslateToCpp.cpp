...
static bool opRequiresTrailingSemicolon(Operation *op) {
  return !isa<cf::CondBranchOp, emitc::DeclareFuncOp, emitc::ForOp,
              emitc::IfOp, emitc::VerbatimOp, pto::SimdVecScopeOp>(op);
}
...
static LogicalResult printOperation(CppEmitter &emitter,
                                    pto::SimdVecScopeOp vecScopeOp) {
  raw_indented_ostream &os = emitter.ostream();
  os << "__VEC_SCOPE__ {\n";
  os.indent();

  Block &body = vecScopeOp.getBody().front();
  for (Operation &op : body.getOperations()) {
    if (failed(emitter.emitOperation(op, opRequiresTrailingSemicolon(&op))))
      return failure();
  }
  ...
  for (Block &block : blocks) {
    ...
    for (Operation &op : block.getOperations()) {
      if (failed(emitter.emitOperation(
              op, /*trailingSemicolon=*/opRequiresTrailingSemicolon(&op))))
        return failure();
    }
  }
...
LogicalResult CppEmitter::emitOperation(Operation &op, bool trailingSemicolon) {
  LogicalResult status =
      llvm::TypeSwitch<Operation *, LogicalResult>(&op)
          ...
          .Case<emitc::AddOp, emitc::ApplyOp, emitc::AssignOp,
                ...
                emitc::VerbatimOp, pto::SimdVecScopeOp>(
              [&](auto op) { return printOperation(*this, op); })
          // Func ops.
  ...
  trailingSemicolon = trailingSemicolon && opRequiresTrailingSemicolon(&op);
...
LogicalResult mlir::pto::translateToCpp(Operation *op, raw_ostream &os,
                                        bool declareVariablesAtTop) {
