...
static bool shouldMaterializeYieldOperand(Operation* owner) { return isa<scf::YieldOp>(owner); }

static bool hasStringAttr(ArrayRef<NamedAttribute> attrs, StringRef name, StringRef value)
{
...
{
    return llvm::any_of(attrs, [name](NamedAttribute attr) { return attr.getName().getValue() == name; });
}
...
static void copyTileHandleAttrs(Operation* from, SmallVectorImpl<NamedAttribute>& attrs)
{
    StringRef names[] = {"pto.view_semantics", kForceDynamicValidShapeAttrName};
    for (StringRef name : names) {
        if (Attribute attr = from->getAttr(name))
            attrs.push_back(NamedAttribute(StringAttr::get(from->getContext(), name), attr));
    }
}
...
static std::optional<AddressSpace> getAddressSpace(Type type)
{
    if (auto memTy = dyn_cast<MemRefType>(type)) {
        if (auto asAttr = dyn_cast_or_null<AddressSpaceAttr>(memTy.getMemorySpace()))
            return asAttr.getAddressSpace();
        return std::nullopt;
    }
    if (auto tileTy = dyn_cast<TileBufType>(type)) {
        if (auto asAttr = dyn_cast_or_null<AddressSpaceAttr>(tileTy.getMemorySpace()))
            return asAttr.getAddressSpace();
    }
    return std::nullopt;
}
...
static TileBufConfigAttr makeTileConfig(MLIRContext* ctx, BLayout bl, SLayout sl)
{
    Builder builder(ctx);
    return TileBufConfigAttr::get(
        ctx, BLayoutAttr::get(ctx, bl), SLayoutAttr::get(ctx, sl), builder.getI32IntegerAttr(kFractalSize512),
        PadValueAttr::get(ctx, PadValue::Null), CompactModeAttr::get(ctx, CompactMode::Null));
}

static void inferConfigForMaterializedUse(
    Operation* owner, unsigned operandNo, Type operandType, TileHandleMetadata& meta, MLIRContext* ctx)
{
    if (meta.explicitConfig)
        return;

    auto colRow = [ctx]() { return makeTileConfig(ctx, BLayout::ColMajor, SLayout::RowMajor); };
    auto rowCol = [ctx]() { return makeTileConfig(ctx, BLayout::RowMajor, SLayout::ColMajor); };
    ...
    if (isa<TMatmulAccOp>(owner)) {
        if (!isA5Target(owner))
            return;
        if (operandNo == kFirstOperandIndex || operandNo == kSecondOperandIndex || operandNo == kFourthOperandIndex)
            meta.config = colRow();
...
static TileHandleMetadata getTileHandleMetadata(Value value, MLIRContext* ctx)
{
...
static FailureOr<std::pair<int64_t, int64_t>> getBoxedTileInnerShape(
    TileBufConfigAttr configAttr, Type elemTy, int32_t slVal)
{
    ...
    switch (fractal) {
        ...
        case kFractalSize32:
            return std::make_pair<int64_t, int64_t>(kFractalSize16, kFractalSize32 / kFractalSize16);
        case kFractalSize512:
            if (slVal == static_cast<int32_t>(SLayout::RowMajor))
                return std::make_pair<int64_t, int64_t>(kFractalSize16, kFractalSize32 / elemBytes);
            if (slVal == static_cast<int32_t>(SLayout::ColMajor))
                return std::make_pair<int64_t, int64_t>(kFractalSize32 / elemBytes, kFractalSize16);
            return failure();
        default:
            return failure();
    }
...
static bool setDenseTilePointerStrides(
    int32_t blVal, int64_t rows, int64_t cols, int64_t& rowStride, int64_t& colStride)
{
    if (blVal == 1) {
        rowStride = 1;
        colStride = rows;
...
static bool setBoxedTilePointerStrides(
    int32_t blVal, int32_t slVal, int64_t rows, int64_t cols, int64_t innerRows, int64_t innerCols, int64_t& rowStride,
    int64_t& colStride)
{
    if (blVal == 1) {
        if (slVal != 1)
...
static bool getTilePointerStrides(
    TileBufConfigAttr configAttr, Type elemTy, int64_t rows, int64_t cols, int64_t& rowStride, int64_t& colStride)
{
    ...
    bool boxed = slVal != 0;
    if (!boxed)
        return setDenseTilePointerStrides(blVal, rows, cols, rowStride, colStride);
    ...
    if (failed(innerShape) || innerShape->first <= 0 || innerShape->second <= 0)
        return false;
    return setBoxedTilePointerStrides(
        blVal, slVal, rows, cols, innerShape->first, innerShape->second, rowStride, colStride);
}

static SmallVec2<int64_t> getMaterializedTileShape(MemRefType memTy, const TileHandleMetadata& meta)
{
    ...
    if (pto::isPTOFloat4PackedType(sourceMrTy.getElementType()) || subRows == ShapedType::kDynamic ||
        subCols == ShapedType::kDynamic)
        return shape;

    SmallVector<int64_t> inheritedStrides;
    int64_t inheritedOffset = ShapedType::kDynamic;
    if (failed(getStridesAndOffset(sourceMrTy, inheritedStrides, inheritedOffset)) ||
        inheritedStrides.size() < kTileRank2D)
        return shape;

    int64_t childRowStride = 0;
    int64_t childColStride = 0;
    if (!getTilePointerStrides(
            meta.config, sourceMrTy.getElementType(), subRows, subCols, childRowStride, childColStride))
        return shape;
    if (inheritedStrides[0] == childRowStride && inheritedStrides[1] == childColStride) {
        shape[0] = subRows;
        shape[1] = subCols;
    }
...
static TileBufType buildTileTypeFromMemRef(MemRefType memTy, const TileHandleMetadata& meta, MLIRContext* ctx)
{
    ...
    return TileBufType::get(ctx, shape, memTy.getElementType(), memTy.getMemorySpace(), validShape, meta.config);
}

static bool isMaterializedTileAnchor(Operation* op) { return isa<BindTileOp, PointerCastOp>(op); }
...
static Value materializeOffset(OpFoldResult ofr, OpBuilder& builder, Location loc)
{
...
static Value mulI64(Value lhs, int64_t rhs, OpBuilder& builder, Location loc)
{
    ...
    if (rhs == 1)
        return lhs;
    return builder.create<arith::MulIOp>(loc, lhs, makeI64Constant(builder, loc, rhs));
}

static Value computeExplicitAddress(Value value, OpBuilder& builder, Location loc);

static Value computeSubviewAddress(memref::SubViewOp subview, OpBuilder& builder, Location loc)
{
    ...
    for (auto [offsetOfr, stride] :
         llvm::zip_equal(mixedOffsets, ArrayRef<int64_t>(sourceStrides).take_front(mixedOffsets.size()))) {
        if (stride == ShapedType::kDynamic)
            return Value();
        Value offset = materializeOffset(offsetOfr, builder, loc);
        if (!offset)
            return Value();
        linearOffset = addI64(linearOffset, mulI64(offset, stride, builder, loc), builder, loc);
    }
...
static bool isControlFlowAddressProducer(Operation* op)
{
    ...
    return name == "scf.if" || name == "scf.for" || name == "scf.while" || name == "scf.execute_region" ||
...
{
    value = peelAddressSource(value);
    auto diag = owner->emitOpError() << "cannot materialize tile handle for local memref because its "
...
static Value lookupMaterializedTileHandle(Value value, DenseMap<Value, Value>& tileHandles)
{
...
static FailureOr<bool> materializeSCFIfResults(ModuleOp module, DenseMap<Value, Value>& tileHandles)
{
    bool changed = false;
    ...
    for (scf::IfOp ifOp : llvm::reverse(ifOps)) {
        ...
        for (auto [idx, result] : llvm::enumerate(ifOp.getResults())) {
            ...
            Value thenTile = lookupMaterializedTileHandle(thenYield.getOperand(idx), tileHandles);
            Value elseTile = lookupMaterializedTileHandle(elseYield.getOperand(idx), tileHandles);
            if (!thenTile || !elseTile)
                continue;
            if (thenTile.getType() != elseTile.getType()) {
                ifOp.emitOpError() << "cannot materialize tile result #" << idx
                                   << " because branch tile types differ: " << thenTile.getType() << " vs "
                                   << elseTile.getType();
                return failure();
            }
...
static FailureOr<bool> materializeSCFForResults(ModuleOp module, DenseMap<Value, Value>& tileHandles)
{
    bool changed = false;
    ...
    for (scf::ForOp forOp : llvm::reverse(forOps)) {
        ...
        for (auto [idx, result] : llvm::enumerate(forOp.getResults())) {
            ...
            Value initTile = lookupMaterializedTileHandle(forOp.getInitArgs()[idx], tileHandles);
            if (!initTile)
                continue;
            ...
            if (initTile.getType() != yieldTy) {
                forOp.emitOpError() << "cannot materialize tile result #" << idx
                                    << " because init/yield tile types differ: " << initTile.getType() << " vs "
                                    << yieldTy;
                return failure();
            }
...
static LogicalResult materializeControlFlowTileResults(ModuleOp module, DenseMap<Value, Value>& tileHandles)
{
    bool changed = false;
    do {
        changed = false;

        FailureOr<bool> ifChanged = materializeSCFIfResults(module, tileHandles);
        if (failed(ifChanged))
            return failure();
        changed = changed || *ifChanged;

        FailureOr<bool> forChanged = materializeSCFForResults(module, tileHandles);
...
static Value getAllocValidOperand(TileBufType tileTy, Value operand, unsigned dim, OpBuilder& builder, Location loc)
{
...
static void copyMaterializedTileAttrs(ArrayRef<NamedAttribute> attrs, Operation* to)
{
    if (Attribute attr = getAttr(attrs, kForceDynamicValidShapeAttrName))
        to->setAttr(kForceDynamicValidShapeAttrName, attr);
}

static void updateResultTypesAfterMaterializingOperand(Operation* op, unsigned operandNo, Type tileTy)
{
...
{
    return viewSemantics && (viewSemantics.getValue() == "treshape" || viewSemantics.getValue() == "bitcast");
}
...
static SmallVector<OpOperand*> collectMaterializedTileUses(Value anchoredValue)
{
    ...
    for (OpOperand& use : anchoredValue.getUses()) {
        if (shouldMaterializeOperand(use.getOwner()) || shouldMaterializeYieldOperand(use.getOwner())) {
...
static Value createMaterializedTileFromSource(
    Operation* anchor, Type tileTy, StringAttr viewSemantics, Value sourceTile, OpBuilder& builder)
{
    if (!(sourceTile && viewSemantics))
        return Value();
    if (viewSemantics.getValue() == "treshape") {
        return builder.create<TReshapeOp>(anchor->getLoc(), tileTy, sourceTile).getResult();
    }
    if (viewSemantics.getValue() == "bitcast") {
        return builder.create<BitcastOp>(anchor->getLoc(), tileTy, sourceTile).getResult();
    }
    return Value();
}

static Value createMaterializedAllocTile(
    Operation* anchor, Value anchoredValue, Type tileTy, const TileHandleMetadata& meta, OpBuilder& builder,
    bool& failedMaterialization)
{
    ...
    auto alloc = builder.create<AllocTileOp>(
        ...
        getAllocValidOperand(cast<TileBufType>(tileTy), meta.validRow, 0, builder, anchor->getLoc()),
        getAllocValidOperand(cast<TileBufType>(tileTy), meta.validCol, 1, builder, anchor->getLoc()));
    copyMaterializedTileAttrs(meta.attrs, alloc);
    return alloc.getResult();
}

static void rewriteMaterializedTileUses(ArrayRef<OpOperand*> usesToRewrite, Value materialized, Type tileTy)
{
...
static Value materializeAnchorResult(
    Operation* anchor, Value anchoredValue, OpBuilder& builder, MLIRContext* ctx, DenseMap<Value, Value>& tileHandles,
    const DenseSet<Value>& mustMaterialize, bool& failedMaterialization)
{
    auto memTy = dyn_cast<MemRefType>(anchoredValue.getType());
    if (!memTy || !isLocalTileMemRef(memTy))
        return Value();

    SmallVector<OpOperand*> usesToRewrite = collectMaterializedTileUses(anchoredValue);
    TileHandleMetadata meta = getTileHandleMetadata(anchoredValue, ctx);
    auto viewSemantics = dyn_cast_or_null<StringAttr>(getAttr(meta.attrs, "pto.view_semantics"));
    bool isTileView = isTileViewSemantics(viewSemantics);
    if (usesToRewrite.empty() && !isTileView && !mustMaterialize.contains(anchoredValue))
        return Value();

    for (OpOperand* use : usesToRewrite)
        inferConfigForMaterializedUse(use->getOwner(), use->getOperandNumber(), anchoredValue.getType(), meta, ctx);
    ...
    Value materialized = createMaterializedTileFromSource(anchor, tileTy, viewSemantics, sourceTile, builder);
    if (!materialized) {
        materialized = createMaterializedAllocTile(anchor, anchoredValue, tileTy, meta, builder, failedMaterialization);
        if (!materialized)
            return Value();
    }
...
static DenseSet<Value> collectMustMaterializeSources(ArrayRef<Operation*> anchors, MLIRContext* ctx)
{
...
static SmallVec32<std::pair<Operation*, unsigned>> collectTileOperandsToRewrite(ModuleOp module)
{
...
static bool rewriteMaterializedTileOperand(
    Operation* op, unsigned operandNo, OpBuilder& builder, MLIRContext* ctx, DenseMap<Value, Value>& tileHandles,
    bool& failedMaterialization)
{
...
static void materializeTileOperands(
    ModuleOp module, OpBuilder& builder, MLIRContext* ctx, DenseMap<Value, Value>& tileHandles,
    bool& failedMaterialization)
{
    for (auto [op, operandNo] : collectTileOperandsToRewrite(module)) {
        if (!rewriteMaterializedTileOperand(op, operandNo, builder, ctx, tileHandles, failedMaterialization)) {
            continue;
        }
    }
}

struct PTOMaterializeTileHandlesPass : public impl::PTOMaterializeTileHandlesBase<PTOMaterializeTileHandlesPass> {
    void runOnOperation() override
    {
        ...
        for (Operation* anchor : anchors) {
            if (anchor->getNumResults() != 1)
                continue;
            materializeAnchorResult(
                anchor, anchor->getResult(0), builder, ctx, tileHandles, mustMaterialize, failedMaterialization);
        }
        ...
        materializeTileOperands(module, builder, ctx, tileHandles, failedMaterialization);
        if (failedMaterialization) {
            signalPassFailure();
            return;
        }
